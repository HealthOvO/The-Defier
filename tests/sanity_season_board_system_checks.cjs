...
function assertSeasonBoardHandoffTarget(game, board, sourceKey, label) {
  ...
  assert(
    typeof game.rewardView?.getRewardSeasonBoardHandoffTarget === 'function',
    `${sourceLabel} should expose getRewardSeasonBoardHandoffTarget`
  );
  const target = game.rewardView.getRewardSeasonBoardHandoffTarget(sourceKey);
...
(function run() {
  ...
  const normalizedRewardMeta = game.rewardView.getRewardExpeditionMeta();
  ...
  const positiveSideHandoff = game.rewardView.getRewardSeasonBoardHandoffTarget('sideVerification');
  ...
  const rewardBrief = game.rewardView.getRewardNarrativeBriefMeta();
  assert(
  ...
  const normalizedLocklineRewardMeta = locklineGame.rewardView.getRewardExpeditionMeta();
  assert(
  ...
  const originalLocklineSwitchCollectionSection = locklineGame.rewardView.switchCollectionSection;
  let locklineFollowSection = '';
  locklineGame.rewardView.switchCollectionSection = (section) => {
    locklineFollowSection = section;
    locklineGame.currentScreen = 'collection';
  };
  assert(
    locklineGame.rewardView.followRewardSeasonBoardHandoff('nextTask') === true
      && locklineFollowSection === normalizedLocklineRewardMeta.seasonBoard.nextWeekGoal?.value
      && locklineGame.rewardView.lastRewardSeasonBoardHandoff?.source === 'settlement',
  ...
  const locklineArrivalNotice = locklineGame.rewardView.pendingRewardSeasonBoardHandoffNotice;
  ...
  const locklineRewardBrief = locklineGame.rewardView.getRewardNarrativeBriefMeta();
  assert(
  ...
  const chapterArcDrillTargets = ['daily', 'weekly', 'global'].map((mode) => chapterArcGame.rewardView.getRewardChapterArcDrillTarget(mode));
  assert(
    chapterArcDrillTargets.every((target, index) => {
      const mode = ['daily', 'weekly', 'global'][index];
      return target
        && target.sourceKey === 'chapterArcDrill'
        && target.action === 'challenge'
        && target.value === mode
        && target.mode === mode
        && target.source === 'chapter_arc'
        && target.sourceId === chapterArc.id
        && target.chapterId
        && target.focusId === `chapter_codex:${target.chapterId}`
        && /章节演练/.test(target.buttonLabel || '')
        && (mode !== 'daily' || /今日天机/.test(target.buttonLabel || ''))
        && (mode !== 'weekly' || /七日劫数/.test(target.buttonLabel || ''))
        && (mode !== 'global' || /众生试炼/.test(target.buttonLabel || ''));
    }),
    `chapter arc reward drill targets should route to daily/weekly/global challenge focus, got ${JSON.stringify(chapterArcDrillTargets)}`
  );
  ...
  chapterArcDrillTargets.forEach((chapterArcDrillTarget) => {
    const mode = chapterArcDrillTarget.mode;
    assert(
      chapterArcGame.rewardView.followRewardChapterArcDrill(chapterArcDrillTarget.chapterId, mode) === true
      ...
      `chapter arc reward drill follow should write chapter focus and open ${mode} challenge hub, got ${JSON.stringify({
  ...
  const normalizedDebtRewardMeta = debtGame.rewardView.getRewardExpeditionMeta();
  assert(
  ...
  const debtRewardBrief = debtGame.rewardView.getRewardNarrativeBriefMeta();
  assert(
  ...
  const normalizedRiskyRewardMeta = aftereffectTransitionGame.rewardView.getRewardExpeditionMeta();
  assert(
  ...
  const riskyRewardBrief = aftereffectTransitionGame.rewardView.getRewardNarrativeBriefMeta();
  assert(
