...
async function safeScreenshot(page, outputPath) {
  try {
    ...
    await page.screenshot({ path: outputPath, fullPage: true, timeout: SCREENSHOT_TIMEOUT_MS, animations: 'disabled' });
  } catch (err) {
...
async function safeElementScreenshot(page, selector, outputPath) {
  try {
    ...
    if (box && box.width > 0 && box.height > 0) {
      await page.screenshot({ path: outputPath, clip: box, timeout: SCREENSHOT_TIMEOUT_MS, animations: 'disabled' });
      return;
    }
    await target.screenshot({ path: outputPath, timeout: SCREENSHOT_TIMEOUT_MS, animations: 'disabled' });
  } catch (err) {
