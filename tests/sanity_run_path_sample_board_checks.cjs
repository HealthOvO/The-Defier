...
(function run() {
  ...
  assert(progress.sampledCharacters === 2, `progress should expose sampled character count, got ${JSON.stringify(progress)}`);const reloadedGame = Object.create(Game.prototype);


  if (typeof reloadedGame.attachHubControllers === 'function') reloadedGame.attachHubControllers();
  reloadedGame.player = {
    characterId: 'linFeng',
    realm: 6
  };
