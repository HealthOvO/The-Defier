...
(function run() {
  ...
  assert(typeof hud.BattleHUD.escapeHtml === 'function', 'escapeHtml should exist');
  assert(typeof hud.BattleHUD.escapeAttr === 'function', 'escapeAttr should exist');
  ...
  assert(hud.BattleHUD.escapeHtml('<boss>') === '&lt;boss&gt;', 'escapeHtml should encode angle brackets');
  ...
  assert(/data-tooltip-safe="[^"]*&#39;危险&#39;"/.test(intentMarkup), 'enemy intent tooltip attr should escape single quotes');
  assert(/data-intent-type="debuff"/.test(intentMarkup), 'enemy intent markup should expose intent type data attr');
  assert(!/onmouseenter|onmouseleave|onclick/.test(intentMarkup), 'enemy intent markup should not contain inline event handlers');
