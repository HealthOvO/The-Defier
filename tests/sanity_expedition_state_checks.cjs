...
(function run() {
  ...
  assert(Array.isArray(buildSnapshot.expedition?.observatoryLink?.trainingTags) && buildSnapshot.expedition.observatoryLink.trainingTags.length >= 1, `build snapshot should retain observatory training tags, got ${JSON.stringify(buildSnapshot.expedition?.observatoryLink)}`);const persistedGame = new Game();


  if (typeof ctx.__attachExpeditionHubController === 'function') ctx.__attachExpeditionHubController(persistedGame);
  ...
  localStorage.removeItem('theDefierActiveExpeditionStateV1');const malformedSeedGame = new Game();

  if (typeof ctx.__attachExpeditionHubController === 'function') ctx.__attachExpeditionHubController(malformedSeedGame);
  ...
  malformedSeedGame.persistActiveExpeditionState();const repairedGame = new Game();

  if (typeof ctx.__attachExpeditionHubController === 'function') ctx.__attachExpeditionHubController(repairedGame);
  ...
  assert(sanctum.goals.some((goal) => goal.id === 'run_slate_archive_goal'), 'sanctum should include a run slate goal');const reloadedGame = new Game();


  if (typeof ctx.__attachExpeditionHubController === 'function') ctx.__attachExpeditionHubController(reloadedGame);
  ...
  if (typeof ctx.__attachExpeditionHubController === 'function') ctx.__attachExpeditionHubController(battleStartGame);
  ...
  localStorage.removeItem('theDefierActiveExpeditionStateV1');const implicitBattleStartGame = new Game();

  if (typeof ctx.__attachExpeditionHubController === 'function') ctx.__attachExpeditionHubController(implicitBattleStartGame);
