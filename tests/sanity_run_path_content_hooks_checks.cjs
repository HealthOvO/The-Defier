...
(function run() {
  ...
  const EventView = vm.runInContext('EventView', gameCtx);
  const RewardView = vm.runInContext('RewardView', gameCtx);
  const InventoryView = vm.runInContext('InventoryView', gameCtx);const game = Object.create(Game.prototype);
  ...
  try { game.inventoryView = new InventoryView(game); } catch(e){}
  game.player = {
    gold: 500,
    heavenlyInsight: 6,
    karma: 2,
  ...
  const research = InventoryView.prototype.getTreasureResearchData.call(game.inventoryView, {
  ...
  const overview = InventoryView.prototype.getTreasureResearchOverviewData.call(game.inventoryView);
