...
(function run() {
  ...
  game.rewardView = new RewardView(game);
  console.log('game.eventView is', typeof game.eventView);
  console.log('EventManager.js getEventModalRefs called with game =', !!game);
  ...
  assert(payload.player.runPath && payload.player.runPath.mutation && payload.player.runPath.mutation.name === '借势落子', `render_game_to_text should expose run path mutation, got ${JSON.stringify(payload.player.runPath)}`);const fallbackGame = Object.create(Game.prototype);


  if (typeof fallbackGame.attachHubControllers === 'function') fallbackGame.attachHubControllers();
  fallbackGame.eventView = new EventView(fallbackGame);
