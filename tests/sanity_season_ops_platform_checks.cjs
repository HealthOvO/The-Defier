...
async function runConcurrentStartupCheck() {
  ...
  try {
    ...
    assert.strictEqual(firstHealth.payload?.schema?.currentMigrationId, '0011_authoritative_fate_chronicle');
    assert.strictEqual(secondHealth.payload?.schema?.currentMigrationId, '0011_authoritative_fate_chronicle');
  } finally {
...
async function runApiChecks(server) {
  ...
  assert.strictEqual(health.payload?.schema?.version, 11, 'season ops should coexist with authoritative fate chronicle schema v11');
  assert.strictEqual(health.payload?.schema?.currentMigrationId, '0011_authoritative_fate_chronicle');
