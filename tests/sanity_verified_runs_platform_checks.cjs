...
async function runChecks() {
  ...
  try {
    ...
    assert.strictEqual(upgradedVersion.payload?.schema?.currentMigrationId, '0008_authoritative_world_rift', 'older databases should advance through verified runs to world rift v8 on restart');
    assert.deepStrictEqual(
