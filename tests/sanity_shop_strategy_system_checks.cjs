...
(function run() {
  ...
  harness.getCardPrice = (card) => {
    let base = 50;
    if (card.rarity === 'uncommon') base = 100;
    if (card.rarity === 'rare') base = 200;
    if (card.rarity === 'legendary') base = 300;
    return base;
  };
  harness.getTreasurePrice = (treasure) => {
    let base = 150;
    if (treasure.rarity === 'uncommon') base = 250;
    if (treasure.rarity === 'rare') base = 400;
    if (treasure.rarity === 'legendary') base = 600;
    return base;
  };
  harness.generateShopData = Game.prototype.generateShopData || function() { return { cards: [], treasures: [] }; };
