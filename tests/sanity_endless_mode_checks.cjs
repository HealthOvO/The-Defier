...
(function run() {
  ...
  const ctx = vm.createContext({
    ...
    JSON,
    Date,
    StrategicView: class StrategicView {
      constructor(game) {
        this.game = game;
      }
      update() {}
      showEndlessBoonSelection(onDone) {
        if (typeof onDone === 'function') onDone();
      }
      showEndlessParanoiaSelection(cycleOverride, onDone) {
        if (typeof this.game.applyEndlessParanoiaChoice === 'function') {
