...
(function run() {
  ...
  const Game = vm.runInContext('Game', ctx);const releaseGame = new Game();


  if (typeof ctx.__attachExpeditionHubController === 'function') ctx.__attachExpeditionHubController(releaseGame);
  ...
  assert(Array.isArray(releasePayload.recentNemesisLogs) && releasePayload.recentNemesisLogs.some((entry) => entry.status === 'released'), 'release payload should retain released history log');const tradeGame = new Game();


  if (typeof ctx.__attachExpeditionHubController === 'function') ctx.__attachExpeditionHubController(tradeGame);
  ...
  assert(Array.isArray(tradePayload.recentNemesisLogs) && tradePayload.recentNemesisLogs.some((entry) => entry.status === 'traded'), 'trade payload should retain traded history log');const evolveGame = new Game();


  if (typeof ctx.__attachExpeditionHubController === 'function') ctx.__attachExpeditionHubController(evolveGame);
