...
(function run() {
  ...
  loadFile(ctx, path.join(root, 'js/views/InventoryView.js'));
  loadFile(ctx, path.join(root, 'js/views/ShopView.js'));
  const Game = vm.runInContext('Game', ctx);
  const InventoryView = vm.runInContext('InventoryView', ctx);
  const ShopManager = vm.runInContext('ShopManager', ctx);
  const ShopView = vm.runInContext('ShopView', ctx);
  assert(typeof Game === 'function', 'Game should exist');

  const harness = {
    game: null, // Will point to harness itself
    player: {
      currentHp: 18,
      maxHp: 60,
      gold: 180,
  ...
  [
    ...
    harness[name] = Game.prototype[name] || 
                    (InventoryView && InventoryView.prototype[name]) || 
                    (ShopManager && ShopManager.prototype[name]) || 
                    (ShopView && ShopView.prototype[name]);
  });
