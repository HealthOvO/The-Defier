export class RunManager {
    ...
    draftRunPathsForCharacter(characterId) {
        ...
        const cached = this.game.pendingRunPathDrafts && Array.isArray(this.game.pendingRunPathDrafts[charId])
            ? this.game.pendingRunPathDrafts[charId].slice()
            : null;
    ...
    applyRunPathMutationSelection(mutationId, realmCleared = 0) {
        ...
        if (
            this.game.map
            && this.game.currentScreen === 'map-screen'
            && typeof this.game.map.updateLegacyMissionTracker === 'function'
        ) {
    ...
    getRunPathShopProfile() {
        const meta = this.game.player && typeof this.game.player.getRunPathMeta === 'function'
            ? this.game.player.getRunPathMeta()
            : (this.game.selectedRunPathId ? this.getRunPathMetaById(this.game.selectedRunPathId) : null);
    ...
    injectRunPathShopServices(baseServices = [], tabId = 'base') {
        ...
        const priceMult = tabId === 'rumor'
            ? this.game.getShopPriceMultiplier(0.02)
            : this.game.getShopPriceMultiplier(0.08);
    ...
    queueRunPathRewardMeta(pathMeta, phaseMeta, options = {}) {
        ...
        const existingMeta = this.game.lastRunPathRewardMeta
            && this.game.lastRunPathRewardMeta.pathId === pathId
            && Array.isArray(this.game.lastRunPathRewardMeta.entries)
    ...
    handleRunPathProgress(eventType, amount = 1, context = {}) {
        ...
        if (isFinalPhase) {
            const completionMeta = pathMeta.completionRecord && typeof pathMeta.completionRecord === 'object'
                ...
                : {};
            const archiveRecord = typeof this.game.recordRunPathCompletion === 'function'
                ? this.game.recordRunPathCompletion(pathMeta, {
                    phaseMeta,
        ...
        if (
            this.game.currentScreen === 'battle-screen'
            || this.game.currentScreen === 'reward-screen'
    ...
    shouldOfferRunVowAfterRealm(realmCleared) {
        ...
        const active = this.game.player && typeof this.game.player.getRunVowMetas === 'function'
    ...
    advanceRunDestinyTier(reason = '') {
        const current = this.game.player && typeof this.game.player.normalizeRunDestiny === 'function'
            ? this.game.player.normalizeRunDestiny(this.game.player.runDestiny)
            : (this.game.player ? this.game.player.runDestiny : null);
        ...
        if (this.game.player && typeof this.game.player.setRunDestiny === 'function') {
            ...
            this.game.player.runDestiny = { id: current.id, tier: nextTier };
        }
        const meta = this.game.player && typeof this.game.player.getRunDestinyMeta === 'function'
            ? this.game.player.getRunDestinyMeta()
            : this.game.getRunDestinyMetaById(current.id, nextTier);
        if (reason && meta) {
    ...
    getDisplayRealmName(realm) {
        const fallbackName = this.game.map && typeof this.game.map.getRealmName === 'function'
            ? this.game.map.getRealmName(realm)
            : `第${realm}重天`;
    ...
    getChapterDisplaySnapshot(realm) {
        ...
        const currentDestiny = (this.game.player && typeof this.game.player.getRunDestinyMeta === 'function')
            ? this.game.player.getRunDestinyMeta()
            : null;
        const currentSpirit = (this.game.player && typeof this.game.player.getSpiritCompanionMeta === 'function')
            ? this.game.player.getSpiritCompanionMeta()
            : null;
        const currentVows = (this.game.player && typeof this.game.player.getRunVowMetas === 'function')
            ? this.game.player.getRunVowMetas()
            : [];
    ...
    applyLegacyRunDoctrine(player, presetId = null, secondaryPresetId = null) {
        ...
        const p2Id = secondaryPresetId !== undefined && secondaryPresetId !== null
            ? secondaryPresetId
            : (this.game.legacyProgress?.secondaryPreset || null);
    ...
    initRealmSelect() {
        ...
        // Auto-select logic
        let targetRealm = 1;
        const unlockedRealms = Array.isArray(this.game.unlockedRealms) && this.game.unlockedRealms.length > 0
            ? this.game.unlockedRealms
            : [1];
    ...
    updateRealmPreview(realmId) {
        ...
        if (realmId === 'endless') {
            ...
            const activeMutators = (state.activeMutators || [])
                .map((id) => this.game.getEndlessMutatorPool().find((m) => m.id === id))
                .filter(Boolean);
    ...
    onRealmComplete() {
        ...
        const clearEssence = this.game.awardLegacyEssence(2 + Math.floor(currentRealm / 2), '破境夺天', { silent: true });
        ...
        if (this.game.player.realm >= 18) {
            const finalEssence = this.game.awardLegacyEssence(18, '逆天终局', { silent: true });
