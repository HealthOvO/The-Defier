export class ShopManager {
    ...
    showLawDetail(law, isCollected = false) {
        ...
        chipsEl.innerHTML = [
            ...
            `<span class="detail-status-chip">${this.game.getLawElementLabel(law.element)}属性</span>`,
            `<span class="detail-status-chip rarity-chip rarity-${rarity}">${this.game.getLawRarityText(rarity)}</span>`
        ].join('');
        ...
        switch (service.id) {
            ...
            case 'soulMortgage': {
                ...
                this.game.showRewardModal(
                    '蚀寿抵押完成',
                    `当前生命压至 ${this.game.player.currentHp}。
