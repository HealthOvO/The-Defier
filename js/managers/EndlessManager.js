export class EndlessManager {
    ...
    normalizeEndlessState(rawState = null) {
        ...
        const progressionRealm = Math.max(
            Number(this.game.player?.maxRealmReached) || 1,
            Math.max(...((Array.isArray(this.game.unlockedRealms) && this.game.unlockedRealms.length > 0) ? this.game.unlockedRealms : [1]))
        );
    ...
    getEndlessActiveMutatorIds() {
        ...
        if (
            ...
            this.game.currentBattleNode.type === 'elite'
        ) {
    ...
    getEndlessCollapseAnalysis() {
        ...
        const hpRatio = this.game.player && Number(this.game.player.maxHp) > 0
            ? Math.max(0, Number(this.game.player.currentHp) || 0) / Math.max(1, Number(this.game.player.maxHp) || 1)
            : 0;
        const nodeType = this.game.currentBattleNode && typeof this.game.currentBattleNode.type === 'string'
            ? this.game.currentBattleNode.type
            : 'enemy';
    ...
    getEndlessDangerProfile(cycleOverride = null) {
        const axisLibrary = typeof this.game.getSharedDangerAxisLibrary === 'function'
            ? this.game.getSharedDangerAxisLibrary()
            : {
                burst: {
                    id: 'burst',
                    label: '先手爆发',
    ...
    handleEndlessRealmComplete() {
        ...
        const boardBeforeVerification = typeof this.game.getSeasonBoardSnapshot === 'function'
        ...
        this.game.awardLegacyEssence(essenceGain, '无尽感悟', { silent: true });
        ...
        if (typeof this.game.recordSeasonVerificationResult === 'function') {
            ...
            this.game.recordSeasonVerificationResult({
                ...
                writebackLine: pendingDebt
                    ? '无尽轮回主验证通过，欠卷会被清账并释放天命强目标。'
                    : '无尽轮回主验证通过，本周押卷会升级为正卷。',
                resolvedRunId: seasonProfile?.id || '',
                chapterIndex: Math.max(0, nextCycle + 1),
                proofQuality: nextCycle >= 4 ? 'decisive' : 'solid',
                lineageStyle: '长压试炼',
                summaryLine: pendingDebt
                    ? '无尽通关已补齐主验证，这笔欠卷可以在季盘上清账。'
                    : '无尽通关证明这条主轴在长压环境下依然成立。',
