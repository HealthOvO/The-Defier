...
export class SeasonBoardManager {
  ...
  normalizeSeasonBoardClaimedLaneRewards(source = null) {
    ...
    return Object.fromEntries(Object.entries(result).sort((a, b) => String(b[0]).localeCompare(String(a[0]))).slice(0, 8));
  }
  getCommittedSeasonBoardFrontierResolution(options = {}) {
    ...
    const sourceState = options.seasonVerification && typeof options.seasonVerification === 'object' ? options.seasonVerification : this.game.ensureSeasonVerificationState({
      weekTag: targetWeekTag,
      weekLabel: options.weekLabel || ''
    });
    const records = [...(Array.isArray(sourceState?.records) ? sourceState.records : []), ...(Array.isArray(sourceState?.history) ? sourceState.history : [])].map((entry, index) => this.game.normalizeSeasonVerificationRecord(entry, index)).filter(entry => {
      if (!entry || typeof entry !== 'object') return false;
      if (targetWeekTag && entry.weekTag !== targetWeekTag) return false;
      return String(entry.recordKind || '').trim() === 'frontier_resolution' || !!entry.frontierResolutionChoiceId || !!entry.frontierResolutionId;
    }).sort((a, b) => {
  ...
  getSeasonBoardFrontierResolutionArchiveRecords(options = {}) {
    const sourceState = options.seasonVerificationState && typeof options.seasonVerificationState === 'object' ? options.seasonVerificationState : this.game.seasonVerificationState && typeof this.game.seasonVerificationState === 'object' ? this.game.seasonVerificationState : null;
    const rawRecords = [...(Array.isArray(sourceState?.records) ? sourceState.records : []), ...(Array.isArray(sourceState?.history) ? sourceState.history : [])];
    const validChoices = new Set(['hold_primary', 'rebalance_support', 'seal_dispute']);
    const seen = new Set();
    return rawRecords.map((entry, index) => this.game.normalizeSeasonVerificationRecord(entry, index)).filter(entry => {
      if (!entry || typeof entry !== 'object') return false;
      const choiceId = String(entry.frontierResolutionChoiceId || '').trim();
      const isFrontierResolution = String(entry.recordKind || '').trim() === 'frontier_resolution' || !!choiceId || !!entry.frontierResolutionId;
      if (!isFrontierResolution || !validChoices.has(choiceId)) return false;
      const key = String(entry.recordId || entry.frontierResolutionId || `${entry.weekTag || 'weekless'}:${choiceId}:${entry.frontierResolutionSubmittedAt || entry.updatedAt || index}`).trim();
  ...
  buildSeasonBoardFrontierChronicleArchive(frontier = null, context = {}) {
    ...
    const dominantStyle = styleEntries.filter(entry => entry.count > 0).sort((a, b) => {
    ...
    return {
      ...
      summaryLine: dominantStyle ? `战役史卷已封存 ${entries.length} 条会审裁记，当前更常呈现【${dominantStyle.label}】。` : '战役史卷正在等待第一条会审裁记封存。',
      detailLine: latestEntry ? `最近封记：${latestEntry.weekLabel || latestEntry.weekTag || '本周'} · ${latestEntry.choiceLabel || '裁记'}${latestEntry.supportLaneLabel ? ` · 副线 ${latestEntry.supportLaneLabel}` : ''}。` : `完成本周【${primaryLaneLabel}】会审后，史卷会开始累计长期裁记风格。`,
  ...
  normalizeSeasonBoardFrontierChronicleArchive(source = null, context = {}) {
    ...
    const normalizeEntry = (entry = null, index = 0) => {
      ...
      const choiceId = ['hold_primary', 'rebalance_support', 'seal_dispute'].includes(String(item.choiceId || '').trim()) ? String(item.choiceId || '').trim() : '';
      if (!choiceId) return null;
      const stanceId = String(item.stanceId || '').trim().slice(0, 48) || (choiceId === 'rebalance_support' ? 'support_balancer' : choiceId === 'seal_dispute' ? 'dispute_archivist' : 'frontier_loyalist');
      const fallbackStanceId = choiceId === 'rebalance_support' ? 'support_balancer' : choiceId === 'seal_dispute' ? 'dispute_archivist' : 'frontier_loyalist';
    ...
    const entries = (Array.isArray(root.entries) ? root.entries : []).map(normalizeEntry).filter(Boolean).slice(0, 6);
    const latestEntry = normalizeEntry(root.latestEntry) || entries[0] || null;
    const styleEntries = (Array.isArray(root.styleEntries) ? root.styleEntries : []).filter(entry => entry && typeof entry === 'object').slice(0, 3).map(entry => {
    ...
    const totalRecords = Math.max(entries.length, Math.max(0, Math.floor(Number(root.totalRecords || root.sealedCount || 0) || 0)));
  ...
  buildSeasonBoardChapterArc(context = {}) {
    ...
    const currentWeekTag = sanitize(context.weekTag || context.weekVerdictLedger?.current?.weekTag || context.frontier?.weekTag || '', 24);
    const currentWeekLabel = sanitize(context.weekLabel || context.weekVerdictLedger?.current?.weekLabel || context.frontier?.weekLabel || currentWeekTag, 32);
    const currentWeekMeta = parseWeekTag(currentWeekTag);
    const currentWeekSlot = currentWeekMeta ? (currentWeekMeta.weekNo - 1) % 3 + 1 : 1;
    const chapterNumber = currentWeekMeta ? Math.max(1, Math.floor((currentWeekMeta.weekNo - 1) / 3) + 1) : 1;
    const windowStartWeekNo = currentWeekMeta ? Math.max(1, currentWeekMeta.weekNo - currentWeekSlot + 1) : 0;
    const windowEndWeekNo = currentWeekMeta ? Math.min(53, windowStartWeekNo + 2) : 0;
    const windowStartIndex = currentWeekMeta ? currentWeekMeta.year * 53 + windowStartWeekNo : 0;
    const windowEndIndex = currentWeekMeta ? currentWeekMeta.year * 53 + windowEndWeekNo : 0;
    ...
    const currentVerdict = context.weekVerdictLedger?.current && typeof context.weekVerdictLedger.current === 'object' ? context.weekVerdictLedger.current : null;
    const records = typeof this.getSeasonBoardFrontierResolutionArchiveRecords === 'function' ? this.getSeasonBoardFrontierResolutionArchiveRecords({
      seasonVerificationState: context.seasonVerificationState
    }) : [];
    const rawEntries = [...(currentVerdict?.frontierResolutionChoiceId ? [currentVerdict] : []), ...records].map(normalizeEntry).filter(Boolean).sort((a, b) => {
    ...
    const summarizeEntries = (entries = []) => {
      ...
      entries.forEach(entry => {
        ...
        if (entry.choiceId === 'rebalance_support' && entry.supportLaneId && entry.submittedAt >= latestSupportLaneAt) {
      ...
      const dominantChoiceId = Object.keys(countsByChoice).sort((a, b) => {
    ...
    const windowEntries = currentWeekMeta ? distinctEntries.filter(entry => entry.weekIndex >= windowStartIndex && entry.weekIndex <= windowEndIndex).slice(0, 3) : distinctEntries.slice(0, 3);
    ...
    const previousWindowEntries = currentWeekMeta && currentWeekSlot === 1 && windowStartWeekNo > 3 ? distinctEntries.filter(entry => entry.weekIndex >= windowStartIndex - 3 && entry.weekIndex < windowStartIndex).slice(0, 3) : [];
    ...
    const rescueReasonLine = hasDebtPressure ? context.debtPack?.summaryLine || '本章仍有欠卷占住强目标位，需要先清账再归章。' : hasRiskPressure ? context.settlement?.summaryLine || '本章押卷仍属险卷，需要补一条外场或旁证。' : rescueOpen ? `本章当前只封 ${sealedCount}/3 周裁记，章末前建议补齐缺口。` : '本章裁记节奏稳定，暂不需要额外救火。';
    const rescueWindow = {
      available: true,
      open: rescueOpen,
      ...
      guideLine: sanitize(rescueOpen ? '救火窗口只提示章内缺口，真正行动仍沿本周季盘 nextTask 推进。' : '继续按本周季盘主线推进，章层只做归卷回看。', 220)
    };
    const chapterClosed = sealedCount >= 3;
    const pressureWindow = {
      available: true,
      open: rescueOpen || hasDebtPressure || hasRiskPressure,
      statusId: hasDebtPressure ? 'debt_pressure' : hasRiskPressure ? 'risk_pressure' : chapterClosed ? 'sealed_pressure' : rescueOpen ? 'rescue_pressure' : 'steady_pressure',
      statusLabel: hasDebtPressure ? '欠卷压章' : hasRiskPressure ? '险卷压章' : chapterClosed ? '章末定型' : rescueOpen ? '章内爬压' : '稳态续压',
      reasonLine: sanitize(hasDebtPressure ? context.debtPack?.summaryLine || '欠卷仍压住强目标位，先清账再谈章内收束。' : hasRiskPressure ? context.settlement?.summaryLine || '险卷仍在抬高章势，需要先补外场或旁证。' : rescueOpen ? `章内仍有 ${sealedCount}/3 周裁记缺口，压力正在抬高。` : chapterClosed ? '三周裁记已齐，章势开始收束。' : '章势正在稳步推进。', 220),
      guideLine: sanitize(rescueOpen || hasDebtPressure || hasRiskPressure ? '章势只负责读出当前压力，不会改写本周季盘的行动真源。' : '继续沿本周主线推进，等章势自然收束。', 220),
      shortLine: sanitize(`${hasDebtPressure ? '欠卷压章' : hasRiskPressure ? '险卷压章' : chapterClosed ? '章末定型' : rescueOpen ? '章内爬压' : '稳态续压'}${hasDebtPressure || hasRiskPressure || rescueOpen || chapterClosed ? ` · ${Math.max(0, Math.min(100, hasDebtPressure ? 100 : hasRiskPressure ? 84 : chapterClosed ? 32 : 68))}分` : ''}`, 120)
    };
    const dominantChoiceLabel = dominantChoiceId ? choiceLabelMap[dominantChoiceId] : '';
    const chapterResultId = chapterClosed ? dominantChoiceId === 'seal_dispute' ? 'archived_dispute' : dominantChoiceId === 'rebalance_support' ? 'balanced_front' : 'guarded_front' : rescueOpen ? 'under_rescue' : 'in_progress';
    const endingPreviewLine = chapterClosed ? `章末预览：本章最终偏向【${dominantChoiceLabel || '裁记风格'}】，下章可据此复盘主线。` : `章末预览：还剩 ${weeksRemaining} 周窗口，三周裁记齐后会形成本章评语。`;
    const finalCommentLine = chapterClosed ? dominantChoiceId === 'rebalance_support' ? '章末评语：本章愿意给副线留下证据，下章普通排班可更重视旁证补样。' : dominantChoiceId === 'seal_dispute' ? '章末评语：本章多次选择封存争议，下章宜先回看证据再开新赌注。' : '章末评语：本章持续守住主战线，下章可以继续把强目标压成稳定成果。' : rescueOpen ? '章中评语：章内证据尚薄，先用本周主线补齐缺口。' : '章中评语：三周节奏正常推进，等待更多裁记沉淀。';
    const feedbackLine = chapterClosed ? dominantChoiceId === 'rebalance_support' ? '本章收成：副线证据已被保住，下章开局更适合先沿旁证节奏铺样。' : dominantChoiceId === 'seal_dispute' ? '本章收成：争议已被压成封存口径，下章开局宜先稳证据、少开新赌注。' : '本章收成：主战线已经稳住，下章开局可以继续把强目标压成成果。' : hasDebtPressure ? '本章反馈：当前仍被欠卷压力拖住，先清强目标，再谈这一章的收成。' : hasRiskPressure ? '本章反馈：当前仍属险卷压强，先补外场或旁证，再判断章末收法。' : rescueOpen ? `本章反馈：第 ${currentWeekSlot}/3 周仍有裁记缺口，先补齐再判断章末收成。` : `本章反馈：当前章势正在形成，继续沿${dominantChoiceLabel ? `【${dominantChoiceLabel}】` : '本周主线'}累积裁记。`;
    const chapterLabel = currentWeekMeta ? `第 ${chapterNumber} 章` : '本章';
    const windowLabel = currentWeekMeta ? `${currentWeekMeta.year} · 第 ${windowStartWeekNo}-${windowEndWeekNo} 周` : '三周窗口';
    ...
    const carryover = (() => {
      ...
      const resultId = previousDominantChoiceId === 'seal_dispute' ? 'archived_dispute' : previousDominantChoiceId === 'rebalance_support' ? 'balanced_front' : 'guarded_front';
      const resultLabel = resultId === 'archived_dispute' ? '封争收束' : resultId === 'balanced_front' ? '补证收束' : '守线收束';
      const preferredLaneId = previousDominantChoiceId === 'rebalance_support' ? previousSummary.latestSupportLaneId || 'expedition' : 'training';
      const preferredLaneLabel = previousDominantChoiceId === 'rebalance_support' ? previousSummary.latestSupportLaneLabel || laneLabelMap[preferredLaneId] || '远征线' : laneLabelMap[preferredLaneId] || '训练线';
      const previousChapterLabel = `第 ${Math.max(1, chapterNumber - 1)} 章`;
      const summaryLine = previousDominantChoiceId === 'rebalance_support' ? `上章承卷：${previousChapterLabel} 以【${previousDominantChoiceLabel || '副线补证'}】收束，新章开局会轻量前移【${preferredLaneLabel}】。` : previousDominantChoiceId === 'seal_dispute' ? `上章承卷：${previousChapterLabel} 以【${previousDominantChoiceLabel || '封存争议'}】收束，新章开局会先回【${preferredLaneLabel}】补证。` : `上章承卷：${previousChapterLabel} 以【${previousDominantChoiceLabel || '守主战线'}】收束，新章开局会先稳住【${preferredLaneLabel}】主轴。`;
      return {
        available: true,
    ...
    const objective = (() => {
      const objectiveLaneId = hasDebtPressure ? 'verification' : hasRiskPressure ? context.nextTask?.laneId || 'verification' : rescueOpen ? dominantChoiceId === 'rebalance_support' ? currentSummary.latestSupportLaneId || context.nextTask?.laneId || 'expedition' : dominantChoiceId === 'seal_dispute' ? context.nextTask?.laneId || 'training' : context.nextTask?.laneId || 'training' : chapterClosed ? dominantChoiceId === 'rebalance_support' ? currentSummary.latestSupportLaneId || context.nextTask?.laneId || 'expedition' : context.nextTask?.laneId || 'training' : context.nextTask?.laneId || 'training';
      const objectiveLaneLabel = laneLabelMap[objectiveLaneId] || '本周主线';
      const statusId = hasDebtPressure ? 'debt_clear' : hasRiskPressure ? 'risk_stabilize' : rescueOpen ? 'chapter_rescue' : chapterClosed ? 'chapter_close' : 'chapter_progress';
      const statusLabel = hasDebtPressure ? '先清债账' : hasRiskPressure ? '先补外场' : rescueOpen ? dominantChoiceId === 'rebalance_support' ? '副线补证' : dominantChoiceId === 'seal_dispute' ? '封存争议' : '守主战线' : chapterClosed ? '章末定型' : '章中推进';
      const summaryLine = hasDebtPressure ? `章目标：本章仍被欠卷压住，先把【${objectiveLaneLabel}】补成可结算事实。` : hasRiskPressure ? `章目标：本章处在险卷压强里，先让【${objectiveLaneLabel}】补上外场或旁证。` : rescueOpen ? `章目标：本章要先补齐 ${sealedCount}/3 周裁记缺口，当前重心偏向【${objectiveLaneLabel}】。` : chapterClosed ? `章目标：本章已收束为【${dominantChoiceLabel || '裁记风格'}】，下章经营会沿这个口径继续。` : `章目标：本章正在形成【${dominantChoiceLabel || '当前主线'}】的收束样貌。`;
      const goalLine = hasDebtPressure ? '优先清掉欠卷占位，再谈章末收束。' : hasRiskPressure ? '先补外场或旁证，再判断章末收法。' : rescueOpen ? `第 ${currentWeekSlot}/3 周仍有裁记缺口，先补齐再判断章末收成。` : chapterClosed ? '三周裁记已齐，接下来只需维持当前章势。' : '继续沿当前主线补齐章内证据。';
      const reasonLine = hasDebtPressure ? context.debtPack?.summaryLine || '本章仍有欠卷占住强目标位，需要先清账再归章。' : hasRiskPressure ? context.settlement?.summaryLine || '本章押卷仍属险卷，需要补一条外场或旁证。' : rescueOpen ? rescueReasonLine : finalCommentLine || endingPreviewLine || '章目标已进入稳定收束阶段。';
      return {
        available: true,
    ...
    return {
      ...
      goalLine: chapterClosed ? '本章三周裁记已齐，章末评语可用于复盘下章主线。' : currentWeekSlot >= 3 ? '章末周优先补齐本章裁记缺口，再回看本章主线是否成立。' : '继续沿本周季盘行动推进，等待三周裁记形成章末评语。',
  ...
  normalizeSeasonBoardChapterArc(source = null, context = {}) {
    ...
    const hasDerivedContext = !!(context.weekTag || context.weekLabel || context.weekVerdictLedger?.current || context.seasonVerificationState);
    ...
    const entries = (Array.isArray(root.entries) ? root.entries : []).filter(entry => entry && typeof entry === 'object').slice(0, 3).map((entry, index) => ({
    ...
    return {
      ...
      pressureWindow: {
        available: pressureRoot.available !== false && !!(pressureRoot.summaryLine || pressureRoot.statusLine || pressureRoot.reasonLine || pressureRoot.guideLine || pressureRoot.shortLine || pressureRoot.statusLabel),
      ...
      objective: {
        available: objectiveRoot.available !== false && !!(objectiveRoot.summaryLine || objectiveRoot.statusLine || objectiveRoot.goalLine || objectiveRoot.reasonLine),
  ...
  followSeasonBoardTask(taskId = '') {
    ...
    const sanctumData = typeof this.game.getSanctumOverviewData === 'function' ? this.game.getSanctumOverviewData() : null;
    if (sanctumData?.seasonBoard && typeof sanctumData.seasonBoard === 'object') {
      boardCandidates.push(sanctumData.seasonBoard);
    }
    const rewardMeta = typeof this.game.rewardView?.getRewardExpeditionMeta === 'function' ? this.game.rewardView.getRewardExpeditionMeta() : null;
    if (rewardMeta?.seasonBoard && typeof rewardMeta.seasonBoard === 'object') {
      boardCandidates.push(rewardMeta.seasonBoard);
    }
    const liveBoard = typeof this.getSeasonBoardSnapshot === 'function' ? this.getSeasonBoardSnapshot() : null;
    if (liveBoard && typeof liveBoard === 'object') {
      boardCandidates.push(liveBoard);
    }
    const board = boardCandidates.find(candidate => candidate && typeof candidate === 'object' && (candidate.nextTask || Array.isArray(candidate.lanes) && candidate.lanes.some(lane => Array.isArray(lane?.tasks) && lane.tasks.length > 0))) || null;
    ...
    const targetTask = tasks.find(entry => entry.id === targetTaskId) || (board.nextTask && typeof board.nextTask === 'object' ? {
      ...board.nextTask,
      laneId: board.nextTask.laneId || '',
      laneLabel: board.nextTask.laneLabel || ''
    } : null) || tasks.find(entry => !entry.completed) || tasks[0] || null;
    ...
    const isCollectionAction = actionType === 'collection' || !actionType && !['challenge', 'pvp', 'endless', 'map'].includes(anchorSection) && !['pvp-screen', 'map-screen'].includes(actionValue);
    const taskFollowNotice = {
      sourceKey: 'task',
      action: isCollectionAction ? 'collection' : actionType || (anchorSection === 'challenge' ? 'challenge' : 'screen'),
      value: isCollectionAction ? actionValue || anchorSection || 'sanctum' : actionValue || anchorSection || '',
    ...
    this.game.lastSeasonBoardTaskFollowNotice = {
      ...taskFollowNotice
    };
  ...
  getSeasonBoardSignalSnapshot(options = {}) {
    const baseSignals = typeof this.game.getHeavenlyMandateSignalSnapshot === 'function' ? this.game.getHeavenlyMandateSignalSnapshot(options) : {};
    const overrideSlate = options.latestSlate && typeof options.latestSlate === 'object' ? options.latestSlate : null;
    const latestSlate = overrideSlate || (baseSignals.latestSlate && typeof baseSignals.latestSlate === 'object' ? baseSignals.latestSlate : null);
    ...
    const latestSlateWeekTag = latestSlateAt > 0 ? String(this.game.getHeavenlyMandateWeekMeta(latestSlateAt)?.weekTag || '').trim() : String(baseSignals.latestSlateWeekTag || '').trim();
    ...
    }) : baseSignals.agendaSnapshot && typeof baseSignals.agendaSnapshot === 'object' ? baseSignals.agendaSnapshot : null;
    ...
    const currentChapterIndex = Math.max(0, Math.floor(Number(expeditionState?.chapterIndex) || 0), Math.floor(Number(latestSlate?.chapterIndex) || 0), Math.floor(Number(lastAgenda?.boundChapterIndex) || 0), Math.floor(Number(activeAgenda?.boundChapterIndex) || 0));
    const lineage = typeof this.game.getFateLineageSnapshot === 'function' ? this.game.getFateLineageSnapshot({
      latestSlate
    }) : null;
    const aftereffects = typeof this.game.getFateAftereffectSnapshot === 'function' ? this.game.getFateAftereffectSnapshot({
      latestRunId: String(latestSlate?.id || ''),
      latestSlate,
      currentChapterIndex,
      expeditionState
    }) : null;
    const seasonVerification = typeof this.game.getSeasonVerificationSnapshot === 'function' ? this.game.getSeasonVerificationSnapshot({
  ...
  getSeasonBoardPhaseMeta(snapshot = null) {
    ...
    const resolvedVerificationCount = Math.max(0, Math.floor(Number(source.seasonVerification?.verifiedCount) || 0)) + Math.max(0, Math.floor(Number(source.seasonVerification?.failedCount) || 0));
    const verificationReady = weeklyScore >= 360 || endlessClears > 0 || pvpSeasonMatchCount > 0 || aftereffectActive > 0 || resolvedVerificationCount > 0;
    ...
    const locklineReady = !!source.latestSlate || !!source.activeAgenda || !!source.lastAgenda || !!source.selectedGuide || !!source.trainingFocus;
    if (locklineReady) {
      return {
        id: 'lockline',
        label: '锁线期',
  ...
  normalizeSeasonBoardVerificationOrder(source = null, index = 0) {
    ...
    const next = {
      ...
      role: ['primary', 'side'].includes(String(root.role || '').trim()) ? String(root.role).trim() : index === 0 ? 'primary' : 'side',
  ...
  buildSeasonBoardSettlementState(signals = {}, phase = null) {
    ...
    const lastAgenda = safeSignals.lastAgenda && typeof safeSignals.lastAgenda === 'object' ? safeSignals.lastAgenda : null;
    const aftereffects = safeSignals.aftereffects && typeof safeSignals.aftereffects === 'object' ? safeSignals.aftereffects : null;
    const primaryAftereffect = aftereffects?.primary && typeof aftereffects.primary === 'object' ? aftereffects.primary : null;
    const seasonVerification = safeSignals.seasonVerification && typeof safeSignals.seasonVerification === 'object' ? safeSignals.seasonVerification : null;
    const primaryVerification = seasonVerification?.primary && typeof seasonVerification.primary === 'object' ? seasonVerification.primary : null;
    const sideVerification = seasonVerification?.side && typeof seasonVerification.side === 'object' ? seasonVerification.side : null;
    ...
    const hasDebt = !!(lastAgenda?.recoveryEligible || lastAgenda?.selectedContractLabel && lastAgenda?.contractResolved && lastAgenda?.contractSuccess === false || ['contract_miss', 'recovery'].includes(String(primaryAftereffect?.outcomeId || '').trim()) || lastAgenda?.outcome && !['success', 'completed'].includes(String(lastAgenda.outcome || '').trim()));
    const sourceRunId = String(lastAgenda?.sourceRunId || primaryAftereffect?.sourceRunId || safeSignals.latestSlate?.id || '').trim();
    const sourceSlate = sourceRunId && Array.isArray(this.game.runSlateArchive) ? this.game.runSlateArchive.find(entry => String(entry?.id || '').trim() === sourceRunId) || null : null;
    const chapterIndex = Math.max(0, Math.floor(Number(lastAgenda?.boundChapterIndex ?? lastAgenda?.chapterIndex ?? primaryAftereffect?.chapterIndex ?? safeSignals.latestSlate?.chapterIndex ?? 0) || 0));
    ...
    const currentWeekIndex = Math.max(0, Math.floor(Number(safeSignals.weekIndex) || 0), toWeekIndex(currentWeekTag));
    const openedAt = Math.max(0, Math.floor(Number(lastAgenda?.updatedAt ?? lastAgenda?.resolvedAt ?? lastAgenda?.completedAt ?? lastAgenda?.selectedAt ?? primaryAftereffect?.createdAt ?? sourceSlate?.timestamp ?? (safeSignals.latestSlate?.id === sourceRunId ? safeSignals.latestSlate?.timestamp : 0) ?? 0) || 0));
    const openedWeekMeta = openedAt > 0 && typeof this.game.getHeavenlyMandateWeekMeta === 'function' ? this.game.getHeavenlyMandateWeekMeta(openedAt) : null;
    const openedWeekTag = String(lastAgenda?.openedWeekTag || primaryAftereffect?.openedWeekTag || openedWeekMeta?.weekTag || '').trim();
    const openedWeekIndex = Math.max(0, Math.floor(Number(openedWeekMeta?.weekIndex) || 0), toWeekIndex(openedWeekTag));
    const weekDelta = hasDebt && currentWeekTag && openedWeekTag && currentWeekTag !== openedWeekTag ? Math.max(1, currentWeekIndex > 0 && openedWeekIndex > 0 ? currentWeekIndex - openedWeekIndex : 1) : 0;
    ...
    const debtStatus = hasDebt ? debtClearedByVerification ? 'cleared' : debtDegradedByVerification ? 'degraded' : debtDegradedByDelay ? 'degraded' : weekDelta > 0 ? 'deferred' : 'open' : '';
    const debtOccupiesStrongSlot = ['open', 'deferred'].includes(debtStatus);
    const occupiedMandateTaskId = debtOccupiesStrongSlot ? `season_mandate_debt_focus_${String(lastAgenda?.agendaId || sourceRunId || currentWeekTag || 'current').trim().slice(0, 40)}` : '';
    const occupationReason = debtStatus === 'deferred' ? `旧周欠卷已带入 ${currentWeekTag || safeSignals.weekLabel || '本周'}，强目标先让位给清账。` : debtDegradedByDelay ? `这笔欠卷已拖延 ${debtDelayCount} 周，当前降级为反证，不再继续占据强目标。` : debtStatus === 'open' ? '本周刚形成欠卷，强目标先切到清账。' : debtStatus === 'cleared' ? '主验证已清账，强目标重新释放给赛季推进。' : '主验证给出反证，欠卷改记为反例，不再继续占据强目标。';
    const debtProgressText = debtStatus === 'deferred' ? `已拖延 ${Math.max(1, weekDelta)} 周` : debtDegradedByDelay ? `拖延 ${debtDelayCount} 周后降级` : debtStatus === 'cleared' ? '已清账' : debtStatus === 'degraded' ? '反证已入账' : primaryAftereffect?.remainingChapters > 0 ? `清账窗口 ${Math.max(1, Math.floor(Number(primaryAftereffect.remainingChapters) || 1))} 章` : '本周内优先清账';
    const debtStatusLine = debtStatus === 'deferred' ? `欠卷已拖延 ${Math.max(1, weekDelta)} 周` : debtDegradedByDelay ? `欠卷已拖延 ${debtDelayCount} 周，现转为反证` : debtStatus === 'cleared' ? primaryVerification?.statusLine || '欠卷已清账' : debtStatus === 'degraded' ? primaryVerification?.statusLine || '欠卷已转为反证' : primaryAftereffect?.statusLine || '欠卷待清账';
    const debtGuideLine = debtStatus === 'deferred' ? `这笔欠卷已带入 ${safeSignals.weekLabel || currentWeekTag || '本周'}，先清账，再决定这一周是否继续冲定榜验证。` : debtDegradedByDelay ? `这笔欠卷已经拖延 ${debtDelayCount} 周，当前先按反证收束主轴，再决定是否重新建立高压验证。` : debtStatus === 'cleared' ? primaryVerification?.writebackLine || '主验证已经完成清账，可以回到定榜推进。' : debtStatus === 'degraded' ? primaryVerification?.writebackLine || '这笔欠卷已经转成反证，先修正主轴再继续扩线。' : '先处理上一道承诺留下的欠卷，再决定这一周是否值得继续冲定榜验证。';
    ...
    if (safePhase.id === 'lockline') {
      ...
      if (primaryVerified) {
        ...
        outcomeId = hasCrossVerification ? 'positive_sheet' : 'risky_sheet';
      }
    }
    const labelCatalog = {
      sampling_sheet: '待采样',
    ...
    const proofQuality = String(primaryVerification?.proofQuality || sideVerification?.proofQuality || (hasCrossVerification ? 'solid' : '')).trim();
    ...
    const resolvedStatus = primaryVerified ? 'verified' : primaryFailed ? 'failed' : sideVerified ? 'reinforced' : debtStatus || 'pending';
    const settlementSourceRunId = String(primaryVerification?.resolvedRunId || sourceRunId || '').trim();
    const settlementChapterIndex = Math.max(chapterIndex, Math.floor(Number(primaryVerification?.chapterIndex) || 0), Math.floor(Number(sideVerification?.chapterIndex) || 0));
    const summaryCatalog = {
      sampling_sheet: '当前仍在采样，本周主轴还没有结成可押卷的赛季答卷。',
      locking_sheet: '当前样本已经开始押卷，但还要继续把章节承诺真正锁进洞府与归卷。',
      positive_sheet: primaryVerified ? debtClearedByVerification ? '主验证已经回写，本周欠卷已清，本周押卷改判为「正卷」。' : '主验证已经回写，本周押卷升级为「正卷」。' : '本周押卷已结成「正卷」，这条主轴已经通过至少一条高压验证。',
      risky_sheet: primaryFailed ? '主验证给出了反证，本周押卷暂降为「险卷」，需要重新校准主轴。' : debtDegradedByDelay ? `这笔欠卷已连续拖延 ${debtDelayCount} 周，当前从强目标位降为「险卷」反证，需要先收紧主轴再继续推进。` : '本周押卷已结成「险卷」，路线已成形，但还需要外场验证决定能否真正定榜。',
      debt_sheet: sideVerified ? '旁验证已经补上，但上一道承诺留下的欠卷仍待主验证清账。' : '本周押卷暂结成「欠卷」，上一道承诺留下了待清的研究债账。'
    };
    const detailCatalog = {
      sampling_sheet: safeSignals.selectedGuide?.title ? `当前主练样本：${safeSignals.selectedGuide.title}。先把它压成一张本周可复盘的章节答卷。` : '先锁一份精选命盘或本周归卷，再决定真正要押哪条主线。',
      locking_sheet: lastAgenda?.phaseLine || lastAgenda?.focusNodeLine || lastAgenda?.summaryLine || '当前押卷正在推进，先把承诺写成真正的章节留痕。',
      positive_sheet: [primaryVerification?.summaryLine || '', primaryVerification?.detailLine || primaryWritebackLine || '', sideVerified ? sideVerification?.summaryLine || sideWritebackLine || '' : '', !primaryVerified && lastAgenda?.contractResolutionLine ? lastAgenda.contractResolutionLine : '', !primaryVerified && endlessClears > 0 ? `无尽已清 ${endlessClears} 轮` : '', !primaryVerified && pvpMatches > 0 ? `天道榜已留 ${pvpMatches} 场账本` : ''].filter(Boolean).slice(0, 2).join('｜'),
      risky_sheet: [debtDegradedByDelay ? primaryAftereffect?.detailLine || '' : primaryFailed ? primaryVerification?.summaryLine || '' : '', debtDegradedByDelay ? `延账 ${debtDelayCount} 周后已从强目标位降级，不再继续占用本周主舞台。` : !primaryFailed && sideVerified ? sideVerification?.summaryLine || sideWritebackLine || '' : '', debtDegradedByDelay ? lastAgenda?.recoveryHintLine || lastAgenda?.contractResolutionLine || '' : primaryFailed ? primaryVerification?.detailLine || primaryWritebackLine || '' : lastAgenda?.contractResolutionLine || '', debtDegradedByDelay || primaryFailed ? '' : '当前主轴还缺最后一轮高压验算，适合继续补无尽或天道榜样本。'].filter(Boolean).slice(0, 2).join('｜'),
      debt_sheet: [primaryAftereffect?.detailLine || '', sideVerified ? sideVerification?.summaryLine || sideWritebackLine || '' : '', lastAgenda?.recoveryHintLine || lastAgenda?.recoveryLine || lastAgenda?.reasonLine || ''].filter(Boolean).slice(0, 2).join('｜')
    };
    const progressCatalog = {
      sampling_sheet: '等待第一张归卷',
      locking_sheet: lastAgenda ? `${Math.max(0, Math.floor(Number(lastAgenda.progress) || 0))}/${Math.max(1, Math.floor(Number(lastAgenda.target) || 1))} 项押卷推进` : '等待洞府承诺立项',
      positive_sheet: primaryVerified ? `${primaryVerification?.sourceModeLabel || '主验证'} 已回写` : `无尽 ${endlessClears}/1 · 天道榜 ${Math.min(pvpMatches, 2)}/2`,
      risky_sheet: primaryFailed ? '反证已入账' : debtDegradedByDelay ? `拖延 ${debtDelayCount} 周后降级` : `待补外场验证 ${hasCrossVerification ? 0 : 1} 条`,
      debt_sheet: primaryAftereffect?.remainingChapters > 0 ? `清账窗口 ${Math.max(1, Math.floor(Number(primaryAftereffect.remainingChapters) || 1))} 章` : '本周内优先清账'
    };
    const guideCatalog = {
      sampling_sheet: '先把本周主练样本压成一张可复盘答卷，再决定要不要立项押卷。',
      locking_sheet: '继续沿洞府承诺推进，先把押卷锁进章节结算，再决定后续扩线方向。',
      positive_sheet: primaryVerified ? sideVerified ? '主验证与旁验证都已回写，可以继续沿当前主修扩样本。' : '主验证已回写，可继续补一张不同节奏的旁验证巩固这条正卷。' : hasCrossVerification ? '当前正卷已经站住脚，主验证可继续补无尽或天道榜，旁验证可去七日劫数复盘。' : '主轴已经成形，但还没留下真正的高压验证样本。',
      risky_sheet: primaryFailed ? '先根据反证收紧路线，再决定是补旁证、换验证入口，还是重新建立主验证。' : debtDegradedByDelay ? `这笔欠卷已经拖延 ${debtDelayCount} 周，当前先按反证收束主轴，再决定是否重新建立高压验证。` : sideVerified ? '旁验证已经补齐，但仍需把这条险卷送去无尽或天道榜做真正的主验证。' : '先把当前险卷送去无尽或天道榜做一次高压验证，再用七日劫数补一张挑战旁证。',
      debt_sheet: sideVerified ? '旁验证已经给出补强，但仍需主验证真正清账后才能释放本周定榜节奏。' : '先处理上一道承诺留下的欠卷，再决定这一周是否值得继续冲定榜验证。'
    };
    const statusCatalog = {
      sampling_sheet: '待形成可押卷主轴',
      locking_sheet: '洞府承诺推进中',
      positive_sheet: primaryVerified ? [debtClearedByVerification ? '欠卷已清' : '主验证通过', primaryVerification?.sourceModeLabel || '', proofQualityLabel].filter(Boolean).join(' · ') : '正卷已通过至少一条外场验证',
      risky_sheet: primaryFailed ? ['主验证失利', primaryVerification?.sourceModeLabel || '', proofQualityLabel].filter(Boolean).join(' · ') : debtDegradedByDelay ? `延账 ${debtDelayCount} 周 · 已降级` : sideVerified ? ['旁验证已补', sideVerification?.sourceModeLabel || '', '险卷待主验证'].filter(Boolean).join(' · ') : '险卷待做外场定榜验证',
      debt_sheet: sideVerified ? ['旁验证已补', sideVerification?.sourceModeLabel || '', debtStatusLine].filter(Boolean).join(' · ') : '欠卷待清账'
    };
    const settlement = this.normalizeSeasonBoardSettlement({
      ...
      resolutionTier: primaryVerified ? debtClearedByVerification ? 'recovered' : 'confirmed' : primaryFailed || debtDegradedByDelay ? 'degraded' : outcomeId === 'risky_sheet' ? 'provisional' : outcomeId === 'debt_sheet' ? 'debt' : outcomeId === 'locking_sheet' ? 'commit' : 'sampling',
      resolvedStatus,
    ...
    const debtPack = debtStatus ? this.normalizeSeasonBoardDebtPack({
      ...
      summaryLine: debtStatus === 'cleared' ? `研究债账已清：${String(primaryVerification?.summaryLine || primaryWritebackLine || lastAgenda?.recoveryLine || '这笔欠卷已经完成清账。').trim()}` : debtStatus === 'degraded' ? `${debtDegradedByDelay ? '研究债账拖延降级' : '研究债账转为反证'}：${String(primaryVerification?.summaryLine || primaryWritebackLine || lastAgenda?.reasonLine || '这笔欠卷已经转成反证。').trim()}` : primaryAftereffect?.summaryLine ? `研究债账：${String(primaryAftereffect.summaryLine || '').trim()}` : `研究债账：${String(lastAgenda?.recoveryLine || lastAgenda?.reasonLine || '上一道押卷承诺仍待清账。').trim()}`,
      detailLine: [debtStatus === 'cleared' || debtStatus === 'degraded' ? primaryVerification?.detailLine || primaryWritebackLine || '' : primaryAftereffect?.detailLine || '', sideVerified ? sideVerification?.summaryLine || sideWritebackLine || '' : '', lastAgenda?.recoveryHintLine || lastAgenda?.contractResolutionLine || ''].filter(Boolean).slice(0, 2).join('｜'),
      guideLine: debtGuideLine,
      statusLine: debtStatusLine,
      progressText: debtProgressText,
      settleWindowText: debtStatus === 'deferred' ? `已带入 ${safeSignals.weekLabel || currentWeekTag || '本周'}` : debtStatus === 'cleared' ? '本周已清' : debtStatus === 'degraded' ? debtDegradedByDelay ? `拖延 ${debtDelayCount} 周后降级` : '反证归档' : primaryAftereffect?.remainingChapters > 0 ? `剩余 ${Math.max(1, Math.floor(Number(primaryAftereffect.remainingChapters) || 1))} 章` : '本周内清账',
      recommendedValidationLabel: debtStatus === 'cleared' ? '清账已回写' : debtStatus === 'degraded' ? '反证归档' : '清债验证',
      recommendedAnchorSection: String(primaryVerification?.anchorSection || sideVerification?.anchorSection || (safeSignals.endlessSeason ? 'endless' : safeSignals.pvpSeason ? 'pvp' : 'sanctum')).trim(),
      status: debtStatus,
    ...
    const challengeSideVerificationOrder = !debtPending && ['positive_sheet', 'risky_sheet'].includes(String(settlement?.outcomeId || '').trim()) ? {
      ...
      summaryLine: settlement?.outcomeId === 'positive_sheet' ? weeklyScore >= weeklyChallengeTarget ? '用七日劫数继续复盘这条正卷，补一张不同节奏的旁验证。' : '去七日劫数打一轮，把当前正卷补成一张挑战旁证。' : weeklyScore >= weeklyChallengeTarget ? '先用七日劫数复盘当前险卷，再决定去无尽还是天道榜冲定榜。' : '先去七日劫数打一轮，把当前险卷压成一张挑战旁证。',
      hintLine: safeSignals.weeklyBundle ? `${weeklyChallengeName} · 当前 ${weeklyScore} 分${weeklyArchiveCount > 0 ? ` · 已归档 ${weeklyArchiveCount} 份样本` : ''}` : '周挑战会用固定命盘复刻这周主练，适合补一张挑战旁证。',
    ...
    const crossModeFollowupOrder = {
      ...
      summaryLine: debtPending ? '欠卷清掉后，再把这条主轴送去无尽或天道榜补齐定榜证明。' : needEndless ? '补一场真实对局账本，让本周主轴留下外场反证。' : '再补一轮无尽样本，确认这条路线在更长压力线上依然成立。',
      hintLine: debtPending ? '先清账，再定榜，避免把未结的旧账继续压到下一周。' : needEndless ? `${safeSignals.pvpSeasonName || '天道榜'} 会更快给出真实外场样本。` : `${safeSignals.endlessSeason?.name || '无尽轮回'} 更适合补长线压强证明。`,
      statusLine: debtPending ? '清账后解锁' : `无尽 ${endlessClears}/1 · 天道榜 ${Math.min(pvpMatches, 2)}/2`,
    ...
    const fallbackPrimaryOrder = debtPending ? {
      ...
      summaryLine: needEndless ? '先补 1 轮无尽轮回，把当前主轴送进更高压环境继续验算。' : needPvp ? '再补 1-2 场天道榜账本，确认这条路线不是只在章节里成立。' : '无尽与天道榜都已留痕，可继续冲更高分或反例样本。',
      hintLine: needEndless ? `${safeSignals.endlessSeason?.name || '无尽轮回'} 当前仍缺 1 轮跨模证明。` : needPvp ? `${safeSignals.pvpSeasonName || '天道榜'} 还缺 ${Math.max(0, 2 - pvpMatches)} 场账本。` : '当前正卷已经具备定榜基础，可继续扩样本或冲更高压证明。',
      statusLine: `无尽 ${endlessClears}/1 · 天道榜 ${Math.min(pvpMatches, 2)}/2`,
      anchorSection: needEndless ? 'endless' : 'pvp',
      priority: 1
    };
    const verificationOrders = [explicitPrimaryOrder || fallbackPrimaryOrder, explicitSideOrder || challengeSideVerificationOrder || crossModeFollowupOrder].map((entry, index) => this.normalizeSeasonBoardVerificationOrder({
      ...entry,
  ...
  formatSeasonBoardLaneRewardGainLine(gains = {}) {
    ...
    return [insight > 0 ? `天机 +${insight}` : '', karma > 0 ? `业果 +${karma}` : '', ringExp > 0 ? `命环经验 +${ringExp}` : '', gold > 0 ? `灵石 +${gold}` : ''].filter(Boolean).join('，') || '赛季留痕 +1';
  }
  getSeasonBoardLaneRewardClaim(weekTag = '', laneId = '') {
    ...
    const state = typeof this.game.normalizeSeasonVerificationState === 'function' ? this.game.normalizeSeasonVerificationState(this.game.seasonVerificationState) : this.game.seasonVerificationState || {};
  ...
  buildSeasonBoardLaneRewards(lanes = [], context = {}) {
    ...
    return (Array.isArray(lanes) ? lanes : []).map(lane => {
      ...
      return {
        ...
        detailLine: completed ? `${laneLabel}已达成 ${completedCount}/${Math.max(1, totalCount)}，本周可结算一次。` : `${laneLabel}还差 ${Math.max(0, totalCount - completedCount)} 格结题。`,
        status,
        statusLabel,
        ready: completed,
  ...
  buildSeasonBoardFrontier(lanes = [], context = {}) {
    const safeLanes = (Array.isArray(lanes) ? lanes : []).filter(lane => lane && typeof lane === 'object').slice(0, 3);
    ...
    const verificationOrders = Array.isArray(context.verificationOrders) ? context.verificationOrders.filter(entry => entry && typeof entry === 'object') : [];
    ...
    const phaseDefaultLaneId = phaseId === 'ranking' ? 'verification' : phaseId === 'lockline' ? 'expedition' : 'training';
    const primaryFrontId = hasOpenDebt || hasDebtVerification ? 'verification' : nextTask?.laneId && laneById.has(nextTask.laneId) ? nextTask.laneId : phaseDefaultLaneId;
    ...
    const firstOpenTaskForLane = (lane = null) => {
      ...
      return lane.tasks.find(task => task && !task.completed) || lane.tasks.find(task => task && typeof task === 'object') || null;
    };
    const actionTask = nextTask?.laneId === primaryFrontId ? nextTask : firstOpenTaskForLane(primaryLane);
    const actionMeta = typeof this.game.getSeasonVerificationActionMeta === 'function' ? this.game.getSeasonVerificationActionMeta(actionTask?.actionValue || actionTask?.anchorSection || primaryCatalog.anchorSection || 'sanctum', {
      fallbackSection: 'sanctum'
    }) : {
    ...
    const pressureScore = hasOpenDebt ? 3 : nextTask?.laneId === primaryFrontId ? 2 : laneComplete ? 0 : phaseId === 'ranking' && primaryFrontId === 'verification' ? 2 : 1;
    ...
    const buildItem = (laneId, index) => {
      ...
      const completed = totalCount > 0 && completedCount >= totalCount;
      const lanePressureScore = laneId === primaryFrontId ? pressureScore : completed ? 0 : 1;
      ...
      const itemActionMeta = typeof this.game.getSeasonVerificationActionMeta === 'function' ? this.game.getSeasonVerificationActionMeta(itemTask?.actionValue || itemTask?.anchorSection || meta.anchorSection || 'sanctum', {
        fallbackSection: 'sanctum'
      }) : {
        actionType: 'collection',
      ...
      return {
        ...
        actionType: String(itemTask?.actionType || itemActionMeta.actionType || '').trim().slice(0, 24) || itemActionMeta.actionType || 'collection',
        actionValue: String(itemTask?.actionValue || itemActionMeta.actionValue || '').trim().slice(0, 40) || itemActionMeta.actionValue || 'sanctum',
        ctaLabel: String(itemTask?.ctaLabel || itemActionMeta.ctaLabel || '前往推进').trim().slice(0, 24) || '前往推进',
        actionTargetLabel: String(itemActionMeta.targetLabel || itemTask?.anchorSection || itemActionMeta.actionValue || '当前主线').trim().slice(0, 24) || '当前主线',
        priority: laneId === primaryFrontId ? 1 : role === 'support' ? 2 : 3
      };
    };
    const items = ['training', 'expedition', 'verification'].map((laneId, index) => buildItem(laneId, index)).filter(Boolean).sort((a, b) => a.priority - b.priority);
    const primaryItem = items.find(item => item.id === primaryFrontId) || items[0] || null;
    const actionType = String(actionTask?.actionType || actionMeta.actionType || '').trim().slice(0, 24) || actionMeta.actionType || 'collection';
    const actionValue = String(actionTask?.actionValue || actionMeta.actionValue || '').trim().slice(0, 40) || actionMeta.actionValue || 'sanctum';
    const ctaLabel = String(actionTask?.ctaLabel || actionMeta.ctaLabel || '前往推进').trim().slice(0, 24) || '前往推进';
    const actionTargetLabel = String(actionMeta.targetLabel || actionTask?.anchorSection || actionValue || '当前主线').trim().slice(0, 24) || '当前主线';
    const source = hasOpenDebt ? 'debt_pack' : String(nextTask?.source || '').trim() || (actionTask ? 'lane' : 'season_board');
    const sourceId = hasOpenDebt ? String(debtPack?.id || '').trim().slice(0, 96) : String(nextTask?.sourceId || actionTask?.sourceId || actionTask?.id || '').trim().slice(0, 96);
    const guideSeed = hasOpenDebt ? debtPack?.guideLine || debtPack?.summaryLine || '先把欠卷清回可验证状态。' : actionTask?.hintLine || actionTask?.statusLine || primaryLane.summaryLine || '沿当前主战线补齐下一格。';
    const frontier = {
      available: true,
      ...
      detailLine: String([primaryItem?.progressText ? `进度 ${primaryItem.progressText}` : '', hasOpenDebt ? debtPack?.progressText || debtPack?.statusLine || '' : '', actionTask?.label || ''].filter(Boolean).join(' · ')).trim().slice(0, 220),
  ...
  buildSeasonBoardFrontierDecree(frontier = null, context = {}) {
    ...
    const statusId = String(root.statusId || '').trim().slice(0, 24) || (pressureScore >= 3 ? 'high_pressure' : pressureScore >= 2 ? 'pressure' : pressureScore >= 1 ? 'pending' : 'stable');
    const statusLabel = String(root.statusLabel || root.pressureLabel || '').trim().slice(0, 24) || (pressureScore >= 3 ? '高压' : pressureScore >= 2 ? '承压' : pressureScore >= 1 ? '待补样' : '稳态');
  ...
  buildSeasonBoardFrontierChronicle(frontier = null, context = {}) {
    ...
    const items = Array.isArray(root.items) ? root.items.filter(entry => entry && typeof entry === 'object').slice(0, 3) : [];
    ...
    const progressParts = laneOrder.map(entryLaneId => {
      const item = itemByLane.get(entryLaneId);
      if (!item) return '';
      return `${laneNameMap[entryLaneId] || item.shortLabel || item.label || '战线'} ${item.progressText || '待同步'}`;
    }).filter(Boolean);
    const fallbackProgress = items.map(item => `${item.shortLabel || item.label || '战线'} ${item.progressText || '待同步'}`).filter(Boolean);
    const progressLine = `三线记录：${(progressParts.length > 0 ? progressParts : fallbackProgress).join(' · ') || '待同步'}。`;
    const lessonSeed = String(decree?.successLine || decree?.focusLine || root.guideLine || '先完成主战线一格，再回季盘复核副线。').trim();
    return {
      available: true,
  ...
  buildSeasonBoardFrontierCouncil(frontier = null, context = {}) {
    ...
    const laneOpinions = items.map((item, index) => {
      ...
      const role = String(item.role || (isPrimary ? 'primary' : completed ? 'reserve' : 'support')).trim().slice(0, 24) || (isPrimary ? 'primary' : completed ? 'reserve' : 'support');
      const stance = isPrimary ? pressureScore >= 2 ? 'press' : 'lead' : completed ? 'reserve' : 'defer';
      const stanceLabel = stance === 'press' ? '优先守线' : stance === 'lead' ? '先行复核' : stance === 'reserve' ? '证据待命' : '暂缓抢线';
      const opinionLaneLabel = String(item.shortLabel || item.label || (isPrimary ? laneLabel : '副战线')).trim().slice(0, 24) || (isPrimary ? laneLabel : '副战线');
      ...
      return {
        ...
        stance,
        stanceLabel,
        noteLine: isPrimary ? `主线意见：先完成【${opinionLaneLabel}】一格，再回季盘复核。` : completed ? `副线意见：【${opinionLaneLabel}】已留证，暂不抢主行动。` : `副线意见：【${opinionLaneLabel}】保持待补${progressText ? ` ${progressText}` : ''}，不新增第二行动。`
      };
    }).filter(opinion => opinion.laneId);
    const supportNames = laneOpinions.filter(opinion => opinion.laneId !== laneId).map(opinion => opinion.laneLabel).filter(Boolean);
    ...
    return {
      ...
      supportLine: supportNames.length > 0 ? `副线意见：${supportNames.join(' / ')}只保留证据，不切走主行动。` : '副线意见：暂无可拆分副线，保持当前主线推进。',
  ...
  buildSeasonBoardFrontierResolution(frontier = null, context = {}) {
    ...
    const debtPack = context.debtPack && typeof context.debtPack === 'object' ? context.debtPack : null;
    const weekVerdictCurrent = context.weekVerdictLedger?.current && typeof context.weekVerdictLedger.current === 'object' ? context.weekVerdictLedger.current : null;
    const council = root.council && typeof root.council === 'object' ? root.council : null;
    const chronicle = root.chronicle && typeof root.chronicle === 'object' ? root.chronicle : null;
    const supportOpinion = Array.isArray(council?.laneOpinions) ? council.laneOpinions.find(opinion => opinion && opinion.laneId && opinion.laneId !== laneId && opinion.role !== 'reserve') || council.laneOpinions.find(opinion => opinion && opinion.laneId && opinion.laneId !== laneId) || null : null;
    const supportLaneId = sanitize(weekVerdictCurrent?.frontierResolutionSupportLaneId || weekVerdictCurrent?.supportLaneId || supportOpinion?.laneId || '', 32);
    const supportLaneLabel = sanitize(weekVerdictCurrent?.frontierResolutionSupportLaneLabel || weekVerdictCurrent?.supportLaneLabel || supportOpinion?.laneLabel || '', 24);
    const settlementOutcomeId = sanitize(settlement?.outcomeId || weekVerdictCurrent?.settlementOutcomeId || '', 32);
    const settlementOutcomeLabel = sanitize(settlement?.outcomeLabel || weekVerdictCurrent?.settlementOutcomeLabel || '', 24);
    ...
    const primaryComplete = Array.isArray(root.items) ? !!root.items.find(item => (item?.laneId === laneId || item?.id === laneId) && item.completed) : false;
    const suggestedChoiceId = hardDebtActive ? 'hold_primary' : supportLaneId && primaryComplete ? 'rebalance_support' : 'hold_primary';
    const choiceCatalog = {
      hold_primary: {
        label: '守主战线',
    ...
    const rawChoiceId = sanitize(weekVerdictCurrent?.frontierResolutionChoiceId || weekVerdictCurrent?.resolutionChoiceId || weekVerdictCurrent?.choiceId || '', 32);
    const submittedChoiceId = Object.prototype.hasOwnProperty.call(choiceCatalog, rawChoiceId) ? rawChoiceId : '';
    const submitted = !!(weekVerdictCurrent?.frontierResolutionId || weekVerdictCurrent?.frontierResolutionSubmittedAt || weekVerdictCurrent?.chronicleSealStatus === 'sealed' || submittedChoiceId);
    ...
    const source = weekVerdictCurrent ? 'week_verdict_ledger' : settlement ? 'settlement' : 'frontier';
    const sourceId = sanitize(weekVerdictCurrent?.ledgerId || settlement?.id || root.id || '', 96);
    const choiceLabel = submitted ? sanitize(weekVerdictCurrent?.frontierResolutionLabel || weekVerdictCurrent?.choiceLabel || choiceMeta.label, 32) : '';
    const summaryLine = sanitize(weekVerdictCurrent?.frontierResolutionSummaryLine || weekVerdictCurrent?.councilResolutionLine || (submitted ? `本周会审裁记：采用【${choiceLabel || choiceMeta.label}】，${effectiveChoiceId === 'rebalance_support' ? `副线【${supportLaneLabel || '副战线'}】获得补证优先。` : effectiveChoiceId === 'seal_dispute' ? '争议先封入史卷，不改写排班。' : `主战线【${laneLabel}】继续优先。`}` : choiceMeta.summary), 220);
    const chronicleSealLine = sanitize(weekVerdictCurrent?.chronicleSealLine || (submitted ? `战役史卷已封记：${choiceLabel || choiceMeta.label} · ${laneLabel}${supportLaneLabel ? ` / ${supportLaneLabel}` : ''}。` : `战役史卷待封记：${chronicle?.nextRecordLine || `完成【${laneLabel}】一格后回季盘封记。`}`), 220);
    const councilResolutionLine = sanitize(weekVerdictCurrent?.councilResolutionLine || (submitted ? `诸界会审裁定：${summaryLine}` : `诸界会审待裁记：${council?.verdictLine || `先守【${laneLabel}】，副线保留证据。`}`), 220);
    return {
      available: true,
      submitted,
      id: sanitize(weekVerdictCurrent?.frontierResolutionId || `season_frontier_resolution_${weekTag || 'current'}_${submitted ? effectiveChoiceId : 'pending'}_${laneId}`, 96),
      weekTag,
      phaseId,
      phaseLabel,
      laneId,
  ...
  normalizeSeasonBoardFrontierDecree(source = null, context = {}) {
    ...
    const statusId = String(canonical.statusId || root.statusId || frontier?.statusId || '').trim().slice(0, 24) || (pressureScore >= 3 ? 'high_pressure' : pressureScore >= 2 ? 'pressure' : pressureScore >= 1 ? 'pending' : 'stable');
    const statusLabel = String(canonical.statusLabel || root.statusLabel || frontier?.statusLabel || frontier?.pressureLabel || '').trim().slice(0, 24) || (pressureScore >= 3 ? '高压' : pressureScore >= 2 ? '承压' : pressureScore >= 1 ? '待补样' : '稳态');
    const tone = String(canonical.tone || root.tone || '').trim().slice(0, 24) || (pressureScore >= 3 ? 'urgent' : pressureScore >= 2 ? 'focused' : pressureScore >= 1 ? 'watch' : 'steady');
    const toneLabel = String(canonical.toneLabel || root.toneLabel || '').trim().slice(0, 24) || (tone === 'urgent' ? '急令' : tone === 'focused' ? '专注' : tone === 'watch' ? '观察' : '稳令');
    const actionTargetLabel = String(canonical.actionTargetLabel || root.actionTargetLabel || frontier?.actionTargetLabel || '当前主线').trim().slice(0, 24) || '当前主线';
    const normalized = {
    ...
    return normalized.summaryLine || normalized.constraintLine || normalized.successLine ? normalized : null;
  }
  normalizeSeasonBoardFrontierChronicle(source = null, context = {}) {
    ...
    return normalized.summaryLine || normalized.currentEntryLine || normalized.progressLine ? normalized : null;
  }
  normalizeSeasonBoardFrontierCouncil(source = null, context = {}) {
    ...
    const statusId = String(canonical.statusId || root.statusId || frontier?.statusId || '').trim().slice(0, 24) || (pressureScore >= 3 ? 'high_pressure' : pressureScore >= 2 ? 'pressure' : pressureScore >= 1 ? 'pending' : 'stable');
    const statusLabel = String(canonical.statusLabel || root.statusLabel || frontier?.statusLabel || frontier?.pressureLabel || '').trim().slice(0, 24) || (pressureScore >= 3 ? '高压' : pressureScore >= 2 ? '承压' : pressureScore >= 1 ? '待补样' : '稳态');
    const rawOpinions = Array.isArray(canonical.laneOpinions) && canonical.laneOpinions.length > 0 ? canonical.laneOpinions : Array.isArray(root.laneOpinions) ? root.laneOpinions : [];
    const laneOpinions = rawOpinions.filter(entry => entry && typeof entry === 'object').slice(0, 3).map(entry => ({
    ...
    return normalized.summaryLine || normalized.verdictLine || normalized.laneOpinions.length > 0 ? normalized : null;
  }
  normalizeSeasonBoardFrontierResolution(source = null, context = {}) {
    ...
    if (!laneId) return null;
    const suggestedChoiceId = ['hold_primary', 'rebalance_support', 'seal_dispute'].includes(String(canonical.suggestedChoiceId || '').trim()) ? String(canonical.suggestedChoiceId || '').trim() : 'hold_primary';
    const choiceId = ['hold_primary', 'rebalance_support', 'seal_dispute'].includes(String(canonical.choiceId || '').trim()) ? String(canonical.choiceId || '').trim() : '';
    ...
    if (!normalized.summaryLine) {
      normalized.summaryLine = normalized.submitted ? `本周会审裁记：采用【${normalized.choiceLabel || '裁记'}】，写入战役史卷。` : `会审裁记待封：建议【${normalized.suggestedChoiceLabel || '守主战线'}】。`;
    }
    if (!normalized.chronicleSealLine) {
      normalized.chronicleSealLine = normalized.submitted ? `战役史卷已封记：${normalized.choiceLabel || normalized.laneLabel}。` : `战役史卷待封记：完成【${normalized.laneLabel}】一格后回季盘封记。`;
    }
    if (!normalized.councilResolutionLine) {
      normalized.councilResolutionLine = normalized.submitted ? `诸界会审裁定：${normalized.summaryLine}` : `诸界会审待裁记：先守【${normalized.laneLabel}】，副线保留证据。`;
    }
    return normalized.summaryLine || normalized.chronicleSealLine || normalized.councilResolutionLine ? normalized : null;
  }
  normalizeSeasonBoardFrontier(source = null, context = {}) {
    ...
    const canonical = derived && typeof derived === 'object' ? derived : root;
    const pressureScore = Math.max(0, Math.min(3, Math.floor(Number(canonical.pressureScore ?? root.pressureScore) || 0)));
    const statusId = String(canonical.statusId || root.statusId || (pressureScore >= 3 ? 'high_pressure' : pressureScore >= 2 ? 'pressure' : pressureScore >= 1 ? 'pending' : 'stable')).trim().slice(0, 24);
    const statusLabel = String(canonical.statusLabel || canonical.pressureLabel || root.statusLabel || root.pressureLabel || (pressureScore >= 3 ? '高压' : pressureScore >= 2 ? '承压' : pressureScore >= 1 ? '待补样' : '稳态')).trim().slice(0, 24);
    const rawItems = Array.isArray(canonical.items) && canonical.items.length > 0 ? canonical.items : Array.isArray(root.items) ? root.items : [];
    const items = rawItems.filter(entry => entry && typeof entry === 'object').slice(0, 3).map((entry, index) => ({
    ...
    const actionMeta = typeof this.game.getSeasonVerificationActionMeta === 'function' ? this.game.getSeasonVerificationActionMeta(canonical.actionValue || canonical.primaryAnchorSection || primaryItem?.anchorSection || root.actionValue || root.primaryAnchorSection || 'sanctum', {
      fallbackSection: 'sanctum'
    }) : {
      actionType: 'collection',
    ...
    return normalized.summaryLine || normalized.guideLine || normalized.items.length > 0 ? normalized : null;
  }
  commitSeasonBoardFrontierResolution(choiceId = '', options = {}) {
    ...
    const board = options?.board && typeof options.board === 'object' ? this.normalizeSeasonBoardSnapshot(options.board) : typeof this.getSeasonBoardSnapshot === 'function' ? this.getSeasonBoardSnapshot(options) : null;
    ...
    if (!weekTag) return failure('missing_week', {
      resolution
    });
    ...
    const supportItem = Array.isArray(frontier.items) ? frontier.items.find(item => item && item.laneId && item.laneId !== primaryLaneId && item.role !== 'reserve') || frontier.items.find(item => item && item.laneId && item.laneId !== primaryLaneId) || null : null;
    const supportLaneId = targetChoiceId === 'rebalance_support' ? sanitizeText(resolution.supportLaneId || supportItem?.laneId || supportItem?.id || '', 32) : '';
    const supportLaneLabel = targetChoiceId === 'rebalance_support' ? sanitizeText(resolution.supportLaneLabel || supportItem?.shortLabel || supportItem?.label || '副战线', 24) : '';
    const submittedAt = Math.max(0, Math.floor(Number(options.submittedAt || Date.now()) || 0));
    const ledger = board.weekVerdictLedger?.current && typeof board.weekVerdictLedger.current === 'object' ? board.weekVerdictLedger.current : null;
    const settlement = board.settlement && typeof board.settlement === 'object' ? board.settlement : null;
    const summaryLine = targetChoiceId === 'rebalance_support' ? `本周会审裁记：给【${supportLaneLabel || '副战线'}】补一份旁证，但不抢【${primaryLaneLabel}】主行动。` : targetChoiceId === 'seal_dispute' ? '本周会审裁记：争议先封入史卷，本周不改写三线排班。' : `本周会审裁记：继续守住【${primaryLaneLabel}】，先完成主战线一格。`;
    const chronicleSealLine = targetChoiceId === 'rebalance_support' ? `战役史卷已封记：${meta.label} · ${primaryLaneLabel}${supportLaneLabel ? ` / ${supportLaneLabel}` : ''}。` : targetChoiceId === 'seal_dispute' ? `战役史卷已封记：${meta.label} · 会审争议暂不改写排班。` : `战役史卷已封记：${meta.label} · ${primaryLaneLabel}继续优先。`;
    ...
    if (!record?.recordId) return failure('record_failed', {
      resolution
    });
    const isSameResolutionRecord = entry => entry && (entry.recordId === record.recordId || entry.weekTag === weekTag && (String(entry.recordKind || '').trim() === 'frontier_resolution' || !!entry.frontierResolutionChoiceId || !!entry.frontierResolutionId));
    const records = [record, ...(Array.isArray(state.records) ? state.records.filter(entry => !isSameResolutionRecord(entry)) : [])].slice(0, 6);
    const history = [record, ...(Array.isArray(state.history) ? state.history.filter(entry => !isSameResolutionRecord(entry)) : [])].slice(0, 18);
  ...
  claimSeasonBoardLaneReward(laneId = '', options = {}) {
    ...
    const board = options?.board && typeof options.board === 'object' ? this.normalizeSeasonBoardSnapshot(options.board) : typeof this.game.getSeasonBoardSnapshot === 'function' ? this.game.getSeasonBoardSnapshot(options) : null;
    const reward = Array.isArray(board?.laneRewards) ? board.laneRewards.find(entry => entry?.laneId === targetLaneId) : null;
    const failure = (reason, detail = {}) => {
    ...
    if (!reward.ready) return failure('not_ready', {
      reward
    });
    if (reward.claimed) return failure('already_claimed', {
      reward
    });
    ...
    if (claimedLaneRewards?.[weekTag]?.[targetLaneId]) {
      return failure('already_claimed', {
        reward
      });
    }
    ...
    const playerSnapshot = this.game.player ? {
      ...
      fateRing: this.game.player.fateRing && typeof this.game.player.fateRing === 'object' ? {
        ...
        json: (() => {
          try {
            return typeof this.game.player.fateRing.toJSON === 'function' ? this.game.player.fateRing.toJSON() : null;
          } catch {
            return null;
          }
        })()
    ...
    const gold = Math.max(0, Math.floor(Number(gains.gold) || 0));
    try {
      strategicGain = typeof this.game.grantStrategicCurrencies === 'function' ? this.game.grantStrategicCurrencies({
      ...
      ringExp = typeof this.game.grantFateRingExp === 'function' ? this.game.grantFateRingExp(gains.ringExp || 0, reason) : 0;
  ...
  normalizeSeasonBoardSnapshot(source = null) {
    const root = source && typeof source === 'object' ? source : null;
    if (!root) return null;
    const lanes = Array.isArray(root.lanes) ? root.lanes.map((lane, index) => this.game.normalizeHeavenlyMandateLane(lane, index)).slice(0, 3) : [];
    ...
    const explicitTotal = Math.max(0, Math.floor(Number(root.totalTaskCount ?? root.progress?.total ?? 0) || 0));
    const totalTaskCount = Math.max(laneTotalTaskCount, explicitTotal);
    const explicitCompleted = Math.max(0, Math.floor(Number(root.completedTaskCount ?? root.progress?.completed ?? 0) || 0));
    const completedTaskCount = totalTaskCount > 0 ? Math.min(totalTaskCount, Math.max(laneCompletedTaskCount, explicitCompleted)) : Math.max(laneCompletedTaskCount, explicitCompleted);
    const progressText = String(root.progress?.progressText || (totalTaskCount > 0 ? `${completedTaskCount}/${totalTaskCount}` : '待同步')).trim() || '待同步';
    const progressRatio = totalTaskCount > 0 ? Math.max(0, Math.min(1, completedTaskCount / Math.max(1, totalTaskCount))) : 0;
    ...
    const basePrioritizedLaneIds = Array.isArray(lanePriorityByPhase[phaseId]) ? lanePriorityByPhase[phaseId] : lanePriorityByPhase.sampling;
    ...
    const verificationOrders = Array.isArray(root.verificationOrders) ? root.verificationOrders.map((entry, index) => this.normalizeSeasonBoardVerificationOrder(entry, index)).filter(Boolean).slice(0, 2) : [];
    ...
    const currentVerdict = weekVerdictLedger?.current && typeof weekVerdictLedger.current === 'object' ? weekVerdictLedger.current : null;
    ...
    const hardVerificationGate = phaseId === 'ranking' || hardSettlementOutcomes.has(settlementOutcomeId) || verificationOrders.some(order => String(order?.type || '').trim() === 'clear_debt');
    const hardDebtGate = !!(debtPack && ['open', 'deferred'].includes(activeDebtStatus) && (debtPack.occupiesStrongSlot || settlementOutcomeId === 'debt_sheet' || verificationOrders.some(order => String(order?.type || '').trim() === 'clear_debt')));
    const explicitNextWeekGoal = root.nextWeekGoal && typeof root.nextWeekGoal === 'object' ? root.nextWeekGoal : null;
    const explicitStrongNextWeekGoal = !!(explicitNextWeekGoal && String(explicitNextWeekGoal.source || '').trim() && String(explicitNextWeekGoal.source || '').trim() !== 'lane');
    const chapterArcPreview = typeof this.buildSeasonBoardChapterArc === 'function' ? this.buildSeasonBoardChapterArc({
    ...
    const chapterArcCarryoverPreview = chapterArcPreview?.carryover && typeof chapterArcPreview.carryover === 'object' ? chapterArcPreview.carryover : null;
    const frontierResolutionChoiceId = String(currentVerdict?.frontierResolutionChoiceId || currentVerdict?.resolutionChoiceId || currentVerdict?.choiceId || '').trim();
    const frontierResolutionSupportLaneId = String(currentVerdict?.frontierResolutionSupportLaneId || currentVerdict?.supportLaneId || '').trim();
    const canApplyFrontierResolutionLaneBias = frontierResolutionChoiceId === 'rebalance_support' && !hardDebtGate && !hardVerificationGate && !explicitStrongNextWeekGoal && basePrioritizedLaneIds.includes(frontierResolutionSupportLaneId) && hasIncompleteLaneTask(frontierResolutionSupportLaneId);
    const chapterArcCarryoverLaneId = String(chapterArcCarryoverPreview?.preferredLaneId || '').trim();
    const canApplyChapterArcCarryoverLaneBias = !canApplyFrontierResolutionLaneBias && !!chapterArcCarryoverPreview?.available && !!chapterArcCarryoverPreview?.openingWeek && !hardDebtGate && !hardVerificationGate && !explicitStrongNextWeekGoal && basePrioritizedLaneIds.includes(chapterArcCarryoverLaneId) && hasIncompleteLaneTask(chapterArcCarryoverLaneId);
    const prioritizedLaneIds = canApplyFrontierResolutionLaneBias ? [frontierResolutionSupportLaneId, ...basePrioritizedLaneIds.filter(laneId => laneId !== frontierResolutionSupportLaneId)] : canApplyChapterArcCarryoverLaneBias ? [chapterArcCarryoverLaneId, ...basePrioritizedLaneIds.filter(laneId => laneId !== chapterArcCarryoverLaneId)] : basePrioritizedLaneIds;
    const prioritizedLanes = [...prioritizedLaneIds.map(laneId => lanes.find(lane => lane && lane.id === laneId)).filter(Boolean), ...lanes.filter(lane => lane && !prioritizedLaneIds.includes(lane.id))];
    const nextTask = prioritizedLanes.reduce((result, lane) => {
      if (result) return result;
      const task = Array.isArray(lane.tasks) ? lane.tasks.find(entry => entry && !entry.completed) : null;
      return task ? {
        laneId: lane.id,
    ...
    const resolveNextTaskSource = (candidate = null) => {
      ...
      const debtStatus = String(debtPack?.status || '').trim();
      if (debtPack && ['open', 'deferred'].includes(debtStatus) && (debtPack.occupiesStrongSlot || settlement?.outcomeId === 'debt_sheet' || verificationOrders.some(order => order?.type === 'clear_debt'))) {
        return 'debt_pack';
      }
      if (candidate?.laneId === 'verification' || phaseId === 'ranking' || ['positive_sheet', 'risky_sheet'].includes(String(settlement?.outcomeId || '').trim())) {
        return 'verification';
      }
    ...
    const nextTaskActionMeta = nextTask ? typeof this.game.getSeasonVerificationActionMeta === 'function' ? this.game.getSeasonVerificationActionMeta(nextTask.task.actionValue || nextTask.task.anchorSection || '', {
      fallbackSection: 'sanctum'
    }) : {
      actionType: 'collection',
    ...
    const nextTaskPayload = nextTask ? {
      ...
      actionType: String(nextTask.task.actionType || nextTaskActionMeta?.actionType || '').trim().slice(0, 24) || nextTaskActionMeta?.actionType || 'collection',
      actionValue: String(nextTask.task.actionValue || nextTaskActionMeta?.actionValue || '').trim().slice(0, 40) || nextTaskActionMeta?.actionValue || 'sanctum',
      ctaLabel: String(nextTask.task.ctaLabel || nextTaskActionMeta?.ctaLabel || '前往推进').trim().slice(0, 24) || '前往推进',
    ...
    const rootNextWeekGoal = root.nextWeekGoal && typeof root.nextWeekGoal === 'object' ? root.nextWeekGoal : null;
    const rootNextWeekGoalMatchesNextTask = !!(rootNextWeekGoal && nextTaskPayload && (!rootNextWeekGoal.taskId || String(rootNextWeekGoal.taskId || '').trim() === nextTaskPayload.id) && (!rootNextWeekGoal.taskSourceId || String(rootNextWeekGoal.taskSourceId || '').trim() === nextTaskPayload.taskSourceId) && (!rootNextWeekGoal.laneId || String(rootNextWeekGoal.laneId || '').trim() === nextTaskPayload.laneId) && (!rootNextWeekGoal.source || normalizeNextTaskSource(rootNextWeekGoal.source) === nextTaskPayload.source) && (!rootNextWeekGoal.sourceId || String(rootNextWeekGoal.sourceId || '').trim() === nextTaskPayload.sourceId));
    ...
    const nextWeekGoal = nextTaskPayload ? {
      ...
      note: String(rootNextWeekGoalSeed?.note || [nextTaskPayload.hintLine || '', nextTaskPayload.statusLine || '', nextTaskPayload.progressText ? `进度 ${nextTaskPayload.progressText}` : ''].filter(Boolean).join(' · ')).trim().slice(0, 220),
      action: String(rootNextWeekGoalSeed?.action || nextTaskPayload.actionType || '').trim().slice(0, 24) || nextTaskPayload.actionType,
      value: String(rootNextWeekGoalSeed?.value || nextTaskPayload.actionValue || '').trim().slice(0, 40) || nextTaskPayload.actionValue,
      buttonLabel: String(rootNextWeekGoalSeed?.buttonLabel || rootNextWeekGoalSeed?.ctaLabel || nextTaskPayload.ctaLabel || '前往推进').trim().slice(0, 24) || '前往推进',
      ...
      taskSource: String(rootNextWeekGoalSeed?.taskSource || nextTaskPayload.taskSource || 'lane').trim().slice(0, 40) || 'lane',
    ...
    const summaryLine = String(root.summaryLine || `赛季天道盘当前围绕【${themeLabel}】展开，已进入「${phaseLabel}」。`).trim();
    const detailLine = String(root.detailLine || [root.seasonName ? `赛季：${String(root.seasonName || '').trim()}` : '', root.weekLabel ? `轮转：${String(root.weekLabel || '').trim()}` : '', root.crossModeSummary ? `验算：${String(root.crossModeSummary || '').trim()}` : ''].filter(Boolean).slice(0, 2).join('｜')).trim();
    const guideLine = String(root.guideLine || (nextTask ? `下一步：${nextTask.task.label}${nextTask.task.hintLine ? ` · ${nextTask.task.hintLine}` : ''}` : '本周季盘任务已齐备，可以继续冲更高分或扩充跨模样本。')).trim();
    const crossModeSummary = String(root.crossModeSummary || '').trim();
    const statusLine = String(root.statusLine || [phaseLabel, `赛季进度 ${progressText}`, crossModeSummary].filter(Boolean).join(' · ')).trim();
    const rewardLine = String(root.rewardLine || `本章归卷会把赛季天道盘推进到 ${progressText}${nextTask ? `，接下来建议 ${nextTask.task.label}` : '，当前主轴已经成型。'}`).trim();
    ...
    const chapterArcCarryover = chapterArc?.carryover && typeof chapterArc.carryover === 'object' ? {
      ...
      summaryLine: canApplyChapterArcCarryoverLaneBias ? chapterArc.carryover.summaryLine || '' : `上章承卷：${chapterArc.carryover.chapterLabel || '上章'} 的章末评语已保留，但当前周强目标更强，开局偏置暂不生效。`,
      statusLabel: canApplyChapterArcCarryoverLaneBias ? '开局偏置' : '让位强目标',
      guideLine: canApplyChapterArcCarryoverLaneBias ? chapterArc.carryover.guideLine || '承卷偏置正在轻量影响当前普通开局排班。' : '上章承卷已保留为章层评语，但当前周目标更强，这道开局偏置暂不生效。'
    } : null;
  ...
  buildSeasonBoardRouteDirective(options = {}) {
    ...
    const defaultLaneId = phaseId === 'ranking' ? 'verification' : phaseId === 'lockline' ? 'expedition' : 'training';
    const boardNextTask = board?.nextTask && typeof board.nextTask === 'object' ? board.nextTask : null;
    const boardNextLaneId = ['training', 'expedition', 'verification'].includes(String(boardNextTask?.laneId || '').trim()) && String(boardNextTask?.source || '').trim() === 'lane' ? String(boardNextTask.laneId || '').trim() : '';
    const nextLaneId = boardNextLaneId || defaultLaneId;
    const chapterArcCarryover = board?.chapterArc?.carryover && typeof board.chapterArc.carryover === 'object' ? board.chapterArc.carryover : null;
    ...
    const normalizeNodeTypes = (value = null, limit = 4) => {
      ...
      return Array.isArray(value) ? value.map(entry => String(entry || '').trim()).filter(Boolean).slice(0, limit) : [];
    };
    ...
    const buildFocusLabel = (nodeTypes = []) => normalizeNodeTypes(nodeTypes).map(nodeType => nodeTypeLabel(nodeType)).join(' / ');
    ...
    if (chapterArcFeedbackLine) {
      directive = {
        ...directive,
        summaryLine: [chapterArcFeedbackLine, directive.summaryLine || ''].filter(Boolean).join(' '),
        rewardLine: [chapterArcFeedbackLine, directive.rewardLine || ''].filter(Boolean).join(' ')
      };
    }
    if (chapterArcCarryover?.available) {
      directive = {
        ...directive,
        summaryLine: [chapterArcCarryover.summaryLine || '', directive.summaryLine || ''].filter(Boolean).join(' '),
        rewardLine: [chapterArcCarryover.guideLine || '', directive.rewardLine || ''].filter(Boolean).join(' ')
      };
    }
    const shift = directive.weightShift && typeof directive.weightShift === 'object' ? typeof this.game.sanitizeSanctumAgendaWeightShift === 'function' ? this.game.sanitizeSanctumAgendaWeightShift(directive.weightShift) : {
  ...
  getSeasonBoardSnapshot(options = {}) {
    ...
    const mandateTheme = typeof this.game.getHeavenlyMandateThemeMeta === 'function' ? this.game.getHeavenlyMandateThemeMeta(signals) : {
      id: phase.id,
      label: phase.label,
      icon: phase.icon,
      summaryLine: ''
    };
    const lineage = signals.lineage && typeof signals.lineage === 'object' ? signals.lineage : null;
    const aftereffects = signals.aftereffects && typeof signals.aftereffects === 'object' ? signals.aftereffects : null;
    const styleTrackCount = [lineage?.characterTrack?.dominantLabel, lineage?.styleTrack?.dominantLabel, lineage?.nodeTrack?.dominantLabel, lineage?.researchTrack?.dominantLabel].filter(Boolean).length;
    ...
    const verificationOrders = Array.isArray(settlementState?.verificationOrders) ? settlementState.verificationOrders : [];
    const trainingLane = {
      id: 'training',
      label: '训练线',
      icon: '🔭',
      summaryLine: signals.selectedGuide?.title ? `当前主练样本已锁到【${signals.selectedGuide.title}】，接下来要把谱系与周挑战结果继续压实。` : signals.trainingFocus?.trainingAdvice ? `当前已有主练建议，下一步要把这份建议继续压成稳定可复用的赛季样本。` : '先锁定一份本周主练样本，再把训练建议与谱系沉淀写成长期主轴。',
      tasks: [{
        ...
        target: 1,
        hintLine: signals.selectedGuide ? `当前精选命盘：${signals.selectedGuide.title} · ${signals.selectedGuide.themeLabel || '赛季样本'}` : signals.trainingFocus ? `当前主练：${signals.trainingFocus.chapterName || '最近归卷'} · ${signals.trainingFocus.trainingAdvice || signals.trainingFocus.sourceTitle || '已给出训练建议'}` : '去观星台锁一份精选命盘，或先补一局七日劫数拿到主练建议。',
        ...
        target: 3,
        hintLine: lineage?.currentFocusLine || lineage?.detailLine || '继续积累角色、流派、节点与研究的长期留痕，把本周主练写成可复刻谱系。',
    ...
    const expeditionLane = {
      ...
      label: '远征线',
      icon: '🧭',
      summaryLine: signals.activeAgenda ? `当前正围绕【${signals.activeAgenda.name}】压卷，章节推进要继续沿 ${signals.activeAgenda.focusNodeLine || '主轴'} 走。` : signals.latestSlate ? `最近一张章节答卷已经写回赛季主轴，下一步要把它继续锁进洞府承诺与章节复盘。` : '先推进一章远征留下第一张章节答卷，再决定这周真正要押哪条主线。',
      tasks: [{
        id: 'season_run_slate',
        ...
        target: 1,
        hintLine: signals.latestSlate ? `最新归卷：${signals.latestSlate.chapterName || '章节'} · ${signals.latestSlate.endingName || '已收卷'}${signals.latestSlate.score ? ` · 评分 ${signals.latestSlate.score}` : ''}` : '推进任意一章裂界远征，把本周主轴先写成第一张可复盘答卷。',
        statusLine: signals.latestSlate?.ratingLabel || '',
        anchorSection: 'slates'
      }, {
        id: 'season_commitment',
        label: signals.activeAgenda ? `推进 ${signals.activeAgenda.name}` : signals.lastAgenda ? `复核 ${signals.lastAgenda.name}` : '立下一道洞府锁线承诺',
        icon: '📜',
        progress: signals.activeAgenda ? Math.max(0, Math.floor(Number(signals.activeAgenda.progress) || 0)) : signals.lastAgenda?.outcome === 'success' ? 1 : 0,
        target: signals.activeAgenda ? Math.max(1, Math.floor(Number(signals.activeAgenda.target) || 1)) : 1,
        hintLine: signals.activeAgenda ? signals.activeAgenda.phaseLine || signals.activeAgenda.focusNodeLine || signals.activeAgenda.summaryLine || '本轮承诺正在推进。' : signals.lastAgenda ? signals.lastAgenda.recoveryLine || signals.lastAgenda.grantedLine || signals.lastAgenda.reasonLine || '上一道洞府承诺已经留下结题结果。' : '从归卷书架挑一份答卷设为本周承诺，让赛季主轴真正锁进章节推进。',
        statusLine: signals.activeAgenda?.phaseLabel || signals.lastAgenda?.outcomeLabel || '',
        anchorSection: 'sanctum'
      }]
    };
    const crossModeSummary = [signals.endlessSeason ? `无尽 ${Math.max(0, Math.floor(Number(signals.endlessClears) || 0))} 轮` : '', signals.pvpSeasonName ? `天道榜 ${Math.max(0, Math.floor(Number(signals.pvpSeasonMatchCount) || 0))} 场` : '', aftereffects?.currentStatusLine || ''].filter(Boolean).join(' · ');
    const seasonVerificationLaneTasks = [{
      ...
      target: 1,
      hintLine: signals.endlessSeason ? `${signals.endlessSeason.name || '当前赛季'} · 已清 ${Math.max(0, Math.floor(Number(signals.endlessClears) || 0))} 轮${signals.endlessScore ? ` · 积分 ${Math.max(0, Math.floor(Number(signals.endlessScore) || 0))}` : ''}` : '进入无尽轮回后，会开始记录这周主轴在高压环境下是否还能成立。',
      ...
      target: 2,
      hintLine: signals.pvpSeasonName ? `${signals.pvpSeasonName} · 已记 ${Math.max(0, Math.floor(Number(signals.pvpSeasonMatchCount) || 0))} 场${signals.pvpRecentOpponentName ? ` · 最近对手 ${signals.pvpRecentOpponentName}` : ''}` : 'PVP 账本会记录本周真实对局，至少打一场先建立赛季外场样本。',
      statusLine: aftereffects?.currentStatusLine || signals.pvpSeasonName || '',
      anchorSection: 'pvp'
    }];
    const seasonDebtFocusTask = typeof this.game.buildHeavenlyMandateDebtFocusTask === 'function' ? this.game.buildHeavenlyMandateDebtFocusTask(debtPack) : null;
    if (seasonDebtFocusTask) {
      let replaceIndex = seasonVerificationLaneTasks.findIndex(task => String(task?.anchorSection || '').trim() === seasonDebtFocusTask.anchorSection);
      if (replaceIndex < 0) {
    ...
    const verificationLane = {
      ...
      label: '验算线',
      icon: '🏁',
      summaryLine: [seasonDebtFocusTask ? debtPack?.status === 'deferred' ? '跨周欠卷已经进入验算线强目标，先清账再恢复常规定榜。' : '本周欠卷已经进入验算线强目标，清账验证优先于普通高压补样。' : '', crossModeSummary ? `本周已经开始做跨模验算：${crossModeSummary}。` : '用无尽轮回、天道榜与长期后效去确认这周主练不是只在单章里成立的错觉。'].filter(Boolean).join('｜'),
      tasks: seasonVerificationLaneTasks
    };
    const lanes = [trainingLane, expeditionLane, verificationLane].map((lane, index) => this.game.normalizeHeavenlyMandateLane(lane, index));
    ...
    const themeId = String(signals.selectedGuide?.themeKey || signals.trainingFocus?.themeKey || signals.activeAgenda?.themeKey || signals.latestSlate?.themeKey || lineage?.styleTrack?.dominantId || mandateTheme.id || phase.id).trim() || phase.id;
    const themeLabel = String(signals.selectedGuide?.themeLabel || signals.trainingFocus?.themeLabel || signals.activeAgenda?.themeLabel || signals.latestSlate?.themeLabel || lineage?.styleTrack?.dominantLabel || mandateTheme.label || '本周主轴').trim() || '本周主轴';
    const seasonSource = signals.endlessSeason?.id && signals.pvpSeason?.id ? 'hybrid' : signals.endlessSeason?.id ? 'endless' : signals.pvpSeason?.id ? 'pvp' : 'weekly';
    const seasonName = seasonSource === 'hybrid' && signals.endlessSeason?.name && signals.pvpSeasonName && signals.endlessSeason.name !== signals.pvpSeasonName ? `${signals.endlessSeason.name} / ${signals.pvpSeasonName}` : signals.endlessSeason?.name || signals.pvpSeasonName || `${signals.weekLabel || signals.weekTag || '本周轮转'} · 赛季天道盘`;
    const committedFrontierResolution = typeof this.getCommittedSeasonBoardFrontierResolution === 'function' ? this.getCommittedSeasonBoardFrontierResolution({
    ...
    const board = this.normalizeSeasonBoardSnapshot({
      ...
      detailLine: [mandateTheme.label ? `轮转题面：${mandateTheme.label}` : '', settlement?.detailLine || '', debtPack?.summaryLine || '', signals.latestSlate?.ratingLabel ? `最近答卷：${signals.latestSlate.ratingLabel}` : '', signals.activeAgenda?.selectedContractLabel ? `锁线契约：${signals.activeAgenda.selectedContractLabel}` : '', aftereffects?.currentStatusLine ? `后效：${aftereffects.currentStatusLine}` : ''].filter(Boolean).slice(0, 2).join('｜'),
      guideLine: [settlement?.guideLine || '', verificationOrders[0]?.summaryLine || ''].filter(Boolean).join(' · '),
      statusLine: [phase.label, settlement?.outcomeLabel ? `当前押卷 ${settlement.outcomeLabel}` : '', debtPack?.progressText || verificationOrders[0]?.statusLine || ''].filter(Boolean).join(' · '),
      rewardLine: [settlement?.summaryLine || '', debtPack?.guideLine || verificationOrders[0]?.summaryLine || ''].filter(Boolean).join(' '),
      crossModeSummary,
      completedTaskCount,
      totalTaskCount,
      settlement,
    ...
    return this.normalizeSeasonBoardSnapshot({
      ...board,
      guideLine: [board.guideLine, routeDirective.summaryLine ? `路线引导：${routeDirective.summaryLine}` : ''].filter(Boolean).join(' '),
      rewardLine: [board.rewardLine, routeDirective.rewardLine || ''].filter(Boolean).join(' ')
    });
  }
