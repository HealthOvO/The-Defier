export class SanctumAgendaManager {
    ...
    createFateAftereffectFromSanctumAgenda(resolved = null, context = {}) {
        ...
        const outcomeId = resolved.recoveryEligible
            ? 'recovery'
            : (resolved.contractResolved
                ? (resolved.contractSuccess ? 'contract_success' : 'contract_miss')
                : (resolved.outcome === 'success' ? 'stabilized' : ''));
        if (!outcomeId) return null;
        const rewardTrackId = String(resolved.rewardTrackId || '').trim();
        const templateId = rewardTrackId === 'forbidden_altar'
            ? 'risk_bias'
            : (rewardTrackId === 'memory_rift' ? 'archive_bias' : 'route_bias');
        const catalog = {
            route_bias: {
                contract_success: {
                    name: '星镜余痕',
                    icon: '🧭',
                    durationChapters: 2,
                    weightShift: { observatory: 0.028, event: 0.018, memory_rift: 0.014, enemy: -0.014, rest: -0.010 },
                ...
                contract_miss: {
                    ...
                    icon: '🧭',
                    durationChapters: 1,
                    weightShift: { observatory: 0.018, event: 0.012, memory_rift: 0.010, rest: -0.016, shop: -0.010 },
                ...
                recovery: {
                    ...
                    icon: '🧭',
                    durationChapters: 1,
                    weightShift: { observatory: 0.012, event: 0.008, memory_rift: 0.006, enemy: -0.004 },
                ...
                stabilized: {
                    ...
                    icon: '🧭',
                    durationChapters: 1,
                    weightShift: { observatory: 0.016, event: 0.012, memory_rift: 0.008, rest: -0.006 },
            ...
            risk_bias: {
                contract_success: {
                    ...
                    icon: '🩸',
                    durationChapters: 2,
                    weightShift: { elite: 0.020, trial: 0.018, forbidden_altar: 0.016, rest: -0.018, shop: -0.012 },
                ...
                contract_miss: {
                    ...
                    icon: '🩸',
                    durationChapters: 1,
                    weightShift: { enemy: 0.018, elite: 0.016, trial: 0.014, rest: -0.018, shop: -0.014 },
                ...
                recovery: {
                    ...
                    icon: '🩸',
                    durationChapters: 1,
                    weightShift: { trial: 0.012, elite: 0.008, enemy: 0.006, rest: -0.006 },
                ...
                stabilized: {
                    ...
                    icon: '🩸',
                    durationChapters: 1,
                    weightShift: { elite: 0.014, trial: 0.012, forbidden_altar: 0.010, rest: -0.008 },
            ...
            archive_bias: {
                contract_success: {
                    ...
                    icon: '🪞',
                    durationChapters: 2,
                    weightShift: { memory_rift: 0.022, observatory: 0.016, spirit_grotto: 0.014, enemy: -0.008, shop: -0.008 },
                ...
                contract_miss: {
                    ...
                    icon: '🪞',
                    durationChapters: 1,
                    weightShift: { memory_rift: 0.018, observatory: 0.012, event: 0.010, shop: -0.014, rest: -0.008 },
                ...
                recovery: {
                    ...
                    icon: '🪞',
                    durationChapters: 1,
                    weightShift: { memory_rift: 0.012, observatory: 0.008, spirit_grotto: 0.006, enemy: -0.004 },
                ...
                stabilized: {
                    ...
                    icon: '🪞',
                    durationChapters: 1,
                    weightShift: { memory_rift: 0.016, observatory: 0.012, spirit_grotto: 0.008, enemy: -0.006 },
        ...
        const record = this.game.normalizeFateAftereffectRecord({
            ...
            sourceLine: [
                resolved.name || '洞府议程',
                resolved.selectedContractLabel ? `契约「${resolved.selectedContractLabel}」` : '',
                !resolved.selectedContractLabel && resolved.selectedDecisionLabel ? `处置「${resolved.selectedDecisionLabel}」` : '',
                resolved.boundChapterName || resolved.chapterName || ''
            ].filter(Boolean).join(' · '),
            ...
            detailLine: [
                `来源：${resolved.name || '洞府议程'}${resolved.selectedContractLabel ? ` · 契约「${resolved.selectedContractLabel}」` : (resolved.selectedDecisionLabel ? ` · 处置「${resolved.selectedDecisionLabel}」` : '')}`,
                `正向：${template.positiveLine}`,
                `代价：${template.negativeLine}`
            ].filter(Boolean).join('｜'),
        ...
        state.records = Array.isArray(state.records)
            ? state.records
                .filter((entry) => entry.recordId !== record.recordId && !(entry.sourceRunId === record.sourceRunId && entry.sourceAgendaId === record.sourceAgendaId))
                .concat(record)
                .slice(-6)
            : [record];
    ...
    normalizeSanctumAgendaNodeTypes(list = [], limit = 4) {
        const allowed = new Set([
            'enemy', 'elite', 'event', 'shop', 'trial', 'forge', 'rest',
            'observatory', 'spirit_grotto', 'forbidden_altar', 'memory_rift'
        ]);
        const result = [];
    ...
    inferSanctumAgendaNodeTypes(values = [], fallbackThemeKey = '') {
        ...
        const directNodeTypes = new Set([
            'enemy', 'elite', 'event', 'shop', 'trial', 'forge', 'rest',
            'observatory', 'spirit_grotto', 'forbidden_altar', 'memory_rift'
        ]);
        const patternMap = [
            { type: 'observatory', regex: /(观星|观测|天象|星镜|星轨|天机)/ },
            { type: 'event', regex: /(事件|机缘|奇遇|留痕)/ },
            { type: 'memory_rift', regex: /(裂隙|归卷|改写|回响|答卷|复盘|镜)/ },
            { type: 'spirit_grotto', regex: /(灵契|护道|调息|养势|休整)/ },
            { type: 'forbidden_altar', regex: /(禁术|血契|高压|搏命|献祭)/ },
            { type: 'trial', regex: /(试炼|检定|压强)/ },
            { type: 'forge', regex: /(锻炉|炼器|补件|器灵)/ },
            { type: 'shop', regex: /(商店|补给|货单|折价|灵石)/ },
            { type: 'rest', regex: /(营地|休整|续航|稳血|恢复)/ },
            { type: 'elite', regex: /(精英|追猎|高危)/ },
            { type: 'enemy', regex: /(战斗|前压|清场|接战)/ }
        ];
        const visit = (value) => {
            if (Array.isArray(value)) {
                value.forEach(visit);
    ...
    getSanctumAgendaNodeMeta(nodeType = '') {
        const catalog = {
            enemy: { icon: '⚔️', label: '战斗' },
            elite: { icon: '💀', label: '精英' },
            event: { icon: '❓', label: '事件' },
            shop: { icon: '🏪', label: '商路' },
            trial: { icon: '⚖️', label: '试炼' },
            forge: { icon: '⚒️', label: '锻炉' },
            rest: { icon: '🏕️', label: '营地' },
            observatory: { icon: '🔭', label: '观星' },
            spirit_grotto: { icon: '🪷', label: '灵契' },
            forbidden_altar: { icon: '🩸', label: '禁术' },
            memory_rift: { icon: '🪞', label: '裂隙' }
        };
        ...
        return Array.isArray(list)
            ? list
                ...
                .map((entry, index) => ({
                    ...
                    summaryLine: sanitizeText(
                        entry.summaryLine || entry.summary || `${agendaName} 已进入章中处置。`,
                        180
                    ),
                    ...
                    acceptedTonesAdd: Array.isArray(entry.acceptedTonesAdd)
                        ? Array.from(new Set(
                            entry.acceptedTonesAdd
                                .map((value) => String(value || '').trim())
                                .filter((value) => allowedTones.includes(value))
                        )).slice(0, 4)
                        : [],
            ...
            ? list
                ...
                .map((entry, index) => {
                    const record = {
                        ...
                        tagLabel: sanitizeText(entry.tagLabel || '', 20),
                        summaryLine: sanitizeText(
                            entry.summaryLine || entry.summary || `${agendaName} 可追加一条锁线契约。`,
                            180
                        ),
                        statusLine: sanitizeText(entry.statusLine || entry.effectLine || '', 180),
                        ...
                        acceptedTones: Array.isArray(entry.acceptedTones)
                            ? Array.from(new Set(
                                entry.acceptedTones
                                    .map((value) => String(value || '').trim())
                                    .filter((value) => allowedTones.includes(value))
    ...
    normalizeSanctumAgendaRecord(source = null) {
        ...
        const sanitizeTags = (value, limit = 4) => Array.isArray(value)
        ...
        const sanitizeAcceptedTones = (value, fallback = ['completed', 'selected']) => {
            ...
            const tones = Array.isArray(value)
                ? value.map((entry) => String(entry || '').trim()).filter((entry) => allowed.includes(entry))
                : [];
            return tones.length > 0
                ? Array.from(new Set(tones)).slice(0, 4)
                : (Array.isArray(fallback) ? Array.from(new Set(fallback)).slice(0, 4) : []);
        };
        const sanitizeHistory = (value) => Array.isArray(value)
            ? value
                .filter((entry) => entry && typeof entry === 'object')
                .slice(-8)
                .map((entry) => ({
            ...
            : [];
        const resolveOutcome = (value) => ['active', 'success', 'failed', 'abandoned'].includes(String(value || '').trim())
            ? String(value || '').trim()
            : 'active';
        const resolveTone = (value, fallback = 'tracking') => ['tracking', 'selected', 'completed', 'failed', 'idle'].includes(String(value || '').trim())
            ? String(value || '').trim()
            : fallback;
        const resolveRatingTone = (value) => ['completed', 'selected', 'suggested', 'idle'].includes(String(value || '').trim())
            ? String(value || '').trim()
            : 'idle';
        const rewardTrackId = ['observatory', 'spirit_grotto', 'forbidden_altar', 'memory_rift'].includes(String(root.rewardTrackId || '').trim())
            ? String(root.rewardTrackId || '').trim()
            : '';
        const record = {
            ...
            decisionState: ['locked', 'pending', 'selected'].includes(String(root.decisionState || '').trim())
                ? String(root.decisionState || '').trim()
                : 'locked',
            ...
            contractState: ['locked', 'pending', 'selected', 'resolved'].includes(String(root.contractState || '').trim())
                ? String(root.contractState || '').trim()
                : 'locked',
            ...
            recoveryTier: ['trace', 'salvage', 'deep'].includes(String(root.recoveryTier || '').trim())
                ? String(root.recoveryTier || '').trim()
                : '',
            ...
            outcomeTone: resolveTone(root.outcomeTone, resolveOutcome(root.outcome) === 'success'
                ? 'completed'
                : (resolveOutcome(root.outcome) === 'failed' || resolveOutcome(root.outcome) === 'abandoned' ? 'failed' : 'tracking')),
        ...
        if (selectedContract) {
            ...
            if (!record.selectedContractLine) {
                const contractParts = [
                    selectedContract.statusLine || selectedContract.summaryLine,
                    selectedContract.signCostLine ? `契押 ${selectedContract.signCostLine}` : '',
                    selectedContract.burdenLine || ''
                ].filter(Boolean);
            ...
            && record.selectedDecisionId
            && record.progress >= record.contractThreshold
            && record.contractOptions.length > 0
        ) {
        ...
        const statusParts = [
            record.phaseLabel ? `阶段 ${record.phaseLabel}` : '',
            record.outcome === 'active' ? `关键节点 ${record.progress}/${record.target}` : '',
            record.selectedDecisionLabel || '',
            record.selectedContractLabel ? `契约 ${record.selectedContractLabel}` : ''
        ].filter(Boolean);
    ...
    normalizeSanctumAgendaState(source = null) {
        ...
        const history = Array.isArray(root.history)
            ? root.history
                .map((entry) => this.normalizeSanctumAgendaRecord(entry))
                .filter((entry) => entry.agendaId)
                .slice(-6)
            : [];
        const activeAgenda = root.activeAgenda && typeof root.activeAgenda === 'object'
            ? this.normalizeSanctumAgendaRecord({
                ...root.activeAgenda,
                outcome: 'active',
        ...
        const lastResolved = root.lastResolved && typeof root.lastResolved === 'object'
            ? this.normalizeSanctumAgendaRecord(root.lastResolved)
            : null;
    ...
    getSanctumAgendaSourceSnapshot() {
        const latestSlate = typeof this.game.getLatestRunSlate === 'function'
            ? this.game.getLatestRunSlate()
            : null;
        const currentTraining = typeof this.game.getObservatoryTrainingFocus === 'function'
            ? this.game.getObservatoryTrainingFocus()
            : null;
        const fallbackTraining = !currentTraining && latestSlate && typeof this.game.buildObservatoryTrainingFocusFromSlate === 'function'
            ? this.game.buildObservatoryTrainingFocusFromSlate(latestSlate)
            : null;
        const trainingFocus = currentTraining || fallbackTraining;
        const selectedGuide = typeof this.game.getSelectedObservatoryExpeditionGuide === 'function'
            ? this.game.getSelectedObservatoryExpeditionGuide({ silentSync: true })
            : null;
        const trainingTags = Array.from(new Set(
            [
                ...(Array.isArray(trainingFocus?.trainingTags) ? trainingFocus.trainingTags : []),
                ...(Array.isArray(selectedGuide?.trainingTags) ? selectedGuide.trainingTags : []),
                ...(Array.isArray(latestSlate?.tags)
                    ? latestSlate.tags
                        .map((entry) => String(entry || '').trim())
                        .filter((entry) => /^训练·/.test(entry))
                        .map((entry) => entry.replace(/^训练·/, ''))
                    : [])
            ].map((entry) => String(entry || '').trim()).filter(Boolean)
        )).slice(0, 4);
        const goalHighlights = Array.isArray(trainingFocus?.goalHighlights) && trainingFocus.goalHighlights.length > 0
            ? trainingFocus.goalHighlights.map((entry) => String(entry || '').trim()).filter(Boolean).slice(0, 3)
            : Array.isArray(latestSlate?.answerReview?.goalHighlights)
                ? latestSlate.answerReview.goalHighlights.map((entry) => String(entry || '').trim()).filter(Boolean).slice(0, 3)
                : [];
        const routeFocusLine = String(
            trainingFocus?.routeFocusLine
            || latestSlate?.practiceTopic?.routeFocusLine
            || latestSlate?.practiceTopic?.routeHint
            || latestSlate?.observatoryLink?.routeFocusLine
            || selectedGuide?.routeFocusLine
            || ''
        ).trim();
        const compareHint = String(
            trainingFocus?.compareHint
            || latestSlate?.practiceTopic?.compareHint
            || latestSlate?.observatoryLink?.compareHint
            || selectedGuide?.compareHint
            || ''
        ).trim();
        const trainingAdvice = String(
            trainingFocus?.trainingAdvice
            || latestSlate?.answerReview?.trainingAdvice
            || selectedGuide?.coachBrief
            || selectedGuide?.drillObjective
            || ''
        ).trim();
        const routeNodeTypes = this.inferSanctumAgendaNodeTypes([
            routeFocusLine,
            compareHint,
            trainingAdvice,
            goalHighlights,
            Array.isArray(selectedGuide?.preferredNodes) ? selectedGuide.preferredNodes : []
        ], trainingFocus?.themeKey || latestSlate?.themeKey || selectedGuide?.themeKey || '');
    ...
    buildSanctumAgendaCatalog() {
        ...
        const routeNodeTypes = this.normalizeSanctumAgendaNodeTypes([
            source.preferredNodes,
            source.routeNodeTypes
        ], 4);
        ...
        return {
            steady_line: buildRecord({
                ...
                decisionOptions: [
                    {
                        ...
                        weightShift: { rest: 0.012, event: 0.008, enemy: -0.006 },
                        rewardDelta: { ringExp: -4 },
                    ...
                    },
                    {
                        ...
                        weightShift: { observatory: 0.012, event: 0.012, rest: -0.004 },
                        rewardDelta: { insight: 1, ringExp: 6 },
                        targetDelta: 1,
                        minCompletedGoalsDelta: 1,
                ...
                contractOptions: [
                    {
                        ...
                        weightShift: { observatory: 0.01, event: 0.01, rest: -0.004 },
                        signCost: { insight: 1, karma: 0 },
                        burdenLine: '立契后会追加 1 点天机契押，路线也会继续压向观星 / 事件，修整窗口更少。',
                        bonusReward: { insight: 1, ringExp: 6 },
                    ...
                    },
                    {
                        ...
                        weightShift: { rest: 0.012, spirit_grotto: 0.01, enemy: -0.006 },
                        signCost: { insight: 0, karma: 1 },
                        burdenLine: '立契后会追加 1 点业果契押，为保住定锚，后续样本会更偏修整 / 灵契，爆发窗口更慢。',
                        bonusReward: { insight: 1, karma: 1, ringExp: 2 },
                    ...
                    },
                    {
                        ...
                        requireNoDeviation: false,
                        acceptedTones: ['selected', 'completed'],
                        weightShift: { memory_rift: 0.014, observatory: 0.008, event: 0.008, rest: -0.006 },
                        signCost: { insight: 1, karma: 1 },
                        burdenLine: '立契后会立即垫付 1 点天机与 1 点业果，后续路线也会更偏裂隙 / 观星，容错更紧。',
                        bonusReward: { insight: 2, karma: 1, ringExp: 6 },
                ...
                summaryLine: source.trainingAdvice
                    || `围绕【${themeLabel}】把路线继续写稳，优先命中情报与修整节点。`
            }),
            pressure_line: buildRecord({
                ...
                decisionOptions: [
                    {
                        ...
                        weightShift: { rest: 0.014, shop: 0.008, elite: -0.006 },
                        rewardDelta: { ringExp: -4 },
                        acceptedTonesAdd: ['suggested'],
                    ...
                    },
                    {
                        ...
                        weightShift: { trial: 0.014, forbidden_altar: 0.014, memory_rift: 0.01, rest: -0.006 },
                        rewardDelta: { karma: 1, ringExp: 6 },
                        targetDelta: 1,
                        minCompletedGoalsDelta: 1,
                ...
                contractOptions: [
                    {
                        ...
                        requireNoDeviation: false,
                        acceptedTones: ['selected', 'completed'],
                        weightShift: { elite: 0.01, trial: 0.012, forbidden_altar: 0.012, rest: -0.006 },
                        signCost: { insight: 0, karma: 1 },
                        burdenLine: '立契后会追加 1 点业果契押，后续精英 / 禁术偏置会更重，章节容错更低。',
                        bonusReward: { karma: 1, ringExp: 8 },
                    ...
                    },
                    {
                        ...
                        weightShift: { memory_rift: 0.012, rest: 0.008, elite: 0.008 },
                        signCost: { insight: 1, karma: 0 },
                    ...
                    },
                    {
                        ...
                        requireNoDeviation: false,
                        acceptedTones: ['selected', 'completed'],
                        weightShift: { forbidden_altar: 0.014, memory_rift: 0.014, trial: 0.01, rest: -0.008, shop: -0.006 },
                        signCost: { insight: 1, karma: 1 },
                        burdenLine: '立契后会立即垫付 1 点天机与 1 点业果，后续路线更少给休整与商路缓冲。',
                        bonusReward: { insight: 1, karma: 2, ringExp: 10 },
                ...
                summaryLine: source.highlightLine
                    || `把【${themeLabel}】里的高压处理拆开研究，优先去精英、试炼与禁术节点做验证。`
            }),
            archive_line: buildRecord({
                ...
                decisionOptions: [
                    {
                        ...
                        weightShift: { observatory: 0.01, event: 0.01, memory_rift: -0.004 },
                        rewardDelta: { ringExp: -2 },
                        minCompletedGoalsDelta: -1,
                        acceptedTonesAdd: ['suggested'],
                    ...
                    },
                    {
                        ...
                        weightShift: { memory_rift: 0.014, spirit_grotto: 0.012, event: -0.004 },
                        rewardDelta: { insight: 1, ringExp: 6 },
                        targetDelta: 1,
                        minCompletedGoalsDelta: 1,
                ...
                contractOptions: [
                    {
                        ...
                        requireNoDeviation: true,
                        acceptedTones: ['selected', 'completed'],
                        weightShift: { memory_rift: 0.014, observatory: 0.01, event: -0.004 },
                        signCost: { insight: 1, karma: 0 },
                        burdenLine: '立契后会追加 1 点天机契押，为保镜段完整，偏题与旁线余地都会更少。',
                        bonusReward: { insight: 1, ringExp: 5 },
                    ...
                    },
                    {
                        ...
                        requireNoDeviation: false,
                        acceptedTones: ['completed'],
                        weightShift: { spirit_grotto: 0.014, event: 0.012, observatory: 0.008 },
                        signCost: { insight: 0, karma: 1 },
                        burdenLine: '立契后会追加 1 点业果契押，为了补全合卷，灵契与事件线要吃更多注意力。',
                        bonusReward: { insight: 1, karma: 1, ringExp: 6 },
                    ...
                    },
                    {
                        ...
                        requireNoDeviation: false,
                        acceptedTones: ['selected', 'completed'],
                        weightShift: { event: 0.012, memory_rift: 0.01, spirit_grotto: 0.01, enemy: -0.006 },
                        signCost: { insight: 1, karma: 1 },
                        burdenLine: '立契后会立即垫付 1 点天机与 1 点业果，后续样本会更偏旁注 / 裂隙，主线收束节奏更紧。',
                        bonusReward: { insight: 1, karma: 1, ringExp: 7 },
                ...
                summaryLine: source.compareHint
                    || `把【${themeLabel}】的样本与归卷档案重新压成研究线，优先看观星、裂隙与灵契节点。`
            })
        };
    }
        ...
        const candidates = Object.values(catalog)
            .map((entry) => {
                ...
                return {
                    ...
                    statusLine: activeMatch
                        ? `进行中 · ${active.progress}/${active.target}`
                        : (!source.ready
                            ? '待归卷提供研究题面'
                            : (affordable ? `立项代价 · ${entry.costLine}` : `资源不足 · 还缺 ${missing.join(' / ')}`)),
                    buttonLabel: activeMatch
                        ? '当前议程'
                        : (active
                            ? '已有本轮议程'
                            : (!source.ready
                                ? '先写成一份归卷'
                                : (affordable ? '立为本轮议程' : '资源不足'))),
    ...
    activateSanctumAgenda(agendaId = '') {
        ...
        const next = this.normalizeSanctumAgendaRecord({
            ...
            matchedNodeIds: [],
            hitHistory: [],
            boundChapterIndex: Math.max(
                0,
                Math.floor(Number(
                    expeditionState?.chapterIndex
                    || ((typeof this.game.getLatestRunSlate === 'function' ? this.game.getLatestRunSlate()?.chapterIndex : 0) || 0) + 1
                ) || 0)
            ),
    ...
    applySanctumAgendaDecisionChoice(activeAgenda = null, decisionOption = null) {
        ...
        agenda.acceptedTones = Array.from(new Set([
            ...(Array.isArray(agenda.acceptedTones) ? agenda.acceptedTones : []),
            ...(Array.isArray(option.acceptedTonesAdd) ? option.acceptedTonesAdd : [])
    ...
    chooseSanctumAgendaDecision(optionId = '') {
        ...
        const option = Array.isArray(activeAgenda.decisionOptions)
            ? activeAgenda.decisionOptions.find((entry) => entry.id === safeId)
            : null;
    ...
    applySanctumAgendaContractChoice(activeAgenda = null, contractOption = null) {
        ...
        const matchedContractIds = Array.isArray(agenda.hitHistory)
            ? Array.from(new Set(
                agenda.hitHistory
                    .filter((entry) => contractNodeTypes.includes(String(entry?.nodeType || '').trim()))
        ...
        agenda.selectedContractLine = [
            option.statusLine || option.summaryLine || '',
            signCostLine ? `契押 ${signCostLine}` : '',
            option.burdenLine || ''
        ].filter(Boolean).join(' · ');
        ...
        agenda.contractAcceptedTones = Array.isArray(option.acceptedTones)
            ? Array.from(new Set(option.acceptedTones.map((entry) => String(entry || '').trim()).filter(Boolean))).slice(0, 4)
            : [];
    ...
    chooseSanctumAgendaContract(optionId = '') {
        ...
        const option = Array.isArray(activeAgenda.contractOptions)
            ? activeAgenda.contractOptions.find((entry) => entry.id === safeId)
            : null;
        if (!option) return null;
    ...
    recordSanctumAgendaNodeProgress(nodeType = '', context = {}) {
        ...
        active.matchedNodeIds = Array.isArray(active.matchedNodeIds)
            ? active.matchedNodeIds.concat(nodeId).slice(-12)
            : [nodeId];
        active.progress = Math.min(active.target, Math.max(0, Math.floor(Number(active.progress) || 0)) + 1);
        active.hitHistory = Array.isArray(active.hitHistory)
            ? active.hitHistory.concat({
        ...
        if (active.selectedContractId && Array.isArray(active.contractNodeTypes) && active.contractNodeTypes.includes(safeNodeType)) {
            if (!Array.isArray(active.contractMatchedNodeIds) || !active.contractMatchedNodeIds.includes(nodeId)) {
                active.contractMatchedNodeIds = Array.isArray(active.contractMatchedNodeIds)
                    ? active.contractMatchedNodeIds.concat(nodeId).slice(-12)
                    : [nodeId];
                active.contractProgress = Math.min(
                    Math.max(1, Math.floor(Number(active.contractTarget) || 1)),
                    Math.max(0, Math.floor(Number(active.contractProgress) || 0)) + 1
                );
            }
        }
        active.updatedAt = Date.now();
        const summaryName = [active.name, active.selectedDecisionLabel, active.selectedContractLabel]
            .filter(Boolean)
            .join(' · ');
        active.summaryLine = active.selectedContractLabel
            ? `${summaryName}：关键节点 ${active.progress}/${active.target}，契约样本 ${active.contractProgress}/${active.contractTarget}。`
            : (active.selectedDecisionLabel
                ? `${summaryName}：已命中 ${active.progress}/${active.target} 次关键节点。`
                : `${active.name}：已命中 ${active.progress}/${active.target} 次关键节点。`);
    ...
    buildSanctumAgendaFailureRecovery(activeAgenda = null, context = {}) {
        ...
        const effortScore = (progressRatio * 0.65)
            + (goalRatio * 0.25)
            + (agenda.selectedDecisionId ? 0.1 : 0)
            + (agenda.selectedContractId ? 0.1 : 0);
        ...
        const primaryResource = (Number(agenda?.cost?.insight) || 0) >= (Number(agenda?.cost?.karma) || 0)
            ? 'insight'
            : 'karma';
        ...
        const rewardLine = rewardParts.length > 0
            ? `，回收 ${rewardParts.join('、')}`
            : '';
    ...
    buildSanctumAgendaFailureRecoveryNotice(resolution = null) {
        ...
        const title = recoveryTierLabel
            ? `${recoveryLabel} · ${recoveryTierLabel}`
            : recoveryLabel;
        const summaryLine = recoveryTierLabel
            ? `洞府已执行${recoveryLabel}（${recoveryTierLabel}）`
            : `洞府已执行${recoveryLabel}`;
        const message = [
            String(source.recoveryLine || '').trim(),
            String(source.recoveryHintLine || '').trim()
        ].filter(Boolean).join('\n');
        return {
            title,
            message,
    ...
    resolveSanctumAgenda(reason = 'realm_clear', context = {}) {
        ...
        const active = state.activeAgenda && state.activeAgenda.agendaId ? state.activeAgenda : null;
        if (!active || active.outcome !== 'active') return null;
        const expeditionState = context.state && typeof context.state === 'object'
            ? context.state
            : (typeof this.game.getExpeditionState === 'function' ? this.game.getExpeditionState() : null);
        const slate = context.slate && typeof context.slate === 'object'
            ? context.slate
            : (typeof this.game.getLatestRunSlate === 'function' ? this.game.getLatestRunSlate() : null);
        const answerSheet = context.answerSheet && typeof context.answerSheet === 'object'
            ? context.answerSheet
            : (expeditionState && typeof this.game.getExpeditionAnswerSheet === 'function'
                ? this.game.getExpeditionAnswerSheet(expeditionState)
                : null);
        const reviewCard = answerSheet?.reviewCard && typeof answerSheet.reviewCard === 'object'
            ? answerSheet.reviewCard
            : (slate?.answerReview && typeof slate.answerReview === 'object' ? slate.answerReview : null);
        const completedGoals = Math.max(
            0,
            Math.floor(Number(answerSheet?.completedGoals) || Number(reviewCard?.completedGoals) || 0)
        );
        const totalGoals = Math.max(
            completedGoals,
            Math.floor(Number(answerSheet?.totalGoals) || 0)
        );
        const ratingTone = ['completed', 'selected', 'suggested', 'idle'].includes(String(answerSheet?.ratingTone || reviewCard?.ratingTone || '').trim())
            ? String(answerSheet?.ratingTone || reviewCard?.ratingTone || '').trim()
            : 'idle';
        const ratingLabel = String(answerSheet?.ratingLabel || reviewCard?.ratingLabel || active.ratingLabel || '').trim();
        const deviated = Array.isArray(answerSheet?.goals)
            ? answerSheet.goals.some((goal) => goal && goal.deviated)
            : false;
        ...
        const resolutionName = active.selectedDecisionLabel
            ? `${active.name}·${active.selectedDecisionLabel}`
            : active.name;
        let outcome = 'failed';
        ...
        const contractSuccess = contractSelected
            && outcome === 'success'
            && contractProgressOk
            && contractGoalOk
            && contractToneOk
        ...
        if (contractSelected) {
            ...
            if (outcome !== 'success') {
                ...
                contractResolutionLine = (active.contractSuccessLine || `锁线契约「${contractName}」兑现：${active.contractBonusLine || '额外奖赏已入账。'}`)
                    + (contractStakeLine ? ` 已支付的契押 ${contractStakeLine} 已一并折成契约收益。` : '');
            } else {
                const contractReason = contractMissReasons.length > 0 ? contractMissReasons.join('；') : '尚未满足额外契约条件。';
                contractResolutionLine = (active.contractFailureLine || `锁线契约「${contractName}」未兑现：${contractReason}`)
                    + (contractStakeLine ? ` 已支付的契押 ${contractStakeLine} 未能折现。` : '');
            }
        }
        const recovery = outcome === 'failed'
            ? this.buildSanctumAgendaFailureRecovery(active, {
                reasonId,
                completedGoals,
                totalGoals
            })
        ...
        const chapterIndex = Math.max(
            0,
            Math.floor(Number(expeditionState?.chapterIndex || slate?.chapterIndex || active.boundChapterIndex) || 0)
        );
        const resolved = this.normalizeSanctumAgendaRecord({
            ...
            reasonId,
            reasonLine,
            summaryLine: outcome === 'success'
                ? `${resolutionName}结题成功：${active.progress}/${active.target} 次关键节点，答卷评级 ${ratingLabel || '成卷'}。${contractSelected ? `｜契约「${active.selectedContractLabel || active.selectedContractId}」${contractSuccess ? '兑现' : '未兑现'}。` : ''}`
                : `${resolutionName}未能结题：${reasonLine}${recovery?.recoveryEligible ? '｜已执行残卷回收。' : ''}`,
        ...
        const grantedLine = outcome === 'success'
            ? this.applySanctumAgendaOutcomeReward(resolved)
            : this.applySanctumAgendaFailureRecovery(resolved);
        resolved.grantedLine = grantedLine || (outcome === 'success'
            ? resolved.rewardLine
            : (resolved.recoveryEligible ? resolved.recoveryLine : ''));
        const aftereffect = this.createFateAftereffectFromSanctumAgenda(resolved, {
            chapterIndex,
            latestRunId: resolved.sourceRunId || ''
        });
        const runtimeAftereffect = aftereffect
            ? this.game.getFateAftereffectRuntimeRecord(aftereffect, { currentChapterIndex: chapterIndex })
            : null;
        resolved.logLine = [
            resolved.summaryLine,
            resolved.recoveryLine || '',
            resolved.contractResolutionLine || '',
            runtimeAftereffect?.summaryLine || '',
            outcome === 'success' ? (resolved.grantedLine || '') : ''
        ].filter(Boolean).join('｜');
        state.activeAgenda = null;
        state.lastResolved = resolved;
        state.history = Array.isArray(state.history)
            ? state.history.concat(resolved).slice(-6)
            : [resolved];
        if (outcome === 'success') {
        ...
        if (typeof Utils !== 'undefined' && typeof Utils.showBattleLog === 'function') {
            Utils.showBattleLog(
                outcome === 'success'
                    ? `【${resolved.name}】结题：${resolved.grantedLine || resolved.contractResolutionLine || resolved.reasonLine}`
                    : `【${resolved.name}】未成：${resolved.recoveryLine || resolved.contractResolutionLine || resolved.reasonLine}`
            );
        }
        if (this.game.currentScreen === 'collection' && this.game.getCollectionHubState?.().section === 'sanctum') {
            this.game.initCollection();
    ...
    getSanctumAgendaExpeditionSnapshot(context = {}) {
        ...
        const active = dashboard.active && dashboard.active.agendaId
            ? {
                ...
                decisionOptions: Array.isArray(dashboard.active.decisionOptions)
                    ? dashboard.active.decisionOptions.map((entry) => ({
                        id: entry.id || '',
                        label: entry.label || '',
                    ...
                    : [],
                contractOptions: Array.isArray(dashboard.active.contractOptions)
                    ? dashboard.active.contractOptions.map((entry) => ({
                        id: entry.id || '',
                        label: entry.label || '',
                        tagLabel: entry.tagLabel || '',
                        summaryLine: entry.summaryLine || '',
            ...
            : null;
        const lastResolved = dashboard.lastResolved
            && dashboard.lastResolved.agendaId
            && (!latestRunId || dashboard.lastResolved.sourceRunId === latestRunId)
            ? {
...
if (typeof window !== 'undefined') {
    window.SanctumAgendaManager = SanctumAgendaManager;
}
