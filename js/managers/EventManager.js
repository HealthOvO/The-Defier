export class EventManager {
    ...
    getChapterEventComposerContext() {
        ...
        const chapterSnapshot = (typeof this.game.getChapterDisplaySnapshot === 'function')
            ? this.game.getChapterDisplaySnapshot(realm)
            : null;
    ...
    recordChapterEventConsequence(payload = {}) {
        const event = payload.event && typeof payload.event === 'object'
            ? payload.event
            : (this.game.currentEvent && typeof this.game.currentEvent === 'object' ? this.game.currentEvent : null);
    ...
    getEndlessEventTuning() {
        if (!this.game.endlessManager) this.game.endlessManager = new EndlessManager(this);
    ...
    executeEventEffect(effect) {
        ...
        switch (effect.type) {
            ...
            case 'openCampfire':
                this.game.closeModal();
                setTimeout(() => {
                    const node = this.game.currentBattleNode || { id: `event-camp-${Date.now()}`, row: 0, type: 'event' };
            ...
            case 'trial':
                ...
                const trialConfig = (typeof this.game.armTrialChallenge === 'function'
                    ? this.game.armTrialChallenge
            ...
            case 'runPathProgress':
            {
                ...
                const applied = !!(
                    runPathMeta
                    && typeof this.game.handleRunPathProgress === 'function'
                    && this.game.handleRunPathProgress(eventType, amount, {
            ...
            case 'law':
                if (effect.random) {
                    ...
                    if (randomLaw && this.game.player.collectLaw({ ...randomLaw })) {
    ...
    getTemporaryEventShopOffers(effect = {}) {
        ...
        const runPathMeta = this.game.player && typeof this.game.player.getRunPathMeta === 'function'
            ? this.game.player.getRunPathMeta()
            : null;
        const pathDoctrineProfile = (this.game.player && typeof this.game.player.getPathDoctrineProfile === 'function')
            ? this.game.player.getPathDoctrineProfile()
            : null;
