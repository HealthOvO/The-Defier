export class MetaProgressionManager {
    ...
    normalizeSeasonVerificationState(source = null) {
        ...
        const lastResolved = root.lastResolved && typeof root.lastResolved === 'object'
            ...
            : (history[0] || null);
        const claimedLaneRewards = typeof this.game.normalizeSeasonBoardClaimedLaneRewards === 'function'
            ? this.game.normalizeSeasonBoardClaimedLaneRewards(root.claimedLaneRewards)
            : {};
        return {
            version: Math.max(1, Math.floor(Number(root.version) || defaults.version)),
            weekTag: String(root.weekTag || '').trim().slice(0, 24),
    ...
    recordSeasonVerificationResult(source = null) {
        ...
        const boardContext = root.seasonBoard && typeof root.seasonBoard === 'object'
            ? root.seasonBoard
            : (typeof this.game.getSeasonBoardSnapshot === 'function'
                ? this.game.getSeasonBoardSnapshot()
                : null);
    ...
    getHeavenlyMandateWeekMeta(dateOverride = null) {
        ...
        const endlessMeta = typeof this.game.getEndlessWeekMeta === 'function'
            ? this.game.getEndlessWeekMeta(dateRef)
            : null;
        const challengeWeekTag = typeof this.game.getChallengeRotationKey === 'function'
            ? String(this.game.getChallengeRotationKey('weekly', dateRef) || '').trim()
            : '';
    ...
    getHeavenlyMandateSignalSnapshot(options = {}) {
        ...
        const expeditionState = typeof this.game.getExpeditionState === 'function'
            ...
            : null;
        const answerSheet = expeditionState && typeof this.game.getExpeditionAnswerSheet === 'function'
            ? this.game.getExpeditionAnswerSheet(expeditionState)
            : null;
        const trainingFocus = typeof this.game.getObservatoryTrainingFocus === 'function'
        ...
        const selectedGuide = typeof this.game.getSelectedObservatoryExpeditionGuide === 'function'
            ...
            : null;
        const weeklyBundle = typeof this.game.buildChallengeBundle === 'function'
            ? this.game.buildChallengeBundle('weekly', weekMeta.dateRef)
            : null;
        const weeklyArchiveEntries = typeof this.game.getObservatoryArchiveEntries === 'function'
            ? this.game.getObservatoryArchiveEntries({
                mode: 'weekly',
        ...
        const agendaSnapshot = typeof this.game.getSanctumAgendaExpeditionSnapshot === 'function'
            ? this.game.getSanctumAgendaExpeditionSnapshot({ latestRunId: String((weeklyLatestSlate || latestSlate)?.id || '') })
            : null;
        const endlessState = typeof this.game.ensureEndlessState === 'function'
            ? this.game.ensureEndlessState()
            : (this.game.endlessState && typeof this.game.endlessState === 'object' ? this.game.endlessState : null);
        const endlessSeason = typeof this.game.getEndlessSeasonProfile === 'function'
            ? this.game.getEndlessSeasonProfile(
    ...
    buildHeavenlyMandateBoard(options = {}) {
        ...
        const seasonSignals = typeof this.game.getSeasonBoardSignalSnapshot === 'function'
            ? this.game.getSeasonBoardSignalSnapshot({
                ...options,
                latestSlate
            })
            : null;
        const seasonPhase = seasonSignals && typeof this.game.getSeasonBoardPhaseMeta === 'function'
            ? this.game.getSeasonBoardPhaseMeta(seasonSignals)
            : null;
        const seasonSettlementState = seasonSignals && typeof this.game.buildSeasonBoardSettlementState === 'function'
            ? this.game.buildSeasonBoardSettlementState(seasonSignals, seasonPhase)
            : null;
    ...
    getHeavenlyMandateExpeditionSnapshot(options = {}) {
        ...
        const seasonBoard = typeof this.game.getSeasonBoardSnapshot === 'function'
            ? this.game.getSeasonBoardSnapshot(options.latestSlate ? { latestSlate: options.latestSlate } : {})
            : null;
...
if (typeof window !== 'undefined') {
    window.MetaProgressionManager = MetaProgressionManager;
}
