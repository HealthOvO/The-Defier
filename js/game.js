...
class Game {
                            ...
                            seasonBoard: expeditionMeta.seasonBoard
                                ? {
                                    ...
                                    weekVerdictLedger: expeditionMeta.seasonBoard.weekVerdictLedger?.current
                                        ...
                                        : null,
                                    verificationOrders: Array.isArray(expeditionMeta.seasonBoard.verificationOrders)
                                        ? expeditionMeta.seasonBoard.verificationOrders.map((entry) => ({
                                            id: entry?.id || '',
                                            type: entry?.type || '',
                                            role: entry?.role || '',
                                            label: entry?.label || '',
                                    ...
                                    lanes: Array.isArray(expeditionMeta.seasonBoard.lanes)
                                        ? expeditionMeta.seasonBoard.lanes.map((lane) => ({
                                            id: lane?.id || '',
                                            label: lane?.label || '',
                                            icon: lane?.icon || '',
                            ...
                            aftereffects: expeditionMeta.aftereffects
                                ? {
                                    ...
                                    primary: expeditionMeta.aftereffects.primary
                                        ...
                                        : null,
                                    records: Array.isArray(expeditionMeta.aftereffects.records)
                                        ? expeditionMeta.aftereffects.records.map((entry) => ({
                                            recordId: entry?.recordId || '',
                                            icon: entry?.icon || '',
                                            name: entry?.name || '',
    ...
    getRunPathCatalog() {
        if (!this.runManager) this.runManager = new RunManager(this);
    ...
    getEventModalRefs() {
        ...
        return this.eventManager.getEventModalRefs();
    }

    resetModalPresentation(modal) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.resetModalPresentation(modal);
    }
    ...
    createDefaultEndlessState() {
        if (!this.endlessManager) this.endlessManager = new EndlessManager(this);
    ...
    createDefaultChapterEventLedger() {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.createDefaultChapterEventLedger();
    }

    normalizeChapterEventLedger(rawLedger = null) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.normalizeChapterEventLedger(rawLedger);
    }

    ensureChapterEventLedger() {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.ensureChapterEventLedger();
    }

    getChapterEventLedgerSaveState() {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.getChapterEventLedgerSaveState();
    }

    applyChapterEventLedgerSaveState(rawLedger = null) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.applyChapterEventLedgerSaveState(rawLedger);
    }

    getChapterEventLedgerSnapshot(options = {}) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.getChapterEventLedgerSnapshot(options);
    }

    getChapterEventComposerContext() {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.getChapterEventComposerContext();
    }

    recordChapterEventConsequence(payload = {}) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.recordChapterEventConsequence(payload);
    }

    buildEventChoiceEffectSummary(choice = {}) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.buildEventChoiceEffectSummary(choice);
    }

    getEventNarrativePresentation(event, node = null) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.getEventNarrativePresentation(event, node);
    }

    applyEventModalPresentation(presentation = {}) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.applyEventModalPresentation(presentation);
    }
    ...
    getEndlessEventTuning() {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.getEndlessEventTuning();
    }
    ...
    getLegacyRunDoctrineForPreset(presetId) {
        if (!this.runManager) this.runManager = new RunManager(this);
        return this.runManager.getLegacyRunDoctrineForPreset(presetId);
    }
    ...
    applyLegacyRunDoctrine(player, presetId = null, secondaryPresetId = null) {
        if (!this.runManager) this.runManager = new RunManager(this);
        return this.runManager.applyLegacyRunDoctrine(player, presetId, secondaryPresetId);
    }

    applyLegacyRunMission(player, presetId = null) {
        if (!this.runManager) this.runManager = new RunManager(this);
        return this.runManager.applyLegacyRunMission(player, presetId);
    }
    ...
    async handlePVPVictory() {
        ...
        if (result && result.rejected) {
            ...
            const weekMeta = typeof this.getHeavenlyMandateWeekMeta === 'function'
                ? this.getHeavenlyMandateWeekMeta()
                : null;
            ...
            const boardBeforeVerification = typeof this.getSeasonBoardSnapshot === 'function'
            ...
            this.recordSeasonVerificationResult({
                ...
                detailLine: [
                    delta ? `积分 ${delta >= 0 ? `+${delta}` : `${delta}`}` : '',
                    Number(result?.coinsAwarded) > 0 ? `天道币 +${Math.floor(Number(result.coinsAwarded) || 0)}` : '',
                    pendingDebt ? '这场胜局会优先清掉季盘欠卷。' : '这场胜局会把本周押卷往正卷推进。'
                ].filter(Boolean).join('｜'),
                statusLine: '天道榜 · 通过',
                anchorSection: 'pvp',
                priority: 1
    ...
    showRewardScreen(gold, canSteal, stealEnemy, ringExp = 0, strategicGain = null) {
        if (!this.rewardView) this.rewardView = new RewardView(this);
    ...
    showEventModal(event, node) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.showEventModal(event, node);
    }
    ...
    showTrialChallengeSelection(node) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.showTrialChallengeSelection(node);
    }

    showForgeChoiceModal(node, costs = {}) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.showForgeChoiceModal(node, costs);
    }

    showForgeCardDraft(node, costs = {}) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.showForgeCardDraft(node, costs);
    }

    describeTreasureWorkshopStatus(treasure) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.describeTreasureWorkshopStatus(treasure);
    }

    showForgeTreasureDraft(node, mode = 'reforge', costs = {}) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.showForgeTreasureDraft(node, mode, costs);
    }
    ...
    executeEventEffect(effect) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.executeEventEffect(effect);
    }

    getTemporaryEventShopOffers(effect = {}) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.getTemporaryEventShopOffers(effect);
    }

    applyTemporaryEventShopOffer(offer) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.applyTemporaryEventShopOffer(offer);
    }

    showTemporaryEventShop(effect = {}) {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.showTemporaryEventShop(effect);
    }

    // 事件中升级卡牌 (Revised with Preview)
    showEventUpgradeCard() {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.showEventUpgradeCard();
    }

    // 事件完成
    onEventComplete() {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.onEventComplete();
    }
    ...
    bindLibraryEvents() {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.bindLibraryEvents();
    }
    ...
    bindRingEvents() {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.bindRingEvents();
    }
    ...
    getStrategicEngineeringEventBiasProfile() {
        if (!this.eventManager) this.eventManager = new EventManager(this);
        return this.eventManager.getStrategicEngineeringEventBiasProfile();
    }
...
最大生命降至 ${this.player.maxHp}。`, '🩸');
    ...
    showTreasureCompendium() {
        if (!this.inventoryView) this.inventoryView = new InventoryView(this);
        this.inventoryView.showTreasureCompendium();
    }
