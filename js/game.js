...
class Game {
    ...
    showPlayerInfo() {
        ...
        if (avatarEl) avatarEl.textContent = char.avatar; // 这里假设avatar是emoji
        if (nameEl) nameEl.textContent = `${char.name} · ${char.title}`;
        if (titleEl) {
            titleEl.textContent = '逆命印记'; // 或者根据角色不同
    ...
    updatePlayerDisplay() {
        ...
        // 也可以更新头像（如果是emoji的话）
        // 目前CSS里的头像可能是背景图，或者emoji容器
        const playerAvatars = document.querySelectorAll('.player-avatar, .char-avatar');
        // 注意：这里可能会覆盖角色选择界面的头像，要小心
        // 我们只针对战斗界面和地图界面的头像
        // 实际上，战斗界面的头像是通过CSS类还是内容？
        // index.html: <div class="player-avatar">..<span>林风</span></div>
        // 我们已经在上面更新了span
    }
}
