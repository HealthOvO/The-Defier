...
export function formatBattleLogTime(timestamp) {
...
export function getBattleLogFilters() {
  return LOG_FILTERS.map(item => ({
    ...item
  }));
}

export function buildBattleLogPanelShellMarkup(activeFilter = "all") {
  return `
...
export function buildBattleLogListMarkup(records = [], activeFilter = "all") {
  ...
  return filteredRecords.map(item => {
    ...
    const time = escapeHtml(formatBattleLogTime(item?.ts));
...
export function buildRewardBattleMetaMarkup(meta = {}) {
  const chips = [];
  const toTierText = value => `${"I".repeat(Math.max(1, Math.min(3, Number(value) || 1)))}阶`;
  if (meta.encounter) {
    chips.push(`<span class="reward-meta-chip chip-encounter">遭遇战利：${escapeHtml(meta.encounter.themeName || "未知轮段")}（${escapeHtml(toTierText(meta.encounter.tierStage))}）</span>`, `<span class="reward-meta-chip chip-gold">遭遇灵石 +${Math.max(0, Math.floor(Number(meta.encounter.goldBonus) || 0))}</span>`, `<span class="reward-meta-chip chip-exp">遭遇命环经验 +${Math.max(0, Math.floor(Number(meta.encounter.ringExpBonus) || 0))}</span>`);
  }
  if (meta.squad) {
    chips.push(`<span class="reward-meta-chip chip-squad">敌阵战利：${escapeHtml(meta.squad.squadName || "未知编队")}</span>`, `<span class="reward-meta-chip chip-gold">编队灵石 +${Math.max(0, Math.floor(Number(meta.squad.goldBonus) || 0))}</span>`, `<span class="reward-meta-chip chip-exp">编队命环经验 +${Math.max(0, Math.floor(Number(meta.squad.ringExpBonus) || 0))}</span>`);
