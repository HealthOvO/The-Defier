...
export const PVPService = {
  ...
  getDivisionRewardMultiplier(scoreOrDivision = null) {
    const cfg = this.seasonConfig && this.seasonConfig.divisionRewardMultipliers ? this.seasonConfig.divisionRewardMultipliers : {};
    let division = null;
  ...
  getCurrentSeasonMeta() {
    ...
    const score = this.currentRankData && typeof this.currentRankData.score === 'number' ? this.currentRankData.score : 1000;
    const division = this.currentRankData && this.currentRankData.division ? this.currentRankData.division : this.getDivisionByScore(score);
    return {
  ...
  normalizePVPDangerProfile(profile = null) {
    ...
    return {
      ...
      tags: Array.isArray(source.tags) ? source.tags.filter(tag => typeof tag === 'string' && tag.trim()).slice(0, 4) : [],
  ...
  getPvpResultReview({
    ...
    const subtitleParts = [safeOpponent.division || this.getDivisionByScore(safeOpponent.score), `第${Math.max(1, Math.floor(Number(safeOpponent.realm) || 1))}层`];
    ...
    return {
      ...
      tags: [profile.dominantAxisLabel, safeOpponent.archetypeLabel, safeOpponent.division].filter(Boolean).slice(0, 3),
  ...
  estimatePvpSnapshotFromRank(rank = null, context = {}) {
    const safeRank = rank && typeof rank === 'object' ? rank : this.getDefaultLocalRank();
    ...
    const seed = this.hashPvpPreviewSeed([safeRank.objectId || '', safeRank.user && safeRank.user.objectId ? safeRank.user.objectId : '', safeRank.user && safeRank.user.username ? safeRank.user.username : '', safeRank.score || 0, safeRank.realm || 1, division, context.listIndex || 0].join('|'));
    ...
    if (scoreGap >= 80) style = 'aggressive';else if (realmGap >= 1 || division === '天穹榜') style = seed % 2 === 0 ? 'fortified' : 'balanced';
    const personality = style === 'aggressive' ? 'slaughter' : style === 'fortified' ? 'longevity' : 'balanced';
    ...
    const guardianFormation = style !== 'aggressive' ? seed % 2 === 0 : seed % 5 === 0;
  ...
  getDeckRoleWeights(deck = [], preferredArchetype = 'balanced') {
    ...
    (Array.isArray(deck) ? deck : []).forEach(card => {
      ...
      if (type === 'attack') counts.attack += 1;else if (type === 'defense') counts.defense += 1;else counts.utility += 1;
    });
  ...
  getPVPDangerProfile(opponentSource = null, context = {}) {
    if (opponentSource && typeof opponentSource === 'object' && opponentSource.axes && opponentSource.dominantAxisId) {
    ...
    const rank = source.rank || source.opponent && source.opponent.rank || (source.user || source.score || source.division ? source : null) || null;
    ...
    const myRank = context.myRank || this.currentRankData || this.loadLocalRank();
    const myScore = Math.max(0, Number(context.myScore !== undefined ? context.myScore : myRank && myRank.score) || 1000);
    const myRealm = Math.max(1, Math.floor(Number(context.myRealm !== undefined ? context.myRealm : myRank && myRank.realm) || 1));
    ...
    const estimatedSnapshot = !nestedBattleData || !nestedBattleData.me ? this.estimatePvpSnapshotFromRank(safeRank, {
      myScore,
      myRealm,
      listIndex: context.listIndex || 0
    }) : null;
    const battleData = this.normalizeBattleData(nestedBattleData && typeof nestedBattleData === 'object' ? nestedBattleData : estimatedSnapshot ? estimatedSnapshot.battleData : {});
    const config = ghost && ghost.config && typeof ghost.config === 'object' ? ghost.config : {};
    ...
    const personalityRules = battleData.personalityRules && typeof battleData.personalityRules === 'object' ? battleData.personalityRules : null;
    const guardianFormation = !!(config.guardianFormation || estimatedSnapshot && estimatedSnapshot.guardianFormation);
    ...
    const burstValue = this.clampPvpDangerValue(18 + divisionDifficulty * 0.34 + Math.max(0, scoreGap) * 0.07 + Math.max(0, realmGap) * 5 + roleWeights.attackRate * 34 + (Math.max(1, Number(battleData.me.energy) || 3) - 3) * 7 + archetypeMeta.burstBonus + Math.max(0, (personalityRules && Number(personalityRules.damageMul) || 1) - 1) * 70, 0, 100);
    const attritionValue = this.clampPvpDangerValue(16 + divisionDifficulty * 0.36 + Math.max(0, Math.floor(Number(battleData.me.maxHp) || 100) - 90) * 0.38 + roleWeights.defenseRate * 34 + roleWeights.utilityRate * 8 + archetypeMeta.attritionBonus + (guardianFormation ? 6 : 0) + Math.max(0, (personalityRules && Number(personalityRules.hpMul) || 1) - 1) * 52, 0, 100);
    const controlValue = this.clampPvpDangerValue(14 + divisionDifficulty * 0.28 + roleWeights.utilityRate * 40 + archetypeMeta.controlBonus + (guardianFormation ? 18 : 0) + Math.max(0, Math.floor(Number(personalityRules && personalityRules.regenEnergyPerTurn) || 0)) * 6 + (confidence === 'estimated' ? 5 : 0), 0, 100);
    const executionValue = this.clampPvpDangerValue(15 + divisionDifficulty * 0.42 + Math.max(0, scoreGap) * 0.05 + Math.max(0, realmGap) * 4 + Math.max(8, Math.floor(Number(roleWeights.total) || 8)) * 0.9 + archetypeMeta.executionBonus + (guardianFormation ? 4 : 0) + (confidence === 'estimated' ? 6 : 2), 0, 100);
    const axes = [{
      id: 'burst',
    ...
    const index = this.clampPvpDangerValue(18 + axisAverage * 0.58 + dominantAxis.value * 0.14 + divisionDifficulty * 0.26 + Math.max(0, scoreGap) * 0.015 + Math.max(0, realmGap) * 1.6, 0, 100);
    ...
    if (scoreGap > 0) contextSignals.push(`榜差 +${scoreGap}`);else if (scoreGap < 0) contextSignals.push(`榜差 ${scoreGap}`);
    ...
    const tags = [safeRank.division || this.getDivisionByScore(safeRank.score), archetypeMeta.label, guardianFormation ? '守阵镜像' : '', scoreGap >= 60 ? '高榜压制' : scoreGap <= -40 ? '可主动抢节奏' : '同段细局'].filter(Boolean).slice(0, 4);
    ...
    const note = confidence === 'estimated' ? '榜单推演基于榜位、境界与套路估算，最终匹配结果可能不同。' : '已按当前残影快照推演本场对局风险，可直接拿来读题。';
  ...
  normalizeFocusRank(rawRank = null) {
    ...
    const objectId = src.objectId || `focus-rank-${this.hashPvpPreviewSeed([safeUser.objectId, safeUser.username, score, realm].join('|')).toString(16)}`;
    return {
      objectId,
      user: safeUser,
      score,
  ...
  loadLocalSnapshot() {
    ...
    try {
      ...
      const normalizedData = this.normalizeBattleData(typeof parsed.data === 'string' ? JSON.parse(parsed.data) : parsed.data);
      return {
        ...parsed,
  ...
  normalizeEconomyState(raw) {
    ...
    const matchHistory = Array.isArray(src.matchHistory) ? src.matchHistory.filter(it => it && typeof it === 'object').slice(-24).map(it => this.normalizeMatchHistoryEntry(it)) : [];
    const equippedSkinId = typeof src.equippedSkinId === 'string' && src.equippedSkinId && ownedItems[src.equippedSkinId] ? src.equippedSkinId : null;
    const equippedTitleId = typeof src.equippedTitleId === 'string' && src.equippedTitleId && ownedItems[src.equippedTitleId] ? src.equippedTitleId : null;
    return {
      version: 1,
  ...
  getShopCatalog() {
    const source = typeof PVP_SHOP_ITEMS !== 'undefined' && PVP_SHOP_ITEMS ? PVP_SHOP_ITEMS : {
      cards: [],
      items: [],
      cosmetics: []
    };
  ...
  getShopItemState(itemId, state = null, itemOverride = null) {
    ...
    const equipped = !!(item.type === 'skin' && economy.equippedSkinId === item.id || item.type === 'title' && economy.equippedTitleId === item.id);
    ...
    let reason = 'ok';
    if (equipped) reason = 'equipped';else if (equippable) reason = 'equippable';else if (blockedByOwnership) reason = 'owned';else if (soldOut) reason = 'sold_out';else if (insufficient) reason = 'insufficient';
    return {
      exists: true,
      buyable,
      owned,
  ...
  calculateRewardBreakdown(options = {}, state = null) {
    ...
    const economy = state ? this.normalizeEconomyState(state) : this.loadEconomyState();
    const myRating = Math.max(0, Number(options.myRating || options.myScore || this.currentRankData && this.currentRankData.score || 1000) || 1000);
    ...
    const streakMultiplier = isWin ? Math.min(1.25, 1 + streakBase * 0.03) : Math.min(1.12, 1 + streakBase * 0.02);
  ...
  getRewardPreview(isWin = true, opponentRating = 1000) {
    const reward = this.calculateRewardBreakdown({
      isWin,
      opponentRating,
      isRanked: true
    });
  ...
  getFocusDuelSlip(focusSource = null, options = {}) {
    ...
    const myRank = options.myRank && typeof options.myRank === 'object' ? options.myRank : this.currentRankData || this.loadLocalRank();
    ...
    const dangerProfile = source.dangerProfile ? this.normalizePVPDangerProfile(source.dangerProfile) : this.getPVPDangerProfile({
      rank
    }, {
      myRank,
      myScore,
    ...
    const ratingPreview = this.getRatingDeltaPreview(rank.score, {
      myRating: myScore
    });
  ...
  getPvpSeasonSegmentMeta({
    ...
    const phaseLabel = scoreGap >= 60 ? '高榜压制' : scoreGap <= -40 ? '可主动抢节奏' : '同段细局';
  ...
  getPvpRankingComparisonMeta({
    ...
    const pool = (Array.isArray(listContext) ? listContext : []).map(entry => {
  ...
  getPvpDossierHistoryMeta({
    ...
    const seasonHistory = this.getRecentMatchHistory(24, state).filter(item => !item.seasonId || item.seasonId === seasonMeta.id);
    const targetDivision = safeRank.division || this.getDivisionByScore(safeRank.score);
    ...
    const trendAvgDri = recentTrend.length > 0 ? Math.round(recentTrend.reduce((sum, item) => sum + (Number(item.dangerIndex) || 0), 0) / recentTrend.length) : 0;
    const trendAvgDelta = recentTrend.length > 0 ? Math.round(recentTrend.reduce((sum, item) => sum + (Number(item.ratingDelta) || 0), 0) / recentTrend.length) : 0;
    const directAvgDelta = directSample.length > 0 ? Math.round(directSample.reduce((sum, item) => sum + (Number(item.ratingDelta) || 0), 0) / directSample.length) : 0;
    const directAvgCoins = directSample.length > 0 ? Math.round(directSample.reduce((sum, item) => sum + (Number(item.coinsAwarded) || 0), 0) / directSample.length) : 0;
    const latestDirect = directSample[0] || null;
    const sameModeCount = brief.modeId ? seasonHistory.filter(item => item.modeId && item.modeId === brief.modeId).length : 0;
    const ledgerScopeChips = [targetDivision, profile.dominantAxisLabel || '', seasonSegment.segmentLabel || '', brief.modeLabel || ''].filter(chip => typeof chip === 'string' && chip.trim()).slice(0, 4);
    ...
    if (seasonHistory.length > 0) {
      ...
      if (trendSample.length > 0) {
        const modeLine = brief.modeLabel && sameModeCount > 0 ? `；${brief.modeLabel} ${sameModeCount} 场` : '';
  ...
  getFocusOpponentDossier(focusSource = null, options = {}) {
    ...
    const duelBrief = source.duelBrief && typeof source.duelBrief === 'object' ? source.duelBrief : this.getFocusDuelSlip({
      rank,
      dangerProfile
    }, {
      myRank,
    ...
    const dossierTags = Array.from(new Set([...(Array.isArray(dangerProfile.tags) ? dangerProfile.tags : []), ...(Array.isArray(duelBrief.tags) ? duelBrief.tags : []), targetDivision, archetypeLabel, seasonMeta.segmentLabel, comparisonMeta.shortTag].filter(tag => typeof tag === 'string' && tag.trim()))).slice(0, 6);
    return {
      targetName: safeName,
      targetRankId: rank.objectId || '',
      targetDivision,
  ...
  getEquippedCosmetics(state = null) {
    ...
    return {
      skin: skinItem ? {
        id: skinItem.id,
        name: skinItem.name,
        skinId: skinItem.skinId || skinItem.id,
        icon: skinItem.icon || '👘'
      } : null,
      title: titleItem ? {
        id: titleItem.id,
        name: titleItem.name,
        titleId: titleItem.titleId || titleItem.id,
        icon: titleItem.icon || '👑'
      } : null
    };
  },
  ...
  unequipCosmeticItem(itemId) {
    ...
    const economy = this.loadEconomyState();
    const isEquipped = !!(item.type === 'skin' && economy.equippedSkinId === item.id || item.type === 'title' && economy.equippedTitleId === item.id);
    if (!isEquipped) {
      return {
        success: false,
        message: '该外观未佩戴',
  ...
  applyShopReward(item, gameRef = null) {
    ...
    const gameInstance = gameRef || (typeof window !== 'undefined' ? window.game : (typeof globalThis !== 'undefined' && globalThis.game ? globalThis.game : null));
    const player = gameInstance && gameInstance.player ? gameInstance.player : null;
    if (item.type === 'card') {
      const cardData = item.data && typeof item.data === 'object' ? JSON.parse(JSON.stringify(item.data)) : null;
      ...
      if (player && typeof player.addCardToDeck === 'function' && cardData) {
        ...
        if (typeof gameInstance.autoSave === 'function') gameInstance.autoSave();
        return {
          applied: true,
          detail: 'card_added'
        };
    ...
    if (item.type === 'skin') {
      return {
        applied: false,
        detail: `skin_unlocked:${item.skinId || item.id}`
      };
    }
    if (item.type === 'title') {
      return {
        applied: false,
        detail: `title_unlocked:${item.titleId || item.id}`
      };
    }
    return {
      applied: false,
      detail: 'no_runtime_effect'
    };
  },
  purchaseShopItem(itemId, options = {}) {
    ...
    if (!itemState.buyable) {
      if (itemState.reason === 'equipped') return {
        success: false,
        message: '该外观已佩戴',
        reason: 'equipped'
      };
      if (itemState.reason === 'equippable') return {
        success: false,
        message: '该外观可直接佩戴',
        reason: 'equippable'
      };
      if (itemState.reason === 'owned') return {
        success: false,
        message: '该商品已拥有',
        reason: 'owned'
      };
      if (itemState.reason === 'sold_out') return {
        success: false,
        message: '该商品已售罄',
        reason: 'sold_out'
      };
      if (itemState.reason === 'insufficient') return {
        success: false,
        message: '天道币不足',
        reason: 'insufficient'
      };
    ...
    let next = this.normalizeEconomyState({
      ...
      ...(item.type === 'skin' && !economy.equippedSkinId ? {
        equippedSkinId: item.id
      } : {}),
      ...(item.type === 'title' && !economy.equippedTitleId ? {
        equippedTitleId: item.id
      } : {}),
  ...
  getPracticeDeck(deckArchetype = 'balanced', realm = 1) {
    ...
    const baseStarter = Array.isArray(typeof STARTER_DECK !== 'undefined' ? STARTER_DECK : null) ? pickValid(STARTER_DECK) : [];
  ...
  createPracticeLeaderboard(baseRank = null) {
    ...
    const bots = [{
      id: 'bot-1',
      name: '镜湖散修',
      score: localRank.score + 40,
      realm: Math.max(1, localRank.realm),
      division: this.getDivisionByScore(localRank.score + 40)
    }, {
      id: 'bot-2',
      name: '玄铁守门人',
      score: Math.max(800, localRank.score - 35),
      realm: Math.max(1, localRank.realm),
      division: this.getDivisionByScore(localRank.score - 35)
    }, {
      id: 'bot-3',
      name: '离火剑客',
      score: localRank.score + 95,
      realm: Math.max(1, localRank.realm + 1),
      division: this.getDivisionByScore(localRank.score + 95)
    }, {
      id: 'bot-4',
      name: '归墟棋手',
      score: Math.max(700, localRank.score - 90),
      realm: Math.max(1, localRank.realm),
      division: this.getDivisionByScore(localRank.score - 90)
    }, {
      id: 'bot-5',
      name: '风雪行者',
      score: localRank.score + 10,
      realm: Math.max(1, localRank.realm),
      division: this.getDivisionByScore(localRank.score + 10)
    }].map((bot, index) => ({
      objectId: bot.id,
      user: {
        objectId: bot.id,
  ...
  createPracticeOpponent(myScore = 1000, myRealm = 1, reason = 'practice', options = {}) {
    ...
    const shouldLockFocus = !!(preferredRank && preferredRank.user && preferredRank.user.objectId && preferredRank.user.objectId !== localUser.objectId);
    const seed = shouldLockFocus ? this.hashPvpPreviewSeed([preferredRank.objectId, preferredRank.user.objectId, preferredRank.user.username, preferredRank.score, preferredRank.realm].join('|')) : this.nextPracticeSeed();
    const styles = ['aggressive', 'fortified', 'balanced'];
    const scoreShift = (seed % 9 - 4) * 15;
    const namePool = ['太虚演武傀儡', '青锋论道者', '星渊镜像', '古碑守阵灵'];
    const estimatedFocus = shouldLockFocus ? this.estimatePvpSnapshotFromRank(preferredRank, {
      myScore,
      myRealm,
      listIndex: 0
    }) : null;
    const style = estimatedFocus && estimatedFocus.style ? estimatedFocus.style : styles[seed % styles.length];
    const opponentScore = shouldLockFocus ? preferredRank.score : Math.max(700, Math.floor((Number(myScore) || 1000) + scoreShift));
    const opponentRealm = shouldLockFocus ? preferredRank.realm : Math.max(1, Math.floor((Number(myRealm) || 1) + (seed % 3 - 1)));
    const opponentName = shouldLockFocus ? preferredRank.user.username : `${namePool[seed % namePool.length]}-${seed % 97 + 1}`;
    ...
    const battleData = estimatedFocus ? estimatedFocus.battleData : this.normalizeBattleData({
      me: {
  ...
  async findOpponent(myScore, myRealm, options = {}) {
    ...
    try {
      ...
      const buildMatchResult = (opponentRank, ghostData, parsedData, meta = {}) => {
        ...
        const resolvedDangerProfile = meta.dangerProfile ? this.normalizePVPDangerProfile(meta.dangerProfile) : this.getPVPDangerProfile({
          rank: opponentRank,
          ghost: ghostData,
          battleData: safeBattleData
        }, {
        ...
        const matchIntent = meta.matchIntent && typeof meta.matchIntent === 'object' ? meta.matchIntent : this.getFocusDuelSlip({
          rank: opponentRank,
        ...
        const dossier = meta.dossier && typeof meta.dossier === 'object' ? meta.dossier : this.getFocusOpponentDossier({
          rank: opponentRank,
          dangerProfile: resolvedDangerProfile,
          duelBrief: matchIntent
        }, {
      ...
      if (preferredRank && preferredRank.objectId && preferredRank.user && preferredRank.user.objectId && preferredRank.user.objectId !== user.objectId) {
        const preferredCandidate = preferredRank.objectId ? (await this.getRankByObjectId(preferredRank.objectId)) || preferredRank : preferredRank;
      ...
      const matchIntent = preferredRank ? this.getFocusDuelSlip({
        rank: opponentRank,
  ...
  getDeckArchetype(deck) {
    ...
    deck.forEach(card => {
      ...
      if (type === 'attack') attack++;else if (type === 'defense') defense++;else utility++;
    });
  ...
  async reportMatchResult(isWin, opponentRankData, matchTicket = null) {
    ...
    const ticketValid = !!(active && !active.consumed && matchTicket && active.ticket === matchTicket && (!active.userId || !user || active.userId === user.objectId) && now - active.issuedAt <= 10 * 60 * 1000 && (!active.opponentRankId || !opponentRankId || active.opponentRankId === opponentRankId) && (!active.opponentUserId || !opponentUserId || active.opponentUserId === opponentUserId));
    if (!ticketValid) {
    ...
    const calcRating = (myRating, oppRating, result) => {
      ...
      return {
        newRating: myRating + fallbackDelta,
        delta: fallbackDelta
      };
    };
    ...
    if (opponentRankId) {
      ...
      if (verifiedOpponentRank && typeof verifiedOpponentRank.score === 'number') {
        const verifiedOpponentUserId = verifiedOpponentRank.user && verifiedOpponentRank.user.objectId ? verifiedOpponentRank.user.objectId : null;
...
if (typeof window !== 'undefined') window.PVPService = PVPService;
