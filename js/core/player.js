import { CHARACTERS, CARDS } from "../data/index.js";
import { SKILLS } from "../data/skills.js";
import { MutatedRing, SealedRing, KarmaRing, AnalysisRing, FateRing } from "./fateRing.js";
...
import { STARTER_DECK, inferDeckArchetype, upgradeCard, cloneCardTemplate, getRandomCard } from "../data/cards.js";
import { FATE_RING } from "../data/fate_ring.js";
import { LAW_RESONANCES, LAWS } from "../data/laws.js";
import { TREASURES } from "../data/treasures.js";
const getRuntimeGame = player => player?.game || (typeof globalThis !== 'undefined' ? globalThis.game || globalThis.window?.game || null : null);
const getRuntimeBattle = player => player?.game?.battle || (typeof globalThis !== 'undefined' ? globalThis.game?.battle || globalThis.window?.game?.battle || null : null);
/**
 * The Defier - 玩家系统
 */
export class Player {
  ...
  normalizeRunDestiny(runDestiny = this.runDestiny) {
    ...
    const catalog = typeof RUN_DESTINIES !== 'undefined' && RUN_DESTINIES && typeof RUN_DESTINIES === 'object' ? RUN_DESTINIES : null;
  ...
  normalizeRunPath(runPath = this.runPath) {
    const catalog = typeof RUN_PATHS !== 'undefined' && RUN_PATHS && typeof RUN_PATHS === 'object' ? RUN_PATHS : null;
  ...
  normalizeRunPathMutationState(runPathMutationState = this.runPathMutationState) {
    ...
    const base = runPath && typeof RUN_PATHS !== 'undefined' && RUN_PATHS ? RUN_PATHS[runPath.id] || null : null;
    const mutations = base && base.mutations && typeof base.mutations === 'object' ? base.mutations : null;
    if (!runPath || !mutations || !runPathMutationState || typeof runPathMutationState !== 'object') {
      this.runPathMutationState = null;
      return null;
    }
    const mutationId = typeof runPathMutationState.mutationId === 'string' ? runPathMutationState.mutationId : '';
  ...
  ensureRunPathProgress() {
    ...
    const progress = this.runPathProgress && typeof this.runPathProgress === 'object' ? this.runPathProgress : {};
    const normalized = {
      pathId: runPath.id,
      ...
      completedPhases: Array.isArray(progress.completedPhases) ? progress.completedPhases.map(entry => String(entry || '')).filter(Boolean) : [],
      rewardHistory: Array.isArray(progress.rewardHistory) ? progress.rewardHistory.map(entry => String(entry || '')).filter(Boolean).slice(0, 12) : [],
  ...
  getRunPathMeta() {
    ...
    const baseTreasureSynergy = base.treasureSynergy && typeof base.treasureSynergy === 'object' ? JSON.parse(JSON.stringify(base.treasureSynergy)) : null;
    const mergedTreasureSynergy = baseTreasureSynergy ? {
      ...baseTreasureSynergy,
      favoredSets: Array.from(new Set([...(Array.isArray(baseTreasureSynergy.favoredSets) ? baseTreasureSynergy.favoredSets : []), ...(Array.isArray(mutationMeta?.treasureSynergy?.favoredSets) ? mutationMeta.treasureSynergy.favoredSets : [])])).slice(0, 4)
    ...
    const mutationEffects = mutationMeta?.effects && typeof mutationMeta.effects === 'object' ? mutationMeta.effects : {};
    ...
    const mutationEventPool = Array.isArray(mutationMeta?.mutationEventPool) ? mutationMeta.mutationEventPool.map(eventId => String(eventId || '').trim()).filter(Boolean).slice(0, 3) : [];
    const mergedEventPool = Array.from(new Set([...baseEventPool, ...mutationEventPool]));
    return {
      id: base.id,
      ...
      currentPhase: currentPhase ? {
        ...(currentPhase || {}),
        rewards: Array.isArray(currentPhase.rewards) ? currentPhase.rewards.map(reward => ({
          ...(reward || {})
        })) : []
      } : null,
  ...
  normalizeRunVows(runVows = this.runVows) {
    const catalog = typeof RUN_VOWS !== 'undefined' && RUN_VOWS && typeof RUN_VOWS === 'object' ? RUN_VOWS : null;
  ...
  setRunVows(runVows = []) {
    this.runVows = Array.isArray(runVows) ? runVows.map(entry => ({
      ...(entry || {})
    })) : [];
  ...
  getRunVowMetas() {
    return this.normalizeRunVows().map(entry => this.getRunVowMeta(entry.id, entry.tier)).filter(Boolean);
  }
  getRunVowEffects() {
    const result = {
      openingBlock: 0,
  ...
  normalizeSpiritCompanion(spiritCompanion = this.spiritCompanion) {
    const catalog = typeof SPIRIT_COMPANIONS !== 'undefined' && SPIRIT_COMPANIONS && typeof SPIRIT_COMPANIONS === 'object' ? SPIRIT_COMPANIONS : null;
  ...
  ensureSpiritCompanionBattleState() {
    ...
    this.spiritCompanionBattleState.charge = Math.max(0, Math.min(chargeMax, Math.floor(Number(this.spiritCompanionBattleState.charge) || 0)));
  ...
  spendSpiritCharge(amount = null) {
    const effects = this.getSpiritCompanionEffects();
    const chargeMax = Math.max(0, Math.floor(Number(effects.chargeMax) || 0));
    const cost = amount == null ? chargeMax : Math.max(0, Math.floor(Number(amount) || 0));
    const state = this.ensureSpiritCompanionBattleState();
  ...
  resolveArchetypeResonance() {
    ...
    this.deck.forEach(card => {
      if (!card) return;
      if (archetypeId === 'hemorrhage') {
        const isHemorrhage = card.synergyGroup === 'hemorrhage' || Array.isArray(card.keywords) && card.keywords.includes('bleed');
        if (isHemorrhage) matchCount += 1;
      } else if (archetypeId === 'precision') {
        const isPrecision = card.synergyGroup === 'precision' || card.synergyGroup === 'stance' || Array.isArray(card.keywords) && (card.keywords.includes('mark') || card.keywords.includes('stance'));
        if (isPrecision) matchCount += 1;
      } else if (archetypeId === 'entropy') {
        const isEntropy = card.synergyGroup === 'entropy' || Array.isArray(card.keywords) && (card.keywords.includes('discard') || card.keywords.includes('mulligan'));
        if (isEntropy) matchCount += 1;
      } else if (archetypeId === 'stormcraft') {
        const isStormcraft = card.synergyGroup === 'stormcraft' || Array.isArray(card.keywords) && (card.keywords.includes('storm') || card.keywords.includes('vulnerable') || card.keywords.includes('chain'));
        if (isStormcraft) matchCount += 1;
      } else if (archetypeId === 'vitalweave') {
        const isVitalweave = card.synergyGroup === 'vitalweave' || Array.isArray(card.keywords) && (card.keywords.includes('vital') || card.keywords.includes('heal') || card.keywords.includes('sustain'));
        if (isVitalweave) matchCount += 1;
      } else if (archetypeId === 'bulwark') {
        const isBulwark = card.synergyGroup === 'bulwark' || Array.isArray(card.keywords) && (card.keywords.includes('guard') || card.keywords.includes('retain'));
        if (isBulwark) matchCount += 1;
      } else if (archetypeId === 'cursebound') {
        const isCursebound = card.synergyGroup === 'cursebound' || Array.isArray(card.keywords) && (card.keywords.includes('curse') || card.keywords.includes('selfharm') || card.keywords.includes('contract'));
        if (isCursebound) matchCount += 1;
      } else if (archetypeId === 'soulforge') {
        const isSoulforge = card.synergyGroup === 'soulforge' || Array.isArray(card.keywords) && (card.keywords.includes('forge') || card.keywords.includes('construct') || card.keywords.includes('array'));
        if (isSoulforge) matchCount += 1;
      } else if (archetypeId === 'mirrorweave') {
        const isMirrorweave = card.synergyGroup === 'mirrorweave' || Array.isArray(card.keywords) && (card.keywords.includes('mirror') || card.keywords.includes('echo') || card.keywords.includes('delay'));
        if (isMirrorweave) matchCount += 1;
      } else if (archetypeId === 'oathbound') {
        const isOathbound = card.synergyGroup === 'oathbound' || Array.isArray(card.keywords) && (card.keywords.includes('oath') || card.keywords.includes('debt') || card.keywords.includes('penance'));
  ...
  triggerArchetypeDiscardProc(discardedCount = 0) {
    ...
    const doctrine = this.legacyRunDoctrine && typeof this.legacyRunDoctrine === 'object' ? this.legacyRunDoctrine : null;
    ...
    const drawCount = Math.max(hasEntropyResonance ? resonance.firstDiscardDraw || 0 : 0, hasLegacyEntropy ? doctrine.entropyLegacyDraw || 0 : 0);
    ...
    const baseDamage = Math.max(hasEntropyResonance ? resonance.discardDamage || 0 : 0, hasLegacyEntropy ? doctrine.entropyLegacyDiscardDamage || 0 : 0);
  ...
  triggerArchetypeBlockProc(blockGained = 0) {
    ...
    const drawCount = Math.max(hasBulwarkResonance ? resonance.firstBlockDraw || 0 : 0, hasLegacyBulwark ? doctrine.bulwarkLegacyDraw || 0 : 0);
    if (drawCount > 0) this.drawCards(drawCount);
    ...
    const target = aliveEnemies[Math.floor(Math.random() * aliveEnemies.length)];
    const damage = Math.max(hasBulwarkResonance ? Number(resonance.blockCounterDamage) || 0 : 0, hasLegacyBulwark ? Number(doctrine.bulwarkLegacyCounterDamage) || 0 : 0, 1);
    const dealt = battle.dealDamageToEnemy(target, damage);
  ...
  triggerArchetypeHealProc(actualHeal = 0) {
    ...
    const ratio = Math.max(hasVitalResonance ? Number(resonance.firstHealBlockRatio) || 0 : 0, hasLegacyVitalweave ? Number(doctrine.vitalweaveLegacyBlockRatio) || 0 : 0);
    const bonusBlock = Math.max(1, Math.floor(healed * ratio));
    const drawCount = Math.max(hasVitalResonance ? Math.floor(Number(resonance.firstHealDraw) || 0) : 0, hasLegacyVitalweave ? Math.floor(Number(doctrine.vitalweaveLegacyDraw) || 0) : 0);
    const burstBase = Math.max(hasVitalResonance ? Math.floor(Number(resonance.healBurstDamage) || 0) : 0, hasLegacyVitalweave ? Math.floor(Number(doctrine.vitalweaveLegacyBurstDamage) || 0) : 0);
    ...
    Utils.showBattleLog(`【回生织脉】回生触发：护盾 +${bonusBlock}${drawCount > 0 ? `，抽牌 +${drawCount}` : ''}` + `，并对 ${target.name} 造成 ${dealt} 伤害`);
  ...
  triggerArchetypeForgeProc(createdCards = []) {
    ...
    if (resonance.procUsedThisTurn) return;
    const forgeCount = cards.filter(card => card.synergyGroup === 'soulforge' || Array.isArray(card.keywords) && (card.keywords.includes('forge') || card.keywords.includes('construct') || card.keywords.includes('array'))).length;
  ...
  rememberPlayedCardForEcho(card) {
    ...
    try {
      ...
      clonedEffects = (card.effects || []).map(effect => ({
        ...(effect || {})
      }));
    }
  ...
  buildEchoScaledEffect(effect, ratio = 1) {
    ...
    switch (scaledEffect.type) {
      ...
      case 'buff':
      case 'debuff':
        scaledEffect.value = scaledEffect.buffType === 'dodgeChance' ? decimal(scaledEffect.value) : discrete(scaledEffect.value);
        break;
      case 'oathDebt':
  ...
  sanitizeDeck() {
    ...
    this.deck = this.deck.map(card => {
      ...
      // 保留实例ID (如果存在)
      if (card.instanceId) freshCard.instanceId = card.instanceId;else freshCard.instanceId = this.generateCardId();
      return freshCard;
    });
  }
  ...
  prepareBattle() {
    ...
    else if (this.realm >= 15) heartDemonCount = 3;else if (this.realm >= 10) heartDemonCount = 2;else if (this.realm >= 5) heartDemonCount = 1;
    ...
    if (Number(vowEffects.battleStartHpLoss) > 0) {
      const hpLoss = Math.min(Math.max(0, Math.floor(Number(vowEffects.battleStartHpLoss) || 0)), Math.max(0, this.currentHp - 1));
      if (hpLoss > 0) {
  ...
  startTurn() {
    ...
    const destinyMeta = this.getRunDestinyMeta();
    const destinyEffects = destinyMeta ? destinyMeta.effects : null;
    if (destinyMeta && destinyEffects && !this.runDestinyBattleState.firstTurnBonusApplied && this.turnNumber === 1) {
    ...
    const vowEffects = this.getRunVowEffects();
    if (this.runVowBattleState && !this.runVowBattleState.firstTurnBonusApplied && this.turnNumber === 1) {
    ...
    if (destinyMeta && destinyEffects) {
      ...
      if (Number.isFinite(healThreshold) && healThreshold > 0 && healAmount > 0 && this.maxHp > 0 && this.currentHp / this.maxHp <= healThreshold) {
    ...
    try {
      keepBlock = this.hasBuff('retainBlock') || this.collectedLaws && this.collectedLaws.some(l => l && l.passive && l.passive.type === 'retainBlock') || this.activeResonances && this.activeResonances.some(r => r.effect && (r.effect.type === 'persistentBlock' || r.effect.type === 'retainBlock'));
    } catch (e) {
  ...
  addBlock(amount) {
    ...
    // 环境：古战场 - 无法获得护盾
    try {
      const activeBattle = getRuntimeBattle(this);
    ...
    const destinyMeta = this.getRunDestinyMeta();
    const destinyEffects = destinyMeta ? destinyMeta.effects : null;
    if (destinyMeta && destinyEffects && Number(destinyEffects.firstBlockGainBonusPct) > 0 && this.runDestinyBattleState && !this.runDestinyBattleState.firstBlockBonusUsed) {
    ...
    if (spiritMeta && Number(spiritEffects.firstBlockBonusPerTurn) > 0 && Math.max(0, Math.floor(Number(this.turnNumber) || 0)) > 0 && this.ensureSpiritCompanionBattleState() && !this.spiritCompanionBattleState.firstBlockBonusUsedThisTurn) {
  ...
  heal(amount) {
    ...
    if (Number.isFinite(Number(vowEffects.healMultiplier)) && Number(vowEffects.healMultiplier) > 0 && Number(vowEffects.healMultiplier) < 1) {
  ...
  takeDamage(amount) {
    ...
    if (hpDamage > 0) {
      ...
      if (this.game && this.game.battle && typeof this.game.battle.handleSpiritCompanionPlayerDamaged === 'function') {
  ...
  playCard(cardIndex, target, options = {}) {
    ...
    try {
      ...
      if (card.type === 'skill') {
        const runPathMeta = this.getRunPathMeta();
        const runPathEffects = this.getRunPathEffects();
        if (runPathMeta && Number(runPathEffects.firstSkillDrawPerTurn) > 0 && this.runPathBattleState && !this.runPathBattleState.firstSkillDrawUsedThisTurn) {
        ...
        const destinyMeta = this.getRunDestinyMeta();
        const destinyEffects = destinyMeta ? destinyMeta.effects : null;
        if (destinyMeta && destinyEffects && Number(destinyEffects.firstSkillDrawPerTurn) > 0 && this.runDestinyBattleState && !this.runDestinyBattleState.firstSkillDrawUsedThisTurn) {
        ...
        if (spiritMeta && Number(spiritPassive.firstSkillDrawPerTurn) > 0 && this.ensureSpiritCompanionBattleState() && !this.spiritCompanionBattleState.firstSkillDrawUsedThisTurn) {
        ...
        if (doctrineProfile.path === 'resonance' && doctrineProfile.tier > 0) {
          ...
          if (doctrineProfile.skillChainDraw > 0 && this.pathDoctrineSkillChainCountThisTurn >= 2 && !this.pathDoctrineSkillChainDrawUsedThisTurn) {
      ...
      // 播放卡牌特效
      const activeGame = getRuntimeGame(this);
      ...
      if (card.isTemp || card.exhaust) {
        ...
        const vowEffects = this.getRunVowEffects();
        if (Number(vowEffects.firstExhaustDrawPerTurn) > 0 && this.runVowBattleState && !this.runVowBattleState.firstExhaustDrawUsedThisTurn) {
  ...
  executeEffect(effect, target, context = {}) {
    ...
    if (effect.type === 'damage' || effect.type === 'penetrate' || effect.type === 'damageAll') {
      ...
      if (doctrineProfile.path === 'destruction' && doctrineProfile.tier > 0 && Math.max(0, Math.floor(Number(this.block) || 0)) <= 5) {
    ...
    if (effect.type === 'damage' || effect.type === 'penetrate' || effect.type === 'damageAll') {
      try {
        const battle = getRuntimeBattle(this);
    ...
    switch (effect.type) {
      ...
      case 'createCard':
        {
          ...
          for (let i = 0; i < count; i++) {
            ...
            if (zone === 'hand') this.hand.push(template);else if (zone === 'draw') this.drawPile.push(template);else this.discardPile.push(template);
      ...
      case 'consumeOathDebt':
        {
          ...
          const consumeLimitRaw = effect.consumeCount !== undefined && effect.consumeCount !== null ? Math.floor(Number(effect.consumeCount) || 0) : currentDebt;
          ...
          if (remainingDebt > 0) this.buffs.oathDebt = remainingDebt;else delete this.buffs.oathDebt;
          const baseDamage = Math.max(0, Math.floor(Number(effect.baseDamage) || 0));
          const damagePerDebt = Math.max(0, Math.floor(effect.damagePerDebt !== undefined && effect.damagePerDebt !== null ? Number(effect.damagePerDebt) || 0 : Number(effect.value) || 0));
      ...
      case 'echoLastPlayedCard':
        {
          ...
          const replayableEffects = source.effects.filter(sourceEffect => sourceEffect && typeof sourceEffect === 'object' && sourceEffect.type && sourceEffect.type !== 'echoLastPlayedCard');
          if (replayableEffects.length <= 0) {
            return {
              type: 'echoLastPlayedCard',
              echoed: false,
              reason: 'noReplayableEffect',
              value: 0,
              results: []
            };
          }
      ...
      case 'blockBurst':
        {
          ...
          const maxConsume = effect.maxConsume !== undefined && effect.maxConsume !== null ? Math.max(0, Math.floor(Number(effect.maxConsume) || 0)) : null;
      ...
      case 'consumeAllEnergy':
        ...
        return {
          type: 'damage',
          value: energy * (effect.damagePerEnergy || 6),
          target: effect.target
        };
      case 'randomCards':
  ...
  addBuff(type, value) {
    ...
    if (buffNames[type]) buffName = buffNames[type];else if (typeof GameData !== 'undefined' && GameData.getBuffName) buffName = GameData.getBuffName(type);
  ...
  processBuffsOnTurnStart() {
    ...
    if (this.buffs.oathDebt) {
      ...
      if (debt > 0) {
        ...
        if (decayed > 0) this.buffs.oathDebt = decayed;else delete this.buffs.oathDebt;
      ...
      if (card) {
        ...
        if (this.realm === 6 || this.buffs.confuse && this.buffs.confuse > 0) {
          ...
          if (this.buffs.confuse) {
            ...
            const r = Math.random();
            let change = 0;
            if (r < 0.2) change = -1;else if (r < 0.5) change = 0;else change = 1;
  ...
  getState() {
    return {
      ...
      // FateRing needs its own compression ideally, but it's small usually
      // 压缩法则列表
      collectedLaws: this.collectedLaws.map(l => ({
        id: l.id
      })),
      ...
      equippedTreasures: (this.equippedTreasures || []).map(t => t.id),
      // 只存ID即可
      ...
      runPathBattleState: this.runPathBattleState ? {
        ...this.runPathBattleState
      } : this.resetRunPathBattleState(),
  ...
  compressCardList(list) {
    return list.map(c => ({
      ...
      upgraded: c.upgraded,
      cost: c.cost,
      // Preserve current cost (e.g. randomized)
      isTemp: c.isTemp
    }));
  }
  ...
  getTreasureById(treasureId, options = {}) {
    ...
    const source = options && options.equippedOnly ? this.equippedTreasures : this.collectedTreasures;
  ...
  getTreasureResearchRoleMeta(treasureId = '') {
    ...
    const coreTreasureIds = new Set(['iron_talisman', 'soul_banner', 'spirit_turtle_shell', 'astral_forge_core', 'fate_lotus_seal', 'ringweaver_anvil', 'five_element_bead']);
    const formTreasureIds = new Set(['metalEssence', 'woodSpiritRoot', 'waterCrystal', 'firePhoenixFeather', 'thickEarthShield', 'vitality_stone', 'blood_orb', 'ring_echo_compass', 'moonblade_sheath', 'hunter_contract', 'matrix_resonator']);
  ...
  isTreasureSpiritInfusionEligible(treasure = null) {
    const treasureId = typeof treasure === 'string' ? treasure : treasure?.id || '';
  ...
  getTreasureSpiritBondLabel(spiritId = '') {
    ...
    if (!safeId) return '器灵';
    const spiritMeta = typeof SPIRIT_COMPANIONS !== 'undefined' && SPIRIT_COMPANIONS ? SPIRIT_COMPANIONS[safeId] || null : null;
  ...
  getTreasureResearchEntry(treasure = null) {
    const resolvedTreasure = typeof treasure === 'string' ? this.getTreasureById(treasure) || (typeof TREASURES !== 'undefined' && TREASURES ? TREASURES[treasure] || null : null) : treasure;
    if (!resolvedTreasure) return null;
    const setTag = typeof resolvedTreasure.setTag === 'string' ? resolvedTreasure.setTag.trim() : '';
    ...
    const totalSetCount = setTag && typeof TREASURES !== 'undefined' && TREASURES ? Object.values(TREASURES).filter(item => item && item.setTag === setTag).length : 0;
    return {
  ...
  getTreasureWorkshopResearchOverview() {
    const catalog = typeof TREASURES !== 'undefined' && TREASURES ? Object.values(TREASURES).filter(Boolean) : [];
    ...
    const setProgress = setIds.map(setTag => {
      ...
      const resonanceStage = pieces >= 3 ? 'full' : pieces >= 2 ? 'active' : owned > 0 ? 'forming' : 'empty';
      const resonanceLabel = resonanceStage === 'full' ? '三段共鸣' : resonanceStage === 'active' ? '二段共鸣' : resonanceStage === 'forming' ? '待收集' : '未起步';
      return {
        id: setTag,
    ...
    const readyInfusions = equipped.filter(treasure => this.isTreasureSpiritInfusionEligible(treasure) && !treasure?.data?.workshopSpiritBond).map(treasure => treasure?.name || treasure?.id || '未知法宝');
    return {
      setProgress,
  ...
  getTreasureWorkshopReforgeMode(treasure = null) {
    ...
    const rarity = treasure && typeof treasure.rarity === 'string' ? treasure.rarity.toLowerCase() : 'common';
  ...
  getTreasureWorkshopSnapshot(scope = 'equipped') {
    const source = scope === 'all' ? Array.isArray(this.collectedTreasures) ? this.collectedTreasures : [] : Array.isArray(this.equippedTreasures) ? this.equippedTreasures : [];
  ...
  applyTreasureSpiritInfusion(treasureId, spiritId = '') {
    const treasure = this.getTreasureById(treasureId);
    const spiritMeta = this.getSpiritCompanionMeta();
    const resolvedSpiritId = typeof spiritId === 'string' && spiritId ? spiritId : spiritMeta ? spiritMeta.id : '';
  ...
  triggerTreasureEffect(triggerType, ...args) {
    ...
    if (triggerType === 'onBattleStart') {
      const workshopLogs = [];
      this.equippedTreasures.forEach(treasure => {
        const data = treasure && treasure.data && typeof treasure.data === 'object' ? treasure.data : null;
        if (!data) return;
      ...
      const spiritMeta = this.getSpiritCompanionMeta();
      const infusedTreasure = this.equippedTreasures.find(treasure => treasure && treasure.data && typeof treasure.data.workshopSpiritBond === 'string' && treasure.data.workshopSpiritBond.length > 0);
      if (infusedTreasure && spiritMeta && infusedTreasure.data.workshopSpiritBond === spiritMeta.id && typeof this.gainSpiritCharge === 'function') {
  ...
  triggerTreasureValueEffect(triggerType, value, ...args) {
    ...
    if (triggerType === 'onBeforeDealDamage') {
      ...
      const bleedStacks = target && target.buffs ? Math.max(0, Math.floor(Number(target.buffs.bleed) || 0)) : 0;
      ...
      const hasDebuff = target && target.buffs && Object.values(target.buffs).some(value => Math.max(0, Math.floor(Number(value) || 0)) > 0);
      const reforgeTreasure = this.equippedTreasures.find(treasure => treasure && treasure.data && treasure.data.workshopReforge === 'rend');
