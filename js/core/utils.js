...
import { escapeAttr, escapeHtml } from "../ui/render-safe.js";
/**
 * The Defier - 工具函数
 */
export const Utils = {
  ...
  createCardElement(card, index = 0, isReward = false, options = {}) {
    ...
    const battleTags = Array.isArray(options?.battleTags) ? options.battleTags.filter(tag => tag && typeof tag === 'object' && tag.label).map(tag => ({
      id: String(tag.id || 'hint'),
      label: String(tag.label || ''),
      tip: String(tag.tip || '')
    })).slice(0, 2) : [];
    const battleTagHtml = !isReward && battleTags.length > 0 ? `<div class="card-live-tags">${battleTags.map(tag => `
  ...
  showCardDetail(card, meta = {}) {
    ...
    const loreHtml = card.lore ? `<div class="detail-dual-panel cd-lore-card"><span class="detail-mini-label">卡牌逸闻</span><p class="cd-lore">"${card.lore}"</p></div>` : '';
    const rarity = card.rarity || 'common';
    const rarityClass = `rarity-${rarity}`;
    const summaryChips = [`<span class="detail-status-chip type-${card.type}">${this.getCardTypeName(card.type)}</span>`, `<span class="detail-status-chip rarity-chip ${rarityClass}">${this.getCardRarityName(rarity)}</span>`, card.lawType ? `<span class="detail-status-chip law">${this.getLawName(card.lawType)}</span>` : ''].filter(Boolean).join('');
    ...
    const extraSummaryRows = Array.isArray(meta.extraSummaryRows) ? meta.extraSummaryRows.map(row => `<div class="cd-summary-row"><span>${row.label}</span><strong>${row.value}</strong></div>`).join('') : '';
  ...
  getEffectDescription(effect) {
    ...
    switch (effect.type) {
      ...
      case 'draw':
        return `抽取 ${effect.value} 张牌`;
      case 'energy':
        return `回复 ${effect.value} 点灵力`;
      case 'conditionalDraw':
        return `条件抽牌: ${effect.condition === 'lowHp' ? '生命<20%' : '未知条件'}`;
      default:
        return `类型: ${effect.type}, 数值: ${effect.value || '-'}`;
    }
  },
  getLawName(type) {
    const map = {
  ...
  createEnemyElement(enemy, index = 0) {
    ...
    let currentPattern;
    if (enemy.isGhost) {
      currentPattern = enemy.currentIntent || {
        type: 'unknown',
        value: '...'
      };
    } else {
      currentPattern = enemy.patterns ? enemy.patterns[enemy.currentPatternIndex || 0] : {
        type: 'none'
      };
    }
    ...
    const isGuardBreaker = !!(isAttackIntent && (enemy.isElite && enemy.eliteType === 'sunder' || enemy.buffs && enemy.buffs.guardBreak > 0 || canBossGuardBreak));
    const describePattern = pattern => {
      if (!pattern || typeof pattern !== 'object') return '未知行动';
      switch (pattern.type) {
        case 'attack':
          return `攻击 ${pattern.value} 点伤害`;
        case 'multiAttack':
          return `连击 ${pattern.value} x ${pattern.count} 次`;
        case 'defend':
          return `获得 ${pattern.value} 点护盾`;
        case 'buff':
          return `强化自身（${pattern.buffType || '增益'}）`;
        case 'debuff':
          return `施加减益（${pattern.buffType || '负面状态'}）`;
        case 'heal':
          return `恢复 ${pattern.value} 点生命`;
        case 'addStatus':
          return `塞入 ${pattern.count || 1} 张状态牌`;
        case 'summon':
          return `召唤 ${pattern.count || 1} 个随从`;
        case 'multiAction':
          return `连续执行 ${(pattern.actions || []).length} 段行动`;
        default:
          return pattern.intent || '未知行动';
      }
    };
    const resolveEnemyRole = (targetEnemy, activePattern) => {
      ...
      const role = targetEnemy && typeof targetEnemy.enemyVariantRole === 'string' && roleLabels[targetEnemy.enemyVariantRole] ? targetEnemy.enemyVariantRole : (() => {
        const sourcePatterns = Array.isArray(targetEnemy?.patterns) && targetEnemy.patterns.length > 0 ? targetEnemy.patterns : [activePattern];
        const stat = {
          attack: 0,
          defend: 0,
          debuff: 0
        ...
        sourcePatterns.forEach(pattern => {
          ...
          if (pattern.type === 'attack' || pattern.type === 'multiAttack' || pattern.type === 'executeDamage') {
            stat.attack += 1;
          } else if (pattern.type === 'defend' || pattern.type === 'heal' || pattern.type === 'buff' && pattern.buffType === 'block') {
            stat.defend += 1;
          } else if (pattern.type === 'debuff' || pattern.type === 'addStatus' || pattern.type === 'summon' || pattern.type === 'multiAction') {
            stat.debuff += 1;
          }
        });
    ...
    const resolvePlayerCounterTools = () => {
      ...
      const markBurstByPattern = pattern => {
        ...
        if ((pattern.type === 'attack' || pattern.type === 'damage' || pattern.type === 'executeDamage') && value >= 10) {
          tools.burst = true;
        }
    ...
    const resolveThreatTags = targetEnemy => {
      ...
      sourcePatterns.forEach(pattern => {
          ...
          if (Array.isArray(pattern.actions)) {
            pattern.actions.forEach(action => {
              ...
              if (action.type === 'attack') {
                ...
                burstScore = Math.max(burstScore, Math.floor((Number(action.value) || 0) * Math.max(1, Number(action.count) || 1)));
              }
            });
          }
        }
        if (pattern.type === 'attack') {
          ...
          burstScore = Math.max(burstScore, Math.floor((Number(pattern.value) || 0) * Math.max(1, Number(pattern.count) || 1)));
    ...
    if (currentPattern.type === 'multiAction' && Array.isArray(currentPattern.actions) && currentPattern.actions.length > 0) {
      const segments = currentPattern.actions.map((act, idx) => `${idx + 1}.${describePattern(act)}`).join('；');
    ...
    if (isGuardBreaker) {
      ...
      const shattered = playerBlock > 0 ? Math.min(playerBlock, Math.max(3, Math.min(shatterCap, Math.floor(playerBlock * shatterRate)))) : 0;
      const bonusDamage = shattered > 0 ? Math.max(1, Math.floor(shattered * (enemy.isElite && enemy.eliteType === 'sunder' ? 0.6 : 0.4))) : 0;
    ...
    const battleHud = typeof globalThis !== 'undefined' && globalThis.DefierBattleHud ? globalThis.DefierBattleHud : null;
    const renderMetaStrip = (stripClass, items = []) => {
      ...
      if (battleHud && typeof battleHud.buildEnemyMetaStripMarkup === 'function') {
        return battleHud.buildEnemyMetaStripMarkup({
          stripClass,
          items: safeItems
        });
      }
    ...
    const intentDisplay = battleHud && typeof battleHud.resolveEnemyIntentDisplay === 'function' ? battleHud.resolveEnemyIntentDisplay(currentPattern) : {
      icon: currentPattern.intent || '❓',
      label: '',
      hasLabel: false
    };
    const intentMarkup = battleHud && typeof battleHud.buildEnemyIntentMarkup === 'function' ? battleHud.buildEnemyIntentMarkup({
      type: currentPattern.type,
      icon: intentDisplay.icon,
      label: intentDisplay.label,
      value: intentValue,
      isGuardBreaker,
      tooltipSafe: intentDesc,
      ariaLabel: intentDesc
    }) : `
                <div class="enemy-intent ${escapeHtml(currentPattern.type || 'unknown')} ${isGuardBreaker ? 'breaker' : ''}"
                     data-tooltip-safe="${escapeAttr(intentDesc)}"
                     data-intent-type="${escapeAttr(currentPattern.type || 'unknown')}">
            ...
            `;
    const counterHintTitle = counterHints.map(item => String(item.detail || '')).join(' | ');
    const primaryMetaMarkup = renderMetaStrip('enemy-meta-strip enemy-meta-primary', [{
      className: `enemy-role-tag role-${enemyRole.id}`,
      text: enemyRole.label
    }, tacticalPlanLabel ? {
      className: 'enemy-plan-tag',
      text: `节奏·${tacticalPlanLabel}`,
      title: `行动节奏：${tacticalPlanLabel}`
    } : null, counterHints.length > 0 ? {
      className: 'enemy-counter-tag',
      text: `反制·${counterHints[0].label}`,
      title: counterHintTitle
    } : null]);
    const secondaryMetaMarkup = renderMetaStrip('enemy-meta-strip enemy-meta-secondary', [encounterTag ? {
      className: 'enemy-encounter-tag',
      text: `⚔️ 遭遇·${encounterTag} ${encounterTierText}`,
      title: encounterDesc || '本场遭遇词条生效中'
    } : null, encounterAffixTag ? {
      className: 'enemy-encounter-affix',
      text: `✦ 词缀·${encounterAffixTag}`,
      title: encounterAffixDesc || '高阶遭遇词缀生效中'
    } : null, squadTag ? {
      className: 'enemy-squad-tag',
      text: `⛭ 编队·${squadTag}${squadRoleLabel ? `·${squadRoleLabel}` : ''}`,
      title: squadDesc || '敌方编队协同中'
    } : null, chapterFormationTag ? {
    ...
    const threatTagsMarkup = renderMetaStrip('enemy-meta-strip enemy-threat-tags', threatTags.map(item => ({
      ...
      newCard.effects.forEach(e => {
        if (['damage', 'block', 'heal', 'penetrate'].includes(e.type)) {
          if (typeof e.value === 'number') {
            ...
            if (e.value < 10) e.value += 3;else e.value = Math.floor(e.value * 1.3);
          }
        }
  ...
  getBattleFeedbackApi() {
    return typeof DefierBattleFeedback !== 'undefined' && DefierBattleFeedback ? DefierBattleFeedback : null;
  },
  getBattleLogFilters() {
    const helper = this.getBattleFeedbackApi();
    if (helper && typeof helper.getBattleLogFilters === 'function') {
      return helper.getBattleLogFilters();
    }
    return [{
      id: 'all',
      label: '全部'
  ...
  formatBattleLogTime(timestamp) {
    const helper = this.getBattleFeedbackApi();
    if (helper && typeof helper.formatBattleLogTime === 'function') {
      return helper.formatBattleLogTime(timestamp);
    }
  ...
  buildBattleLogPanelShellMarkup(activeFilter = 'all') {
    const helper = this.getBattleFeedbackApi();
    if (helper && typeof helper.buildBattleLogPanelShellMarkup === 'function') {
      return helper.buildBattleLogPanelShellMarkup(activeFilter);
    }
    const filters = this.getBattleLogFilters();
    return `
                    ...
                    <div class="battle-log-panel-filters" role="toolbar" aria-label="战斗记录筛选">
                        ${filters.map(item => `
                            <button type="button"
                                    ...
                                    data-filter="${escapeAttr(item.id)}"
                   ...
                   class="battle-log-panel-list"></div>
                `;
  },
  buildBattleLogListMarkup(records = [], activeFilter = 'all') {
    const helper = this.getBattleFeedbackApi();
    if (helper && typeof helper.buildBattleLogListMarkup === 'function') {
      return helper.buildBattleLogListMarkup(records, activeFilter);
    }
    ...
    return filteredRecords.map(item => `
                <div class="battle-log-item log-${escapeHtml(item?.category || 'system')}">
                    <div class="battle-log-item-time">${escapeHtml(this.formatBattleLogTime(item?.ts))}</div>
                    <div class="battle-log-item-text">${escapeHtml(item?.message || '')}</div>
                </div>
            `).join('');
  },
  classifyBattleLog(message = '') {
    ...
    if (text.includes('不足') || text.includes('失败') || text.includes('异常') || text.includes('无法') || text.includes('免疫')) {
      return 'warning';
    }
    if (text.includes('伤害') || text.includes('流血') || text.includes('中毒') || text.includes('灼烧') || text.includes('斩杀')) {
      return 'damage';
    }
    if (text.includes('恢复') || text.includes('护盾') || text.includes('获得') || text.includes('减益') || text.includes('强化') || text.includes('虚弱') || text.includes('眩晕')) {
      return 'status';
    }
    if (text.includes('奖励') || text.includes('掉落') || text.includes('灵石') || text.includes('卡牌') || text.includes('战斗胜利')) {
      return 'reward';
    }
    return 'system';
  },
  ensureBattleLogPanel() {
    ...
    if (!panel) {
      ...
      panel.innerHTML = this.buildBattleLogPanelShellMarkup(this._battleLogFilter || 'all');
  ...
  renderBattleLogPanel() {
    ...
    const helper = this.getBattleFeedbackApi();
    panel.dataset.renderer = helper ? 'battle-feedback' : 'legacy';
    list.dataset.renderer = helper ? 'battle-feedback' : 'legacy';
    list.innerHTML = this.buildBattleLogListMarkup(this._battleLogHistory, filter);
  },
...
// upgradeCard = Utils.upgradeCard.bind(Utils);
