...
import { CHARACTERS, ENEMIES, ENEMY_ECOLOGY_TEMPLATES, CHAPTER_ELITE_COMBOS } from "../data/index.js";
...
const collectionHubMethods = Object.create(null);
  ...
  let originalHandleBossDefeated = null;
  ...
  const toStringArray = (value, max = 8) => Array.isArray(value) ? value.map(item => String(item || '').trim()).filter(Boolean).slice(0, max) : [];
  const normalizeRatingTone = value => ['completed', 'selected', 'suggested', 'idle'].includes(String(value || '')) ? String(value) : 'idle';
  ...
  const buildHeavenlyMandateOverviewModel = (source = null, fallback = null) => {
    ...
    const lanes = mandate ? readArray(mandate.lanes).filter(lane => lane && typeof lane === 'object').sort((a, b) => getHeavenlyMandateLaneOrder(a.id) - getHeavenlyMandateLaneOrder(b.id)).slice(0, 3).map((lane, laneIndex) => {
      const tasks = readArray(lane.tasks).filter(task => task && typeof task === 'object').slice(0, 4).map((task, taskIndex) => {
        ...
        return {
          ...
          ctaLabel: String(task.ctaLabel || actionMeta.ctaLabel || (completed ? '沿此复核' : '前往推进')).trim() || (completed ? '沿此复核' : '前往推进')
        };
      });
    ...
    const completedTaskCount = mandate ? Math.min(lanes.reduce((sum, lane) => sum + lane.tasks.filter(task => task.completed).length, 0), Math.max(0, clampInt(mandate.completedTaskCount, 0, 9999))) : 0;
    const totalTaskCount = mandate ? Math.max(lanes.reduce((sum, lane) => sum + lane.tasks.length, 0), clampInt(mandate.totalTaskCount, 0, 9999)) : 0;
    const focusTaskSource = mandate?.focusTask && typeof mandate.focusTask === 'object' ? mandate.focusTask : legacy?.focusTask && typeof legacy.focusTask === 'object' ? legacy.focusTask : null;
    ...
    const summaryLine = String(mandate?.summaryLine || legacy?.summaryLine || `天道敕令：${weekLabel} · 当前轮转「${themeLabel}」`).trim();
    const detailLine = String(legacy?.detailLine || (nextTask ? `当前优先补「${nextTask.label}」${nextTask.progressText ? ` · ${nextTask.progressText}` : ''}${nextTask.hintLine ? ` · ${nextTask.hintLine}` : ''}` : mandate ? `${themeKicker || '本周天道敕令'}${totalTaskCount > 0 ? ` · 已完成 ${completedTaskCount}/${totalTaskCount}` : ''}` : '')).trim();
    const mandateActionMeta = resolveSeasonBoardActionMeta(String(mandate?.actionValue || nextTask?.anchorSection || legacy?.actionValue || '').trim(), 'sanctum');
    const mandateGuideTargetLabel = String(mandateActionMeta.targetLabel || '').trim();
    const guideLine = String(legacy?.guideLine || (nextTask ? `先沿${mandateGuideTargetLabel ? `【${mandateGuideTargetLabel}】` : '当前主线'}补完「${nextTask.label}」，再回洞府复核其余两条玩法线。` : mandate && lanes.length > 0 ? `本周共 ${lanes.length} 条任务线，适合按远征、训练、对抗三线同步补进度。` : '')).trim();
    return {
      ...
      goalProgressText: nextTask ? nextTask.progressText || (nextTask.completed ? '已完成' : '待推进') : totalTaskCount > 0 ? `${completedTaskCount}/${totalTaskCount} 项敕令已成` : '待同步',
      completedTaskCount,
      totalTaskCount,
  ...
  const getSeasonBoardVerificationOrderPair = (orders = []) => {
    ...
    const secondary = list.find(entry => entry !== primary && String(entry.role || '').trim() === 'side') || list.find(entry => entry !== primary) || null;
    return {
      list,
      primary,
      secondary
  ...
  const buildSeasonBoardOverviewModel = (source = null) => {
    ...
    const normalizeSeasonBoardChapterArcEntry = (entry = null, index = 0) => {
      const root = entry && typeof entry === 'object' ? entry : {};
      return {
        id: String(root.id || `season_chapter_arc_entry_${index + 1}`).trim() || `season_chapter_arc_entry_${index + 1}`,
        chapterLabel: String(root.chapterLabel || '').trim(),
        arcLabel: String(root.arcLabel || '').trim(),
        windowLabel: String(root.windowLabel || '').trim(),
        weekSlot: String(root.weekSlot || '').trim(),
        progressText: String(root.progressText || '').trim(),
        summaryLine: String(root.summaryLine || '').trim(),
        statusLine: String(root.statusLine || '').trim(),
        goalLine: String(root.goalLine || '').trim(),
        shortLine: String(root.shortLine || root.summaryLine || root.statusLine || [String(root.chapterLabel || '').trim(), String(root.arcLabel || '').trim(), String(root.progressText || '').trim()].filter(Boolean).join(' · ')).trim()
      };
    };
    const normalizeSeasonBoardChapterArcPanel = (panel = null, fallbackId = '') => {
      const root = panel && typeof panel === 'object' ? panel : null;
      if (!root) return null;
      const summaryLine = String(root.summaryLine || '').trim();
      const statusLine = String(root.statusLine || '').trim();
      const goalLine = String(root.goalLine || '').trim();
      const reasonLine = String(root.reasonLine || '').trim();
      const guideLine = String(root.guideLine || '').trim();
      const endingPreviewLine = String(root.endingPreviewLine || '').trim();
      const finalCommentLine = String(root.finalCommentLine || '').trim();
      const label = String(root.label || '').trim();
      const available = root.available !== false && !!(summaryLine || statusLine || goalLine || reasonLine || guideLine || endingPreviewLine || finalCommentLine || label);
      return {
        id: String(root.id || fallbackId || '').trim(),
        label,
        available,
        open: !!root.open,
        statusId: String(root.statusId || '').trim(),
        statusLabel: String(root.statusLabel || '').trim(),
        focusLaneId: String(root.focusLaneId || '').trim(),
        focusLaneLabel: String(root.focusLaneLabel || '').trim(),
        summaryLine,
        statusLine,
        goalLine,
        reasonLine,
        ...
        endingPreviewLine,
        finalCommentLine,
        shortLine: String(root.shortLine || summaryLine || statusLine || goalLine || reasonLine || finalCommentLine || endingPreviewLine || guideLine || '').trim()
      };
    };
    ...
    const lanes = board ? readArray(board.lanes).filter(lane => lane && typeof lane === 'object').sort((a, b) => getSeasonBoardLaneOrder(a.id) - getSeasonBoardLaneOrder(b.id)).slice(0, 3).map((lane, laneIndex) => {
      const tasks = readArray(lane.tasks).filter(task => task && typeof task === 'object').slice(0, 4).map((task, taskIndex) => {
        const rawProgress = clampInt(task.progress, 0, 9999);
        const target = Math.max(1, clampInt(task.target, 1, 9999));
        const completed = !!task.completed || rawProgress >= target;
    ...
    const topLevelLaneRewards = board ? readArray(board.laneRewards).map(reward => normalizeLaneReward(reward)).filter(Boolean) : [];
    const laneRewards = topLevelLaneRewards.length > 0 ? topLevelLaneRewards : lanes.map(lane => lane.reward).filter(Boolean);
    ...
    const resolveSummaryCount = (value, fallback = 0) => {
      const num = Number(value);
      return Number.isFinite(num) ? Math.max(0, Math.min(99, Math.floor(num))) : fallback;
    };
    ...
    const completedTaskCount = board ? Math.min(lanes.reduce((sum, lane) => sum + lane.tasks.filter(task => task.completed).length, 0), Math.max(0, clampInt(board.completedTaskCount, 0, 9999))) : 0;
    const totalTaskCount = board ? Math.max(lanes.reduce((sum, lane) => sum + lane.tasks.length, 0), clampInt(board.totalTaskCount, 0, 9999)) : 0;
    const progressText = String(board?.progress?.progressText || (totalTaskCount > 0 ? `${completedTaskCount}/${totalTaskCount}` : '待同步')).trim() || '待同步';
    const chapterArcEntries = board ? readArray(board.chapterArc?.entries).filter(entry => entry && typeof entry === 'object').slice(0, 2).map(normalizeSeasonBoardChapterArcEntry) : [];
    const chapterArcReview = normalizeSeasonBoardChapterArcPanel(board?.chapterArc?.review, 'season_chapter_arc_review');
    const chapterArcRescueWindow = normalizeSeasonBoardChapterArcPanel(board?.chapterArc?.rescueWindow, 'season_chapter_arc_rescue');
    const chapterArcPressureWindow = normalizeSeasonBoardChapterArcPanel(board?.chapterArc?.pressureWindow, 'season_chapter_arc_pressure_window');
    const chapterArcCarryover = normalizeSeasonBoardChapterArcPanel(board?.chapterArc?.carryover, 'season_chapter_arc_carryover');
    const chapterArcObjective = normalizeSeasonBoardChapterArcPanel(board?.chapterArc?.objective, 'season_chapter_arc_objective');
    const chapterArc = board?.chapterArc && typeof board.chapterArc === 'object' ? (() => {
      const root = board.chapterArc;
      const chapterLabel = String(root.chapterLabel || '').trim();
      const arcLabel = String(root.arcLabel || '').trim();
      const windowLabel = String(root.windowLabel || '').trim();
      const weekSlot = String(root.weekSlot || '').trim();
      const summaryLine = String(root.summaryLine || [chapterLabel, arcLabel, windowLabel || weekSlot || String(root.progressText || '').trim()].filter(Boolean).join(' · ')).trim();
      const statusLine = String(root.statusLine || chapterArcPressureWindow?.shortLine || chapterArcRescueWindow?.shortLine || chapterArcReview?.shortLine || '').trim();
      const goalLine = String(root.goalLine || chapterArcEntries[0]?.goalLine || chapterArcEntries[0]?.shortLine || '').trim();
      const feedbackLine = String(root.feedbackLine || chapterArcPressureWindow?.guideLine || chapterArcReview?.shortLine || chapterArcRescueWindow?.shortLine || goalLine || '').trim();
      return {
        available: root.available !== false && !!(summaryLine || chapterLabel || arcLabel || chapterArcEntries.length > 0),
        id: String(root.id || 'season_board_chapter_arc').trim() || 'season_board_chapter_arc',
        chapterLabel,
        arcLabel,
        windowLabel,
        weekSlot,
        sealedWeeks: clampInt(root.sealedWeeks, 0, 3),
        targetWeeks: clampInt(root.targetWeeks || 3, 1, 3),
        weeksRemaining: clampInt(root.weeksRemaining, 0, 99),
        progressText: String(root.progressText || '').trim(),
        summaryLine,
        statusLine,
        goalLine,
        ...
        entries: chapterArcEntries
      };
    })() : {
      available: false,
      id: 'season_board_chapter_arc',
      chapterLabel: '',
      arcLabel: '',
      windowLabel: '',
      weekSlot: '',
    ...
    const verificationOrders = board ? readArray(board.verificationOrders).filter(entry => entry && typeof entry === 'object').slice(0, 3).map((entry, index) => ({
    ...
    const verificationArchiveEntries = board?.verificationArchive && typeof board.verificationArchive === 'object' ? readArray(board.verificationArchive.entries).filter(entry => entry && typeof entry === 'object').slice(0, 6).map(normalizeSeasonVerificationArchiveEntry) : [];
    const verificationArchiveLatest = board?.verificationArchive?.latestEntry && typeof board.verificationArchive.latestEntry === 'object' ? normalizeSeasonVerificationArchiveEntry(board.verificationArchive.latestEntry) : verificationArchiveEntries[0] || null;
    ...
    const frontier = board?.frontier && typeof board.frontier === 'object' ? (() => {
      ...
      const council = councilRoot ? {
        ...
        laneOpinions: readArray(councilRoot.laneOpinions).filter(entry => entry && typeof entry === 'object').slice(0, 3).map(entry => ({
      ...
      const chronicleArchive = archiveRoot ? {
        ...
        styleEntries: readArray(archiveRoot.styleEntries).filter(entry => entry && typeof entry === 'object').slice(0, 3).map(entry => ({
        ...
        entries: readArray(archiveRoot.entries).filter(entry => entry && typeof entry === 'object').slice(0, 6).map(entry => ({
      ...
      return {
        ...
        resolution,
        chronicleArchive,
        items: readArray(root.items).filter(entry => entry && typeof entry === 'object').slice(0, 3).map((entry, index) => ({
  ...
  const getSeasonBoardNextTaskLine = (seasonNextTask = null, fallback = '优先补当前季盘行动。') => {
    ...
    return String(seasonNextTask.hintLine || (seasonNextTask.label ? `当前季盘行动：${seasonNextTask.label}` : '') || seasonNextTask.statusLine || seasonNextTask.progressText || fallback).trim() || fallback;
  };
  const resolveSeasonBoardActionMeta = (target = '', fallbackTarget = 'sanctum') => {
    const normalizedTarget = String(target || '').trim();
    const fallback = SECTION_META[fallbackTarget] ? fallbackTarget : 'sanctum';
    ...
    if (SECTION_META[normalizedTarget]) {
      ...
      return collectionAction(normalizedTarget, collectionCtaLabelMap[normalizedTarget] || '前往推进');
    }
  ...
  collectionHubMethods.ensureCollectionHubBootState = function () {
  ...
  collectionHubMethods.escapeCollectionHtml = function (value) {
    return escapeHtml(value);
  };
  collectionHubMethods.normalizeCollectionHubState = function (rawState = null) {
  ...
  collectionHubMethods.getCollectionHubState = function () {
    this.ensureCollectionHubBootState();
    return this.collectionHubState;
  };
  collectionHubMethods.getCollectionSectionMeta = function (section = 'laws') {
    return SECTION_META[section] || SECTION_META.laws;
  };
  collectionHubMethods.getRewardSeasonBoardHandoffArrivalNotice = function (section = '') {
    const notice = (this.rewardView?.pendingRewardSeasonBoardHandoffNotice) || (this.pendingRewardSeasonBoardHandoffNotice && typeof this.pendingRewardSeasonBoardHandoffNotice === 'object' ? this.pendingRewardSeasonBoardHandoffNotice : null);
  ...
  collectionHubMethods.renderRewardSeasonBoardHandoffArrival = function (section = '') {
    if (typeof document === 'undefined') return;
    const heading = document.querySelector('#collection .collection-heading-group');
    const fallbackTitle = document.getElementById('collection-title');
    const mountHost = heading || fallbackTitle?.parentElement || document.querySelector('#collection .collection-main') || document.querySelector('#collection');
    ...
    if (!noticeEl) {
      ...
      mountHost.appendChild(noticeEl);
    }
    ...
    noticeEl.innerHTML = `
            ...
            <button type="button" class="collection-inline-btn collection-handoff-arrival-btn"
                ...
                data-season-board-handoff-lane-id="${escapeHtml(notice.laneId)}">${escapeHtml(notice.focusLabel)}</button>
        `;
    const focusBtn = noticeEl.querySelector('[data-season-board-handoff-focus="true"]');
    if (focusBtn && !focusBtn.__rewardSeasonBoardFocusBound) {
      focusBtn.addEventListener('click', () => {
        if (typeof this.focusRewardSeasonBoardHandoffArrival === 'function') {
          this.focusRewardSeasonBoardHandoffArrival();
        }
      });
      focusBtn.__rewardSeasonBoardFocusBound = true;
    }
    ...
    if (renderedPendingNotice && typeof setTimeout === 'function') {
      setTimeout(() => {
        ...
        if (this.rewardView?.pendingRewardSeasonBoardHandoffNotice === renderedPendingNotice) {
          this.rewardView.pendingRewardSeasonBoardHandoffNotice = null;
        }
      }, 0);
    }
  };
  collectionHubMethods.getRewardSeasonBoardHandoffArrivalFocusNotice = function () {
    const pending = typeof this.getRewardSeasonBoardHandoffArrivalNotice === 'function' ? this.getRewardSeasonBoardHandoffArrivalNotice(this.collectionHubState?.section || '') : null;
    const raw = pending || (this.lastRewardSeasonBoardHandoffArrivalNotice && typeof this.lastRewardSeasonBoardHandoffArrivalNotice === 'object' ? this.lastRewardSeasonBoardHandoffArrivalNotice : null);
    if (!raw) return null;
  ...
  collectionHubMethods.clearRewardSeasonBoardHandoffArrivalFocus = function () {
    if (typeof document === 'undefined') return;
  ...
  collectionHubMethods.findRewardSeasonBoardHandoffArrivalTarget = function (notice = {}) {
    ...
    const findNode = (selector, predicate) => Array.from(document.querySelectorAll(selector)).find(el => {
      try {
    ...
    const goal = findNode('[data-season-board-goal="true"]', (_el, ds) => matchesTaskLane(ds.seasonBoardGoalTaskId, ds.seasonBoardGoalLaneId) || matchesSource(ds.seasonBoardGoalSource, ds.seasonBoardGoalSourceId, ds.seasonBoardGoalTaskSource, ds.seasonBoardGoalTaskSourceId) || textIncludes(ds.seasonBoardGoalId, taskId) || textIncludes(ds.seasonBoardGoalId, sourceId));
    if (goal) {
      return {
        kind: 'goal',
        element: goal,
    ...
    const research = findNode('[data-season-board-research="true"]', (_el, ds) => matchesTaskLane(ds.seasonBoardResearchTaskId, ds.seasonBoardResearchLaneId) || matchesSource(ds.seasonBoardResearchSource, ds.seasonBoardResearchSourceId, ds.seasonBoardResearchTaskSource, ds.seasonBoardResearchTaskSourceId) || textIncludes(ds.seasonBoardResearchId, taskId) || textIncludes(ds.seasonBoardResearchId, sourceId));
    if (research) {
      return {
        kind: 'research',
        element: research,
    ...
    const task = findNode('[data-season-board-task="true"]', (_el, ds) => matchesTaskLane(ds.seasonBoardTaskId, ds.seasonBoardLaneId) || !!taskId && ds.seasonBoardTaskId === taskId);
    if (task) {
      return {
        kind: 'task',
        element: task,
    ...
    const lane = findNode('[data-season-board-lane="true"]', (_el, ds) => !!laneId && ds.seasonBoardLaneId === laneId);
    if (lane) {
      return {
        kind: 'lane',
        element: lane,
    ...
    const fallbackGoal = sourceKey ? findNode('[data-season-board-goal="true"], [data-season-board-research="true"]', (el, ds) => textIncludes(ds.seasonBoardGoalId || ds.seasonBoardResearchId || el.textContent || '', sourceKey)) : null;
    return fallbackGoal ? {
  ...
  collectionHubMethods.focusRewardSeasonBoardHandoffArrival = function (retryAttempt = 0) {
  ...
  collectionHubMethods.getSeasonBoardTaskFollowArrivalNotice = function (section = '') {
    const notice = this.pendingSeasonBoardTaskFollowNotice && typeof this.pendingSeasonBoardTaskFollowNotice === 'object' ? this.pendingSeasonBoardTaskFollowNotice : null;
    if (!notice) return null;
    const action = String(notice.action || '').trim();
    if (action !== 'collection') return null;
    const targetSection = String(notice.value || notice.anchorSection || 'sanctum').trim() || 'sanctum';
  ...
  collectionHubMethods.renderSeasonBoardTaskFollowArrival = function (section = '') {
    if (typeof document === 'undefined') return;
    const heading = document.querySelector('#collection .collection-heading-group');
    ...
    noticeEl.innerHTML = `
            ...
            <button type="button" class="collection-inline-btn collection-handoff-arrival-btn"
                ...
                >${escapeHtml(notice.focusLabel)}</button>
        `;
  ...
  collectionHubMethods.getSeasonBoardTaskFollowArrivalFocusNotice = function () {
    const pending = typeof this.getSeasonBoardTaskFollowArrivalNotice === 'function' ? this.getSeasonBoardTaskFollowArrivalNotice(this.collectionHubState?.section || '') : null;
    const raw = pending || (this.lastSeasonBoardTaskFollowArrivalNotice && typeof this.lastSeasonBoardTaskFollowArrivalNotice === 'object' ? this.lastSeasonBoardTaskFollowArrivalNotice : null);
    if (!raw) return null;
    const action = String(raw.action || '').trim();
    if (action !== 'collection') return null;
    const value = String(raw.value || raw.anchorSection || 'sanctum').trim() || 'sanctum';
  ...
  collectionHubMethods.clearSeasonBoardTaskFollowArrivalFocus = function () {
    if (typeof document === 'undefined') return;
  ...
  collectionHubMethods.findSeasonBoardTaskFollowArrivalTarget = function (notice = {}) {
    if (typeof document === 'undefined') return null;
    const taskId = String(notice.taskId || '').trim();
    const laneId = String(notice.laneId || '').trim();
    const findNode = (selector, predicate) => Array.from(document.querySelectorAll(selector)).find(el => {
    ...
    const matchesTaskLane = (taskValue = '', laneValue = '') => !!taskId && String(taskValue || '') === taskId && (!laneId || String(laneValue || '') === laneId);
    const task = findNode('[data-season-board-task="true"]', (_el, ds) => matchesTaskLane(ds.seasonBoardTaskId, ds.seasonBoardLaneId) || !!taskId && ds.seasonBoardTaskId === taskId);
    if (task) {
      return {
        kind: 'task',
    ...
    return typeof this.findRewardSeasonBoardHandoffArrivalTarget === 'function' ? this.findRewardSeasonBoardHandoffArrivalTarget(notice) : null;
  };
  collectionHubMethods.focusSeasonBoardTaskFollowArrival = function (retryAttempt = 0) {
    if (typeof document === 'undefined') return false;
  ...
  collectionHubMethods.loadCollectionUnlockHistory = function () {
    try {
      const raw = typeof localStorage !== 'undefined' ? localStorage.getItem(COLLECTION_HISTORY_KEY) : null;
      const parsed = raw ? JSON.parse(raw) : [];
      if (!Array.isArray(parsed)) return [];
      return parsed.filter(entry => entry && typeof entry === 'object' && entry.type && entry.itemId).map(entry => ({
      ...
      })).sort((a, b) => b.timestamp - a.timestamp).slice(0, 24);
    } catch (error) {
      return [];
    }
  };
  collectionHubMethods.persistCollectionUnlockHistory = function () {
    try {
  ...
  collectionHubMethods.loadBossMemoryRecords = function () {
    try {
      const raw = typeof localStorage !== 'undefined' ? localStorage.getItem(BOSS_MEMORY_RECORDS_KEY) : null;
      const parsed = raw ? JSON.parse(raw) : {};
  ...
  collectionHubMethods.persistBossMemoryRecords = function () {
    try {
      if (typeof localStorage === 'undefined') return;
  ...
  collectionHubMethods.loadRunPathRecords = function () {
    try {
      const raw = typeof localStorage !== 'undefined' ? localStorage.getItem(RUN_PATH_RECORDS_KEY) : null;
      const parsed = raw ? JSON.parse(raw) : {};
      if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return {};
      return Object.keys(parsed).reduce((records, pathId) => {
        const record = parsed[pathId];
        if (!record || typeof record !== 'object') return records;
        const favoredSets = Array.isArray(record.favoredSets) ? record.favoredSets.map(item => String(item || '').trim()).filter(Boolean).slice(0, 4) : [];
        records[pathId] = {
  ...
  collectionHubMethods.persistRunPathRecords = function () {
    try {
      if (typeof localStorage === 'undefined') return;
  ...
  collectionHubMethods.getRunPathRecord = function (pathId = '') {
    this.ensureCollectionHubBootState();
    const safePathId = String(pathId || '').trim();
    const source = safePathId && this.runPathRecords && this.runPathRecords[safePathId] ? this.runPathRecords[safePathId] : null;
    return {
      pathId: safePathId,
  ...
  collectionHubMethods.recordRunPathCompletion = function (pathMeta = null, options = {}) {
    ...
    const completionRecord = pathMeta?.completionRecord && typeof pathMeta.completionRecord === 'object' ? pathMeta.completionRecord : {};
    ...
    const mutationMeta = pathMeta?.mutation && typeof pathMeta.mutation === 'object' ? pathMeta.mutation : null;
    const favoredSets = Array.isArray(pathMeta?.treasureSynergy?.favoredSets) ? pathMeta.treasureSynergy.favoredSets.map(item => String(item || '').trim()).filter(Boolean).slice(0, 4) : previous.favoredSets;
    const next = {
      pathId: safePathId,
  ...
  collectionHubMethods.getCompletedRunPathCount = function () {
    this.ensureCollectionHubBootState();
    return Object.values(this.runPathRecords || {}).filter(record => clampInt(record?.clears || 0, 0) > 0).length;
  };
  collectionHubMethods.getTotalRunPathClearCount = function () {
    this.ensureCollectionHubBootState();
    return Object.values(this.runPathRecords || {}).reduce((sum, record) => sum + clampInt(record?.clears || 0, 0), 0);
  };
  collectionHubMethods.getLatestRunPathRecord = function () {
    this.ensureCollectionHubBootState();
    const records = Object.values(this.runPathRecords || {}).filter(record => record && clampInt(record.lastCompletedAt || 0, 0) > 0).sort((a, b) => clampInt(b.lastCompletedAt || 0, 0) - clampInt(a.lastCompletedAt || 0, 0));
    return records[0] || null;
  };
  collectionHubMethods.loadRunPathBossSamples = function () {
    try {
      const raw = typeof localStorage !== 'undefined' ? localStorage.getItem(RUN_PATH_BOSS_SAMPLES_KEY) : null;
      const parsed = raw ? JSON.parse(raw) : [];
      if (!Array.isArray(parsed)) return [];
      return parsed.filter(sample => sample && typeof sample === 'object' && sample.pathId && sample.bossId).map(sample => {
        const realm = clampInt(sample.realm || 0, 0, 9999);
        const chapter = realm > 0 && typeof this.getChapterProfileForRealm === 'function' ? this.getChapterProfileForRealm(realm) : null;
        const favoredSets = Array.isArray(sample.favoredSets) ? sample.favoredSets.map(item => String(item || '').trim()).filter(Boolean).slice(0, 4) : [];
        return {
      ...
      }).sort((a, b) => clampInt(b.completedAt || 0, 0) - clampInt(a.completedAt || 0, 0)).slice(0, 60);
    } catch (error) {
      return [];
    }
  };
  collectionHubMethods.persistRunPathBossSamples = function () {
    try {
      if (typeof localStorage === 'undefined') return;
  ...
  collectionHubMethods.recordRunPathBossSample = function (pathMeta = null, bossMeta = null, options = {}) {
    this.ensureCollectionHubBootState();
    const safePathId = String(pathMeta?.id || '').trim();
    ...
    const characterMeta = characterId ? getCharacterMeta(characterId) : null;
    const favoredSets = Array.isArray(pathMeta?.treasureSynergy?.favoredSets) ? pathMeta.treasureSynergy.favoredSets.map(item => String(item || '').trim()).filter(Boolean).slice(0, 4) : [];
  ...
  collectionHubMethods.getRunPathBossSamples = function (options = {}) {
    this.ensureCollectionHubBootState();
  ...
  collectionHubMethods.getRunPathSampleSetLabel = function (setId = '') {
  ...
  collectionHubMethods.buildRunPathSampleRecommendation = function (samples = [], options = {}) {
    ...
    const rankByCountAndTurn = (source = []) => source.slice().sort((a, b) => {
    ...
    const resolveChapterMeta = (sample = null) => {
      ...
      const realm = clampInt(sample.realm || 0, 0, 9999);
      const chapterIndex = realm > 0 ? getChapterIndexForRealm(realm) : parseChapterIndexFromText(sample.chapterName || sample.chapterLabel || '');
      const chapterProfile = chapterIndex > 0 && typeof this.getChapterProfileForRealm === 'function' ? this.getChapterProfileForRealm((chapterIndex - 1) * 3 + 1) : null;
      return {
        index: chapterIndex,
    ...
    samples.forEach(sample => {
      ...
      (Array.isArray(sample.favoredSets) ? sample.favoredSets : []).map(setId => String(setId || '').trim()).filter(Boolean).forEach(setId => {
      ...
      const chapterKey = chapterMeta.index > 0 ? `chapter_${chapterMeta.index}` : String(chapterMeta.name || '').trim();
      if (chapterKey) {
    ...
    const topChapter = topChapterRaw ? (() => {
      const targetChapterIndex = targetChapter.index > 0 ? targetChapter.index : topChapterRaw.index > 0 ? topChapterRaw.index : 0;
      const targetChapterName = targetChapter.name || (targetChapterIndex > 0 ? `第${targetChapterIndex}章` : '');
      const chapterDistance = targetChapterIndex > 0 && topChapterRaw.index > 0 ? Math.abs(topChapterRaw.index - targetChapterIndex) : 0;
      const coverageScore = Math.round((topChapterRaw.count || 0) / Math.max(1, samples.length) * 65);
      const speedScore = topChapterRaw.bestTurn < 9999 ? Math.max(0, 18 - topChapterRaw.bestTurn) : 0;
      const chapterAlignScore = targetChapterIndex > 0 && topChapterRaw.index > 0 ? Math.max(0, 25 - chapterDistance * 10) : 12;
    ...
    if (topChapter) {
      const chapterTargetHint = topChapter.targetName && topChapter.targetName !== topChapter.name ? `，当前章节 ${topChapter.targetName}` : '';
    ...
    if (topBoss && !isBossFocused) {
      lines.push(topBoss.bestTurn < 9999 ? `推荐目标：优先复刻 ${topBoss.name}（最快 ${topBoss.bestTurn} 回合）的收官模板。` : `推荐目标：优先补 ${topBoss.name} 的限时回合样本，后续更容易校准收官节奏。`);
    }
    return {
  ...
  collectionHubMethods.buildRunPathBossSampleBoard = function (options = {}) {
    const displayLimit = Math.max(1, clampInt(options.limit || 3, 1, 6));
    const samples = typeof this.getRunPathBossSamples === 'function' ? this.getRunPathBossSamples({
      ...options,
      limit: 60
    }) : [];
    ...
    const bestTurn = timedSamples.length > 0 ? timedSamples.reduce((best, sample) => Math.min(best, clampInt(sample.turns || 0, 0, 9999)), 9999) : 0;
    const entries = samples.slice(0, displayLimit).map(sample => ({
      ...sample,
      headline: [sample.characterName || '未定角色', sample.pathName || '未定命途', sample.mutationName || ''].filter(Boolean).join(' · '),
      subtitle: `${sample.bossName || sample.bossId} · 第 ${Math.max(1, clampInt(sample.realm || 1, 1, 9999))} 重${sample.turns > 0 ? ` · ${sample.turns} 回合` : ''}`,
      tagLine: [sample.mutationBranch && sample.mutationName ? `${sample.mutationBranch}·${sample.mutationName}` : '', ...(sample.favoredSets || []).slice(0, 2).map(setId => this.getRunPathSampleSetLabel(setId)).filter(Boolean)].filter(Boolean)
    }));
    const recommendation = typeof this.buildRunPathSampleRecommendation === 'function' ? this.buildRunPathSampleRecommendation(samples, options) : {
      character: null,
      mutation: null,
      chapter: null,
      sets: [],
    ...
    return {
      title: focusPathName && focusBossName ? `${focusPathName} × ${focusBossName}` : focusBossName ? `${focusBossName} 样本对照` : focusPathName ? `${focusPathName} 样本对照` : '近期通关样本',
      ...
      emptyText: focusBossName ? '当前还没有这位主宰的通关样本，先去打一场主线 Boss 再回来对照。' : focusPathName ? '当前命途还没有沉淀出 Boss 通关样本，先用它完成一场章节 Boss。' : '当前还没有可用于对照的通关样本。',
      entries,
      recommendation
    };
  };
  collectionHubMethods.getBossMemoryRecord = function (bossId = '') {
    this.ensureCollectionHubBootState();
    const safeBossId = String(bossId || '').trim();
    const source = safeBossId && this.bossMemoryRecords && this.bossMemoryRecords[safeBossId] ? this.bossMemoryRecords[safeBossId] : null;
    return {
      bossId: safeBossId,
  ...
  collectionHubMethods.recordBossMemoryResult = function (bossId = '', result = 'defeat', turns = 0) {
    const safeBossId = String(bossId || '').trim();
    ...
    const next = {
      ...
      bestTurn: isVictory ? previous.bestTurn > 0 ? Math.min(previous.bestTurn, Math.max(1, clampInt(turns || 0, 1, 9999))) : Math.max(1, clampInt(turns || 0, 1, 9999)) : previous.bestTurn,
  ...
  collectionHubMethods.getCollectionUnlockHistory = function (limit = 6) {
    this.ensureCollectionHubBootState();
    return (this.collectionUnlockHistory || []).slice(0, Math.max(0, clampInt(limit, 0, 24)));
  };
  collectionHubMethods.formatCollectionTimestamp = function (timestamp = 0) {
  ...
  collectionHubMethods.getFateLineageSnapshot = function (options = {}) {
    this.ensureCollectionHubBootState();
    const latestSlate = options.latestSlate && typeof options.latestSlate === 'object' ? options.latestSlate : typeof this.getLatestRunSlate === 'function' ? this.getLatestRunSlate() : null;
    const currentRunPath = this.player && typeof this.player.getRunPathMeta === 'function' ? this.player.getRunPathMeta() : this.selectedRunPathId && typeof this.getRunPathMetaById === 'function' ? this.getRunPathMetaById(this.selectedRunPathId) : null;
    const currentDestiny = this.player && typeof this.player.getRunDestinyMeta === 'function' ? this.player.getRunDestinyMeta() : null;
    const currentCharacter = getCharacterMeta(this.player?.characterId || '');
    const samples = typeof this.getRunPathBossSamples === 'function' ? this.getRunPathBossSamples({
      limit: 24,
      sortBy: 'recent'
    }) : [];
    const selectedGuide = typeof this.getSelectedObservatoryExpeditionGuide === 'function' ? this.getSelectedObservatoryExpeditionGuide({
      silentSync: true
    }) : null;
    const trainingFocus = typeof this.getObservatoryTrainingFocus === 'function' ? this.getObservatoryTrainingFocus() : null;
    const seasonVerification = typeof this.getSeasonVerificationSnapshot === 'function' ? this.getSeasonVerificationSnapshot() : null;
    const primarySeasonVerification = seasonVerification?.primary && typeof seasonVerification.primary === 'object' ? seasonVerification.primary : null;
    const secondarySeasonVerification = seasonVerification?.side && typeof seasonVerification.side === 'object' ? seasonVerification.side : null;
    const seasonVerificationHistoryCount = Array.isArray(seasonVerification?.history) ? seasonVerification.history.length : 0;
    const frontierResolutionRecords = typeof this.getSeasonBoardFrontierResolutionArchiveRecords === 'function' ? this.getSeasonBoardFrontierResolutionArchiveRecords() : [];
    const frontierResolutionHistoryCount = Array.isArray(frontierResolutionRecords) ? frontierResolutionRecords.length : 0;
    const slateFocus = latestSlate && typeof this.buildObservatoryTrainingFocusFromSlate === 'function' ? this.buildObservatoryTrainingFocusFromSlate(latestSlate) : null;
    const archiveEntries = typeof this.getObservatoryArchiveEntries === 'function' ? this.getObservatoryArchiveEntries({
      limit: 12,
      types: ['challenge', 'replay'],
      sortBy: 'recent'
    }) : [];
    const agendaDashboard = typeof this.getSanctumAgendaDashboard === 'function' ? this.getSanctumAgendaDashboard() : {
      active: null,
      lastResolved: null,
      history: []
    };
    const activeAgenda = agendaDashboard?.active && typeof agendaDashboard.active === 'object' ? agendaDashboard.active : null;
    const lastResolved = agendaDashboard?.lastResolved && typeof agendaDashboard.lastResolved === 'object' ? agendaDashboard.lastResolved : null;
    const historyRecords = Array.isArray(agendaDashboard?.history) ? agendaDashboard.history.filter(entry => entry && typeof entry === 'object' && entry.agendaId).slice(-6) : [];
    const researchHistoryCount = Math.max(historyRecords.length, seasonVerificationHistoryCount);
    const runPathRecords = Object.keys(this.runPathRecords || {}).map(pathId => typeof this.getRunPathRecord === 'function' ? this.getRunPathRecord(pathId) : this.runPathRecords?.[pathId] || null).filter(record => record && clampInt(record.clears || 0, 0, 9999) > 0).sort((a, b) => {
    ...
    const rankEntries = (source = [], limit = 4) => source.slice().sort((a, b) => {
    ...
    const classifySeasonVerdictStyle = (source = null) => {
      ...
      const resultStatus = String(source.resultStatus || source.primaryVerificationResultStatus || source.resolvedStatus || '').trim();
      const writebackMode = String(source.writebackMode || source.primaryWritebackMode || '').trim();
      ...
      } else if (writebackMode === 'carry_forward' || resultStatus === 'deferred' || writebackMode === 'pending') {
        id = 'deferred_cleanup';
        label = '延账收尾';
        icon = '⏳';
        fallbackNote = '这笔账被继续带入后续周转，谱系会把拖延与收尾节奏一起记下来。';
      } else if (['upgrade_verdict', 'degrade'].includes(writebackMode) || ['verified', 'failed'].includes(resultStatus)) {
        id = 'risky_push';
        label = '押榜抢线';
        icon = '⚔️';
      ...
      const noteLine = [[sourceModeLabel, statusLabelMap[resultStatus] || ''].filter(Boolean).join(' · '), detailLine || fallbackNote].filter(Boolean).slice(0, 2).join('｜') || fallbackNote;
      return {
        id,
        label,
        icon,
        ...
        sourceKey: String(source.recordId || source.ledgerId || `${source.weekTag || 'current'}:${role || 'primary'}:${writebackMode || resultStatus || 'verdict'}`).trim()
      };
    };
    ...
    const verdictStyleEntries = verdictStyleFallbacks.map(meta => {
      ...
      return {
        ...
        icon: meta.icon,
        value: clampInt(entry?.value || 0, 0, 9999),
        valueText: entry?.value > 0 ? `${meta.dominantLabel} · ${entry.value} 次` : `等待${meta.label}`,
    ...
    const dominantVerdictStyle = rankEntries(verdictStyleEntries.filter(entry => entry.value > 0).map(entry => ({
      ...entry,
    ...
    const frontierResolutionEntries = (Array.isArray(frontierResolutionRecords) ? frontierResolutionRecords : []).map((record, index) => {
      ...
      const sourceKey = String(record.recordId || record.frontierResolutionId || `${record.weekTag || 'weekless'}:${choiceId}:${record.frontierResolutionSubmittedAt || record.updatedAt || index}`).trim();
      ...
      existing.tags = Array.from(new Set([...(existing.tags || []), record.weekLabel || record.weekTag || '', record.frontierResolutionSupportLaneLabel || ''].filter(Boolean))).slice(0, 3);
    ...
    samples.forEach(sample => {
      ...
      existing.tags = [...Array.from(existing.pathNames).slice(0, 2), ...Array.from(existing.bossNames).slice(0, 1)].filter(Boolean).slice(0, 3);
      existing.noteLine = [sample.pathName ? `最近样本 · ${sample.pathName}` : '', sample.bossName ? `对 ${sample.bossName}` : '', turns > 0 ? `${turns} 回合` : ''].filter(Boolean).join(' · ');
    ...
    if (characterMap.size <= 0 && currentCharacter) {
      characterMap.set(currentCharacter.id, {
        ...
        value: 0,
        valueText: '当前主修',
        noteLine: currentRunPath?.name ? `当前正沿【${currentRunPath.name}】推进样本。` : '当前角色尚未留下稳定实战样本。',
    ...
    const characterTrack = buildTrack({
      ...
      label: '角色谱系',
      icon: '🧍',
      summaryLine: dominantCharacter ? dominantCharacter.value > 0 ? `实战样本当前更常由【${dominantCharacter.label}】收官，${dominantCharacter.valueText}已经开始成形。` : `当前主修角色暂定为【${dominantCharacter.label}】，后续样本会继续把角色画像压实。` : '角色谱系仍待第一份实战样本落档。',
      progressText: dominantCharacter?.value > 0 ? `已记录 ${characterEntries.filter(entry => entry.value > 0).length} 名角色` : '等待角色样本',
    ...
    runPathRecords.slice(0, 3).forEach(record => {
      const pathMeta = typeof this.getRunPathMetaById === 'function' ? this.getRunPathMetaById(record.pathId || '') : null;
      styleEntries.push({
        ...
        noteLine: [record.lastCharacterName ? `最近由 ${record.lastCharacterName}` : '', record.lastMutationName ? `裂变 ${record.lastMutationName}` : '', record.lastRealm > 0 ? `第 ${record.lastRealm} 重` : ''].filter(Boolean).join(' · ') || '当前流派正在等待新的圆满样本。',
        tags: [record.lastMutationBranch && record.lastMutationName ? `${record.lastMutationBranch}·${record.lastMutationName}` : '', ...(record.favoredSets || []).slice(0, 2).map(setId => this.getRunPathSampleSetLabel(setId))].filter(Boolean).slice(0, 3),
        anchorSection: 'builds',
    ...
    const styleTrack = buildTrack({
      ...
      summaryLine: dominantStyle ? `长期流派当前收束到【${dominantStyle.label}】${currentDestiny ? `，命格签名是【${currentDestiny.name || currentDestiny.id}】。` : '。'}` : '流派谱系还没有稳定落在某条长期主线上。',
      progressText: dominantStyle?.value > 0 ? `${dominantStyle.valueText}` : currentRunPath?.name ? '当前命途已挂接' : '等待流派战录',
    ...
    const addNodeSource = (sourceKeyBase = '', values = [], fallbackThemeKey = '', sourceLabel = '', noteLine = '') => {
      ...
      const nodeTypes = typeof this.inferSanctumAgendaNodeTypes === 'function' ? this.inferSanctumAgendaNodeTypes(values, fallbackThemeKey) : [];
      nodeTypes.forEach(nodeType => {
        const meta = typeof this.getSanctumAgendaNodeMeta === 'function' ? this.getSanctumAgendaNodeMeta(nodeType) : null;
    ...
    addNodeSource(currentRunPath?.id ? makeSourceKey('path', currentRunPath.id) : '', [currentRunPath?.routeHint, currentRunPath?.playstyle, currentRunPath?.description], currentRunPath?.id || '', '当前命途', currentRunPath?.routeHint || currentRunPath?.playstyle || '当前命途会把节点偏好压成长期路线。');
    addNodeSource(selectedGuide?.id ? makeSourceKey('guide', selectedGuide.id) : '', [selectedGuide?.routeFocusLine, selectedGuide?.compareHint, ...(selectedGuide?.trainingTags || [])], selectedGuide?.themeKey || '', '精选命盘', selectedGuide?.routeFocusLine || selectedGuide?.compareHint || '当前精选命盘会提供一条稳定的节点抓手。');
    addNodeSource(trainingFocus?.sourceRunId ? makeSourceKey('run', trainingFocus.sourceRunId) : '', [trainingFocus?.routeFocusLine, trainingFocus?.compareHint, ...(trainingFocus?.trainingTags || [])], trainingFocus?.themeKey || '', '当前主练', trainingFocus?.routeFocusLine || trainingFocus?.trainingAdvice || '当前主练已经开始沉淀节点偏好。');
    addNodeSource(slateFocus?.sourceRunId ? makeSourceKey('run', slateFocus.sourceRunId) : '', [slateFocus?.routeFocusLine, slateFocus?.compareHint, ...(slateFocus?.trainingTags || [])], slateFocus?.themeKey || '', '最近答卷', slateFocus?.routeFocusLine || slateFocus?.trainingAdvice || '最近答卷会继续反哺节点偏好。');
    archiveEntries.forEach(entry => {
      addNodeSource(makeSourceKey('archive', entry.archiveEntryId || entry.id || entry.at || nodeMap.size), [entry.preferredNodes, ...(entry.trainingTags || []), entry.note || '', entry.title || ''], entry.themeKey || '', entry.themeLabel || entry.originLabel || '观星档案', entry.note || entry.summary || entry.title || '观星档案已经留下节点偏好。');
    });
    [activeAgenda, ...historyRecords].filter(Boolean).forEach(entry => {
      const runSourceKey = entry.sourceRunId ? makeSourceKey('run', entry.sourceRunId) : makeSourceKey('agenda', `${entry.agendaId || 'agenda'}_${entry.updatedAt || entry.selectedAt || 0}`);
      addNodeSource(runSourceKey, [entry.focusNodeTypes, entry.contractNodeTypes, entry.focusNodeLine, entry.selectedContractLine, entry.selectedDecisionLine], entry.themeKey || '', entry.outcome === 'active' ? '当前议程' : '洞府结题', entry.focusNodeLine || entry.summaryLine || entry.selectedContractLine || entry.selectedDecisionLine || '洞府研究已经给出节点偏好。');
    });
    ...
    const nodeTrack = buildTrack({
      ...
      summaryLine: nodeEntries.length > 0 ? `路线留痕当前更常落在【${nodeEntries.slice(0, 3).map(entry => entry.label).join(' / ')}】。` : '节点谱系仍待从观星、归卷和议程里继续抽出长期主轴。',
    ...
    historyRecords.forEach(record => {
      const runKey = record.sourceRunId ? makeSourceKey('run', record.sourceRunId) : makeSourceKey('agenda', `${record.agendaId || 'agenda'}_${record.updatedAt || record.selectedAt || 0}`);
      addFacetValue('agenda', record.themeLabel || record.name || record.agendaId, runKey, `${record.name || record.themeLabel || '洞府议程'} · ${record.outcomeLabel || '研究留痕'}`);
      if (record.selectedDecisionLabel) {
        addFacetValue('decision', record.selectedDecisionLabel, runKey, record.selectedDecisionLine || record.reasonLine || record.summaryLine || '');
      }
      if (record.selectedContractLabel) {
        addFacetValue('contract', record.selectedContractLabel, runKey, record.contractResolutionLine || record.selectedContractLine || record.summaryLine || '');
      }
      const outcomeLabel = record.outcome === 'success' ? '结题成功' : record.recoveryEligible ? `残卷回收${record.recoveryTierLabel ? ` · ${record.recoveryTierLabel}` : ''}` : '研究未成';
      addFacetValue('outcome', outcomeLabel, runKey, record.recoveryLine || record.grantedLine || record.reasonLine || record.summaryLine || '');
    });
    const pickFacetTop = (facetId = '') => rankEntries(Array.from(facetMaps[facetId] || []).map(([label, entry]) => ({
      id: `${facetId}_${label}`,
      label,
    ...
    const dominantResearchBase = agendaFacet || (activeAgenda ? {
      label: activeAgenda.name,
      id: activeAgenda.agendaId || 'active'
    } : null);
    ...
    const researchTrack = buildTrack({
      ...
      label: '研究谱系',
      icon: '📚',
      summaryLine: dominantVerdictStyle ? `研究谱系当前更常留下【${dominantVerdictStyle.label}】这类赛季裁定习惯${dominantResearchBase?.label ? `，并继续围绕【${dominantResearchBase.label}】推进。` : '。'}` : dominantResearchBase ? `研究谱系当前更常围绕【${dominantResearchBase.label}】推进，处置与契约倾向也开始留下稳定留痕。` : '研究谱系仍待第一批议程结果真正沉淀下来。',
      progressText: researchHistoryCount > 0 || dominantVerdictStyle ? `${researchHistoryCount} 份研究留痕 · ${verdictStyleEntries.filter(entry => entry.value > 0).length} 类赛季裁定` : '等待研究结题',
    ...
    const frontierTrack = buildTrack({
      ...
      summaryLine: dominantFrontierStyle ? `会审裁记当前更常留下【${dominantFrontierStyle.label}】风格，已累计 ${frontierResolutionHistoryCount} 条战役史卷。` : '战线裁记仍待第一条会审封记，之后会沉淀成长期处理分歧的风格。',
      progressText: frontierResolutionHistoryCount > 0 ? `已封 ${frontierResolutionHistoryCount} 条裁记` : '等待会审封记',
    ...
    const available = !!(currentCharacter || currentRunPath || currentDestiny || activeAgenda || lastResolved || samples.length > 0 || archiveEntries.length > 0 || runPathRecords.length > 0 || seasonVerification?.available || frontierResolutionHistoryCount > 0);
    const summaryParts = [dominantCharacter?.label || currentCharacter?.name || '', dominantStyle?.label || currentRunPath?.name || currentDestiny?.name || '', dominantNode?.label || '', dominantResearchBase?.label || ''].filter(Boolean);
    const summaryLine = summaryParts.length > 0 ? `长期主修正在向【${summaryParts.join(' / ')}】收束${dominantVerdictStyle?.label ? `，赛季裁定更常留下【${dominantVerdictStyle.label}】` : ''}${dominantFrontierStyle?.label ? `${dominantVerdictStyle?.label ? '，' : '，'}会审裁记偏向【${dominantFrontierStyle.label}】` : ''}。` : dominantVerdictStyle?.label ? `命盘谱系已经开始记录【${dominantVerdictStyle.label}】这类赛季裁定习惯${dominantFrontierStyle?.label ? `，会审裁记偏向【${dominantFrontierStyle.label}】。` : '。'}` : dominantFrontierStyle?.label ? `命盘谱系已经开始记录【${dominantFrontierStyle.label}】这类会审裁记风格。` : '命盘谱系仍待第一批长期留痕落档。';
    const baseDetailLine = activeAgenda ? `当前研究：${activeAgenda.name} · ${activeAgenda.phaseLabel || activeAgenda.selectedDecisionLabel || activeAgenda.selectedContractLabel || activeAgenda.summaryLine || '洞府样本正在推进。'}` : lastResolved ? `最近研究：${lastResolved.name} · ${lastResolved.outcomeLabel || '研究留痕'}${lastResolved.recoveryLine ? ` · ${lastResolved.recoveryLine}` : ''}` : dominantCharacter?.noteLine || dominantStyle?.noteLine || dominantNode?.noteLine || '角色、流派、节点与研究会在这里合成一份长期身份档案。';
    const detailLine = [baseDetailLine, primarySeasonVerification?.writebackLine ? `赛季回写：${primarySeasonVerification.writebackLine}` : dominantVerdictStyle?.noteLine ? `裁定留痕：${dominantVerdictStyle.noteLine}` : latestFrontierResolution?.noteLine ? `会审留痕：${latestFrontierResolution.noteLine}` : secondarySeasonVerification?.writebackLine ? `旁证回写：${secondarySeasonVerification.writebackLine}` : ''].filter(Boolean).slice(0, 2).join('｜');
    const currentFocusLine = [selectedGuide?.title ? `精选命盘【${selectedGuide.title}】` : '', trainingFocus?.chapterName ? `当前主练 ${trainingFocus.chapterName}` : '', primarySeasonVerification?.label ? `主验证 ${primarySeasonVerification.label}` : '', latestSlate?.chapterName ? `最近答卷 ${latestSlate.chapterName}` : '', !primarySeasonVerification?.label && secondarySeasonVerification?.label ? `旁证 ${secondarySeasonVerification.label}` : '', !primarySeasonVerification?.label && !secondarySeasonVerification?.label && dominantVerdictStyle?.label ? `当前裁定 ${dominantVerdictStyle.label}` : ''].filter(Boolean).slice(0, 3).join(' · ');
    const nextTargets = [];
    ...
    if (researchHistoryCount <= 0) {
      nextTargets.push(activeAgenda ? `把【${activeAgenda.name}】结成一次留痕，让研究谱系开始记录处置、契约与回收倾向。` : '回洞府立下一道议程并真正结题，研究谱系才会开始稳定成长。');
    ...
    return {
      ...
      researchTrack,
      frontierTrack,
      recentRecords: [dominantCharacter?.noteLine || '', dominantStyle?.noteLine || '', lastResolved?.summaryLine || '', dominantVerdictStyle?.noteLine || '', latestFrontierResolution?.noteLine || ''].filter(Boolean).slice(0, 3)
    };
  };
  collectionHubMethods.buildRuntimeSaveSnapshot = function () {
    ...
    return {
      version: '5.1.0',
      player: this.player && typeof this.player.getState === 'function' ? this.player.getState() : null,
      map: {
      ...
      unlockedRealms: Array.isArray(this.unlockedRealms) && this.unlockedRealms.length > 0 ? this.unlockedRealms.slice() : [1],
      ...
      endlessMeta: typeof this.ensureEndlessState === 'function' ? this.ensureEndlessState() : this.endlessState,
      encounterMeta: typeof this.ensureEncounterState === 'function' ? this.ensureEncounterState() : this.encounterState,
      schemaMigratedAt: Date.now(),
      timestamp: Date.now()
    };
  };
  collectionHubMethods.captureBossMemorySession = function (bossId = '') {
    this.ensureCollectionHubBootState();
    const safeBossId = String(bossId || '').trim();
  ...
  collectionHubMethods.restoreBossMemorySession = function () {
  ...
  collectionHubMethods.startBossMemoryBattle = function (bossId = '') {
    this.ensureCollectionHubBootState();
    const safeBossId = String(bossId || '').trim();
    ...
    const entry = typeof this.getBossArchiveEntries === 'function' ? this.getBossArchiveEntries().find(item => item.id === safeBossId) || null : null;
    ...
    if (this.player && typeof this.player.getRunDestinyMeta === 'function' && !this.player.getRunDestinyMeta() && typeof this.player.setRunDestiny === 'function') {
      const runDestinyId = this.selectedRunDestinyId || (typeof this.resolveDefaultRunDestinyId === 'function' ? this.resolveDefaultRunDestinyId(characterId) : null);
    ...
    if (this.player && typeof this.player.getSpiritCompanionMeta === 'function' && !this.player.getSpiritCompanionMeta() && typeof this.player.setSpiritCompanion === 'function') {
      const spiritCompanionId = this.selectedSpiritCompanionId || (typeof this.resolveDefaultSpiritCompanionId === 'function' ? this.resolveDefaultSpiritCompanionId(characterId) : null);
    ...
    const bossInstance = typeof Utils !== 'undefined' && typeof Utils.deepClone === 'function' ? Utils.deepClone(catalogBoss) : JSON.parse(JSON.stringify(catalogBoss));
  ...
  collectionHubMethods.finishBossMemoryBattle = function (result = 'defeat', payload = {}) {
    ...
    if (firstClear && typeof this.awardLegacyEssence === 'function') {
      legacyGain = this.awardLegacyEssence(4, '伏魔台记忆战', {
        silent: true
      });
    }
    if (isVictory && typeof this.recordCollectionUnlock === 'function') {
      this.recordCollectionUnlock('boss_memory', {
        ...
        note: firstClear ? `伏魔台首胜 · 最快 ${record.bestTurn || Math.max(1, normalizedTurns)} 回合` : `伏魔台再破记忆 · 最快 ${record.bestTurn || Math.max(1, normalizedTurns)} 回合`
      });
    }
    const title = isVictory ? '伏魔台记忆战胜利' : '伏魔台记忆战留痕';
    const lines = isVictory ? [`${bossName} 的记忆残响已被镇压。`, `累计胜场 ${record.clears} · 最快 ${record.bestTurn || Math.max(1, normalizedTurns)} 回合`, firstClear && legacyGain > 0 ? `首胜奖励：传承精魄 +${legacyGain}` : '重复演武不会消耗主线进度，可继续刷新更快轮次。'] : [`${bossName} 的记忆尚未平复。`, '本次演武已留档，失败不会污染主线存档。', `累计试作 ${record.attempts} 次${record.bestTurn > 0 ? ` · 当前最快 ${record.bestTurn} 回合` : ''}`];
    if (typeof Utils !== 'undefined' && Utils.showBattleLog) {
      Utils.showBattleLog(isVictory ? `【伏魔台】记忆战告捷：${bossName}` : `【伏魔台】记忆战留档：${bossName}`);
    }
  ...
  collectionHubMethods.showCollection = function (section = 'laws') {
    this.ensureCollectionHubBootState();
    this.collectionHubState = this.normalizeCollectionHubState({
  ...
  collectionHubMethods.switchCollectionSection = function (section = 'laws') {
    this.ensureCollectionHubBootState();
    this.collectionHubState = this.normalizeCollectionHubState({
      ...this.collectionHubState,
      section
  ...
  collectionHubMethods.setSpiritCodexSearchQuery = function (query = '') {
    this.collectionHubState = this.normalizeCollectionHubState({
  ...
  collectionHubMethods.setSpiritCodexFocusFilter = function (value = 'all') {
    this.collectionHubState = this.normalizeCollectionHubState({
      ...this.getCollectionHubState(),
      spiritFocus: value,
      section: 'spirits'
  ...
  collectionHubMethods.setChapterCodexSearchQuery = function (query = '') {
    this.collectionHubState = this.normalizeCollectionHubState({
      ...this.getCollectionHubState(),
      chapterQuery: query,
      section: 'chapters'
  ...
  collectionHubMethods.setChapterCodexFocusFilter = function (value = 'all') {
    this.collectionHubState = this.normalizeCollectionHubState({
      ...this.getCollectionHubState(),
      chapterFocus: value,
      section: 'chapters'
  ...
  collectionHubMethods.setEnemyCodexSearchQuery = function (query = '') {
    this.collectionHubState = this.normalizeCollectionHubState({
      ...this.getCollectionHubState(),
      enemyQuery: query,
      section: 'enemies'
  ...
  collectionHubMethods.setEnemyCodexFocusFilter = function (value = 'all') {
    this.collectionHubState = this.normalizeCollectionHubState({
      ...this.getCollectionHubState(),
      enemyFocus: value,
      section: 'enemies'
  ...
  collectionHubMethods.setBossArchiveSearchQuery = function (query = '') {
    this.collectionHubState = this.normalizeCollectionHubState({
      ...this.getCollectionHubState(),
      bossQuery: query,
      section: 'bosses'
  ...
  collectionHubMethods.setBossArchiveFocusFilter = function (value = 'all') {
    this.collectionHubState = this.normalizeCollectionHubState({
      ...this.getCollectionHubState(),
      bossFocus: value,
      section: 'bosses'
  ...
  collectionHubMethods.setRunSlateShelfThemeFilter = function (value = 'all') {
    this.collectionHubState = this.normalizeCollectionHubState({
      ...this.getCollectionHubState(),
      slateTheme: value,
      section: 'slates'
  ...
  collectionHubMethods.setRunSlateShelfChapterFilter = function (value = 'all') {
    this.collectionHubState = this.normalizeCollectionHubState({
      ...this.getCollectionHubState(),
      slateChapter: value,
      section: 'slates'
  ...
  collectionHubMethods.setRunSlateShelfRatingFilter = function (value = 'all') {
    this.collectionHubState = this.normalizeCollectionHubState({
      ...this.getCollectionHubState(),
      slateRating: value,
      section: 'slates'
  ...
  collectionHubMethods.getCollectionRealmProgress = function () {
    ...
    const unlockedRealms = Array.isArray(this.unlockedRealms) && this.unlockedRealms.length > 0 ? this.unlockedRealms.map(realm => clampInt(realm, 1, 18)) : [1];
    const unlockedPeak = Math.max(...unlockedRealms);
    const clearedRealm = Math.max(maxClearedFromStats, Math.max(0, unlockedPeak - 1), Math.max(0, maxReached - 1));
    return {
      currentRealm,
      clearedRealm,
  ...
  collectionHubMethods.getCollectionProgressSnapshot = function () {
    this.ensureCollectionHubBootState();
    ...
    const runPathCatalog = typeof this.getRunPathCatalog === 'function' ? this.getRunPathCatalog() : typeof RUN_PATHS !== 'undefined' && RUN_PATHS ? Object.values(RUN_PATHS).filter(item => item && item.id) : [];
    const enemyCatalog = typeof ENEMIES !== 'undefined' && ENEMIES ? Object.values(ENEMIES).filter(enemy => enemy && !enemy.isBoss && !enemy.isMinion) : [];
    const bossCatalog = typeof ENEMIES !== 'undefined' && ENEMIES ? Object.values(ENEMIES).filter(enemy => enemy && enemy.isBoss) : [];
    const realmProgress = this.getCollectionRealmProgress();
    const seenSpiritIds = new Set(this.getCollectionUnlockHistory(24).filter(entry => entry.type === 'spirit').map(entry => entry.itemId));
    const seenEnemyIds = new Set(this.getCollectionUnlockHistory(24).filter(entry => entry.type === 'enemy').map(entry => entry.itemId));
    const currentSpirit = this.player && typeof this.player.getSpiritCompanionMeta === 'function' ? this.player.getSpiritCompanionMeta() : null;
    if (currentSpirit?.id) seenSpiritIds.add(currentSpirit.id);
    const bossMemoryClears = typeof this.getBossMemoryClearCount === 'function' ? this.getBossMemoryClearCount() : 0;
    const bossMemoryAttempts = typeof this.getBossMemoryAttemptCount === 'function' ? this.getBossMemoryAttemptCount() : 0;
    ...
    const completedRunPaths = typeof this.getCompletedRunPathCount === 'function' ? this.getCompletedRunPathCount() : 0;
    const totalRunPathClears = typeof this.getTotalRunPathClearCount === 'function' ? this.getTotalRunPathClearCount() : 0;
    const forgeOverview = this.player && typeof this.player.getTreasureWorkshopResearchOverview === 'function' ? this.player.getTreasureWorkshopResearchOverview() : null;
    return {
      ...
      unclaimedAchievements: Array.isArray(this.achievementSystem?.unlockedAchievements) ? this.achievementSystem.unlockedAchievements.filter(id => !this.achievementSystem.claimedAchievements.includes(id)).length : 0
    };
  };
  collectionHubMethods.getSpiritCodexEntries = function () {
    const catalog = typeof SPIRIT_COMPANIONS !== 'undefined' && SPIRIT_COMPANIONS && typeof SPIRIT_COMPANIONS === 'object' ? SPIRIT_COMPANIONS : {};
    const selectedCharacterId = this.player?.characterId || this.selectedCharacterId || 'linFeng';
    const currentSpirit = this.player && typeof this.player.getSpiritCompanionMeta === 'function' ? this.player.getSpiritCompanionMeta() : null;
    const historySpiritIds = new Set(this.getCollectionUnlockHistory(24).filter(entry => entry.type === 'spirit').map(entry => entry.itemId));
    const draftSpiritIds = Array.isArray(this.pendingSpiritCompanionDrafts?.[selectedCharacterId]) ? this.pendingSpiritCompanionDrafts[selectedCharacterId] : [];
    ...
    return Object.keys(catalog).map(spiritId => {
      const baseMeta = typeof this.getSpiritCompanionMetaById === 'function' ? this.getSpiritCompanionMetaById(spiritId, 1) : null;
      if (!baseMeta) return null;
      const storyProfile = typeof this.getSpiritStoryProfile === 'function' ? this.getSpiritStoryProfile(spiritId) : null;
      const nextTier = Math.min(baseMeta.maxTier || 1, clampInt(baseMeta.tier || 1, 1, 3) + 1);
      const nextMeta = typeof this.getSpiritCompanionMetaById === 'function' ? this.getSpiritCompanionMetaById(spiritId, nextTier) : null;
      const affinityNames = (baseMeta.affinities || []).map(characterId => getCharacterMeta(characterId)?.name || characterId);
      const chapterFits = Object.values(chapterCatalog).filter(chapter => Array.isArray(chapter?.recommendedSpirits) && chapter.recommendedSpirits.includes(spiritId)).map(chapter => chapter.name);
      ...
      const sourceText = isCurrent ? `当前已与【${baseMeta.name}】缔约，可在${storyProfile?.source || '灵契窟'}继续升至更高契阶。` : isAligned ? `${selectedCharacter?.name || '当前角色'} 与它更易共鸣，可在开局灵契三选或${storyProfile?.source || '灵契窟'}追索。` : isSeen ? `你已见过它的回响，继续在${storyProfile?.source || '灵契窟或章节事件'}中深挖更容易稳定缔约。` : '尚未留下完整纪录，推测需通过灵契窟、章节事件或研究项继续追索。';
      const unlockClue = isHidden ? `线索：更适合 ${affinityNames.join('、')} 出手追索。` : `线索：优先在灵契窟升契，或围绕 ${splitKeywords(baseMeta.playstyle, 2).join(' / ') || '核心节奏'} 构筑。`;
      return {
        id: spiritId,
        name: baseMeta.name,
        ...
        nextGrowthText: isCurrent && nextMeta && nextMeta.tier > baseMeta.tier ? `下一阶：${nextMeta.summary || nextMeta.passiveDesc || '灵契护道进一步强化。'}` : isCurrent ? '当前已是最高契阶，可围绕它补足章节与法宝协同。' : '尚未缔约，先解锁来源线索再决定是否围绕它构筑。',
        affinityNames,
  ...
  collectionHubMethods.passesSpiritCodexFilter = function (entry) {
    ...
    const haystack = [entry.id, entry.name, entry.title, entry.description, entry.playstyle, entry.story, entry.sourceText, entry.unlockClue, entry.storyProfile?.acquisitionTitle, entry.storyProfile?.acquisitionSummary, entry.storyProfile?.witnessTitle, entry.storyProfile?.witnessSummary, ...(entry.affinityNames || []), ...(entry.chapterFits || [])].join(' ').toLowerCase();
    return haystack.includes(state.spiritQuery.toLowerCase());
  };
  collectionHubMethods.selectSpiritCodexEntry = function (spiritId = '') {
  ...
  collectionHubMethods.getChapterCodexEntries = function () {
    const entries = [];
    const realmProgress = this.getCollectionRealmProgress();
    ...
    for (let chapterIndex = 1; chapterIndex <= 6; chapterIndex += 1) {
      ...
      const profile = typeof this.getChapterProfileForRealm === 'function' ? this.getChapterProfileForRealm(chapterRealm) : null;
      if (!profile) continue;
      const narrativeProfile = typeof this.getChapterNarrativeProfile === 'function' ? this.getChapterNarrativeProfile(chapterIndex) : null;
      ...
      const ecologyTemplates = typeof ENEMY_ECOLOGY_TEMPLATES !== 'undefined' && ENEMY_ECOLOGY_TEMPLATES && ENEMY_ECOLOGY_TEMPLATES[chapterIndex] ? ENEMY_ECOLOGY_TEMPLATES[chapterIndex] : null;
      const eliteCombo = typeof CHAPTER_ELITE_COMBOS !== 'undefined' && CHAPTER_ELITE_COMBOS && CHAPTER_ELITE_COMBOS[chapterIndex] ? CHAPTER_ELITE_COMBOS[chapterIndex] : null;
  ...
  collectionHubMethods.passesChapterCodexFilter = function (entry) {
    const state = this.getCollectionHubState();
    if (!entry) return false;
    ...
    const haystack = [entry.id, entry.name, entry.fullName, entry.mechanic, entry.mood, entry.skyOmen?.name, entry.skyOmen?.desc, entry.leyline?.name, entry.leyline?.desc, entry.routePrompt, entry.bossPrompt, entry.ecologyTemplates?.formation?.name, entry.ecologyTemplates?.formation?.desc, entry.eliteCombo?.name, entry.eliteCombo?.summary, entry.narrativeProfile?.summary, entry.narrativeProfile?.finaleRecall?.summary, ...(entry.narrativeProfile?.beats || []).map(beat => `${beat?.stage || ''} ${beat?.title || ''} ${beat?.summary || ''}`), ...(entry.focusTags || []), ...(entry.ecologyTags || []), ...entry.enemies.map(enemy => enemy?.name || ''), ...entry.bosses.map(boss => boss?.name || '')].join(' ').toLowerCase();
    return haystack.includes(state.chapterQuery.toLowerCase());
  };
  collectionHubMethods.selectChapterCodexEntry = function (chapterId = '') {
  ...
  collectionHubMethods.getEnemyThreatTags = function (enemy = null) {
    const tags = new Set();
  ...
  collectionHubMethods.getEnemyCounterHints = function (enemy = null) {
    const hints = [];
    const patterns = Array.isArray(enemy?.patterns) ? enemy.patterns : [];
  ...
  collectionHubMethods.getEnemyCodexEntries = function () {
    const enemyCatalog = typeof ENEMIES !== 'undefined' && ENEMIES ? Object.values(ENEMIES).filter(enemy => enemy && !enemy.isBoss && !enemy.isMinion) : [];
    const realmProgress = this.getCollectionRealmProgress();
    const unlockedPeak = Array.isArray(this.unlockedRealms) && this.unlockedRealms.length > 0 ? Math.max(...this.unlockedRealms.map(realm => clampInt(realm, 1, 18))) : 1;
    const maxReachedRealm = Math.max(realmProgress.currentRealm, clampInt(this.player?.maxRealmReached || 1, 1, 18), unlockedPeak);
    const seenEnemyIds = new Set(this.getCollectionUnlockHistory(24).filter(entry => entry.type === 'enemy').map(entry => entry.itemId));
    return enemyCatalog.map(enemy => {
      const realm = clampInt(enemy.realm || 1, 1, 18);
      const chapter = typeof this.getChapterProfileForRealm === 'function' ? this.getChapterProfileForRealm(realm) : null;
      ...
      const statusLabel = status === 'logged' ? '已归档' : status === 'scouted' ? '已遭遇' : '未来敌影';
      const elementLabel = enemy.element && typeof this.getLawElementLabel === 'function' ? this.getLawElementLabel(enemy.element) : enemy.element || '';
      const stealLaw = enemy.stealLaw && typeof LAWS !== 'undefined' && LAWS ? LAWS[enemy.stealLaw] || null : null;
      const resistTags = enemy.resistances ? Object.entries(enemy.resistances).filter(([, value]) => Number(value)).map(([element, value]) => {
        const label = typeof this.getLawElementLabel === 'function' ? this.getLawElementLabel(element) : element;
  ...
  collectionHubMethods.passesEnemyCodexFilter = function (entry) {
    const state = this.getCollectionHubState();
    if (!entry) return false;
    ...
    const haystack = [entry.id, entry.name, entry.realmLabel, entry.chapterName, entry.chapterFullName, entry.roleLabel, entry.firstMoveText, entry.stealLawName, entry.elementLabel, ...(entry.threatTags || []), ...(entry.counterHints || []), ...(entry.patternPreview || []), ...(entry.resistTags || []), ...(entry.ecologyTags || []), entry.ecologyLabel, ...(entry.elitePartnerIds || [])].join(' ').toLowerCase();
    return haystack.includes(state.enemyQuery.toLowerCase());
  };
  collectionHubMethods.selectEnemyCodexEntry = function (enemyId = '') {
  ...
  collectionHubMethods.resolveBossPressureLabel = function (pressure = 1) {
  ...
  collectionHubMethods.formatEnemyPatternSummary = function (pattern = {}) {
    if (!pattern || typeof pattern !== 'object') return '未知招式';
    switch (pattern.type) {
      case 'attack':
        return `攻击 ${clampInt(pattern.value || 0, 0)}`;
      case 'multiAttack':
        return `${clampInt(pattern.count || 1, 1)} 连击 × ${clampInt(pattern.value || 0, 0)}`;
      case 'defend':
        return `获得 ${clampInt(pattern.value || 0, 0)} 护盾`;
      case 'buff':
        return `强化 ${pattern.buffType || '状态'} +${clampInt(pattern.value || 0, 0)}`;
      case 'debuff':
        return `施加 ${pattern.buffType || '负面'} ${clampInt(pattern.value || 0, 0)} 层`;
      default:
        return pattern.intent || pattern.type || '特殊招式';
    }
  };
  collectionHubMethods.getBossBreakHint = function (boss = null, mechanic = null) {
    ...
    switch (mechanicType) {
      case 'summon':
        return '优先准备清场或点杀手段，避免召唤物拖长节奏。';
      case 'rage':
        return '需要尽早压血线，别让力量层数在中后段失控。';
      case 'burn_aura':
        return '净化、护盾与冰系反制更关键，拖回合会被灼烧蚕食。';
      case 'slow':
        return '尽量在偶数回合前交完关键牌，别让麻痹断掉核心节奏。';
      case 'gravity':
        return '提前压低平均费用或保留回能牌，避免高费整回合卡死。';
      case 'reflect':
        return '分段压血并留足护盾，避免一次性高伤被镜返。';
      case 'chaos':
        return '依赖低费与通用牌更稳，高费爆发牌要留作收尾。';
      case 'devour':
        return '更适合快速收束，越拖越容易被放逐与禁疗掐死。';
      default:
        if (boss?.realm >= 13) return '建议提前准备法宝反制与多轴构筑，不要只押单一爆发。';
        return '先确认其章节规则，再决定是抢节奏还是稳血线。';
    }
  };
  collectionHubMethods.getBossArchiveMemoryProfile = function (boss = null) {
    ...
    return profileMap[bossId] || {
      key: 'seal_card',
      name: '封识诏令'
    };
  };
  collectionHubMethods.resolveRunPathBossArchiveGuidance = function (boss = null, mechanic = null, runPathMeta = null) {
    if (!runPathMeta || typeof this.resolveRunPathBossMatchup !== 'function') return null;
    const memory = typeof this.getBossArchiveMemoryProfile === 'function' ? this.getBossArchiveMemoryProfile(boss) : null;
    const chapter = typeof this.getChapterProfileForRealm === 'function' ? this.getChapterProfileForRealm(boss?.realm || 1) : null;
    ...
    return {
      ...
      counterText: [resolved.fitLabel ? `适配评级 ${resolved.fitLabel}` : '', resolved.chapterCue ? `章节场域 ${resolved.chapterCue}` : '', resolved.chapterFocus ? `章节补题 ${resolved.chapterFocus}` : '', resolved.chapterCounter ? `场域拆法 ${resolved.chapterCounter}` : '', resolved.focus, resolved.counter, resolved.reward ? `收益：${resolved.reward}` : ''].filter(Boolean).join(' ｜ ')
    };
  };
  collectionHubMethods.getBossArchiveEntries = function () {
    const enemyCatalog = typeof ENEMIES !== 'undefined' && ENEMIES ? Object.values(ENEMIES) : [];
    const bossCatalog = enemyCatalog.filter(enemy => enemy && enemy.isBoss);
    const realmProgress = this.getCollectionRealmProgress();
    const runPathMeta = this.player && typeof this.player.getRunPathMeta === 'function' ? this.player.getRunPathMeta() : this.selectedRunPathId && typeof this.getRunPathMetaById === 'function' ? this.getRunPathMetaById(this.selectedRunPathId) : null;
    return bossCatalog.map(boss => {
      const mechanic = typeof BOSS_MECHANICS !== 'undefined' && BOSS_MECHANICS ? BOSS_MECHANICS[boss.id] || null : null;
      const chapter = typeof this.getChapterProfileForRealm === 'function' ? this.getChapterProfileForRealm(boss.realm || 1) : null;
      const counterTreasures = Array.isArray(mechanic?.countersBy) ? mechanic.countersBy.map(treasureId => {
      ...
      const memoryRecord = typeof this.getBossMemoryRecord === 'function' ? this.getBossMemoryRecord(boss.id) : {
        attempts: 0,
        clears: 0,
        bestTurn: 0,
        lastResult: '',
      ...
      const memoryStatus = !memoryReady ? 'hidden' : memoryRecord.clears > 0 ? 'logged' : memoryRecord.attempts > 0 ? 'ready' : 'pending';
      const memoryStatusLabel = !memoryReady ? '需先击破' : memoryRecord.clears > 0 ? '已留痕' : memoryRecord.attempts > 0 ? '试作中' : '未演武';
      const memorySummary = !memoryReady ? '需先在主线击破本体，伏魔台才会开放这场记忆战。' : memoryRecord.clears > 0 ? `已完成 ${memoryRecord.clears} 次记忆战，当前最快 ${Math.max(1, memoryRecord.bestTurn || 0)} 回合。` : memoryRecord.attempts > 0 ? `已试作 ${memoryRecord.attempts} 次，仍在摸索稳定破局窗口。` : '尚未发起记忆战，可用当前构筑直接检验破局窗口。';
      const actPreview = Array.isArray(boss.phaseConfig) ? boss.phaseConfig.map((phase, index) => {
        const label = phase?.name || `转幕 ${index + 2}`;
        const threshold = Math.round(Math.max(0, Math.min(1, Number(phase?.threshold) || 0)) * 100);
        return `${label} · ${threshold}%`;
      }).filter(Boolean).slice(0, 2) : [];
      const runPathMatchup = runPathMeta && typeof this.resolveRunPathBossArchiveGuidance === 'function' ? this.resolveRunPathBossArchiveGuidance(boss, mechanic, runPathMeta) : null;
      let sampleBoard = typeof this.buildRunPathBossSampleBoard === 'function' ? this.buildRunPathBossSampleBoard({
        bossId: boss.id,
  ...
  collectionHubMethods.passesBossArchiveFilter = function (entry) {
    const state = this.getCollectionHubState();
    if (!entry) return false;
    ...
    const haystack = [entry.id, entry.name, entry.chapterName, entry.chapterFullName, entry.mechanicText, entry.breakHint, entry.openingStance, entry.counterWindow, entry.finisher, entry.visualCue, entry.bossPrompt, entry.runPathName, entry.runPathFitLabel, entry.runPathCounterText, entry.memoryStatusLabel, entry.memorySummary, ...(entry.sampleBoard?.entries || []).map(sample => `${sample.headline} ${sample.subtitle} ${(sample.tagLine || []).join(' ')}`), ...entry.counterTreasures.map(item => item.name), ...(entry.patternPreview || []), ...(entry.actPreview || [])].join(' ').toLowerCase();
    return haystack.includes(state.bossQuery.toLowerCase());
  };
  collectionHubMethods.selectBossArchiveEntry = function (bossId = '') {
  ...
  collectionHubMethods.getRunSlateShelfEntries = function () {
    this.ensureCollectionHubBootState();
    const archive = Array.isArray(this.runSlateArchive) ? this.runSlateArchive.slice() : [];
    const activeTrainingFocus = typeof this.getObservatoryTrainingFocus === 'function' ? this.getObservatoryTrainingFocus() : null;
    return archive.filter(entry => entry && typeof entry === 'object' && entry.id).map((entry, index) => {
      const answerReviewSource = entry.answerReview && typeof entry.answerReview === 'object' ? entry.answerReview : null;
      ...
      const trainingFocus = typeof this.buildObservatoryTrainingFocusFromSlate === 'function' ? this.buildObservatoryTrainingFocusFromSlate(entry) : null;
      const themeKey = String(trainingFocus?.themeKey || extractTagValue(entry.tags, '课题·') || extractTagValue(entry.tags, '观星·') || '').trim();
      const themeLabel = String(trainingFocus?.themeLabel || extractTagValue(entry.tags, '课题·') || extractTagValue(entry.tags, '观星·') || '').trim();
      ...
      const trainingTags = (() => {
        ...
        return entry.tags.filter(tag => String(tag || '').startsWith('训练·')).map(tag => String(tag || '').replace(/^训练·/, '').trim()).filter(Boolean).slice(0, 4);
      })();
      const summaryTags = [...new Set([...toStringArray(entry.tags, 6), ...toStringArray(answerReview?.tags, 4)])].slice(0, 8);
  ...
  collectionHubMethods.getRunSlateShelfEntryById = function (runId = '') {
    const safeRunId = String(runId || '').trim();
    return this.getRunSlateShelfEntries().find(entry => entry.id === safeRunId) || null;
  };
  collectionHubMethods.passesRunSlateShelfFilter = function (entry) {
    const state = this.getCollectionHubState();
    if (!entry) return false;
  ...
  collectionHubMethods.selectRunSlateShelfEntry = function (runId = '') {
  ...
  collectionHubMethods.applyRunSlateShelfTrainingFocus = function (runId = '', options = {}) {
    const safeRunId = String(runId || '').trim();
    ...
    const nextFocus = this.setObservatoryTrainingFocus(focus, {
      silent: true
    });
  ...
  collectionHubMethods.reviewRunSlateInObservatory = function (runId = '', tab = '') {
    const safeTab = ['daily', 'weekly', 'global'].includes(String(tab || this.challengeHubState?.tab || '')) ? String(tab || this.challengeHubState?.tab || '') : 'daily';
    const focus = this.applyRunSlateShelfTrainingFocus(runId, {
      silent: true
    });
    const maybeChallengeHub = typeof this.showChallengeHub === 'function'
      ? this.showChallengeHub(safeTab)
      : null;
    if (typeof this.showChallengeHub === 'function') {
      if (focus && typeof this.applyObservatoryTrainingFocus === 'function') {
        const applyFocus = () => {
          if (typeof this.applyObservatoryTrainingFocus === 'function') {
            this.applyObservatoryTrainingFocus(safeTab);
          }
        };
        if (maybeChallengeHub && typeof maybeChallengeHub.then === 'function') {
          void maybeChallengeHub.then(() => {
            applyFocus();
          }).catch(() => {
            applyFocus();
          });
    ...
    } else if (focus && typeof this.applyObservatoryTrainingFocus === 'function') {
      this.applyObservatoryTrainingFocus(safeTab);
    }
    return !!focus;
  };
  collectionHubMethods.getBuildSnapshotData = function () {
    const profile = typeof this.buildPlayerDeckProfile === 'function' ? this.buildPlayerDeckProfile() : {
      size: 0,
      counts: {
        attack: 0,
        defense: 0,
    ...
    const spirit = this.player && typeof this.player.getSpiritCompanionMeta === 'function' ? this.player.getSpiritCompanionMeta() : null;
    const runPath = this.player && typeof this.player.getRunPathMeta === 'function' ? this.player.getRunPathMeta() : this.selectedRunPathId && typeof this.getRunPathMetaById === 'function' ? this.getRunPathMetaById(this.selectedRunPathId) : null;
    const runPathRecord = runPath && typeof this.getRunPathRecord === 'function' ? this.getRunPathRecord(runPath.id) : null;
    let runPathSampleBoard = typeof this.buildRunPathBossSampleBoard === 'function' && runPath?.mutation?.mutationId ? this.buildRunPathBossSampleBoard({
      pathId: runPath.id,
    ...
    const runPathSampleRecommendation = runPathSampleBoard && runPathSampleBoard.recommendation && typeof runPathSampleBoard.recommendation === 'object' ? runPathSampleBoard.recommendation : null;
    const currentCharacterMeta = getCharacterMeta(this.player?.characterId || '');
    const vows = this.player && typeof this.player.getRunVowMetas === 'function' ? this.player.getRunVowMetas() : [];
    const loadedLawIds = this.player?.fateRing && typeof this.player.fateRing.getSocketedLaws === 'function' ? this.player.fateRing.getSocketedLaws() : [];
    ...
    const workshopSnapshot = this.player && typeof this.player.getTreasureWorkshopSnapshot === 'function' ? this.player.getTreasureWorkshopSnapshot('equipped') : [];
    const chapter = typeof this.getChapterDisplaySnapshot === 'function' ? this.getChapterDisplaySnapshot(this.player?.realm || 1) : null;
    const currentChapterIndex = clampInt(chapter?.chapterIndex || Math.floor((Math.max(1, clampInt(this.player?.realm || 1, 1, 18)) - 1) / 3) + 1, 1, 6);
    const nextChapter = currentChapterIndex < 6 && typeof this.getChapterDisplaySnapshot === 'function' ? this.getChapterDisplaySnapshot(currentChapterIndex * 3 + 1) : null;
    ...
    const lineage = typeof this.getFateLineageSnapshot === 'function' ? this.getFateLineageSnapshot() : null;
    const aftereffects = typeof this.getFateAftereffectSnapshot === 'function' ? this.getFateAftereffectSnapshot() : null;
    const latestSlate = typeof this.getLatestRunSlate === 'function' ? this.getLatestRunSlate() : null;
    const seasonBoard = typeof this.getSeasonBoardSnapshot === 'function' ? this.getSeasonBoardSnapshot(latestSlate ? {
      latestSlate
    } : {}) : null;
    ...
    const seasonNextTaskLine = !shouldSurfaceSeasonVerification && seasonNextTask ? getSeasonBoardNextTaskLine(seasonNextTask) : '';
    const seasonBoardBuildTargetLine = seasonBoard ? seasonNextTaskLine || seasonBoard.guideLine || seasonBoard.statusLine || seasonBoard.summaryLine || '继续补齐本周主轴。' : '';
    ...
    if (seasonBoard?.summaryLine) {
      ...
      if (seasonBoard.chapterArc?.available) {
        strengths.unshift(`三周一章当前推进到【${seasonBoard.chapterArc.arcLabel || seasonBoard.chapterArc.chapterLabel || '当前章程'}】第 ${seasonBoard.chapterArc.weekSlot || 1}/${seasonBoard.chapterArc.targetWeeks || 3} 周。`);
        if (seasonBoard.chapterArc.objective?.available) {
          strengths.unshift(`章目标：${seasonBoard.chapterArc.objective.shortLine || seasonBoard.chapterArc.objective.summaryLine || '当前章目标待同步。'}`);
        }
      }
    ...
    const nextChapterRiskTags = Array.from(new Set([nextChapter?.dangerProfile?.dominantLabel ? `下一章高危：${nextChapter.dangerProfile.dominantLabel}` : '', nextChapter?.dangerProfile?.tierLabel ? `${nextChapter.dangerProfile.tierLabel}压力` : '', ...(Array.isArray(nextChapter?.focusTags) ? nextChapter.focusTags.slice(0, 2).map(tag => `高危·${tag}`) : [])].filter(Boolean))).slice(0, 4);
    const gaps = [];
    ...
    if (seasonBoard) {
      if (seasonBoard.chapterArc?.available) {
        nextTargets.unshift(`章程跟进：${seasonBoard.chapterArc.objective?.summaryLine || seasonBoard.chapterArc.objective?.goalLine || seasonBoard.chapterArc.feedbackLine || (seasonBoard.chapterArc.rescueWindow?.open ? seasonBoard.chapterArc.rescueWindow.guideLine || seasonBoard.chapterArc.rescueWindow.reasonLine || seasonBoard.chapterArc.statusLine || '继续跟随当前主线救火。' : seasonBoard.chapterArc.review?.summaryLine || seasonBoard.chapterArc.goalLine || seasonBoard.chapterArc.summaryLine || '继续沿当前章程推进。')}`);
      }
    ...
    const buildPriorityQueue = priorityQueue.slice(0, 3).map((entry, index) => ({
      rank: index + 1,
      label: entry.label,
      detail: entry.detail
    }));
    return {
      ...
      highlights: [{
        label: '最高连击',
        value: clampInt(stats.maxCombo || 0, 0),
        note: '来自全局成就统计'
      }, {
        label: '单次最高伤害',
        value: clampInt(stats.singleDamage || 0, 0),
        note: '当前账号历史高光'
      }, {
        label: '已击破 Boss',
        value: clampInt(stats.bossesDefeated || 0, 0),
        note: '当前账号累计'
      }, {
        label: '遭遇连胜',
        value: clampInt(encounter.maxStreak || 0, 0),
        note: '遭遇主题最佳连胜段'
      }, {
        label: '无尽已清轮次',
        value: clampInt(endless.clearedCycles || 0, 0),
        note: '无尽轮回进度'
      }, {
        label: '最高通关重数',
        value: clampInt(stats.realmCleared || 0, 0),
        note: '章节推进里程碑'
      }]
    };
  };
  ...
  collectionHubMethods.getSanctumOverviewData = function () {
    ...
    const latestRunPathRecord = typeof this.getLatestRunPathRecord === 'function' ? this.getLatestRunPathRecord() : null;
    const latestSample = typeof this.getRunPathBossSamples === 'function' ? this.getRunPathBossSamples({
      limit: 1,
      sortBy: 'recent'
    })[0] || null : null;
    const forgeOverview = this.player && typeof this.player.getTreasureWorkshopResearchOverview === 'function' ? this.player.getTreasureWorkshopResearchOverview() || {
      coreOwned: 0,
      coreTotal: 0,
      formOwned: 0,
      formTotal: 0,
    ...
    const unclaimedAchievementIds = Array.isArray(this.achievementSystem?.unlockedAchievements) ? this.achievementSystem.unlockedAchievements.filter(id => !this.achievementSystem.claimedAchievements.includes(id)) : [];
    const lineage = typeof this.getFateLineageSnapshot === 'function' ? this.getFateLineageSnapshot() : null;
    const aftereffects = typeof this.getFateAftereffectSnapshot === 'function' ? this.getFateAftereffectSnapshot() : null;
    const latestSlate = typeof this.getLatestRunSlate === 'function' ? this.getLatestRunSlate() : null;
    const seasonBoard = buildSnapshot?.seasonBoard && typeof buildSnapshot.seasonBoard === 'object' ? buildSnapshot.seasonBoard : typeof this.getSeasonBoardSnapshot === 'function' ? this.getSeasonBoardSnapshot(latestSlate ? {
      latestSlate
    } : {}) : null;
    const seasonHonorShowcase = (() => {
      try {
        if (typeof PVPService === 'undefined' || !PVPService || typeof PVPService.getSeasonHonorShowcase !== 'function') return null;
        return PVPService.getSeasonHonorShowcase();
      } catch (error) {
        return null;
      }
    })();
    ...
    const seasonNextWeekGoal = seasonBoard?.nextWeekGoal && typeof seasonBoard.nextWeekGoal === 'object' ? seasonBoard.nextWeekGoal : null;
    const seasonFrontierResolution = seasonBoard?.frontier?.resolution && typeof seasonBoard.frontier.resolution === 'object' ? seasonBoard.frontier.resolution : null;
    ...
    const seasonFrontierResolutionSuggestedChoiceId = seasonFrontierResolutionChoiceLabels[seasonFrontierResolution?.suggestedChoiceId] ? seasonFrontierResolution.suggestedChoiceId : 'hold_primary';
    const seasonNextTaskLine = !shouldSurfaceSeasonVerification && seasonNextTask ? getSeasonBoardNextTaskLine(seasonNextTask) : '';
    const seasonNextTaskNoteLine = seasonNextTaskLine ? [seasonNextTask.progressText || '', seasonNextTask.statusLine || '', seasonNextTask.anchorSection ? `去向：${SECTION_META[seasonNextTask.anchorSection]?.title || seasonNextTask.anchorSection}` : ''].filter(Boolean).join(' · ') : '';
    const heavenlyMandate = (() => {
      ...
      const mandateSnapshot = typeof this.getHeavenlyMandateExpeditionSnapshot === 'function' ? this.getHeavenlyMandateExpeditionSnapshot() : null;
      if (mandateSnapshot && typeof mandateSnapshot === 'object') {
        const lanes = Array.isArray(mandateSnapshot.lanes) ? mandateSnapshot.lanes.filter(lane => lane && typeof lane === 'object').map(lane => {
          const tasks = Array.isArray(lane.tasks) ? lane.tasks.filter(task => task && typeof task === 'object').map((task, index) => {
        ...
        const goalProgressText = nextTask ? nextTask.progressText || (nextTask.completed ? '已完成' : '待推进') : totalTaskCount > 0 ? `${completedTaskCount}/${totalTaskCount} 项敕令已成` : '等待本周题面落定';
        const detailLine = nextTask ? `当前优先补「${nextTask.label}」${nextTask.progressText ? ` · ${nextTask.progressText}` : ''}${nextTask.hintLine ? ` · ${nextTask.hintLine}` : ''}` : mandateSnapshot.summaryLine || fallback.detailLine;
        ...
        const guideLine = nextTask ? `先沿${focusTargetLabel ? `【${focusTargetLabel}】` : '当前主线'}补完「${nextTask.label}」，再回洞府复核其余两条玩法线。` : `本周敕令已全部成卷，可转去更高压模式验证这套主练是否稳定。`;
        return {
          available: true,
          source: 'mandate',
      ...
      const seasonProfile = typeof this.getEndlessSeasonProfile === 'function' ? this.getEndlessSeasonProfile() : null;
      if (!seasonProfile || typeof seasonProfile !== 'object') return fallback;
      const goals = Array.isArray(seasonProfile.goals) ? seasonProfile.goals.filter(item => item && typeof item === 'object') : [];
      ...
      const weekLabel = weekNo > 0 && year > 0 ? `${year} · 第 ${weekNo} 周` : weekTag || fallback.weekLabel;
      ...
      const goalTitle = activeGoal ? `${activeGoal.tierLabel || '本周刻印'} · ${activeGoal.title || '留痕校卷'}` : `${goalTierLabel} · 本周留痕`;
    ...
    const heavenlyMandateFocusTask = heavenlyMandate?.focusTask && typeof heavenlyMandate.focusTask === 'object' ? heavenlyMandate.focusTask : heavenlyMandate?.nextTask && typeof heavenlyMandate.nextTask === 'object' ? heavenlyMandate.nextTask : null;
    const achievements = unclaimedAchievementIds.slice(0, 3).map(achievementId => {
      const achievement = typeof ACHIEVEMENTS !== 'undefined' && ACHIEVEMENTS ? ACHIEVEMENTS[achievementId] : null;
      if (!achievement) return null;
      const rewardText = typeof getAchievementRewardText === 'function' ? getAchievementRewardText({
        ...achievement,
        unlocked: true,
        claimed: false
      }) : '领取奖励';
    ...
    const rooms = [{
      ...
      note: latestSample ? `最近对照：${latestSample.characterName || latestSample.pathName} 用【${latestSample.pathName}】收下 ${latestSample.bossName}，${latestSample.turns > 0 ? `${latestSample.turns} 回合` : '已留样'}。` : latestRunPathRecord ? `最近战录：${latestRunPathRecord.recordName || latestRunPathRecord.name} · ${latestRunPathRecord.lastCharacterName || latestRunPathRecord.name} 完成于第 ${latestRunPathRecord.lastRealm || latestRunPathRecord.bestRealm || 1} 重天。` : '首条命途圆满后，会自动把命途定位、推荐套装与 Boss 读法收入洞府。',
      actionLabel: '查看构筑快照',
      actionType: 'collection',
      actionValue: 'builds'
    }, {
      ...
      note: forgeOverview.activeWorkshops > 0 ? `已激活 ${forgeOverview.activeWorkshops} 条炼器铭刻，当前 ${forgeOverview.fullSets} 组套装已进入三段共鸣。` : `当前已整理出 ${forgeOverview.coreOwned}/${forgeOverview.coreTotal} 件核心法宝与 ${forgeOverview.formOwned}/${forgeOverview.formTotal} 件形态件，可继续追套装与器灵目标。`,
      actionLabel: '进入炼器研究',
      actionType: 'treasure',
      ...
      note: progress.runPathBossSampleCount > 0 ? `已归档 ${progress.runPathBossSampleCount} 份角色 × 命途 × Boss 对照样本，伏魔台可继续压更快轮次。` : progress.clearedBossMemories > 0 ? `已留下 ${progress.clearedBossMemories} 份主宰记忆战记录，可继续压最快轮次。` : '普通敌影与主宰机制都已开始归档，现已开放首版 Boss 记忆战。',
      actionLabel: '查看敌影档案',
      actionType: 'collection',
      actionValue: 'enemies'
    }];
    ...
    if (agendaDashboard.active) {
      agendaResearches.push({
        ...
        noteLine: [agendaDashboard.active.phaseLabel ? `当前阶段：${agendaDashboard.active.phaseLabel}` : '', agendaDashboard.active.selectedDecisionLabel ? `已选处置：${agendaDashboard.active.selectedDecisionLabel}` : '', agendaDashboard.active.selectedContractLabel ? `已立契约：${agendaDashboard.active.selectedContractLabel}` : '', agendaDashboard.active.focusNodeLine || agendaDashboard.active.sourceLine || ''].filter(Boolean).join(' · '),
        section: 'slates',
        actionType: 'collection',
        actionValue: 'slates',
        buttonLabel: '查看归卷书架',
      ...
      if (agendaDashboard.active.contractState === 'pending' && Array.isArray(agendaDashboard.active.contractOptions)) {
        agendaDashboard.active.contractOptions.forEach(contract => {
          agendaResearches.push({
            ...
            noteLine: [contract.statusLine || agendaDashboard.active.contractPromptLine || '', contract.signCostLine ? `契押 ${contract.signCostLine}` : '', contract.burdenLine || ''].filter(Boolean).join(' · '),
            section: 'sanctum',
    ...
    agendaDashboard.candidates.forEach(candidate => {
      agendaResearches.push({
        ...
        noteLine: candidate.active ? candidate.focusNodeLine || candidate.sourceLine || '' : candidate.sourceLine || candidate.focusNodeLine || '',
        section: 'slates',
        ...
        progressText: candidate.active ? `${candidate.progress || 0}/${candidate.target || 1}` : candidate.statusLine || candidate.costLine || '待立项',
    ...
    if (lineage?.available) {
      const readyTracks = [lineage.characterTrack, lineage.styleTrack, lineage.nodeTrack, lineage.researchTrack].filter(track => track && Array.isArray(track.entries) && track.entries.length > 0).length;
    ...
    if (seasonBoard) {
      ...
      const seasonVerificationArchive = seasonBoard.verificationArchive && typeof seasonBoard.verificationArchive === 'object' ? seasonBoard.verificationArchive : null;
      const latestSeasonVerificationArchive = seasonVerificationArchive?.latestEntry && typeof seasonVerificationArchive.latestEntry === 'object' ? seasonVerificationArchive.latestEntry : Array.isArray(seasonVerificationArchive?.entries) ? seasonVerificationArchive.entries[0] : null;
      ...
      if (seasonSettlement) {
        seasonBoardResearches.push({
          ...
          noteLine: [seasonSettlement.detailLine || '', seasonSettlement.contractResolutionLine || seasonSettlement.statusLine || seasonSettlement.guideLine || ''].filter(Boolean).join(' · '),
          section: 'sanctum',
          actionType: 'collection',
          actionValue: 'sanctum',
          buttonLabel: '查看裁定',
      ...
      if (seasonDebtPack) {
        seasonBoardResearches.push({
          ...
          noteLine: [seasonDebtPack.detailLine || '', seasonDebtPack.guideLine || seasonDebtPack.progressText || seasonDebtPack.settleWindowText || ''].filter(Boolean).join(' · '),
      ...
      if (primarySeasonVerification) {
        seasonBoardResearches.push({
          ...
          noteLine: [primarySeasonVerification.hintLine || '', primarySeasonVerification.statusLine || '', secondarySeasonVerification?.summaryLine ? `次张：${secondarySeasonVerification.summaryLine}` : ''].filter(Boolean).join(' · '),
      ...
      if (secondarySeasonVerification) {
        seasonBoardResearches.push({
          ...
          noteLine: [secondarySeasonVerification.hintLine || '', secondarySeasonVerification.statusLine || ''].filter(Boolean).join(' · '),
      ...
      if (seasonVerificationArchive?.available) {
        seasonBoardResearches.push({
          ...
          noteLine: [latestSeasonVerificationArchive?.noteLine || '', seasonVerificationArchive.detailLine || ''].filter(Boolean).slice(0, 2).join(' · '),
          section: 'sanctum',
          actionType: 'challenge',
          actionValue: 'weekly',
          buttonLabel: '查看全部周判',
    ...
    const fallbackGoals = combinedResearches.filter(research => research.ready).slice(0, 2).map(research => ({
      id: research.id,
      title: research.name,
    ...
    const heavenlyMandateGoal = {
      ...
      followTaskAnchorSection: heavenlyMandateFocusTask?.anchorSection || '',
      followTaskActionType: heavenlyMandateFocusTask?.actionType || heavenlyMandate.actionType || '',
      followTaskActionValue: heavenlyMandateFocusTask?.actionValue || heavenlyMandate.actionValue || '',
    ...
    const seasonBoardLaneRewardGoals = seasonBoard && Array.isArray(seasonBoard.laneRewards) ? seasonBoard.laneRewards.filter(reward => reward && reward.claimable).map(reward => ({
    ...
    return {
      ...
      lineage,
      aftereffects,
      goals: [heavenlyMandateGoal, ...(seasonBoardGoal ? [seasonBoardGoal] : []), ...seasonBoardLaneRewardGoals, ...(seasonBoardSettlementGoal ? [seasonBoardSettlementGoal] : []), ...(seasonBoardDebtGoal ? [seasonBoardDebtGoal] : []), ...(seasonBoardNextTaskGoal ? [seasonBoardNextTaskGoal] : []), ...(seasonBoardVerificationGoal ? [seasonBoardVerificationGoal] : []), ...(seasonBoardSideVerificationGoal ? [seasonBoardSideVerificationGoal] : []), ...(lineageGoal ? [lineageGoal] : []), ...(aftereffectGoal ? [aftereffectGoal] : []), ...goalPool],
      recentUnlocks: this.getCollectionUnlockHistory(6)
    };
  };
  collectionHubMethods.renderCollectionHubChrome = function () {
    if (typeof document === 'undefined') return;
    const state = this.getCollectionHubState();
  ...
  collectionHubMethods.renderSpiritCodex = function () {
    ...
    grid.innerHTML = filtered.length > 0 ? filtered.map(entry => `
                <button type="button"
                    ...
                    data-collection-action="select-codex-entry"
                    data-collection-target="spirit"
            ...
            `).join('') : '<div class="codex-empty-state">当前检索条件下没有匹配的灵契记录，试试放宽检索或切换关注对象。</div>';
    if (!selected) {
    ...
    detail.innerHTML = `
            <div class="collection-detail-shell">
                ...
                <section class="collection-detail-card">
                    ...
                    <div class="collection-card-tags">
                        ${(selected.chapterFits || []).length > 0 ? selected.chapterFits.map(name => `<span class="collection-tag">${escapeHtml(name)}</span>`).join('') : '<span class="collection-tag muted">等待更多章节数据验证</span>'}
                    </div>
                </section>
            </div>
        `;
    ...
    summary.innerHTML = ['<span class="codex-side-kicker">灵契总览</span>', '<h3>灵契收录状态</h3>', '<div class="codex-summary-grid two-cols">', `<div class="codex-summary-chip"><strong>${entries.length}</strong><span>总条目</span></div>`, `<div class="codex-summary-chip"><strong>${alignedCount}</strong><span>当前角色更易共鸣</span></div>`, `<div class="codex-summary-chip"><strong>${entries.filter(entry => entry.isCurrent).length}</strong><span>当前同行</span></div>`, `<div class="codex-summary-chip"><strong>${hiddenCount}</strong><span>未补全线索</span></div>`, '</div>', `<p class="codex-side-note">当前可见 ${filtered.length} 条灵契记录，优先补齐与当前角色共鸣的灵契会更容易形成章节闭环。</p>`].join('');
    const currentSpirit = entries.find(entry => entry.isCurrent) || null;
    research.innerHTML = ['<span class="codex-side-kicker">研究方向</span>', '<h3>灵契追索建议</h3>', '<ul class="codex-side-list compact">', `<li>${escapeHtml(currentSpirit ? `当前灵契【${currentSpirit.name}】已挂接，可优先围绕 ${currentSpirit.roleTags.join(' / ') || '护道节奏'} 补牌。` : '当前还没有挂接灵契，建议先从开局灵契草案或灵契窟补一个稳定护道。')}</li>`, `<li>${escapeHtml(selected.selectedCharacterName)} 更容易与 ${(selected.affinityNames || []).join('、') || '特定灵契'} 建立共鸣。</li>`, `<li>章节推荐灵契会同步出现在章节档案中，适合和天象 / 地脉一起看。</li>`, '</ul>'].join('');
  };
  collectionHubMethods.renderChapterCodex = function () {
    if (typeof document === 'undefined') return;
    ...
    grid.innerHTML = filtered.length > 0 ? filtered.map(entry => `
                <button type="button"
                    ...
                    data-collection-action="select-codex-entry"
                    data-collection-target="chapter"
                    data-entry-id="${escapeHtml(entry.id)}">
                    <div class="collection-card-top">
    ...
    detail.innerHTML = `
            <div class="collection-detail-shell">
                ...
                <section class="collection-detail-card">
                    ...
                    <div class="collection-mini-grid">
                        ${Array.isArray(selected.narrativeProfile?.beats) && selected.narrativeProfile.beats.length > 0 ? selected.narrativeProfile.beats.map(beat => `
                            <div class="collection-mini-card">
                                <strong>${escapeHtml(beat.stage || '章节片段')}</strong>
                                <span>${escapeHtml(beat.title || '命线未定')}</span>
                                <p>${escapeHtml(beat.summary || '')}</p>
                            </div>
                        `).join('') : '<div class="collection-mini-card"><strong>归档中</strong><span>当前未展示额外叙事节点</span><p>本章资料正在整理为标准档案。</p></div>'}
                    </div>
                </section>
        ...
        `;
    summary.innerHTML = ['<span class="codex-side-kicker">章节总览</span>', '<h3>世界规则进度</h3>', '<div class="codex-summary-grid two-cols">', `<div class="codex-summary-chip"><strong>${entries.length}</strong><span>章节档案</span></div>`, `<div class="codex-summary-chip"><strong>${entries.filter(entry => entry.isCleared).length}</strong><span>已贯通章节</span></div>`, `<div class="codex-summary-chip"><strong>${entries.filter(entry => entry.isCurrent).length}</strong><span>当前章节</span></div>`, `<div class="codex-summary-chip"><strong>${entries.reduce((sum, entry) => sum + entry.bosses.length, 0)}</strong><span>已编入主宰</span></div>`, '</div>', `<p class="codex-side-note">章节档案会把“天象 / 地脉 / 生态 / Boss 传闻”压在同一页，方便在开局前就判断路线。</p>`].join('');
    hints.innerHTML = ['<span class="codex-side-kicker">复盘提示</span>', '<h3>怎么读章节</h3>', '<ul class="codex-side-list compact">', '<li>先看天象和地脉，确定这章要放大的是什么玩法。</li>', '<li>再看生态模板，判断是输出压力、控制压力还是资源压力更高。</li>', '<li>最后再用 Boss 传闻回头修正路线，不要把精英和营地价值看成固定不变。</li>', '</ul>'].join('');
  };
  collectionHubMethods.renderEnemyCodex = function () {
    if (typeof document === 'undefined') return;
    ...
    grid.innerHTML = filtered.length > 0 ? filtered.map(entry => `
            ...
            `).join('') : '<div class="codex-empty-state">当前检索条件下没有匹配的敌影档案。</div>';
    if (!selected) {
            ...
            <div class="collection-detail-shell">
                ...
                <div class="collection-detail-grid">
                    ...
                    <section class="collection-detail-card">
                        ...
                        <div class="collection-card-tags">
                            ${(selected.threatTags || []).length > 0 ? selected.threatTags.map(tag => `<span class="collection-tag">${escapeHtml(tag)}</span>`).join('') : '<span class="collection-tag muted">威胁标签未记录</span>'}
                        </div>
                    </section>
                    <section class="collection-detail-card">
                    ...
                    <section class="collection-detail-card">
                        ...
                        <div class="collection-card-tags">
                            ${(selected.resistTags || []).length > 0 ? selected.resistTags.map(tag => `<span class="collection-tag">${escapeHtml(tag)}</span>`).join('') : '<span class="collection-tag muted">当前未记录显著抗性</span>'}
                        </div>
                    </section>
                </div>
                <section class="collection-detail-card">
                ...
                <section class="collection-detail-card">
                    ...
                    <div class="collection-card-tags">
                        ${(selected.ecologyTags || []).length > 0 ? selected.ecologyTags.map(tag => `<span class="collection-tag">${escapeHtml(tag)}</span>`).join('') : '<span class="collection-tag muted">该敌影的章节焦点仍在整理中</span>'}
                    </div>
                </section>
            </div>
        `;
    summary.innerHTML = ['<span class="codex-side-kicker">敌影总览</span>', '<h3>敌影档案进度</h3>', '<div class="codex-summary-grid two-cols">', `<div class="codex-summary-chip"><strong>${entries.length}</strong><span>敌影总数</span></div>`, `<div class="codex-summary-chip"><strong>${entries.filter(entry => entry.isScouted).length}</strong><span>已遭遇</span></div>`, `<div class="codex-summary-chip"><strong>${entries.filter(entry => entry.isUpcoming).length}</strong><span>未来敌影</span></div>`, `<div class="codex-summary-chip"><strong>${entries.filter(entry => entry.aiProfile === 'control').length}</strong><span>控场型</span></div>`, '</div>', '<p class="codex-side-note">敌影档案会把常驻敌人的压制手段、首拍动作和章节归属压在同一页，方便提前做路线与解法准备。</p>'].join('');
    hints.innerHTML = ['<span class="codex-side-kicker">复盘提示</span>', '<h3>怎么读敌影</h3>', '<ul class="codex-side-list compact">', '<li>先看战术画像和首拍速记，判断这只敌影是在抢血线、压状态还是拖回合。</li>', '<li>再看元素与抗性，确认当前主输出轴会不会被天然克制。</li>', '<li>最后回到章节档案，判断它是单卡问题还是整章生态都在逼你补同一种应对。</li>', '</ul>'].join('');
  };
  collectionHubMethods.renderBossArchive = function () {
    if (typeof document === 'undefined') return;
    ...
    grid.innerHTML = filtered.length > 0 ? filtered.map(entry => `
            ...
            `).join('') : '<div class="codex-empty-state">当前检索条件下没有匹配的 Boss 档案。</div>';
    if (!selected) {
    ...
    detail.innerHTML = `
            <div class="collection-detail-shell">
                ...
                <div class="collection-detail-grid">
                    ...
                    <section class="collection-detail-card">
                        ...
                        <p>${escapeHtml(selected.runPathMatchup ? `${selected.runPathName} · ${selected.runPathCounterText}` : '当前未挂命途，挂接命途后这里会给出适配评级与拆招建议。')}</p>
                        ${selected.runPathMatchup ? `<div class="collection-card-tags"><span class="collection-tag">${escapeHtml(selected.runPathName || '当前命途')}</span><span class="collection-tag">${escapeHtml(selected.runPathFitLabel || '常规拆解')}</span></div>` : ''}
                    </section>
                    <section class="collection-detail-card">
                    ...
                    <section class="collection-detail-card">
                        ...
                        ${(selected.actPreview || []).length > 0 ? `<div class="collection-card-tags">${selected.actPreview.map(item => `<span class="collection-tag">${escapeHtml(item)}</span>`).join('')}</div>` : ''}
                    </section>
                    <section class="collection-detail-card collection-memory-panel">
                        ...
                        <div class="collection-detail-actions">
                            <button type="button"
                                ...
                                data-collection-action="start-boss-memory-battle"
                                data-boss-id="${escapeHtml(selected.id)}"
                                ${selected.memoryReady ? '' : 'disabled'}
                                >${selected.memoryReady ? '发起记忆战' : '需先击破本体'}</button>
                            <button type="button"
                                class="collection-inline-btn secondary"
                                data-collection-action="switch-section"
                                data-collection-section="chapters">对照章节档案</button>
                        </div>
                    </section>
                    <section class="collection-detail-card">
                        <span class="detail-mini-label">建议反制法宝</span>
                        <div class="collection-card-tags">
                            ${selected.counterTreasures.length > 0 ? selected.counterTreasures.map(item => `<span class="collection-tag">${escapeHtml(item.icon)} ${escapeHtml(item.name)}</span>`).join('') : '<span class="collection-tag muted">暂无明确法宝反制</span>'}
                        </div>
                    </section>
                    <section class="collection-detail-card">
        ...
        `;
    summary.innerHTML = ['<span class="codex-side-kicker">主宰总览</span>', '<h3>Boss 档案进度</h3>', '<div class="codex-summary-grid two-cols">', `<div class="codex-summary-chip"><strong>${entries.length}</strong><span>主宰总数</span></div>`, `<div class="codex-summary-chip"><strong>${entries.filter(entry => entry.status === 'defeated').length}</strong><span>已击破</span></div>`, `<div class="codex-summary-chip"><strong>${entries.filter(entry => entry.memoryRecord.clears > 0).length}</strong><span>记忆战留痕</span></div>`, `<div class="codex-summary-chip"><strong>${entries.reduce((sum, entry) => sum + clampInt(entry.memoryRecord.attempts || 0, 0), 0)}</strong><span>总试作次数</span></div>`, `<div class="codex-summary-chip"><strong>${entries.filter(entry => entry.pressureScore >= 1.45).length}</strong><span>高压主宰</span></div>`, `<div class="codex-summary-chip"><strong>${progress.runPathBossSampleCount || 0}</strong><span>样本对照</span></div>`, '</div>', `<p class="codex-side-note">Boss 档案现在会把章节背景、核心机制、反制法宝与伏魔台记忆战记录压在同一页${entries.some(entry => entry.runPathMatchup) ? '，并按当前命途补充适配评级与拆招建议' : ''}${(progress.runPathBossSampleCount || 0) > 0 ? '，同时把已归档的角色 × 命途 × Boss 样本放进对照榜' : ''}，方便边读边练。</p>`].join('');
    hints.innerHTML = ['<span class="codex-side-kicker">复盘提示</span>', '<h3>怎么读主宰</h3>', '<ul class="codex-side-list compact">', '<li>先看主机制决定这场是抢节奏、稳资源，还是必须带净化 / 反制法宝。</li>', '<li>再看招式速记，提前规划哪一拍要留防、哪一拍能转攻。</li>', '<li>最后用伏魔台记忆战把理论过一遍，最快轮次会比单次胜负更能说明你是否真的读懂了这位主宰。</li>', '</ul>'].join('');
  };
  collectionHubMethods.renderBuildSnapshot = function () {
    if (typeof document === 'undefined') return;
    ...
    const lineage = snapshot.lineage && typeof snapshot.lineage === 'object' ? snapshot.lineage : null;
    ...
    hero.innerHTML = `
            <div class="collection-detail-shell">
                <section class="collection-detail-hero">
                    ...
                    ` : ''}
                    ${snapshot.sampleMismatchWarning ? `<p class="collection-alert danger">${escapeHtml(snapshot.sampleMismatchWarning.title)}：${escapeHtml(snapshot.sampleMismatchWarning.text)}</p>` : ''}
                </section>
            </div>
        `;
    metrics.innerHTML = `
            ...
            <div class="collection-mini-grid">
                ...
                <div class="collection-mini-card">
                    ...
                    <span>命途碑廊</span>
                    <p>${escapeHtml(snapshot.runPathRecord?.clears > 0 ? `已收录 ${snapshot.runPathRecord.clears} 份战录，最近完成于${this.formatCollectionTimestamp(snapshot.runPathRecord.lastCompletedAt)}。` : '当前命途尚未圆满，完成后会把战录自动收入洞府。')}</p>
                </div>
            </div>
        `;
    notes.innerHTML = `
            <div class="collection-detail-grid">
                ...
                <section class="collection-detail-card">
                    ...
                    <ul class="collection-detail-list">
                        ${snapshot.priorityQueue.length > 0 ? snapshot.priorityQueue.map(entry => `<li>${entry.rank}. ${escapeHtml(entry.label)}：${escapeHtml(entry.detail)}</li>`).join('') : snapshot.nextTargets.map(line => `<li>${escapeHtml(line)}</li>`).join('')}
                    </ul>
                </section>
                <section class="collection-detail-card">
                    <span class="detail-mini-label">当前装配</span>
                    <div class="collection-card-tags">
                        ${snapshot.loadedLaws.length > 0 ? snapshot.loadedLaws.map(law => `<span class="collection-tag">${escapeHtml(law.icon || '📜')} ${escapeHtml(law.name || law.id)}</span>`).join('') : '<span class="collection-tag muted">命环暂未装满</span>'}
                        ${snapshot.equippedTreasures.length > 0 ? snapshot.equippedTreasures.slice(0, 3).map(treasure => `<span class="collection-tag">${escapeHtml(treasure.icon || '🏺')} ${escapeHtml(treasure.name || treasure.id)}</span>`).join('') : '<span class="collection-tag muted">法宝位未装配</span>'}
                    </div>
                </section>
                <section class="collection-detail-card">
    ...
    highlights.innerHTML = `
        ...
        `;
    summary.innerHTML = ['<span class="codex-side-kicker">构筑概览</span>', '<h3>这一套在打什么</h3>', '<ul class="codex-side-list compact">', `<li>${escapeHtml(snapshot.archetypeLabel)}</li>`, `<li>主类型：${escapeHtml(profile.dominantType)} · 当前章节：${escapeHtml(snapshot.chapter?.name || '未定')}</li>`, `<li>当前誓约：${escapeHtml(snapshot.vows.map(meta => meta.name).join('、') || '暂无')}</li>`, `<li>当前命途：${escapeHtml(snapshot.runPath?.name || '未挂')} · 已完成命途 ${escapeHtml(snapshot.completedRunPaths || 0)} / ${escapeHtml(snapshot.totalRunPaths || 0)}</li>`, `<li>样本对照：${escapeHtml(snapshot.runPathSampleBoard?.count || 0)} 份 · ${snapshot.runPathSampleBoard?.bestTurn > 0 ? `当前最快 ${escapeHtml(snapshot.runPathSampleBoard.bestTurn)} 回合` : '尚未形成最快轮次'}</li>`, `<li>当前精选命盘：${escapeHtml(selectedGuide ? `${selectedGuide.title} · ${selectedGuide.themeLabel || '观星样本'}` : '未锁定，可先去观星台精选一份命盘答卷')}</li>`, lineage?.available ? `<li data-fate-lineage-summary="true">命盘谱系：${escapeHtml(lineage.summaryLine || '长期主修待沉淀')}</li>` : '', `<li>下一章风险：${escapeHtml(snapshot.nextChapterRiskTags.join(' / ') || '终章前暂无额外镜像')}</li>`, snapshot.sampleMismatchWarning ? `<li>误配告警：${escapeHtml(snapshot.sampleMismatchWarning.text)}</li>` : '', '</ul>'].join('');
    guide.innerHTML = ['<span class="codex-side-kicker">阅读顺序</span>', '<h3>怎么用这页复盘</h3>', '<ul class="codex-side-list compact">', '<li>先看“下一章风险镜像”，确认下一章真正高危的是爆发、续航、控场还是资源税。</li>', '<li>再看“补件优先级队列”，按 1/2/3 的顺序补，不要平均摊资源。</li>', '<li>样本对照会告诉你这条命途到底是谁在打、打哪位 Boss 最稳、最快能压到多少回合。</li>', lineage?.available ? `<li data-fate-lineage-guide="build">${escapeHtml(lineage.guideLine || lineage.detailLine || '命盘谱系会把角色、流派、节点与研究压成一份长期身份画像。')}</li>` : '', selectedGuide ? `<li>当前精选命盘：${escapeHtml(selectedGuide.title)} · ${escapeHtml(selectedGuide.themeLabel || '观星样本')}，适合先按这份开局答卷补前两手资源与路线。</li>` : '<li>若还没有锁定精选命盘，先去观星台选一份高分答卷，再回这里对照补件顺序。</li>', '<li>若出现“误配告警”，优先纠偏场域，而不是继续硬抬当前章的随机收益。</li>', '</ul>'].join('');
  };
  collectionHubMethods.renderRunSlateShelf = function () {
    if (typeof document === 'undefined') return;
    ...
    grid.innerHTML = filtered.length > 0 ? filtered.map(entry => `
            ...
            `).join('') : '<div class="codex-empty-state">当前筛面下还没有匹配的归卷答卷，试着切回全部主题或先完成一章远征。</div>';
    summary.innerHTML = ['<span class="codex-side-kicker">书架总览</span>', '<h3>这轮都留下了什么</h3>', '<div class="codex-summary-grid two-cols">', `<div class="codex-summary-chip"><strong>${entries.length}</strong><span>已归卷答卷</span></div>`, `<div class="codex-summary-chip"><strong>${filtered.length}</strong><span>当前筛中</span></div>`, `<div class="codex-summary-chip"><strong>${entries.reduce((max, entry) => Math.max(max, entry.score), 0)}</strong><span>最高评分</span></div>`, `<div class="codex-summary-chip"><strong>${entries.filter(entry => entry.trainingReady).length}</strong><span>可回流主练</span></div>`, '</div>', '<ul class="codex-side-list compact">', activeTrainingFocus?.trainingAdvice ? `<li>当前主练：${escapeHtml(activeTrainingFocus.chapterName || '最近归卷')} · ${escapeHtml(activeTrainingFocus.trainingAdvice)}</li>` : '<li>当前还没有主练回流，可从任意一份归卷答卷里指定新的训练参考。</li>', selected ? `<li>选中答卷：${escapeHtml(selected.chapterName)} · ${escapeHtml(selected.ratingLabel)} · 记录于 ${escapeHtml(selected.timestampLabel)}</li>` : '<li>先完成一章远征，书架就会自动收进第一份章节答卷。</li>', selected?.bountyLine ? `<li>悬赏收官：${escapeHtml(selected.bountyLine)}</li>` : '', selected?.factionLine ? `<li>势力走向：${escapeHtml(selected.factionLine)}</li>` : '', selected?.nemesisName ? `<li>追猎结果：${escapeHtml(`${selected.nemesisName} · ${selected.nemesisStatusLabel || '待定'}`)}</li>` : '', '</ul>'].join('');
    guide.innerHTML = ['<span class="codex-side-kicker">使用建议</span>', '<h3>怎么把书架喂回主线</h3>', '<ul class="codex-side-list compact">', '<li>先按主题筛一遍，看自己最近是在补哪类题，不要把不同课题的答卷混着比。</li>', '<li>再按章节和评级缩小范围，优先复盘“高分但仍有偏题提醒”的卷子，这类最适合转成稳定主练。</li>', '<li>“设为当前训练参考”只会更新这轮主练，不会清空观星留痕或旧样本。</li>', '<li>“回观星复盘”会带着当前答卷的主练主题回到观察站，方便直接继续筛留痕。</li>', selected?.compareHint ? `<li>当前卷的对照抓手：${escapeHtml(selected.compareHint)}</li>` : '', '</ul>'].join('');
    if (!selected) {
      detail.innerHTML = '<div class="codex-empty-state">归卷书架当前为空，先打一章远征再回来复盘。</div>';
      return;
    }
    const canSetTraining = selected.trainingReady && typeof this.buildObservatoryTrainingFocusFromSlate === 'function' && typeof this.setObservatoryTrainingFocus === 'function';
            ...
            <div class="collection-detail-shell">
                ...
                <div class="collection-action-row">
                    <button type="button" class="collection-inline-btn"
                        data-run-slate-action="train-focus"
                    ...
                    <button type="button" class="collection-inline-btn secondary"
                        data-run-slate-action="review-observatory"
                ...
                <section class="collection-detail-card">
                    ...
                    <div class="collection-card-tags">
                        ${(selected.tags.length > 0 ? selected.tags : ['归卷书架', selected.ratingLabel, selected.themeLabel || '未定课题']).filter(Boolean).map(tag => `<span class="collection-tag">${escapeHtml(tag)}</span>`).join('')}
  ...
  collectionHubMethods.renderSanctumOverview = function () {
    if (typeof document === 'undefined') return;
    ...
    const seasonBoard = buildSeasonBoardOverviewModel(data.seasonBoard && typeof data.seasonBoard === 'object' ? data.seasonBoard : null);
    const seasonHonorShowcase = data.seasonHonorShowcase && typeof data.seasonHonorShowcase === 'object' ? data.seasonHonorShowcase : null;
    ...
    const heavenlyMandate = buildHeavenlyMandateOverviewModel(data.mandate && typeof data.mandate === 'object' ? data.mandate : null, data.heavenlyMandate && typeof data.heavenlyMandate === 'object' ? data.heavenlyMandate : null);
    const heavenlyMandateFocusTask = heavenlyMandate.focusTask && typeof heavenlyMandate.focusTask === 'object' ? heavenlyMandate.focusTask : heavenlyMandate.nextTask && typeof heavenlyMandate.nextTask === 'object' ? heavenlyMandate.nextTask : null;
    const heavenlyMandateFocusActionMeta = resolveSeasonBoardActionMeta(heavenlyMandateFocusTask?.anchorSection, 'sanctum');
    ...
    const seasonSettlement = seasonBoard.settlement && typeof seasonBoard.settlement === 'object' ? seasonBoard.settlement : null;
    const seasonDebtPack = seasonBoard.debtPack && typeof seasonBoard.debtPack === 'object' ? seasonBoard.debtPack : null;
    const seasonFrontier = seasonBoard.frontier && typeof seasonBoard.frontier === 'object' ? seasonBoard.frontier : null;
    const seasonFrontierDecree = seasonFrontier?.decree && typeof seasonFrontier.decree === 'object' ? seasonFrontier.decree : null;
    const seasonFrontierChronicle = seasonFrontier?.chronicle && typeof seasonFrontier.chronicle === 'object' ? seasonFrontier.chronicle : null;
    const seasonFrontierChronicleArchive = seasonFrontier?.chronicleArchive && typeof seasonFrontier.chronicleArchive === 'object' ? seasonFrontier.chronicleArchive : null;
    const seasonFrontierChronicleArchiveEntries = Array.isArray(seasonFrontierChronicleArchive?.entries) ? seasonFrontierChronicleArchive.entries.filter(entry => entry && typeof entry === 'object').slice(0, 4) : [];
    const seasonFrontierChronicleArchiveLatest = seasonFrontierChronicleArchive?.latestEntry && typeof seasonFrontierChronicleArchive.latestEntry === 'object' ? seasonFrontierChronicleArchive.latestEntry : seasonFrontierChronicleArchiveEntries[0] || null;
    const seasonFrontierCouncil = seasonFrontier?.council && typeof seasonFrontier.council === 'object' ? seasonFrontier.council : null;
    const seasonFrontierResolution = seasonFrontier?.resolution && typeof seasonFrontier.resolution === 'object' ? seasonFrontier.resolution : null;
    const seasonChapterArc = seasonBoard.chapterArc && typeof seasonBoard.chapterArc === 'object' ? seasonBoard.chapterArc : null;
    const seasonChapterArcObjective = seasonChapterArc?.objective && typeof seasonChapterArc.objective === 'object' ? seasonChapterArc.objective : null;
    const seasonChapterArcPressureWindow = seasonChapterArc?.pressureWindow && typeof seasonChapterArc.pressureWindow === 'object' ? seasonChapterArc.pressureWindow : null;
    const seasonFrontierResolutionChoiceLabels = {
      hold_primary: '守主战线',
      rebalance_support: '副线补证',
      seal_dispute: '封存争议'
    ...
    const seasonFrontierResolutionSuggestedChoiceId = seasonFrontierResolutionChoiceLabels[seasonFrontierResolution?.suggestedChoiceId] ? seasonFrontierResolution.suggestedChoiceId : 'hold_primary';
    const seasonVerificationArchive = seasonBoard.verificationArchive && typeof seasonBoard.verificationArchive === 'object' ? seasonBoard.verificationArchive : null;
    const seasonVerificationArchiveAllEntries = Array.isArray(seasonVerificationArchive?.entries) ? seasonVerificationArchive.entries.filter(entry => entry && typeof entry === 'object') : [];
    const seasonVerificationArchiveEntries = seasonVerificationArchiveAllEntries.slice(0, 3);
    const seasonVerificationArchiveLatest = seasonVerificationArchive?.latestEntry && typeof seasonVerificationArchive.latestEntry === 'object' ? seasonVerificationArchive.latestEntry : seasonVerificationArchiveEntries[0] || null;
    const shouldSurfaceSeasonVerification = shouldSurfaceSeasonBoardVerification(seasonBoard, seasonSettlement);
    const seasonVerificationOrders = shouldSurfaceSeasonVerification && Array.isArray(seasonBoard.verificationOrders) ? seasonBoard.verificationOrders.filter(entry => entry && typeof entry === 'object') : [];
    const seasonVerificationOrderPair = getSeasonBoardVerificationOrderPair(seasonVerificationOrders);
    const primarySeasonVerification = seasonVerificationOrderPair.primary;
    const seasonChapterArcFollowTask = seasonChapterArc?.available && seasonBoard?.nextTask?.id ? seasonBoard.nextTask : null;
    const seasonChapterArcClosed = !!(seasonChapterArc && Number(seasonChapterArc.sealedWeeks || 0) >= Number(seasonChapterArc.targetWeeks || 3));
    const seasonChapterArcFollowLabel = seasonChapterArc?.rescueWindow?.open ? '按本周主线救火' : seasonChapterArcClosed ? '按本周主线复盘' : '跟进本周主线';
    const seasonChapterArcFollowLine = seasonChapterArcFollowTask ? `章程不另开任务，当前仍跟随「${seasonChapterArcFollowTask.label || seasonChapterArcFollowTask.id || '本周季盘行动'}」。` : '';
    const seasonChapterArcObjectiveLine = seasonChapterArcObjective ? [seasonChapterArcObjective.summaryLine || '', seasonChapterArcObjective.goalLine || '', seasonChapterArcObjective.reasonLine || ''].filter(Boolean).join(' · ') : '';
    const seasonChapterArcPressureLine = seasonChapterArcPressureWindow ? [seasonChapterArcPressureWindow.reasonLine || seasonChapterArcPressureWindow.shortLine || '', seasonChapterArcPressureWindow.guideLine || ''].filter(Boolean).join(' · ') : '';
    const heavenlyMandateBoard = Array.isArray(heavenlyMandate.lanes) ? heavenlyMandate.lanes : [];
    const seasonHonorSummaryMarkup = seasonHonorShowcase ? `<p class="collection-muted"
        ...
        data-season-honor-showcase-visibility="self_only_ranked_economy">${escapeHtml(`赛季荣誉陈列：${seasonHonorShowcase.summaryLine || '正式论道外观收藏已同步'} · ${seasonHonorShowcase.recordLine || `本季 ${seasonHonorShowcase.gamesPlayed || 0} 场`}`)}</p>` : '';
    const seasonHonorUnlockedChipMarkup = seasonHonorShowcase ? `<div class="codex-summary-chip"
        data-season-honor-showcase-chip="unlocked"
        data-season-honor-showcase-report="pvp-live-season-honor-showcase-v1"><strong>${escapeHtml(`${seasonHonorShowcase.totalUnlocked || 0}/${seasonHonorShowcase.totalRewards || 0}`)}</strong><span>赛季荣誉</span></div>` : '';
    const seasonHonorNextChipMarkup = seasonHonorShowcase?.nextReward ? `<div class="codex-summary-chip"
        data-season-honor-showcase-chip="next"><strong>${escapeHtml(seasonHonorShowcase.nextReward.remainingGames > 0 ? `${seasonHonorShowcase.nextReward.remainingGames} 场` : '已达成')}</strong><span>${escapeHtml(seasonHonorShowcase.nextReward.rewardName || '下一档荣誉')}</span></div>` : '';
    const seasonHonorBoardMarkup = seasonHonorShowcase ? `<div class="collection-detail-grid" data-season-honor-showcase-board="true">
                    <section class="collection-detail-card"
                        ...
                        <span class="detail-mini-label">${escapeHtml(`${seasonHonorShowcase.seasonName || '开天赛季'} · 正式论道`)}</span>
                        <strong>${escapeHtml(seasonHonorShowcase.latestReward?.rewardName || '赛季荣誉陈列')}</strong>
                        <p>${escapeHtml(seasonHonorShowcase.progressLine || '完成正式论道后，赛季荣誉会自动入库。')}</p>
                        <ul class="collection-detail-list compact">
                            <li>${escapeHtml(seasonHonorShowcase.summaryLine || '本季荣誉陈列尚未点亮。')}</li>
                            <li>${escapeHtml(seasonHonorShowcase.nextReward?.label || '当前最高档荣誉目标已达成。')}</li>
                            <li>${escapeHtml(seasonHonorShowcase.visibilityLine || '仅本人洞府只读可见，不进入公开回放或审计回放。')}</li>
                            <li>${escapeHtml(seasonHonorShowcase.boundary || '赛季荣誉陈列只展示正式论道外观收藏，不授予卡牌、属性、资源、起手、匹配或战斗效果。')}</li>
                        </ul>
                        <div class="collection-action-row">
                            <button type="button" class="collection-inline-btn secondary"
                                data-season-honor-showcase-cta="true"
                                data-collection-action="show-screen"
                                data-screen-id="pvp-screen">${escapeHtml(seasonHonorShowcase.ctaLabel || '继续正式论道')}</button>
                        </div>
                    </section>
                </div>` : '';
    const resolveResearchAction = research => {
      ...
      if (research.actionType === 'treasure') {
        return {
          ...
          disabled: false,
          extraAttrs: 'data-collection-action="show-treasure-compendium"'
        };
      }
      if (research.actionType === 'challenge') {
        return {
          label: research.buttonLabel || '查看成果',
          disabled: false,
          extraAttrs: `data-collection-action="show-challenge-hub" data-challenge-mode="${escapeHtml(research.actionValue || 'daily')}"`
        };
      }
      if (research.actionType === 'screen') {
        return {
          ...
          disabled: false,
          extraAttrs: `data-collection-action="show-screen" data-screen-id="${escapeHtml(research.actionValue || 'map-screen')}"`
        };
      }
      ...
      return {
        ...
        disabled: false,
        extraAttrs: `data-collection-action="switch-section" data-collection-section="${escapeHtml(research.section || research.actionValue || 'builds')}"`
      };
    };
    roomGrid.innerHTML = data.rooms.map(room => `
            <article class="sanctum-room-card">
                ...
                <button type="button" class="collection-inline-btn"
                    data-collection-action="${room.actionType === 'treasure' ? 'show-treasure-compendium' : room.actionType === 'challenge' ? 'show-challenge-hub' : 'switch-section'}"
                    ${room.actionType === 'challenge' ? `data-challenge-mode="${escapeHtml(room.actionValue || 'daily')}"` : room.actionType === 'treasure' ? '' : `data-collection-section="${escapeHtml(room.actionValue)}"`}
                    >${escapeHtml(room.actionLabel)}</button>
            </article>
        `).join('');
    ...
    goalList.innerHTML = data.goals.length > 0 ? data.goals.map(goal => {
      ...
      return `
                <article class="sanctum-goal-item"
                    ...
                    <button type="button" class="collection-inline-btn"
                        ...
                        ${isHeavenlyMandateGoal && goal.followTaskId ? `data-collection-action="follow-heavenly-mandate-task" data-heavenly-mandate-task-id="${escapeHtml(goal.followTaskId)}" data-heavenly-mandate-task-anchor-section="${escapeHtml(goal.followTaskAnchorSection || '')}" data-heavenly-mandate-task-action-type="${escapeHtml(goal.followTaskActionType || '')}" data-heavenly-mandate-task-action-value="${escapeHtml(goal.followTaskActionValue || '')}"` : goal.action === 'claim' ? `data-collection-action="claim-achievement" data-achievement-id="${escapeHtml(goal.id)}"` : goal.action === 'treasure' ? 'data-collection-action="show-treasure-compendium"' : goal.action === 'challenge' ? `data-collection-action="show-challenge-hub" data-challenge-mode="${escapeHtml(goal.value || 'global')}"` : goal.action === 'screen' ? `data-collection-action="show-screen" data-screen-id="${escapeHtml(goal.value || 'map-screen')}"` : goal.action === 'season_board_lane_reward' ? '' : `data-collection-action="switch-section" data-collection-section="${escapeHtml(goal.value || 'builds')}"`}
                        ${goal.disabled ? 'disabled' : ''}
                        >${goal.action === 'claim' ? '领取奖励' : escapeHtml(goal.buttonLabel || '前往查看')}</button>
                </article>
            `;
    }).join('') : '<div class="codex-empty-state">当前没有待领取目标，可以继续推进章节、法则或灵契研究。</div>';
    unlockFeed.innerHTML = data.recentUnlocks.length > 0 ? data.recentUnlocks.map(entry => `
            ...
            `).join('') : '<div class="codex-empty-state">近期还没有新的藏经阁记录，去打一局或补一条研究线索吧。</div>';
    summary.innerHTML = ['<span class="codex-side-kicker">洞府概览</span>', '<h3>局外中枢进度</h3>', activeAgenda ? `<p data-sanctum-agenda-summary="true">${escapeHtml(`当前议程：${activeAgenda.name} · ${activeAgenda.progress}/${activeAgenda.target}${activeAgenda.phaseLabel ? ` · ${activeAgenda.phaseLabel}` : ''}`)}</p>` : lastAgenda ? `<p data-sanctum-agenda-summary="true">${escapeHtml(`最近结题：${lastAgenda.name} · ${lastAgenda.outcomeLabel || '研究留痕'}`)}</p>` : '<p data-sanctum-agenda-summary="true">当前还没有立下洞府议程，可先从归卷书架挑一份答卷作为本轮承诺。</p>', activeAgenda ? `<p class="collection-muted">${escapeHtml(activeAgenda.selectedContractLabel ? `已立契约：${activeAgenda.selectedContractLabel} · ${activeAgenda.selectedContractLine || activeAgenda.phaseLine || activeAgenda.focusNodeLine || ''}` : activeAgenda.selectedDecisionLabel ? `已选处置：${activeAgenda.selectedDecisionLabel} · ${activeAgenda.selectedDecisionLine || activeAgenda.phaseLine || activeAgenda.focusNodeLine || ''}` : activeAgenda.phaseLine || activeAgenda.focusNodeLine || activeAgenda.summaryLine || '')}</p>` : lastAgenda && (lastAgenda.recoveryLine || lastAgenda.grantedLine || lastAgenda.reasonLine || lastAgenda.summaryLine) ? `<p class="collection-muted">${escapeHtml(lastAgenda.recoveryLine || lastAgenda.grantedLine || lastAgenda.reasonLine || lastAgenda.summaryLine)}</p>` : '', heavenlyMandate.available ? `<p data-heavenly-mandate-summary="true" data-heavenly-mandate-week="${escapeHtml(heavenlyMandate.weekTag || '')}" data-heavenly-mandate-directive="${escapeHtml(heavenlyMandate.directiveName || '')}">${escapeHtml(heavenlyMandate.summaryLine || '天道敕令待启封')}</p>` : '', heavenlyMandate.detailLine ? `<p class="collection-muted" data-heavenly-mandate-detail="true">${escapeHtml(heavenlyMandate.detailLine)}</p>` : '', seasonBoard.available ? `<p data-season-board-summary="true">${escapeHtml(seasonBoard.summaryLine || '赛季天道盘待同步')}</p>` : '', seasonHonorSummaryMarkup, seasonChapterArc?.available ? `<p class="collection-muted" data-season-board-chapter-arc="true">${escapeHtml(seasonChapterArc.summaryLine || seasonChapterArc.statusLine || '章节弧线待同步')}</p>` : '', seasonChapterArcObjective?.available ? `<p class="collection-muted" data-season-board-chapter-arc-objective="true">${escapeHtml(seasonChapterArcObjectiveLine || seasonChapterArcObjective.shortLine || '章目标待同步')}</p>` : '', seasonChapterArc?.available && seasonChapterArc.feedbackLine ? `<p class="collection-muted" data-season-board-chapter-arc-feedback="true">${escapeHtml(seasonChapterArc.feedbackLine)}</p>` : '', seasonBoard.detailLine ? `<p class="collection-muted" data-season-board-detail="true">${escapeHtml(seasonBoard.detailLine)}</p>` : '', seasonFrontier ? `<p class="collection-muted" data-season-board-frontier="true" data-season-board-frontier-id="${escapeHtml(seasonFrontier.primaryFrontId || '')}" data-season-board-frontier-pressure="${escapeHtml(seasonFrontier.statusId || '')}">${escapeHtml(seasonFrontier.summaryLine || `诸界战线：${seasonFrontier.primaryFrontLabel || '主战线'} · ${seasonFrontier.pressureLabel || seasonFrontier.statusLabel || '稳态'}`)}</p>` : '', seasonFrontierDecree ? `<p class="collection-muted" data-season-board-frontier-decree="true" data-season-board-frontier-decree-id="${escapeHtml(seasonFrontierDecree.id || '')}" data-season-board-frontier-decree-lane-id="${escapeHtml(seasonFrontierDecree.laneId || '')}">${escapeHtml([seasonFrontierDecree.summaryLine || seasonFrontierDecree.title || '', seasonFrontierDecree.constraintLine || ''].filter(Boolean).join(' · '))}</p>` : '', seasonFrontierChronicle ? `<p class="collection-muted" data-season-board-frontier-chronicle="true" data-season-board-frontier-chronicle-id="${escapeHtml(seasonFrontierChronicle.id || '')}" data-season-board-frontier-chronicle-lane-id="${escapeHtml(seasonFrontierChronicle.laneId || '')}">${escapeHtml([seasonFrontierChronicle.summaryLine || seasonFrontierChronicle.title || '', seasonFrontierChronicle.progressLine || ''].filter(Boolean).join(' · '))}</p>` : '', seasonFrontierChronicleArchive ? `<p class="collection-muted" data-season-board-frontier-chronicle-archive="true" data-season-board-frontier-chronicle-archive-id="${escapeHtml(seasonFrontierChronicleArchive.id || '')}" data-season-board-frontier-chronicle-archive-total="${escapeHtml(String(seasonFrontierChronicleArchive.totalRecords || 0))}">${escapeHtml([seasonFrontierChronicleArchive.summaryLine || '', seasonFrontierChronicleArchive.detailLine || ''].filter(Boolean).join(' · '))}</p>` : '', seasonFrontierCouncil ? `<p class="collection-muted" data-season-board-frontier-council="true" data-season-board-frontier-council-id="${escapeHtml(seasonFrontierCouncil.id || '')}" data-season-board-frontier-council-lane-id="${escapeHtml(seasonFrontierCouncil.laneId || '')}">${escapeHtml([seasonFrontierCouncil.summaryLine || seasonFrontierCouncil.title || '', seasonFrontierCouncil.verdictLine || ''].filter(Boolean).join(' · '))}</p>` : '', seasonFrontierResolution ? `<p class="collection-muted" data-season-board-frontier-resolution="true" data-season-board-frontier-resolution-id="${escapeHtml(seasonFrontierResolution.id || '')}" data-season-board-frontier-resolution-submitted="${seasonFrontierResolution.submitted ? 'true' : 'false'}" data-season-board-frontier-resolution-choice-id="${escapeHtml(seasonFrontierResolution.choiceId || seasonFrontierResolution.suggestedChoiceId || '')}">${escapeHtml([seasonFrontierResolution.summaryLine || '', seasonFrontierResolution.submitted ? seasonFrontierResolution.chronicleSealLine || '' : `建议：${seasonFrontierResolution.suggestedChoiceLabel || seasonFrontierResolutionChoiceLabels[seasonFrontierResolutionSuggestedChoiceId] || '守主战线'}`].filter(Boolean).join(' · '))}</p>` : '', seasonSettlement ? `<p class="collection-muted" data-season-board-settlement="true">${escapeHtml(`季押卷：${seasonSettlement.outcomeLabel || '待押卷'}${seasonSettlement.summaryLine ? ` · ${seasonSettlement.summaryLine}` : ''}`)}</p>` : '', seasonDebtPack ? `<p class="collection-muted" data-season-board-debt="true">${escapeHtml(`债账：${seasonDebtPack.summaryLine || seasonDebtPack.guideLine || seasonDebtPack.progressText || '待清账'}`)}</p>` : '', primarySeasonVerification ? `<p class="collection-muted" data-season-board-verification="true">${escapeHtml(`结业验证：${primarySeasonVerification.summaryLine || primarySeasonVerification.hintLine || primarySeasonVerification.statusLine || primarySeasonVerification.label || '待验证'}`)}</p>` : '', lineage?.available ? `<p data-fate-lineage-summary="true">${escapeHtml(lineage.summaryLine || '命盘谱系待同步')}</p>` : '', lineage?.detailLine ? `<p class="collection-muted" data-fate-lineage-detail="true">${escapeHtml(lineage.detailLine)}</p>` : '', aftereffects?.summaryLine ? `<p data-fate-aftereffect-summary="true">${escapeHtml(aftereffects.summaryLine)}</p>` : '', aftereffects?.detailLine ? `<p class="collection-muted" data-fate-aftereffect-detail="true">${escapeHtml(aftereffects.detailLine)}</p>` : '', '<div class="codex-summary-grid two-cols">', `${activeAgenda ? `<div class="codex-summary-chip"><strong>${escapeHtml(`${activeAgenda.progress}/${activeAgenda.target}`)}</strong><span>当前议程</span></div>` : ''}`, `${heavenlyMandate.available ? `<div class="codex-summary-chip" data-heavenly-mandate-chip="directive" data-heavenly-mandate-theme="true"><strong>${escapeHtml(heavenlyMandate.directiveName || '待启敕令')}</strong><span>天道敕令</span></div>` : ''}`, `${heavenlyMandate.available ? `<div class="codex-summary-chip" data-heavenly-mandate-chip="week"><strong>${escapeHtml(heavenlyMandate.weekLabel || heavenlyMandate.weekTag || '本周轮转')}</strong><span>当前周签</span></div>` : ''}`, `${heavenlyMandate.available ? `<div class="codex-summary-chip" data-heavenly-mandate-chip="progress" data-heavenly-mandate-progress="true"><strong>${escapeHtml(heavenlyMandate.progressText || '待同步')}</strong><span>周进度</span></div>` : ''}`, `${seasonBoard.available ? `<div class="codex-summary-chip" data-season-board-chip="phase"><strong>${escapeHtml(seasonBoard.phaseLabel || '采样期')}</strong><span>季盘阶段</span></div>` : ''}`, `${seasonBoard.available ? `<div class="codex-summary-chip" data-season-board-chip="theme"><strong>${escapeHtml(seasonBoard.themeLabel || '本周主轴')}</strong><span>赛季主轴</span></div>` : ''}`, `${seasonBoard.available ? `<div class="codex-summary-chip" data-season-board-chip="status"><strong>${escapeHtml(seasonBoard.progressText || '待同步')}</strong><span>季盘进度</span></div>` : ''}`, seasonHonorUnlockedChipMarkup, seasonHonorNextChipMarkup, `${seasonChapterArc?.available ? `<div class="codex-summary-chip" data-season-board-chip="chapter-arc"><strong>${escapeHtml(seasonChapterArc.arcLabel || seasonChapterArc.chapterLabel || '章节弧线')}</strong><span>${escapeHtml(seasonChapterArc.weekSlot || seasonChapterArc.windowLabel || seasonChapterArc.progressText || '本周窗口')}</span></div>` : ''}`, `${seasonChapterArcPressureWindow?.available ? `<div class="codex-summary-chip" data-season-board-chip="chapter-arc-pressure"><strong>${escapeHtml(seasonChapterArcPressureWindow.statusLabel || '章势压强')}</strong><span>${escapeHtml(`章势 · ${seasonChapterArcPressureWindow.shortLine || seasonChapterArcPressureWindow.reasonLine || '待同步'}`)}</span></div>` : ''}`, `${seasonChapterArcObjective?.available ? `<div class="codex-summary-chip" data-season-board-chip="chapter-arc-objective"><strong>${escapeHtml(seasonChapterArcObjective.statusLabel || seasonChapterArcObjective.label || '章目标')}</strong><span>${escapeHtml(`章目标 · ${seasonChapterArcObjective.focusLaneLabel || '经营目标'}`)}</span></div>` : ''}`, `${seasonFrontier ? `<div class="codex-summary-chip" data-season-board-chip="frontier" data-season-board-frontier-chip="true"><strong>${escapeHtml(seasonFrontier.primaryFrontShortLabel || seasonFrontier.primaryFrontLabel || '主战线')}</strong><span>${escapeHtml(seasonFrontier.pressureLabel || seasonFrontier.statusLabel || '战线态势')}</span></div>` : ''}`, `${seasonFrontierDecree ? `<div class="codex-summary-chip" data-season-board-chip="frontier-decree" data-season-board-frontier-decree-chip="true"><strong>${escapeHtml(seasonFrontierDecree.laneLabel || seasonFrontier.primaryFrontShortLabel || '主战线')}</strong><span>${escapeHtml(`法旨 · ${seasonFrontierDecree.toneLabel || '本周'}`)}</span></div>` : ''}`, `${seasonFrontierChronicle ? `<div class="codex-summary-chip" data-season-board-chip="frontier-chronicle" data-season-board-frontier-chronicle-chip="true"><strong>${escapeHtml(seasonFrontierChronicle.laneLabel || seasonFrontier.primaryFrontShortLabel || '主战线')}</strong><span>${escapeHtml(`史卷 · ${seasonFrontierChronicle.phaseLabel || '本周'}`)}</span></div>` : ''}`, `${seasonFrontierChronicleArchive ? `<div class="codex-summary-chip" data-season-board-chip="frontier-chronicle-archive" data-season-board-frontier-chronicle-archive-chip="true"><strong>${escapeHtml(seasonFrontierChronicleArchive.dominantStanceLabel || seasonFrontierChronicleArchiveLatest?.stanceLabel || '待沉淀')}</strong><span>${escapeHtml(seasonFrontierChronicleArchive.progressText || `史卷 ${seasonFrontierChronicleArchive.totalRecords || 0} 条`)}</span></div>` : ''}`, `${seasonFrontierCouncil ? `<div class="codex-summary-chip" data-season-board-chip="frontier-council" data-season-board-frontier-council-chip="true"><strong>${escapeHtml(seasonFrontierCouncil.laneLabel || seasonFrontier.primaryFrontShortLabel || '主战线')}</strong><span>${escapeHtml(`会审 · ${seasonFrontierCouncil.phaseLabel || '本周'}`)}</span></div>` : ''}`, `${seasonFrontierResolution ? `<div class="codex-summary-chip" data-season-board-chip="frontier-resolution" data-season-board-frontier-resolution-chip="true"><strong>${escapeHtml(seasonFrontierResolution.submitted ? seasonFrontierResolution.choiceLabel || '已封记' : seasonFrontierResolution.suggestedChoiceLabel || seasonFrontierResolutionChoiceLabels[seasonFrontierResolutionSuggestedChoiceId] || '待裁记')}</strong><span>${escapeHtml(seasonFrontierResolution.submitted ? '裁记已封' : '会审裁记')}</span></div>` : ''}`, `${seasonBoardLaneRewards.length > 0 ? `<div class="codex-summary-chip" data-season-board-chip="lane-reward"><strong>${escapeHtml(`${seasonBoardClaimableLaneRewards.length}/${seasonBoardLaneRewards.length}`)}</strong><span>分线结题赏</span></div>` : ''}`, `${seasonSettlement ? `<div class="codex-summary-chip" data-season-board-chip="settlement"><strong>${escapeHtml(seasonSettlement.outcomeLabel || '待押卷')}</strong><span>季押卷</span></div>` : ''}`, `${seasonDebtPack ? `<div class="codex-summary-chip" data-season-board-chip="debt"><strong>${escapeHtml(seasonDebtPack.progressText || seasonDebtPack.settleWindowText || seasonDebtPack.debtThemeLabel || '待清账')}</strong><span>债账窗口</span></div>` : ''}`, `${primarySeasonVerification ? `<div class="codex-summary-chip" data-season-board-chip="verification"><strong>${escapeHtml(primarySeasonVerification.label || '待验证')}</strong><span>结业验证</span></div>` : ''}`, `${lineage?.available ? `<div class="codex-summary-chip" data-fate-lineage-chip="style"><strong>${escapeHtml(lineage.styleTrack?.dominantLabel || '待沉淀')}</strong><span>主修流派</span></div>` : ''}`, `${lineage?.available ? `<div class="codex-summary-chip" data-fate-lineage-chip="research"><strong>${escapeHtml(lineage.researchTrack?.dominantLabel || '待结题')}</strong><span>研究倾向</span></div>` : ''}`, `${lineage?.frontierTrack ? `<div class="codex-summary-chip" data-fate-lineage-chip="frontier"><strong>${escapeHtml(lineage.frontierTrack.dominantLabel || '待封记')}</strong><span>会审风格</span></div>` : ''}`, `${primaryAftereffect?.templateLabel ? `<div class="codex-summary-chip" data-fate-aftereffect-chip="template"><strong>${escapeHtml(primaryAftereffect.templateLabel || '跨章偏置')}</strong><span>界痕类型</span></div>` : ''}`, `${primaryAftereffect?.statusLabel ? `<div class="codex-summary-chip" data-fate-aftereffect-chip="status"><strong>${escapeHtml(primaryAftereffect.statusLabel || '待生效')}</strong><span>当前状态</span></div>` : ''}`, `<div class="codex-summary-chip"><strong>${data.progress.sanctumAgendaCompleted || 0}</strong><span>议程结题</span></div>`, `<div class="codex-summary-chip"><strong>${data.progress.sanctumAgendaFailed || 0}</strong><span>未成研究</span></div>`, `<div class="codex-summary-chip"><strong>${data.rooms.length}</strong><span>房间总览</span></div>`, `<div class="codex-summary-chip"><strong>${data.researches.filter(item => item.ready).length}</strong><span>已满足研究</span></div>`, `<div class="codex-summary-chip"><strong>${data.progress.clearedChapters}</strong><span>已贯通章节</span></div>`, `<div class="codex-summary-chip"><strong>${data.progress.clearedBossMemories || 0}</strong><span>记忆战留痕</span></div>`, `<div class="codex-summary-chip"><strong>${data.progress.completedRunPaths || 0}</strong><span>命途战录</span></div>`, `<div class="codex-summary-chip"><strong>${data.progress.runPathBossSampleCount || 0}</strong><span>样本对照</span></div>`, `${data.progress.runSlateArchives !== undefined ? `<div class="codex-summary-chip"><strong>${data.progress.runSlateArchives || 0}</strong><span>归卷书架</span></div>` : ''}`, `<div class="codex-summary-chip"><strong>${data.progress.forgeActiveWorkshops || 0}</strong><span>炼器铭刻</span></div>`, `<div class="codex-summary-chip"><strong>${data.progress.forgeFullSets || 0}</strong><span>三段套装</span></div>`, `${data.progress.observatoryTraces !== undefined ? `<div class="codex-summary-chip"><strong>${data.progress.observatoryTraces || 0}</strong><span>观星留痕</span></div>` : ''}`, `<div class="codex-summary-chip"><strong>${data.progress.unclaimedAchievements}</strong><span>可领取目标</span></div>`, '</div>', seasonHonorBoardMarkup, seasonBoard.available ? `<div class="collection-detail-grid"
                        ...
                        <div
                            ...
                            data-season-board-chapter-arc-pressure-status="${escapeHtml(seasonChapterArcPressureWindow?.statusId || '')}">
                                ...
                                ${seasonChapterArc.entries.length > 0 ? `
                                    <ul class="collection-detail-list compact">
                                        ${seasonChapterArc.entries.map(entry => `
                                            <li data-season-board-chapter-arc-entry="true">${escapeHtml(entry.shortLine || entry.summaryLine || '章节节点待同步')}</li>
                                        `).join('')}
                                    </ul>
                                ` : ''}
                                ${seasonChapterArc.rescueWindow?.shortLine ? `<p class="collection-muted" data-season-board-chapter-arc-rescue="true">${escapeHtml(seasonChapterArc.rescueWindow.shortLine)}</p>` : seasonChapterArc.statusLine ? `<p class="collection-muted" data-season-board-chapter-arc-rescue="true">${escapeHtml(seasonChapterArc.statusLine)}</p>` : ''}
                                ${seasonChapterArc.review?.shortLine ? `<p class="collection-muted" data-season-board-chapter-arc-review="true">${escapeHtml(seasonChapterArc.review.shortLine)}</p>` : seasonChapterArc.goalLine ? `<p class="collection-muted" data-season-board-chapter-arc-review="true">${escapeHtml(seasonChapterArc.goalLine)}</p>` : ''}
                                ${seasonChapterArcFollowTask ? `
                                    <div class="season-board-task-action-row" data-season-board-chapter-arc-follow-row="true">
                                        <button type="button" class="collection-inline-btn secondary compact"
                                            data-season-board-chapter-arc-follow="true"
                                            data-season-board-chapter-arc-follow-task-id="${escapeHtml(seasonChapterArcFollowTask.id || '')}"
                                            data-season-board-chapter-arc-follow-source="${escapeHtml(seasonChapterArcFollowTask.source || '')}"
                                            data-season-board-chapter-arc-follow-lane-id="${escapeHtml(seasonChapterArcFollowTask.laneId || '')}"
                                            >${escapeHtml(seasonChapterArcFollowLabel)}</button>
                                        <span class="collection-muted" data-season-board-chapter-arc-follow-line="true">${escapeHtml(seasonChapterArcFollowLine)}</span>
                                    </div>
                                ` : ''}
                            </div>
                        ` : ''}
                    ...
                    ${seasonFrontierResolution ? `
                        <section class="collection-detail-card"
                            ...
                            <ul class="collection-detail-list compact">
                                ${[seasonFrontierResolution.councilResolutionLine || '', seasonFrontierResolution.chronicleSealLine || '', seasonFrontierResolution.supportLaneLabel ? `副线候选：${seasonFrontierResolution.supportLaneLabel}` : '', seasonFrontierResolution.settlementOutcomeLabel ? `押卷结果：${seasonFrontierResolution.settlementOutcomeLabel}` : ''].filter(Boolean).map(line => `<li>${escapeHtml(line)}</li>`).join('')}
                            </ul>
                            ${seasonFrontierResolution.submitted ? `<p class="collection-muted" data-season-board-frontier-resolution-submitted="true">${escapeHtml(`已封记：${seasonFrontierResolution.choiceLabel || '裁记'}${seasonFrontierResolution.submittedAt ? ` · ${new Date(seasonFrontierResolution.submittedAt).toLocaleString('zh-CN')}` : ''}`)}</p>` : `<div class="season-board-task-action-row" data-season-board-frontier-resolution-choice-row="true">
                    ...
                    ${seasonFrontierChronicleArchive ? `
                        <section class="collection-detail-card"
                            ...
                            <ul class="collection-detail-list compact">
                                ${seasonFrontierChronicleArchiveEntries.length > 0 ? seasonFrontierChronicleArchiveEntries.map(entry => `
                                    <li
                                ...
                                `).join('') : `<li>${escapeHtml(seasonFrontierChronicleArchive.detailLine || '完成一次会审裁记后，战役史卷会开始累计长期风格。')}</li>`}
                            </ul>
                    ...
                    ${seasonSettlement ? `
                        <section class="collection-detail-card" data-season-board-settlement-card="true">
                            ...
                            <ul class="collection-detail-list compact">
                                ${[seasonSettlement.summaryLine || '', seasonSettlement.detailLine || '', seasonSettlement.guideLine ? `下一步：${seasonSettlement.guideLine}` : '', seasonSettlement.contractResolutionLine ? `回执：${seasonSettlement.contractResolutionLine}` : ''].filter(Boolean).map(line => `<li>${escapeHtml(line)}</li>`).join('')}
                            </ul>
                        </section>
                    ` : ''}
                    ${seasonDebtPack ? `
                        <section class="collection-detail-card" data-season-board-debt-card="true">
                            ...
                            <ul class="collection-detail-list compact">
                                ${[seasonDebtPack.summaryLine || '', seasonDebtPack.detailLine || '', seasonDebtPack.guideLine ? `清账路径：${seasonDebtPack.guideLine}` : '', seasonDebtPack.recommendedValidationLabel ? `推荐验证：${seasonDebtPack.recommendedValidationLabel}` : ''].filter(Boolean).map(line => `<li>${escapeHtml(line)}</li>`).join('')}
                            </ul>
                        </section>
                    ` : ''}
                    ...
                    ${seasonVerificationArchive?.available ? `
                        <section class="collection-detail-card"
                            ...
                            <ul class="collection-detail-list compact">
                                ${seasonVerificationArchiveEntries.length > 0 ? seasonVerificationArchiveEntries.map(entry => `
                                    <li
                                        ...
                                        <button type="button" class="collection-inline-btn secondary"
                                            ...
                                            >${escapeHtml(entry.ctaLabel || '沿此复核')}</button>
                                    </li>
                                `).join('') : `<li>${escapeHtml(seasonVerificationArchive.detailLine || '当前还没有真正落档的周判记录，先去补一张主验证或旁验证。')}</li>`}
                            </ul>
                            ...
                            ${lane.reward ? `
                                <div class="season-board-task-action-row"
                                    ...
                                    <button type="button" class="collection-inline-btn secondary compact"
                                        ...
                                        >${escapeHtml(lane.reward.buttonLabel || (lane.reward.claimable ? '领取结题赏' : lane.reward.statusLabel || '未结题'))}</button>
                                </div>
                            ` : ''}
                            <ul class="collection-detail-list compact">
                                ${(lane.tasks.length > 0 ? lane.tasks : [{
      id: `${lane.id || 'lane'}_empty`,
      label: '等待任务同步',
      progressText: '',
      completed: false,
      hintLine: '',
      ...
      anchorSection: ''
                                        ...
                                        ${task.actionType || task.anchorSection || task.actionValue ? `
                                            <div class="season-board-task-action-row">
                                                ...
                                                <button type="button" class="collection-inline-btn secondary compact"
                                                    ...
                                                    >${escapeHtml(task.ctaLabel || (task.completed ? '沿此复核' : '前往推进'))}</button>
                                            </div>
                                        ` : ''}
                                    </li>
                                `).join('')}
                    ...
                    `).join('')}
                </div>` : '', heavenlyMandateBoard.length > 0 ? `<div class="collection-detail-grid"
                    ...
                    ${heavenlyMandateFocusTask ? `
                        <section class="collection-detail-card"
                            ...
                            <ul class="collection-detail-list compact">
                                ${[heavenlyMandateFocusTask.hintLine || '', heavenlyMandateFocusTask.statusLine || '', heavenlyMandateFocusTask.anchorSection ? `去向：${heavenlyMandateFocusActionMeta.targetLabel || heavenlyMandateFocusTask.anchorSection}` : ''].filter(Boolean).map(line => `<li>${escapeHtml(line)}</li>`).join('')}
                            </ul>
                            <button type="button" class="collection-inline-btn"
                                data-heavenly-mandate-focus-action="true"
                                data-heavenly-mandate-focus-task-id="${escapeHtml(heavenlyMandateFocusTask.id || '')}"
                                data-collection-action="follow-heavenly-mandate-task"
                                data-heavenly-mandate-task-id="${escapeHtml(heavenlyMandateFocusTask.id || '')}"
                                data-heavenly-mandate-task-anchor-section="${escapeHtml(heavenlyMandateFocusTask.anchorSection || '')}"
                                data-heavenly-mandate-task-action-type="${escapeHtml(heavenlyMandateFocusTask.actionType || '')}"
                                data-heavenly-mandate-task-action-value="${escapeHtml(heavenlyMandateFocusTask.actionValue || '')}">${escapeHtml(heavenlyMandateFocusTask.ctaLabel || heavenlyMandate.ctaLabel || '前往推进')}</button>
                        </section>
                    ` : ''}
      ...
      anchorSection: ''
                                    ...
                                    <li
                                        ...
                                        <button type="button" class="collection-inline-btn secondary"
                                            ...
                                            data-collection-action="follow-heavenly-mandate-task"
                                            data-heavenly-mandate-task-id="${escapeHtml(task.id)}"
                                            data-heavenly-mandate-task-anchor-section="${escapeHtml(task.anchorSection || '')}"
                                            data-heavenly-mandate-task-action-type="${escapeHtml(task.actionType || '')}"
                                            data-heavenly-mandate-task-action-value="${escapeHtml(task.actionValue || '')}">${escapeHtml(task.ctaLabel || (task.completed ? '沿此复核' : '前往推进'))}</button>
                                    </li>
                                `).join('')}
                            </ul>
                        </section>
    ...
    progressCard.innerHTML = ['<span class="codex-side-kicker">图鉴进度</span>', '<h3>收藏总览</h3>', '<ul class="codex-side-list compact">', `<li>法则：${data.progress.collectedLaws} / ${data.progress.totalLaws}</li>`, `<li>法宝：${data.progress.collectedTreasures} / ${data.progress.totalTreasures}</li>`, `<li>炼器研究：核心件 ${data.progress.forgeCoreOwned || 0} / ${data.progress.forgeCoreTotal || 0} · 形态件 ${data.progress.forgeFormOwned || 0} / ${data.progress.forgeFormTotal || 0}</li>`, `<li>套装共鸣：${data.progress.forgeResonantSets || 0} 组达到二段阈值 / ${data.progress.forgeFullSets || 0} 组达到三段共鸣</li>`, `<li>炼器铭刻：重铸 ${data.progress.forgeReforges || 0} / 器灵 ${data.progress.forgeInfusions || 0} / 套装修正 ${data.progress.forgeSetEchoes || 0}</li>`, `<li>灵契线索：${data.progress.seenSpirits} / ${data.progress.totalSpirits}</li>`, `<li>敌影档案：${data.progress.seenEnemies} / ${data.progress.totalEnemies}</li>`, `<li>Boss 档案：${data.progress.defeatedBosses} / ${data.progress.totalBosses}</li>`, `<li>伏魔台记忆战：${data.progress.clearedBossMemories || 0} 次留痕 / ${data.progress.totalBossMemoryAttempts || 0} 次试作</li>`, `<li>命途碑廊：${data.progress.completedRunPaths || 0} / ${data.progress.totalRunPaths || 0} 条命途留痕 · 累计 ${data.progress.totalRunPathClears || 0} 次圆满</li>`, `<li>样本对照：${data.progress.runPathBossSampleCount || 0} 份实战样本 · 涉及 ${data.progress.sampledCharacters || 0} 名角色 / ${data.progress.sampledBosses || 0} 位主宰</li>`, `${data.progress.runSlateArchives !== undefined ? `<li>归卷书架：${data.progress.runSlateArchives || 0} 份章节答卷 · 最高评分 ${data.progress.topRunSlateScore || 0}</li>` : ''}`, `${data.progress.observatoryTraces !== undefined ? `<li>观星留痕：${data.progress.observatoryTraces || 0} 条归档 / ${data.progress.observatoryReplays || 0} 次回放</li>` : ''}`, `<li>洞府议程：${data.progress.sanctumAgendaCompleted || 0} 次结题 / ${data.progress.sanctumAgendaFailed || 0} 次未成</li>`, `${heavenlyMandate.available ? `<li data-heavenly-mandate-progress-row="true">天道敕令：${escapeHtml(heavenlyMandate.weekLabel || heavenlyMandate.weekTag || '本周轮转')} · ${escapeHtml(heavenlyMandate.themeLabel || heavenlyMandate.directiveName || '待启敕令')} · ${escapeHtml(heavenlyMandate.progressText || '待同步')}</li>` : ''}`, `${seasonBoard.available ? `<li data-season-board-progress-row="true">赛季天道盘：${escapeHtml(seasonBoard.weekLabel || seasonBoard.weekTag || '本周轮转')} · ${escapeHtml(seasonBoard.phaseLabel || '采样期')} · ${escapeHtml(seasonBoard.progressText || '待同步')}</li>` : ''}`, `${seasonSettlement ? `<li data-season-board-progress-settlement="true">季押卷：${escapeHtml(seasonSettlement.outcomeLabel || '待押卷')} · ${escapeHtml(seasonSettlement.progressText || seasonSettlement.statusLine || seasonSettlement.summaryLine || '待裁定')}</li>` : ''}`, `${lineage?.available ? `<li data-fate-lineage-progress-row="true">命盘谱系：角色 ${escapeHtml(data.progress.lineageCharacters || 0)} / 流派 ${escapeHtml(data.progress.lineageStyles || 0)} / 节点 ${escapeHtml(data.progress.lineageNodes || 0)} / 研究 ${escapeHtml(data.progress.lineageResearchHistory || 0)} / 会审 ${escapeHtml(data.progress.lineageFrontierResolutions || 0)}</li>` : ''}`, `${aftereffects?.available ? `<li data-fate-aftereffect-progress-row="true">界痕后效：生效 ${escapeHtml(data.progress.fateAftereffectActive || 0)} / 待生效 ${escapeHtml(data.progress.fateAftereffectPending || 0)} · ${escapeHtml(primaryAftereffect?.templateLabel || primaryAftereffect?.name || '跨章偏置')}</li>` : ''}`, '</ul>'].join('');
    guide.innerHTML = ['<span class="codex-side-kicker">使用建议</span>', '<h3>洞府怎么喂主线</h3>', '<ul class="codex-side-list compact">', activeAgenda ? `<li>当前议程：${escapeHtml(activeAgenda.name)} · ${escapeHtml(activeAgenda.sourceLine || activeAgenda.themeLabel || '主练样本')}。</li>` : lastAgenda ? `<li>最近结题：${escapeHtml(lastAgenda.name)} · ${escapeHtml(lastAgenda.outcomeLabel || '研究留痕')}，${escapeHtml(lastAgenda.recoveryLine || lastAgenda.grantedLine || lastAgenda.reasonLine || '可回归卷书架继续校卷。')}</li>` : '<li>若还没有立项，先从归卷书架挑一份答卷设为主练，再回洞府选一个本轮议程。</li>', activeAgenda?.phaseLabel ? `<li>当前阶段：${escapeHtml(activeAgenda.phaseLabel)} · ${escapeHtml(activeAgenda.phaseLine || activeAgenda.summaryLine || '本轮研究正在推进。')}</li>` : '', activeAgenda?.decisionState === 'pending' ? `<li>章中处置：${escapeHtml(activeAgenda.decisionPromptLine || '已解锁一轮议程处置，可在研究列表里二选一。')}</li>` : '', activeAgenda?.contractState === 'pending' ? `<li>锁线契约：${escapeHtml(activeAgenda.contractPromptLine || '已解锁一条锁线契约，可回研究列表里补签 bonus 条件。')}</li>` : '', activeAgenda?.selectedDecisionLabel ? `<li>当前处置：${escapeHtml(activeAgenda.selectedDecisionLabel)} · ${escapeHtml(activeAgenda.selectedDecisionLine || '本章研究条件已按所选处置更新。')}</li>` : '', activeAgenda?.selectedContractLabel ? `<li>当前契约：${escapeHtml(activeAgenda.selectedContractLabel)} · ${escapeHtml(activeAgenda.selectedContractLine || '本章已追加一条 bonus 锁线条件。')}</li>` : '', activeAgenda?.selectedContractLabel && activeAgenda?.contractSignCostLine ? `<li>契押代价：${escapeHtml(activeAgenda.contractSignCostLine)}。</li>` : '', activeAgenda?.selectedContractLabel && activeAgenda?.contractBurdenLine ? `<li>契约负担：${escapeHtml(activeAgenda.contractBurdenLine)}</li>` : '', activeAgenda ? `<li>${escapeHtml(activeAgenda.focusNodeLine || '优先节点暂未锁定，先从观星与归卷线补出一条主轴。')}</li>` : '', activeAgenda ? `<li>结题门槛：${escapeHtml(activeAgenda.successLine || '命中关键节点并把答卷维持在贴题以上。')}</li>` : '', heavenlyMandate.available ? `<li data-heavenly-mandate-guide="overview">天道敕令：${escapeHtml(heavenlyMandate.weekTag || heavenlyMandate.weekLabel || '本周轮转')} 当前轮转「${escapeHtml(heavenlyMandate.directiveName || '待启敕令')}」，${escapeHtml(heavenlyMandate.detailLine || '本周题面正在整理。')}</li>` : '', heavenlyMandate.guideLine ? `<li data-heavenly-mandate-guide="goal">${escapeHtml(heavenlyMandate.guideLine)}</li>` : '', seasonBoard.available ? `<li data-season-board-guide="overview">赛季天道盘：${escapeHtml(seasonBoard.summaryLine || '赛季主轴正在同步。')}${seasonBoard.guideLine ? ` · ${escapeHtml(seasonBoard.guideLine)}` : ''}</li>` : '', seasonFrontier ? `<li data-season-board-frontier-guide="true">${escapeHtml(seasonFrontier.guideLine || seasonFrontier.summaryLine || `诸界战线当前主压在 ${seasonFrontier.primaryFrontLabel || '主战线'}。`)}</li>` : '', seasonFrontierDecree ? `<li data-season-board-frontier-decree-guide="true">${escapeHtml([seasonFrontierDecree.title || '本周法旨', seasonFrontierDecree.successLine || seasonFrontierDecree.riskLine || '优先补主战线一格。'].filter(Boolean).join('：'))}</li>` : '', seasonFrontierChronicle ? `<li data-season-board-frontier-chronicle-guide="true">${escapeHtml([seasonFrontierChronicle.title || '战役史卷', seasonFrontierChronicle.nextRecordLine || seasonFrontierChronicle.lessonLine || '完成主战线后回季盘复核史卷。'].filter(Boolean).join('：'))}</li>` : '', seasonFrontierChronicleArchive ? `<li data-season-board-frontier-chronicle-archive-guide="true">${escapeHtml(['战役史卷回看', seasonFrontierChronicleArchive.summaryLine || seasonFrontierChronicleArchive.detailLine || '多周会审裁记会在这里累计成长期风格。'].filter(Boolean).join('：'))}</li>` : '', seasonFrontierCouncil ? `<li data-season-board-frontier-council-guide="true">${escapeHtml([seasonFrontierCouncil.title || '诸界会审', seasonFrontierCouncil.verdictLine || seasonFrontierCouncil.supportLine || '先守主线，副线保留证据。'].filter(Boolean).join('：'))}</li>` : '', seasonFrontierResolution ? `<li data-season-board-frontier-resolution-guide="true">${escapeHtml([seasonFrontierResolution.submitted ? '会审裁记已封' : '会审裁记待封', seasonFrontierResolution.submitted ? seasonFrontierResolution.councilResolutionLine || seasonFrontierResolution.summaryLine || '' : seasonFrontierResolution.summaryLine || `建议先封记「${seasonFrontierResolution.suggestedChoiceLabel || seasonFrontierResolutionChoiceLabels[seasonFrontierResolutionSuggestedChoiceId] || '守主战线'}」。`].filter(Boolean).join('：'))}</li>` : '', seasonSettlement ? `<li data-season-board-guide="settlement">${escapeHtml(`季押卷：${seasonSettlement.summaryLine || seasonSettlement.detailLine || seasonSettlement.guideLine || seasonSettlement.outcomeLabel || '等待裁定同步。'}`)}</li>` : '', seasonDebtPack ? `<li data-season-board-guide="debt">${escapeHtml(`债账包：${seasonDebtPack.guideLine || seasonDebtPack.summaryLine || seasonDebtPack.progressText || '优先把欠卷清回可验证状态。'}`)}</li>` : '', primarySeasonVerification ? `<li data-season-board-guide="verification">${escapeHtml(`结业验证：${primarySeasonVerification.summaryLine || primarySeasonVerification.hintLine || primarySeasonVerification.statusLine || primarySeasonVerification.label || '等待验证同步。'}`)}</li>` : '', lineage?.available ? `<li data-fate-lineage-guide="overview">${escapeHtml(lineage.detailLine || lineage.guideLine || lineage.summaryLine || '命盘谱系正在同步。')}</li>` : '', aftereffects?.available ? `<li data-fate-aftereffect-guide="overview">${escapeHtml(aftereffects.guideLine || aftereffects.currentStatusLine || aftereffects.detailLine || '界痕后效正在同步。')}</li>` : '', ...seasonBoardLanes.map(lane => `<li data-season-board-guide-lane="${escapeHtml(lane.id)}">${escapeHtml(`${lane.label}：${lane.summaryLine || `当前已完成 ${lane.completedCount}/${lane.totalCount}`}。`)}</li>`), ...(lineage?.tracks || []).map(track => `<li data-fate-lineage-track="${escapeHtml(track.id || '')}">${escapeHtml(`${track.label || '谱系'}：${track.summaryLine || track.progressText || '等待留痕。'}`)}</li>`), ...heavenlyMandateBoard.slice(0, 3).map(lane => `<li data-heavenly-mandate-guide-lane="${escapeHtml(lane.id)}">${escapeHtml(`${lane.label}：${lane.summaryLine || `当前已完成 ${lane.completedCount}/${lane.totalCount}`}。`)}</li>`), ...(aftereffects?.records || []).slice(0, 2).map(entry => `<li data-fate-aftereffect-track="${escapeHtml(entry.recordId || '')}">${escapeHtml(`${entry.name || entry.templateLabel || '界痕'}：${entry.statusLine || entry.summaryLine || entry.positiveLine || '后效已登记。'}`)}</li>`), activeAgenda?.selectedContractLabel ? `<li>契约奖赏：${escapeHtml(activeAgenda.contractBonusLine || '若锁线条件兑现，会额外结算一笔契约奖赏。')}</li>` : '', !activeAgenda && lastAgenda?.recoveryEligible ? `<li>失败回收：${escapeHtml(lastAgenda.recoveryLine || '本轮未能结题，但洞府已回收一部分残卷。')}</li>` : '', !activeAgenda && lastAgenda?.recoveryHintLine ? `<li>${escapeHtml(lastAgenda.recoveryHintLine)}</li>` : '', activeAgenda ? `<li>风险提醒：${escapeHtml(activeAgenda.failureLine || '若路线偏题或章节折损，本轮研究不会结成结构奖励。')}</li>` : '', '<li>先从可领取目标拿到即时收益，再回到章节或 Boss 档案定路线。</li>', '<li>研究项全部偏“解锁信息与入口”，不直接堆数值，方便后续继续扩系统。</li>', '<li>样本对照榜会把角色、命途裂变和 Boss 收官轮次压在一起，适合开局前先找一份稳定模板。</li>', '<li>命途碑廊会把圆满后的命途样本长期保存下来，适合拿来决定下一轮该追哪条主线、补哪组套装、怎么读 Boss。</li>', `${data.progress.runSlateArchives !== undefined ? '<li>归卷书架会长期保存章节答卷与训练建议，适合先挑一份高分卷设为当前主练，再回观星台继续筛留痕。</li>' : ''}`, selectedGuide ? `<li>当前精选命盘：${escapeHtml(selectedGuide.title)} · ${escapeHtml(selectedGuide.themeLabel || '观星样本')}，它会作为洞府里最值得先复刻的一份命盘档案。</li>` : '<li>若还没有锁定当前精选命盘，先去观星台选一份答卷，再回来把它当作洞府里的主参考样本。</li>', '<li>炼器室现在会标出核心件、形态件与器灵灌注资格，适合先在图鉴里定研究目标，再决定路线要去商店、精英还是事件。</li>', '<li>伏魔台的记忆战更适合拿来检验“我是否真的读懂了 Boss 出题”，而不是单纯比一次输赢。</li>', `${data.progress.observatoryTraces !== undefined ? '<li>观星台现在会沉淀命盘签和留痕，适合把高分轮换或好用命盘重新回放验证。</li>' : ''}`, '</ul>'].join('');
  };
  collectionHubMethods.renderCollectionHub = function () {
  ...
  collectionHubMethods.bindSeasonBoardDelegates = function () {
    if (typeof document === 'undefined' || this.__seasonBoardDelegatesBound) return;
    const root = document.querySelector('#collection-screen') || document.querySelector('#collection');
    if (!root) return;
    root.addEventListener('click', event => {
      const target = event.target;
      if (!target || typeof target.closest !== 'function') return;
      const codexEntryBtn = target.closest('[data-collection-action="select-codex-entry"]');
      if (codexEntryBtn) {
        const targetType = String(codexEntryBtn.dataset.collectionTarget || '');
        const entryId = String(codexEntryBtn.dataset.entryId || '');
        if (!entryId) return;
        if (targetType === 'spirit' && typeof this.selectSpiritCodexEntry === 'function') {
          this.selectSpiritCodexEntry(entryId);
          return;
        }
        if (targetType === 'chapter' && typeof this.selectChapterCodexEntry === 'function') {
          this.selectChapterCodexEntry(entryId);
          return;
        }
        if (targetType === 'enemy' && typeof this.selectEnemyCodexEntry === 'function') {
          this.selectEnemyCodexEntry(entryId);
          return;
        }
        if (targetType === 'boss' && typeof this.selectBossArchiveEntry === 'function') {
          this.selectBossArchiveEntry(entryId);
          return;
        }
      }
      const collectionActionBtn = target.closest('[data-collection-action]');
      if (collectionActionBtn && !collectionActionBtn.disabled) {
        const action = String(collectionActionBtn.dataset.collectionAction || '');
        if (action === 'start-boss-memory-battle') {
          const bossId = String(collectionActionBtn.dataset.bossId || '');
          if (bossId && typeof this.startBossMemoryBattle === 'function') {
            this.startBossMemoryBattle(bossId);
            return;
          }
        }
        if (action === 'switch-section') {
          const section = String(collectionActionBtn.dataset.collectionSection || '');
          if (section && typeof this.switchCollectionSection === 'function') {
            this.switchCollectionSection(section);
            return;
          }
        }
        if (action === 'show-treasure-compendium') {
        ...
        if (action === 'claim-achievement') {
          const achievementId = String(collectionActionBtn.dataset.achievementId || '');
          if (achievementId && typeof this.claimAchievement === 'function') {
            this.claimAchievement(achievementId);
            return;
          }
        }
        if (action === 'follow-heavenly-mandate-task') {
          const taskId = String(collectionActionBtn.dataset.heavenlyMandateTaskId || '');
          const taskAnchorSection = String(collectionActionBtn.dataset.heavenlyMandateTaskAnchorSection || '');
          const taskActionType = String(collectionActionBtn.dataset.heavenlyMandateTaskActionType || '');
          const taskActionValue = String(collectionActionBtn.dataset.heavenlyMandateTaskActionValue || '');
          if (taskActionType === 'collection' && typeof this.switchCollectionSection === 'function') {
            this.switchCollectionSection(taskActionValue || taskAnchorSection || 'sanctum');
            return;
          }
          if (taskActionType === 'challenge' && typeof this.showChallengeHub === 'function') {
            this.showChallengeHub(taskActionValue || 'weekly');
            return;
          }
          ...
          if (taskAnchorSection && typeof this.jumpToHeavenlyMandateAnchor === 'function') {
            this.jumpToHeavenlyMandateAnchor(taskAnchorSection, {
              fallbackSection: 'sanctum',
              taskId
            });
            return;
          }
          if (taskId && typeof this.followHeavenlyMandateTask === 'function') {
            this.followHeavenlyMandateTask(taskId);
            return;
          }
        }
      }
      const runSlateActionBtn = target.closest('[data-run-slate-action]');
      if (runSlateActionBtn) {
        const runId = String(runSlateActionBtn.dataset.runSlateId || '');
        const action = String(runSlateActionBtn.dataset.runSlateAction || '');
        if (!runId) return;
        if (action === 'select' && typeof this.selectRunSlateShelfEntry === 'function') {
          this.selectRunSlateShelfEntry(runId);
          return;
        }
        if (action === 'train-focus' && !runSlateActionBtn.disabled && typeof this.applyRunSlateShelfTrainingFocus === 'function') {
          this.applyRunSlateShelfTrainingFocus(runId);
          return;
        }
        if (action === 'review-observatory' && !runSlateActionBtn.disabled && typeof this.reviewRunSlateInObservatory === 'function') {
          this.reviewRunSlateInObservatory(runId);
          return;
        }
      }
      const arrivalBtn = target.closest('[data-season-board-task-arrival-focus="true"]');
      if (arrivalBtn) {
        this.focusSeasonBoardTaskFollowArrival();
        return;
      }
      const chapterArcFollowBtn = target.closest('[data-season-board-chapter-arc-follow="true"]');
      if (chapterArcFollowBtn) {
        const taskId = String(chapterArcFollowBtn.dataset.seasonBoardChapterArcFollowTaskId || '');
        if (taskId) {
          this.followSeasonBoardTask(taskId);
        }
        return;
      }
      const frontierResolutionBtn = target.closest('[data-season-board-frontier-resolution-choice="true"]');
      if (frontierResolutionBtn) {
        const choiceId = String(frontierResolutionBtn.dataset.seasonBoardFrontierResolutionChoiceId || '');
        if (choiceId) {
          this.commitSeasonBoardFrontierResolution(choiceId);
        }
        return;
      }
      const archiveActionBtn = target.closest('[data-season-board-archive-action="true"]');
      if (archiveActionBtn) {
        const entry = archiveActionBtn.closest('[data-season-board-archive-entry="true"]');
        const recordId = String(entry?.dataset?.seasonBoardArchiveRecordId || '');
        if (recordId) {
          this.followSeasonVerificationRecord(recordId);
        }
        return;
      }
      const laneRewardBtn = target.closest('[data-season-board-lane-reward-claim="true"]');
      if (laneRewardBtn) {
      ...
      const taskActionBtn = target.closest('[data-season-board-task-action="true"]');
      if (taskActionBtn) {
        const taskId = String(taskActionBtn.dataset.seasonBoardTaskActionId || '');
        if (taskId) {
          this.followSeasonBoardTask(taskId);
          return;
        }
      }
      const agendaActivateBtn = target.closest('[data-sanctum-agenda-activate="true"]');
      if (agendaActivateBtn) {
        const agendaId = String(agendaActivateBtn.dataset.sanctumAgendaId || '');
        if (agendaId && typeof this.activateSanctumAgenda === 'function') {
          this.activateSanctumAgenda(agendaId);
        }
        return;
      }
      const agendaDecisionBtn = target.closest('[data-sanctum-agenda-decision="true"]');
      if (agendaDecisionBtn) {
        const decisionId = String(agendaDecisionBtn.dataset.sanctumAgendaDecisionId || '');
        if (decisionId && typeof this.chooseSanctumAgendaDecision === 'function') {
          this.chooseSanctumAgendaDecision(decisionId);
        }
        return;
      }
      const agendaContractBtn = target.closest('[data-sanctum-agenda-contract="true"]');
      if (agendaContractBtn) {
        const contractId = String(agendaContractBtn.dataset.sanctumAgendaContractId || '');
        if (contractId && typeof this.chooseSanctumAgendaContract === 'function') {
          this.chooseSanctumAgendaContract(contractId);
        }
      }
    });
    this.__seasonBoardDelegatesBound = true;
  };
  collectionHubMethods.initCollection = function () {
    this.ensureCollectionHubBootState();
    if (typeof originalInitCollection === 'function') {
      originalInitCollection.call(this);
    }
    this.renderCollectionHub();
    this.bindSeasonBoardDelegates();
  };
  if (typeof Player !== 'undefined') {
    let originalSetSpiritCompanion = Player.prototype.setSpiritCompanion;
    let originalCollectLaw = Player.prototype.collectLaw;
    let originalAddTreasure = Player.prototype.addTreasure;
  ...
  if (typeof AchievementSystem !== 'undefined') {
    let originalUnlockAchievement = AchievementSystem.prototype.unlockAchievement;
  ...
  collectionHubMethods.startBattle = function (enemies, node = null) {
    const enemyList = Array.isArray(enemies) ? enemies : [enemies];
    if (typeof this.recordCollectionUnlock === 'function') {
      enemyList.filter(enemy => enemy && enemy.id && !enemy.isBoss && !enemy.isMinion).forEach(enemy => {
    ...
    return typeof originalStartBattle === 'function' ? originalStartBattle.call(this, enemies, node) : undefined;
  };
  collectionHubMethods.onBattleWon = async function (enemies) {
    ...
    return typeof originalOnBattleWon === 'function' ? originalOnBattleWon.call(this, enemies) : undefined;
  };
  collectionHubMethods.onBattleLost = async function () {
    if (this.currentBattleNode && this.currentBattleNode.type === 'boss_memory') {
  ...
  collectionHubMethods.handleBossDefeated = async function (bossEnemy = null, enemyList = [], ringExp = 0) {
    const isBossMemoryBattle = this.currentBattleNode?.type === 'boss_memory';
    const liveRunPathMeta = !isBossMemoryBattle && this.player && typeof this.player.getRunPathMeta === 'function' ? this.player.getRunPathMeta() : null;
    const liveTurnCount = !isBossMemoryBattle ? clampInt(this.battle?.turnNumber || 0, 0, 9999) : 0;
    ...
    if (!isBossMemoryBattle && bossEnemy && typeof this.recordCollectionUnlock === 'function') {
      const chapter = typeof this.getChapterProfileForRealm === 'function' ? this.getChapterProfileForRealm(bossEnemy.realm || this.player?.realm || 1) : null;
  ...
  collectionHubMethods.onRealmComplete = function () {
    const clearedRealm = clampInt(this.player?.realm || 1, 1, 18);
    const shouldRecordChapter = clearedRealm % 3 === 0;
    const chapter = shouldRecordChapter && typeof this.getChapterProfileForRealm === 'function' ? this.getChapterProfileForRealm(clearedRealm) : null;
...
const rebindCollectionHubOriginals = game => {
  if (game && typeof game.showCollection === 'function') originalShowCollection = game.showCollection;
  if (game && typeof game.initCollection === 'function') originalInitCollection = game.initCollection;
  ...
  if (game && typeof game.handleBossDefeated === 'function') originalHandleBossDefeated = game.handleBossDefeated;
  if (game && typeof game.onRealmComplete === 'function') originalOnRealmComplete = game.onRealmComplete;
};
const bindCollectionHubFacade = (controller, game) => {
  Object.entries(collectionHubMethods).forEach(([name, fn]) => {
...
class CollectionHubController {
  constructor(game) {
    this.game = game;
    rebindCollectionHubOriginals(game);
    bindCollectionHubFacade(this, game);
  }
}
function attachCollectionHubController(game) {
...
if (collectionHubGlobal) {
  collectionHubGlobal.__attachCollectionHubController = attachCollectionHubController;
}
return {
  CollectionHubController,
  attachCollectionHubController
};
})();
export const CollectionHubController = CollectionHubModule.CollectionHubController;
export const attachCollectionHubController = CollectionHubModule.attachCollectionHubController;
