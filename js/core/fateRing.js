...
export class FateRing {
  ...
  initSlots() {
    ...
    const existing = Array.isArray(this.slots) ? this.slots.map(s => ({
      law: s.law,
      unlocked: s.unlocked,
      type: s.type
    })) : [];
  ...
  levelUp(levelsGained = 1) {
    ...
    if (FATE_RING.levels[this.level]) {
      ...
      if (levelData.slots > this.maxSlots) {
        const oldSlots = Array.isArray(this.slots) ? this.slots.map(slot => ({
          ...(slot || {})
        })) : [];
  ...
  loadFromJSON(data) {
    ...
    if (Array.isArray(data.slots)) {
      const safeSlots = data.slots.map(slot => ({
        ...(slot || {})
      }));
...
// 林风：变异命环
export class MutatedRing extends FateRing {
  constructor(player) {
...
export class SealedRing extends FateRing {
  constructor(player) {
    super(player, 'sealed');
    this.name = '玉十二环·封印';
...
export class KarmaRing extends FateRing {
  constructor(player) {
    super(player, 'karma');
    this.name = '功德金轮';
...
export class AnalysisRing extends FateRing {
  constructor(player) {
