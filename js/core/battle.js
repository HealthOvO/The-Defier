import { BOSS_MECHANICS, BossMechanicsHandler } from "../data/boss_mechanics.js";
import { TREASURES } from "../data/treasures.js";
import { Utils } from "./utils.js";
import { cloneCardTemplate, inferDeckArchetype } from "../data/cards.js";
import { CARDS, ENEMY_ECOLOGY_TEMPLATES, CHAPTER_ELITE_COMBOS, ENEMIES, CHARACTERS } from "../data/index.js";
...
export class Battle {
  ...
  createBossThreeActState(enemy) {
    ...
    const mech = typeof BOSS_MECHANICS !== 'undefined' && enemy.id && BOSS_MECHANICS[enemy.id] ? BOSS_MECHANICS[enemy.id] : null;
    const memory = this.getBossThreeActMemoryProfile(enemy);
    const counterTreasure = Array.isArray(mech?.countersBy) && typeof TREASURES !== 'undefined' ? mech.countersBy.map(id => TREASURES[id]?.name || id).slice(0, 2).join(' / ') : '';
  ...
  resolveRunPathBossCounterplay(enemy, mech, memory, runPathMeta) {
    ...
    if (this.game && typeof this.game.resolveRunPathBossMatchup === 'function') {
      const resolved = this.game.resolveRunPathBossMatchup(runPathMeta, {
        ...
        memory,
        memoryKey: memory?.key,
        chapterSnapshot: this.game && typeof this.game.getChapterDisplaySnapshot === 'function' ? this.game.getChapterDisplaySnapshot(enemy?.realm || this.player?.realm || this.game?.player?.realm || 1) : null,
        chapterBattlefield: typeof this.getChapterBattlefieldDisplayState === 'function' ? this.getChapterBattlefieldDisplayState() : null
      });
      if (resolved) return resolved;
    }
    const matchupCatalog = runPathMeta.bossMatchups && typeof runPathMeta.bossMatchups === 'object' ? runPathMeta.bossMatchups : null;
    const chapterSnapshot = this.game && typeof this.game.getChapterDisplaySnapshot === 'function' ? this.game.getChapterDisplaySnapshot(enemy?.realm || this.player?.realm || this.game?.player?.realm || 1) : null;
    const chapterField = typeof this.getChapterBattlefieldDisplayState === 'function' ? this.getChapterBattlefieldDisplayState() : null;
    ...
    const chapterMatchup = matchupCatalog?.chapters?.[chapterSnapshot?.id || ''] && typeof matchupCatalog.chapters[chapterSnapshot?.id || ''] === 'object' ? matchupCatalog.chapters[chapterSnapshot.id] : null;
    const resolved = {
      id: runPathMeta.id,
    ...
    resolved.chapterCue = [resolved.chapterName, chapterField?.omen?.phaseLabel || chapterSnapshot?.skyOmen?.name || '', chapterField?.leyline?.activeLabel || chapterSnapshot?.leyline?.name || ''].filter(Boolean).join(' · ');
  ...
  resolveBossActCounterChips(displayState) {
    ...
    if (state.runPathCounterplay) {
      const runPathTip = [state.runPathCounterplay.fitLabel ? `适配评级：${state.runPathCounterplay.fitLabel}` : '', state.runPathCounterplay.chapterCue ? `章节场域：${state.runPathCounterplay.chapterCue}` : '', state.runPathCounterplay.chapterFocus ? `章节补题：${state.runPathCounterplay.chapterFocus}` : '', state.runPathCounterplay.chapterCounter ? `场域拆法：${state.runPathCounterplay.chapterCounter}` : '', state.runPathCounterplay.focus, state.runPathCounterplay.counter, state.runPathCounterplay.reward].filter(Boolean).join(' ');
  ...
  shouldUseCompactBattleHud() {
    if (typeof DefierBattleHud !== 'undefined' && DefierBattleHud && typeof DefierBattleHud.shouldUseCompactBattleHud === 'function') {
      return DefierBattleHud.shouldUseCompactBattleHud(typeof window !== 'undefined' ? window.innerWidth : 0);
    }
  ...
  updateBossActUI() {
    ...
    panel.innerHTML = DefierBattleHud.buildBossActPanelMarkup({
      bossName: boss.name,
      hpPercent: hpPercent * 100,
      currentActName: act.name,
      currentIndex: index,
      acts: state.acts,
      counterChips,
      lines: [{
        ...
        value: [state.runPathCounterplay.fitLabel ? `适配评级：${state.runPathCounterplay.fitLabel}` : '', state.runPathCounterplay.chapterCue ? `章节场域：${state.runPathCounterplay.chapterCue}` : '', state.runPathCounterplay.chapterFocus ? `章节补题：${state.runPathCounterplay.chapterFocus}` : '', state.runPathCounterplay.chapterCounter ? `场域拆法：${state.runPathCounterplay.chapterCounter}` : '', state.runPathCounterplay.focus, state.runPathCounterplay.counter, state.runPathCounterplay.reward ? `收益：${state.runPathCounterplay.reward}` : ''].filter(Boolean).join(' ｜ ')
      }] : []), {
        id: 'fail',
        label: '失败原因',
        value: act.fail
  ...
  applyBossActTransition(enemy, nextIndex) {
    ...
    if (Array.isArray(act.patterns) && act.patterns.length > 0) {
      enemy.patterns = act.patterns.map(pattern => ({
        ...pattern
      }));
  ...
  checkBossThreeActTransition(enemy) {
    ...
    while (state.currentActIndex + 1 < state.acts.length && hpPercent <= Number(state.acts[state.currentActIndex + 1].threshold)) {
  ...
  async processElementalCombos(target, targetIndex) {
    ...
    const combo = [this.elementalTracker[len - 3], this.elementalTracker[len - 2], this.elementalTracker[len - 1]].map(Utils.getCanonicalElement).join('+');
  ...
  getEnemyVariationBlueprint(enemyData, patterns = [], maxHp = 1) {
    ...
    const attackValues = (Array.isArray(patterns) ? patterns : []).filter(pattern => pattern && typeof pattern === 'object' && Number.isFinite(Number(pattern.value)) && (pattern.type === 'attack' || pattern.type === 'multiAttack' || pattern.type === 'executeDamage')).map(pattern => Math.max(1, Math.floor(Number(pattern.value) || 1)));
    const avgAttack = attackValues.length > 0 ? Math.max(2, Math.floor(attackValues.reduce((sum, val) => sum + val, 0) / attackValues.length)) : Math.max(2, Math.floor(realm * 0.75 + 2));
    ...
    const variantPool = {
      early: [{
        ...
        appendPatterns: [{
          type: 'defend',
          value: Math.max(6, Math.floor(avgAttack * 1.2)),
          intent: '🛡️固守'
        }]
      }, {
        id: 'hex_nudge',
      ...
      mid: [{
        ...
        appendPatterns: [{
          type: 'multiAttack',
          value: Math.max(3, Math.floor(avgAttack * 0.76)),
          count: 2,
          intent: '⚔️裂阵连击'
        }]
      }, {
        ...
        appendPatterns: [{
          type: 'defend',
          value: Math.max(8, Math.floor(avgAttack * 1.3)),
          intent: '🧿稳压护体'
        }]
      }, {
        id: 'siphon_curse',
      ...
      late: [{
        ...
        appendPatterns: [{
          type: 'heal',
          value: Math.max(12, Math.floor(Math.max(1, maxHp) * 0.1)),
          intent: '🌊回潮修复'
        }]
      }]
    };
    const pool = variantPool[tier] || variantPool.early;
    const candidates = pool.filter(variant => Array.isArray(variant.roles) && (variant.roles.includes(archetype) || variant.roles.includes('any')));
    const source = candidates.length > 0 ? candidates : pool;
    const pick = source[seed % source.length];
    if (!pick) return null;
    const appendPatterns = (pick.appendPatterns || []).filter(pattern => !!pattern && typeof pattern === 'object').map(pattern => ({
      ...pattern
    }));
  ...
  resolveChapterEcologyFormation(realm = 1, nodeType = 'enemy', enemies = []) {
    ...
    const templates = typeof ENEMY_ECOLOGY_TEMPLATES !== 'undefined' && ENEMY_ECOLOGY_TEMPLATES ? ENEMY_ECOLOGY_TEMPLATES[chapterIndex] : null;
    ...
    const comboCatalog = typeof CHAPTER_ELITE_COMBOS !== 'undefined' && CHAPTER_ELITE_COMBOS ? CHAPTER_ELITE_COMBOS[chapterIndex] : null;
    const enemyIds = new Set((Array.isArray(enemies) ? enemies : []).map(enemy => String(enemy?.id || '')));
    const comboActive = comboCatalog && Array.isArray(comboCatalog.anchorEnemyIds) ? comboCatalog.anchorEnemyIds.every(enemyId => enemyIds.has(String(enemyId))) : false;
    return {
      ...picked,
      chapterIndex,
  ...
  resolveEnemySquadFormation(enemies = []) {
    const combatants = Array.isArray(enemies) ? enemies.filter(enemy => enemy && enemy.currentHp > 0 && !enemy.isBoss && !enemy.isGhost && !enemy.isMinion) : [];
  ...
  applyEnemySquadEcology() {
    ...
    const combatants = this.enemies.filter(enemy => enemy && enemy.currentHp > 0 && !enemy.isBoss && !enemy.isGhost && !enemy.isMinion && Array.isArray(enemy.patterns));
    if (combatants.length < 2) return;
    const formation = this.resolveEnemySquadFormation(combatants);
    if (!formation) return;
    const seeded = combatants.map(enemy => ({
      enemy,
      seed: this.hashSquadSeed(`${formation.seed}:${enemy.id || enemy.name || 'enemy'}`)
    })).sort((a, b) => a.seed - b.seed).map(entry => entry.enemy);
    ...
    seeded.forEach((enemy, index) => {
      ...
      const attackPatterns = enemy.patterns.filter(pattern => pattern && typeof pattern === 'object' && (pattern.type === 'attack' || pattern.type === 'multiAttack' || pattern.type === 'executeDamage') && Number.isFinite(Number(pattern.value)));
  ...
  resolveEncounterThemeProfile() {
    ...
    const encounterState = this.game && typeof this.game.ensureEncounterState === 'function' ? this.game.ensureEncounterState() : null;
    const lastThemeId = encounterState && typeof encounterState.currentStreakId === 'string' ? encounterState.currentStreakId : '';
    const lastThemeStreak = encounterState ? Math.max(0, Math.floor(Number(encounterState.currentStreak) || 0)) : 0;
    const seedSource = [realm, node.id || 0, node.row || 0, node.col || 0, node.type || 'enemy', this.enemies.length].join(':');
    let seed = 0;
    for (let i = 0; i < seedSource.length; i += 1) {
  ...
  applyEncounterSignatureAffix(enemy, theme, tierStage = 1) {
    ...
    const attackValues = enemy.patterns.filter(pattern => pattern && (pattern.type === 'attack' || pattern.type === 'multiAttack')).map(pattern => Math.max(1, Math.floor(Number(pattern.value) || 1)));
  ...
  applyEncounterAffixPayload(enemy, payload) {
    ...
    if (Array.isArray(payload.appendPatterns) && payload.appendPatterns.length > 0) {
      enemy.patterns.push(...payload.appendPatterns.filter(pattern => pattern && typeof pattern === 'object').map(pattern => ({
        ...pattern
      })));
    }
  }
  applyEncounterThemeProfile(theme) {
    ...
    const tierStage = this.game && typeof this.game.registerEncounterThemeStart === 'function' ? this.game.registerEncounterThemeStart(theme.id) : 1;
    const stage = Math.max(1, Math.min(3, Math.floor(Number(tierStage) || 1)));
  ...
  consumeSquadEcologyVictoryBonusSummary() {
    ...
    const rewardMap = {
      squad_pincer_hunt: {
        ...
        exp: 6,
        buffs: [{
          id: 'firstTurnEnergyBoostBattles',
          charges: 1,
          label: '首回合灵力'
        }]
      },
    ...
    const adventureBuffRewards = Array.isArray(pack.buffs) ? pack.buffs.map(item => ({
      ...item
    })) : [];
    let synergy = null;
    if (this.game && typeof this.game.isEndlessActive === 'function' && this.game.isEndlessActive() && typeof this.game.getEndlessCycleThemeProfile === 'function') {
  ...
  getChapterBattlefieldPlayerTurn() {
    return Math.max(1, Math.floor(Number(this.player?.turnNumber) || Number(this.turnNumber) || 1));
  }
  ...
  getChapterBattlefieldActiveVowCount() {
    ...
    return Array.isArray(this.player?.runVows) ? this.player.runVows.filter(Boolean).length : 0;
  }
  getChapterBattlefieldSynergyState() {
    const destinyMeta = this.player && typeof this.player.getRunDestinyMeta === 'function' ? this.player.getRunDestinyMeta() : null;
    const spiritMeta = this.player && typeof this.player.getSpiritCompanionMeta === 'function' ? this.player.getSpiritCompanionMeta() : null;
    const vowCount = this.getChapterBattlefieldActiveVowCount();
    const lawCount = this.getChapterBattlefieldLoadedLawCount();
    const equippedTreasures = Array.isArray(this.player?.equippedTreasures) ? this.player.equippedTreasures.filter(Boolean) : Array.isArray(this.player?.treasures) ? this.player.treasures.filter(Boolean) : [];
    const treasureCount = equippedTreasures.length;
    const setCounts = this.player && typeof this.player.getEquippedTreasureSetCounts === 'function' ? this.player.getEquippedTreasureSetCounts() : equippedTreasures.reduce((result, treasure) => {
    ...
    const axes = [destinyActive, vowCount > 0, lawCount > 0, treasureCount > 0, spiritActive].filter(Boolean).length;
    return {
      axes,
      destinyActive,
      vowCount,
  ...
  getBattleHudLoadedLawMetas() {
    ...
    return loadedLawIds.filter(Boolean).map(lawId => {
      const meta = typeof LAWS !== 'undefined' && LAWS && LAWS[lawId] ? LAWS[lawId] : null;
      return {
  ...
  getBattleHudActiveLawResonances(loadedLawIds = null) {
    const ids = Array.isArray(loadedLawIds) ? loadedLawIds.filter(Boolean) : this.getBattleHudLoadedLawMetas().map(law => law.id).filter(Boolean);
    const active = [];
    ...
    return Array.isArray(this.player?.activeResonances) ? this.player.activeResonances.filter(Boolean).map(resonance => ({
      id: resonance.id || '',
      name: resonance.name || '',
      description: resonance.description || ''
    })) : [];
  ...
  getBattleHudTreasureSetState() {
    const equippedTreasures = Array.isArray(this.player?.equippedTreasures) ? this.player.equippedTreasures.filter(Boolean) : [];
    const setCounts = this.player && typeof this.player.getEquippedTreasureSetCounts === 'function' ? this.player.getEquippedTreasureSetCounts() : equippedTreasures.reduce((result, treasure) => {
      const setTag = treasure && typeof treasure.setTag === 'string' ? treasure.setTag.trim() : '';
      if (!setTag) return result;
      const setEcho = treasure && treasure.data && treasure.data.workshopSetEcho ? 1 : 0;
      result[setTag] = (result[setTag] || 0) + 1 + setEcho;
      return result;
    }, {});
    const activeSets = Object.entries(setCounts || {}).map(([setTag, pieces]) => {
      const label = this.player && typeof this.player.getTreasureSetLabel === 'function' ? this.player.getTreasureSetLabel(setTag) : setTag;
      const meta = this.player && typeof this.player.getTreasureSetMeta === 'function' ? this.player.getTreasureSetMeta(setTag) : null;
      return {
        id: setTag,
        label,
        icon: meta?.icon || '✦',
    ...
    }).filter(entry => entry.pieces >= 2).sort((a, b) => b.pieces - a.pieces || a.label.localeCompare(b.label, 'zh-Hans-CN'));
    return {
  ...
  getBattleSystemDisplayState() {
    ...
    const vows = this.player && typeof this.player.getRunVowMetas === 'function' ? this.player.getRunVowMetas().filter(Boolean) : [];
    const spirit = this.resolveBattleSpiritCompanionDisplay();
    const chapter = this.game && typeof this.game.getChapterDisplaySnapshot === 'function' ? this.game.getChapterDisplaySnapshot(this.player?.realm || this.game?.player?.realm || 1) : null;
    ...
    const omenValue = chapterField?.omen?.phaseLabel || chapterField?.omen?.name || chapter?.skyOmen?.name || chapter?.stageLabel || chapter?.name || '未定';
    const leylineValue = chapterField?.leyline?.activeLabel || chapterField?.leyline?.name || chapter?.leyline?.name || '未显';
    ...
    const lawMeta = activeResonances.length > 0 ? `${loadedLaws.length} 法则在位${pathMeta?.name ? ` · ${pathMeta.name}` : ''}` : loadedLaws.length > 0 ? `${loadedLaws.length} 法则待编织${pathMeta?.name ? ` · ${pathMeta.name}` : ''}` : pathMeta?.name ? `命环 ${pathMeta.name}` : '命环待刻印';
    const treasureValue = treasure.activeCount > 0 ? `${treasure.activeCount} 组激活` : treasure.equippedCount > 0 ? '套装待成型' : '未装备';
    const treasureMeta = treasure.activeCount > 0 ? treasureNames.slice(0, 2).join(' / ') : treasure.equippedCount > 0 ? `${treasure.equippedCount} 件法宝在位` : '暂无法宝';
    const stripItems = [{
      id: 'runPath',
      label: '命途',
      icon: runPath?.icon || '🧭',
      value: runPath?.name || '未定',
      meta: runPathPhase ? `${runPathPhase.label || `阶段${(runPath?.phaseIndex || 0) + 1}`} · ${runPathProgress}/${runPathTarget}` : '本轮尚未定式',
      detail: runPathPhase ? `${runPathPhase.title || runPath?.name}：${runPathPhase.desc || runPath?.playstyle || runPath?.description || ''}` : '当前没有命途主线。',
      ...
      tone: destiny ? destiny.category === '赌博' ? 'warning' : 'fate' : 'muted'
    }, {
      id: 'vows',
      label: '誓约',
      icon: vows[0]?.icon || '✧',
      ...
      value: spiritValue,
      meta: spiritMeta,
      detail: spirit ? `${spirit.passiveLabel || '灵契被动'}：${spirit.passiveDesc || ''}` : '当前没有灵契护道。',
      ...
      value: omenValue,
      meta: leylineValue,
      detail: [chapter?.name ? `${chapter.icon || '☯️'} ${chapter.name}${chapter.stageLabel ? ` · ${chapter.stageLabel}` : ''}` : '', formationValue ? `阵面：${formationValue}${chapterField?.formation?.detail ? ` · ${chapterField.formation.detail}` : ''}` : ''].filter(Boolean).join(' / '),
      ...
      value: lawValue,
      meta: lawMeta,
      detail: [resonanceNames.length > 0 ? `共鸣：${resonanceNames.join(' / ')}` : '', pathMeta?.name ? `命环：${pathMeta.icon || '◈'} ${pathMeta.name}` : '', loadedLawNames.length > 0 ? `法则：${loadedLawNames.join(' / ')}` : ''].filter(Boolean).join(' / '),
      ...
      value: treasureValue,
      meta: treasureMeta,
      detail: treasure.activeCount > 0 ? treasure.activeSets.map(entry => `${entry.icon || '✦'} ${entry.label} ${entry.pieces} 件`).join(' / ') : treasure.equippedCount > 0 ? '当前已带入战斗，但尚未达到 2 件共鸣阈值。' : '当前没有已装备法宝。',
  ...
  refreshChapterBattlefieldState() {
    ...
    if (field.chapterIndex === 2) {
      ...
      field.leyline.activeDesc = activeParts.length > 0 ? `当前答卷由 ${activeParts.join(' / ')} 共同支撑；轴数越完整，终章回报越高。` : '当前仍是单轴答卷，终焉命庭会持续追问你缺的那几块拼图。';
      if (field.formation && field.formation.active && !field.formation.broken) {
  ...
  pickChapterFormationAnchor(combatants = [], strategy = 'highestAttack') {
    const fighters = Array.isArray(combatants) ? combatants.filter(enemy => enemy && enemy.currentHp > 0 && Array.isArray(enemy.patterns)) : [];
  ...
  buildChapterFormationState(profile) {
    ...
    const combatants = Array.isArray(this.enemies) ? this.enemies.filter(enemy => enemy && enemy.currentHp > 0 && !enemy.isBoss && !enemy.isGhost && !enemy.isMinion) : [];
    ...
    combatants.forEach(enemy => {
      ...
      enemy.chapterFormationRoleLabel = enemy === anchor ? formationState.anchorRoleLabel : formationState.escortRoleLabel;
  ...
  getChapterFormationAnchorEnemy() {
    ...
    return (this.enemies || []).find(enemy => enemy && enemy.currentHp > 0 && String(enemy.id || enemy.name || '') === String(formation.anchorEnemyId)) || null;
  }
  ...
  updateChapterBattlefieldOmenPhase(options = {}) {
    const field = this.activeChapterBattlefield;
    const resolvePhase = field?.profile?.omen && typeof field.profile.omen.resolvePhase === 'function' ? field.profile.omen.resolvePhase : null;
    ...
    const changed = field.omen.phaseId !== String(phase.id || '') || field.omen.phaseLabel !== String(phase.label || '');
  ...
  applyChapterBattlefieldTurnStart(actor = 'player', options = {}) {
    ...
    if (field.chapterIndex === 1) {
      ...
      if (phase.id === 'smelt_pressure') {
        ...
        const block = 3 + stageIndex;
        const treasureCount = Array.isArray(this.player?.equippedTreasures) ? this.player.equippedTreasures.filter(Boolean).length : 0;
        this.player.addBlock(block);
        if (treasureCount > 0) {
          this.player.gainEnergy(1);
      ...
      if (phase.id === 'blood_rise') {
        const hpLoss = Math.min(Math.max(0, Math.floor(Number(this.player?.currentHp || 0)) - 1), stageIndex >= 3 ? 2 : 1);
        if (hpLoss > 0) {
        ...
        if (hpRatio <= 0.5) {
          ...
          const hpLoss = Math.min(Math.max(0, Math.floor(Number(this.player?.currentHp || 0)) - 1), 2);
          if (hpLoss > 0) {
            this.player.currentHp = Math.max(1, Math.floor(Number(this.player.currentHp) || 1) - hpLoss);
          }
  ...
  applyChapterBattlefieldPlayerDamageModifiers(enemy, amount, sourceElement = null) {
    ...
    if (field.chapterIndex === 1 && hpRatio <= 0.55) {
      ...
      if (Math.max(0, Math.floor(Number(synergy.axes) || 0)) >= 2) {
        const bonus = Math.max(1, Math.floor(amount * (0.03 * Math.max(0, Math.floor(Number(synergy.axes) || 0)))) + Math.min(3, Math.max(0, Math.floor(Number(synergy.lawCount) || 0))));
        amount += bonus;
  ...
  resolveBattleCommandLoadout() {
    ...
    const node = this.game && this.game.currentBattleNode ? this.game.currentBattleNode : null;
    const idSource = (Array.isArray(this.enemies) ? this.enemies : []).map(enemy => String(enemy && (enemy.id || enemy.name) || 'enemy')).join('|');
    const seedSource = [this.player && this.player.realm ? this.player.realm : 1, node ? node.id : 0, node ? node.type : 'battle', idSource].join(':');
    let seed = 0;
    for (let i = 0; i < seedSource.length; i += 1) {
      seed = (seed * 37 + seedSource.charCodeAt(i)) % 2147483647;
    }
    const pool = catalog.map(item => ({
      ...item
    }));
    const loadout = [];
    ...
    if (endlessActive) {
      ...
      const extraEndlessPool = pool.filter(item => item && (item.id === 'phase_anchor_order' || item.id === 'void_pursuit_order' || item.id === 'horizon_barter_order' || item.id === 'resonance_matrix_order'));
  ...
  getBattleCommandPowerScale(command = null) {
    ...
    const encounterStage = this.activeEncounterTheme ? Math.max(1, Math.min(3, Math.floor(Number(this.activeEncounterTheme.tierStage) || 1))) : 1;
    ...
    const resonanceId = String(this.player?.archetypeResonance?.id || '');
    const hpRatio = Number(this.player?.maxHp) > 0 ? (Number(this.player?.currentHp) || 0) / Number(this.player.maxHp) : 1;
    let synergyBonus = 0;
    ...
    const treasureCount = Array.isArray(this.player?.equippedTreasures) ? this.player.equippedTreasures.length : Array.isArray(this.player?.treasures) ? this.player.treasures.length : 0;
  ...
  resolveBattleCommandEffectiveCost(command) {
    ...
    if (path === 'destruction' && Math.max(0, Math.floor(Number(this.player?.block) || 0)) <= 5) {
      discount += Math.max(0, Math.floor(Number(doctrineProfile.commandCostDiscount) || 0));
    }
  ...
  initializeBattleCommandSystem() {
    ...
    this.commandState.maxPoints = baseCap + (path === 'insight' ? 1 : 0) + Math.max(0, Math.floor(Number(vowEffects.battleCommandPointCapBonus) || 0));
    this.commandState.points = Math.min(this.commandState.maxPoints, 3 + (pressure >= 8 ? 1 : 0) + Math.max(0, Math.floor(Number(vowEffects.initialCommandPointsBonus) || 0)));
    ...
    this.on('cardPlayed', payload => {
      ...
      const cardKeywords = Array.isArray(card.keywords) ? card.keywords.map(kw => String(kw || '').toLowerCase()) : [];
      const hasSelfHarmCard = Array.isArray(card.effects) && card.effects.some(effect => effect && (effect.type === 'selfDamage' || effect.type === 'addStatus'));
      const hasForgeCard = Array.isArray(card.effects) && card.effects.some(effect => effect && effect.type === 'createCard');
      const hasEchoCard = cardKeywords.includes('mirror') || cardKeywords.includes('echo') || cardKeywords.includes('delay') || Array.isArray(card.effects) && card.effects.some(effect => effect && effect.type === 'echoLastPlayedCard');
      const hasOathCard = cardKeywords.includes('oath') || cardKeywords.includes('debt') || cardKeywords.includes('penance') || Array.isArray(card.effects) && card.effects.some(effect => effect && (effect.type === 'consumeOathDebt' || effect.type === 'buff' && effect.buffType === 'oathDebt'));
      let gain = 1;
  ...
  onBattleCommandTurnStart() {
    ...
    let baseGain = 1;
    const hpRatio = this.player && Number(this.player.maxHp) > 0 ? (Number(this.player.currentHp) || 0) / Number(this.player.maxHp) : 1;
  ...
  async activateBattleCommand(commandId) {
    ...
    if (!ok) {
      // 保底回退，防止未知错误吞资源
      this.commandState.points = Math.min(Math.max(1, Math.floor(Number(this.commandState.maxPoints) || 12)), this.commandState.points + cost);
  ...
  consumeResonanceMatrixSignalMode() {
    ...
    const signalOrder = [{
      buff: 'matrixGuardSignal',
      mode: 'guard'
    }, {
      buff: 'matrixBreakSignal',
      mode: 'break'
    }, {
      buff: 'matrixCleanseSignal',
      mode: 'cleanse'
    }, {
      buff: 'matrixBurstSignal',
      mode: 'burst'
    }];
  ...
  handleTacticalAdvisorHotkey(rawKey = '') {
    ...
    const hasMatrixCommand = Array.isArray(state.commands) && state.commands.some(command => command && command.id === 'resonance_matrix_order');
  ...
  async resolveResonanceMatrixMode(command = null, threatProfile = null) {
    ...
    const profile = threatProfile && typeof threatProfile === 'object' ? threatProfile : this.resolveCounterplayThreatProfile();
    const recommendId = profile.needDefend ? 'guard' : profile.needBreak ? 'break' : profile.needCleanse ? 'cleanse' : 'burst';
  ...
  async executeBattleCommandEffect(command) {
    ...
    if (command.id === 'tempo_order') {
      ...
      this.player.buffs.nextAttackBonus = Math.max(Math.floor(Number(this.player.buffs.nextAttackBonus) || 0), nextHitBonus);
    ...
    if (command.id === 'phase_anchor_order') {
      ...
      Utils.showBattleLog(`相位锚定：获得 ${blockGain} 护盾，净化 ${cleaned} 层减益` + `${pressureShift > 0 ? `，轮回压力 -${pressureShift}` : ''}` + `${path === 'resonance' && doctrineTier > 0 ? '，回响教义回收 1 点指令槽' : ''}` + `${path === 'wisdom' && doctrineTier >= 2 ? '，并获得 1 次战术过牌' : ''}` + `${path === 'destruction' && doctrineTier > 0 ? `，毁灭教义提升 ${doctrineTier} 点力量` : ''}` + `${exposed > 0 ? '，但自身暴露 1 层易伤' : ''}`);
      await Utils.sleep(160);
      return true;
    }
    if (command.id === 'void_pursuit_order') {
      ...
      const targetIndex = this.enemies.indexOf(target);
      target.buffs = target.buffs || {};
      const extraScale = path === 'destruction' && doctrineTier > 0 ? 1 + doctrineTier * 0.06 : 1;
      const primaryDamageRaw = Math.max(14, Math.floor((16 + pressure * 1.9 + this.turnNumber * 0.75) * scale * extraScale));
      ...
      const markStack = (pressure >= 6 ? 3 : 2) + (path === 'convergence' && doctrineTier > 0 ? doctrineTier : 0);
      target.buffs.mark = Math.max(0, Math.floor(Number(target.buffs.mark) || 0)) + markStack;
      let splashTotal = 0;
      if (pressure >= 5) {
        const splashRatioBase = pressure >= 8 ? 0.45 : 0.35;
        const splashRatio = Math.min(0.6, splashRatioBase + (path === 'wisdom' && doctrineTier > 0 ? doctrineTier * 0.03 : 0));
      ...
      if (path === 'convergence' && doctrineTier > 0 && target.currentHp <= 0) {
        const refund = this.resolveCommandRefundAfterCounter(1, antiRefund);
        if (refund > 0) {
          this.gainBattleCommandPoints(refund, 'voidPursuit');
        }
      }
      if (path === 'resonance' && doctrineTier > 0 && splashTotal > 0 && typeof this.player?.drawCards === 'function') {
        this.player.drawCards(1);
      }
      ...
      Utils.showBattleLog(`裂界追猎：重击 ${target.name} 造成 ${Math.max(0, dealtPrimary)} 伤害` + `${splashTotal > 0 ? `，余震扩散 ${splashTotal}` : ''}` + `${path === 'convergence' && doctrineTier > 0 ? `，并叠加 ${markStack} 层破绽` : ''}` + `${path === 'resonance' && doctrineTier > 0 && splashTotal > 0 ? '，回响教义抽取 1 张牌' : ''}` + `${antiBurst > 0 ? '，敌方爆发抑制削弱了伤害' : ''}` + `${hpCost > 0 ? `，并消耗 ${hpCost} 生命稳压` : ''}`);
      await Utils.sleep(180);
      return true;
    }
    if (command.id === 'resonance_matrix_order') {
      ...
      if (branchId === 'guard') {
        ...
        const guardRefund = path === 'convergence' && doctrineTier >= 2 ? this.resolveCommandRefundAfterCounter(1, antiRefund) : 0;
        if (guardRefund > 0) {
          this.gainBattleCommandPoints(guardRefund, 'resonanceMatrixGuard');
        }
        Utils.showBattleLog(`命环共振·守势回路${modeText}：获得 ${blockGain} 护盾，净化 ${cleaned} 层减益` + `${path === 'resonance' && doctrineTier > 0 ? '，并抽取 1 张牌' : ''}` + `${path === 'convergence' && doctrineTier >= 2 && guardRefund <= 0 && antiRefund > 0 ? '，但回收被敌方封锁' : ''}`);
        await Utils.sleep(160);
        return true;
      }
      if (branchId === 'break') {
        ...
        const breakRefund = path === 'convergence' && doctrineTier >= 2 && breakAmount >= 10 ? this.resolveCommandRefundAfterCounter(1, antiRefund) : 0;
        if (breakRefund > 0) {
        ...
        Utils.showBattleLog(`命环共振·破阵回路${modeText}：击碎 ${target.name} 护盾 ${Math.min(beforeBlock, breakAmount)} 点，造成 ${Math.max(0, damage)} 伤害并施加 ${vulnStack} 层易伤` + `${path === 'resonance' && doctrineTier > 0 && breakAmount >= 10 ? '，并抽取 1 张牌' : ''}` + `${antiBurst > 0 ? '，敌方爆发抑制生效' : ''}` + `${path === 'convergence' && doctrineTier >= 2 && breakRefund <= 0 && antiRefund > 0 ? '，但回收被敌方封锁' : ''}`);
        await Utils.sleep(170);
        return true;
      }
      if (branchId === 'cleanse') {
        ...
        const cleaned = this.cleansePlayerDebuffs(cleanCap);
        const drawCount = Math.max(1, 2 + (path === 'resonance' && doctrineTier > 0 ? 1 : 0) - antiDraw);
        if (typeof this.player?.drawCards === 'function') {
          this.player.drawCards(drawCount);
        }
        let pressureDelta = 0;
        ...
        const cleanseRefund = path === 'convergence' && doctrineTier > 0 ? this.resolveCommandRefundAfterCounter(1, antiRefund) : 0;
        if (cleanseRefund > 0) {
          this.gainBattleCommandPoints(cleanseRefund, 'resonanceMatrixCleanse');
        }
        Utils.showBattleLog(`命环共振·净域回路${modeText}：净化 ${cleaned} 层减益并抽 ${drawCount}` + `${pressureDelta < 0 ? `，轮回压力 ${pressureDelta}` : ''}` + `${antiStabilize ? '，敌方稳压封锁生效' : ''}` + `${path === 'convergence' && doctrineTier > 0 && cleanseRefund <= 0 && antiRefund > 0 ? '，且回收被敌方封锁' : ''}`);
        await Utils.sleep(150);
        return true;
      }
      let target = aliveEnemies[0];
      ...
      const burstRefund = path === 'convergence' && doctrineTier > 0 && primaryDamage >= 16 ? this.resolveCommandRefundAfterCounter(1, antiRefund) : 0;
      if (burstRefund > 0) {
      ...
      Utils.showBattleLog(`命环共振·歼灭回路${modeText}：重创 ${target.name} ${Math.max(0, primaryDamage)} 并余震扩散 ${splashTotal}` + `${burstRefund > 0 ? `，回收指令槽 +${burstRefund}` : ''}` + `${path === 'convergence' && doctrineTier > 0 && primaryDamage >= 16 && burstRefund <= 0 && antiRefund > 0 ? '，但回收被敌方封锁' : ''}` + `${antiBurst > 0 ? '，敌方爆发抑制生效' : ''}`);
      await Utils.sleep(170);
      return true;
    }
    if (command.id === 'horizon_barter_order') {
      ...
      const drawCount = Math.max(1, 1 + effectiveCandy + Math.max(0, Math.floor(Number(modeProfile.drawBonus) || 0)) + (path === 'resonance' && doctrineTier > 0 ? 1 : 0) - antiDraw);
      if (typeof this.player?.drawCards === 'function') {
        this.player.drawCards(drawCount);
      }
      const rawEnergyGain = Math.max(1, 1 + (effectiveCandy >= 2 ? 1 : 0) + Math.max(0, Math.floor(Number(modeProfile.energyBonus) || 0)) + (path === 'convergence' && doctrineTier >= 2 ? 1 : 0));
      ...
      const primaryRaw = Math.max(8, Math.floor((9 + pressure * 1.4 + effectiveCandy * 3 + this.turnNumber * 0.5) * scale * damageScale * Math.max(0.6, Number(modeProfile.damageMul) || 1)));
      ...
      const rawRefund = effectiveCandy >= 1 ? 1 + (path === 'convergence' && doctrineTier > 0 ? 1 : 0) : 0;
      ...
      Utils.showBattleLog(`界隙交易（${modeProfile.label}）：耗费 ${spentCandy} 点奶糖${antiCandy > 0 ? `（受压制有效 ${effectiveCandy}）` : ''}，抽 ${drawCount} 并回能 ${energyGain}，对 ${target.name} 造成 ${Math.max(0, primaryDamage)} 伤害` + `${splashTotal > 0 ? `（扩散 ${splashTotal}）` : ''}` + `${refund > 0 ? `，回收指令槽 +${refund}` : ''}` + `${rawRefund > 0 && refund <= 0 && antiRefund > 0 ? '，回收被敌方封锁' : ''}` + `${pressureDelta < 0 ? `，轮回压力 ${pressureDelta}` : ''}` + `${pressureDelta > 0 ? `，轮回压力 +${pressureDelta}` : ''}` + `${antiStabilize ? '，敌方稳压封锁生效' : ''}` + `${antiEnergy > 0 ? '，敌方断流抑制生效' : ''}` + `${antiBurst > 0 ? '，敌方爆发抑制生效' : ''}`);
      await Utils.sleep(170);
      return true;
    }
    return false;
  ...
  getBattleCommandSnapshot() {
    ...
    return {
      ...
      commands: Array.isArray(state.commands) ? state.commands.map(command => ({
        id: command.id,
        name: command.name,
  ...
  createEnemyTacticalPlan(enemy) {
    ...
    if (role === 'striker') {
      id = 'strike_chain';
      label = '疾攻链';
      queue = [firstAttack, take(attackIndexes, firstAttack, 1), firstDebuff, take(attackIndexes, firstAttack, 2), firstUtility, take(attackIndexes, firstAttack, 3)];
    } else if (role === 'guardian') {
      id = 'guard_cycle';
      label = '守御轮转';
      queue = [firstDefend, firstAttack, take(defendIndexes, firstDefend, 1), firstUtility, take(attackIndexes, firstAttack, 1), firstDebuff];
    } else if (role === 'hexer') {
      id = 'hex_rotation';
      label = '咒压轮换';
      queue = [firstDebuff, firstAttack, take(debuffIndexes, firstDebuff, 1), firstUtility, take(attackIndexes, firstAttack, 1), firstDefend];
    }
    const sanitized = queue.map(idx => Math.max(0, Math.min(patterns.length - 1, Math.floor(Number(idx) || 0)))).filter((idx, pos, source) => source.length <= 1 || pos === 0 || idx !== source[pos - 1]);
    return {
      id,
      label,
  ...
  refreshEnemyTacticalPlan(enemy, force = false) {
    ...
    const needRefresh = force || !enemy.tacticalPlanId || !Array.isArray(enemy.tacticalQueue) || enemy.__tacticalPatternCount !== enemy.patterns.length;
  ...
  createEnemyInstance(enemyData) {
    ...
    const patterns = sourcePatterns.map(p => ({
      ...p
    }));
    ...
    if (variation) {
      ...
      if (Array.isArray(variation.appendPatterns) && variation.appendPatterns.length > 0) {
        enemy.patterns.push(...variation.appendPatterns.map(pattern => ({
          ...pattern
        })));
      }
    }
  ...
  parseGhostPatterns(enemy) {
    ...
    payload.deck.forEach(card => {
      const rawCard = CARDS ? CARDS[card.id] : null;
      if (!rawCard) return;
      const isUpgraded = card.upgraded;
      let val = rawCard.value || 0;
      if (isUpgraded && rawCard.upgradeBonus) val += rawCard.upgradeBonus;
      if (rawCard.type === 'attack') attacks.push(val);else if (rawCard.type === 'defend') defends.push(val);else magics.push(val);
    });
    ...
    if (attacks.length > 0) {
      enemy.patterns.push({
        type: 'attack',
        value: Math.floor(avgAtk + 10 + realmMultiplier * 4),
        intent: '残影绝学',
        effect: 'pierce'
      });
    }
    ...
    if (defends.length > 0) {
      enemy.patterns.push({
        type: 'defend',
        value: Math.floor(avgDef + 15 + realmMultiplier * 5),
        intent: '残影罡气'
      });
    }
    // 法术模式 (多段打击)
    if (magics.length > 0) {
      enemy.patterns.push({
        type: 'attack',
        value: Math.floor(avgMag + 5 + realmMultiplier * 2),
        intent: '残影法器',
        count: 2
      });
    }
  ...
  updateLegacyMissionTracker() {
    ...
    if (progressText) {
      progressText.textContent = mission.completed ? `已达成 ${target}/${target}` : `${progress}/${target}`;
    }
  }
  ...
  updateEnemiesUI() {
    ...
    if (!container) return;
    this.bindBattleIntentTooltipDelegation(container);
    container.innerHTML = '';
    this.enemies.forEach((enemy, index) => {
      ...
      enemyEl.addEventListener('click', () => {
        ...
        if (this.currentTurn === 'player' && !this.battleEnded && !this.isProcessingCard && !this.isTurnTransitioning && this.targetingMode && this.selectedCardIndex !== undefined && this.selectedCardIndex !== -1) {
  ...
  resolveCounterplayThreatProfile() {
    ...
    const aliveEnemies = (Array.isArray(this.enemies) ? this.enemies : []).filter(enemy => enemy && enemy.currentHp > 0);
    ...
    aliveEnemies.forEach(enemy => {
      ...
      if (enemy.isElite && enemy.eliteType === 'sunder' || Math.max(0, Math.floor(Number(enemy?.buffs?.guardBreak) || 0)) > 0) {
        hasGuardBreak = true;
      }
  ...
  resolveCardCounterTags(card, threatProfile = null) {
    ...
    const keywords = Array.isArray(card.keywords) ? card.keywords.map(kw => String(kw || '').toLowerCase()) : [];
    const tags = [];
    ...
    const hasDefend = card.type === 'defense' || Number(card.block) > 0 || effects.some(effect => effect && (effect.type === 'block' || effect.type === 'buff' && effect.buffType === 'nextTurnBlock'));
    const hasBurst = card.type === 'attack' && Math.max(0, Number(card.damage) || 0) >= 10 || keywords.includes('burst') || keywords.includes('chain') || keywords.includes('vulnerable') || keywords.includes('execute') || effects.some(effect => {
      if (!effect || typeof effect !== 'object') return false;
    ...
    const hasEcho = keywords.includes('mirror') || keywords.includes('echo') || keywords.includes('delay') || effects.some(effect => effect && effect.type === 'echoLastPlayedCard');
    const hasOath = keywords.includes('oath') || keywords.includes('debt') || keywords.includes('penance') || effects.some(effect => effect && (effect.type === 'consumeOathDebt' || effect.type === 'buff' && effect.buffType === 'oathDebt'));
  ...
  resolveBattleTempoRail(threatProfile = null, recommendation = null) {
    const profile = threatProfile && typeof threatProfile === 'object' ? threatProfile : this.resolveCounterplayThreatProfile();
    const pickedRecommendation = recommendation && typeof recommendation === 'object' ? recommendation : this.resolveTacticalAdvisorRecommendation(profile);
    ...
    const guardScore = clamp((profile.needDefend ? 54 : 18) + Math.min(24, Math.max(0, Math.floor(Number(profile.burstScore) || 0))) + (profile.hasGuardBreak ? 16 : 0) + Math.max(0, Math.floor(Number(profile.aliveEnemyCount) || 0) - 1) * 6 - Math.min(16, Math.floor(playerBlock / 3)));
    const breakScore = clamp((profile.needBreak ? 56 : 16) + Math.min(28, Math.floor((Number(profile.totalEnemyBlock) || 0) * 1.5)) + Math.max(0, Math.floor(Number(profile.guardianRoleCount) || 0)) * 12 + Math.max(0, Math.floor(Number(profile.healActions) || 0)) * 10);
    const cleanseScore = clamp((profile.needCleanse ? 58 : 14) + Math.max(0, Math.floor(Number(profile.debuffActions) || 0)) * 14 + Math.max(0, Math.floor(Number(profile.hexerRoleCount) || 0)) * 16);
    const burstScore = clamp((profile.needBurst ? 48 : 22) + Math.max(0, Math.floor(Number(profile.strikerRoleCount) || 0)) * 12 + Math.max(0, Math.floor(Number(profile.summonActions) || 0)) * 14 + ((Number(profile.lowestEnemyHp) || 999) <= 24 ? 14 : 0) + Math.min(10, commandPoints * 2));
    const segments = [{
      id: 'guard',
      label: '守势',
      score: guardScore,
      tip: '先稳血线，避免被敌方抢到斩杀窗口。'
    }, {
      id: 'break',
      label: '破阵',
      score: breakScore,
      ...
      label: '净域',
      score: cleanseScore,
      tip: '优先处理控制与减益，确保行动链不断。'
    }, {
      id: 'burst',
      label: '歼灭',
      score: burstScore,
      tip: '当前可以主动抢节奏，尽快压低敌方血线。'
    }].map(item => ({
      ...item,
  ...
  resolveBattleStatusIslands(state = null, threatProfile = null) {
    const profile = threatProfile && typeof threatProfile === 'object' ? threatProfile : this.resolveCounterplayThreatProfile();
    const runtimeState = state && typeof state === 'object' ? state : this.commandState || this.createDefaultBattleCommandState();
    ...
    const readyCount = commands.reduce((count, command) => {
      ...
      const cost = this.resolveBattleCommandEffectiveCost(command);
      const cooldownRemaining = Math.max(0, Math.floor(Number(command.cooldownRemaining) || 0));
      const ready = this.currentTurn === 'player' && !this.battleEnded && !this.isProcessingCard && !this.isTurnTransitioning && cooldownRemaining === 0 && points >= cost;
    ...
    pushIsland('command', '指令', readyCount > 0 ? `${readyCount} 项就绪` : `${points}/${maxPoints} 槽`, readyCount > 0 ? 'command_ready' : 'command');
    pushIsland('guard', '护势', playerBlock > 0 ? `${playerBlock} 护盾` : profile.needDefend ? '需补盾' : '平稳', playerBlock > 0 ? 'guard' : profile.needDefend ? 'warning' : 'calm');
    if (resonance && (resonance.name || resonance.id)) {
      pushIsland('resonance', '共鸣', `${resonance.name || resonance.id} T${Math.max(1, Math.floor(Number(resonance.tier) || 1))}`, 'resonance');
    } else {
      pushIsland('resonance', '共鸣', '未成型', 'muted');
    }
    if (destiny && destiny.name) {
      pushIsland('destiny', '命格', `${destiny.icon || '✦'} ${destiny.name}`, destiny.category === '赌博' ? 'warning' : 'fate');
    }
    if (Array.isArray(activeVows) && activeVows.length > 0) {
      ...
      pushIsland('vow', '誓约', `${leadVow.icon || '✧'} ${leadVow.name}${extraTag}`, leadVow.category === '赌博' ? 'warning' : 'oath');
    }
    if (bossDisplay && bossDisplay.act) {
      ...
      pushIsland('finish', '收束', Number(profile.lowestEnemyHp) <= 20 ? `可收头 ${profile.lowestEnemyHp}` : `最低血 ${profile.lowestEnemyHp}`, Number(profile.lowestEnemyHp) <= 20 ? 'finish' : 'enemy');
    }
  ...
  resolveBattleAdvisorInspectCardIndex(cardPlanSteps = null) {
    ...
    if (isValidIndex(selectedIndex)) {
      return {
        index: selectedIndex,
        source: this.targetingMode ? 'selected' : 'preview'
      };
    }
  ...
  resolveBattleAdvisorExecutionChain(threatProfile = null, recommendation = null, cardPlanSteps = null) {
    ...
    const effectItems = Array.isArray(card.effects) ? card.effects.map(effect => this.describeBattleAdvisorEffect(effect)).filter(Boolean) : [];
    ...
    const needsTarget = Array.isArray(card.effects) && card.effects.some(effect => effect && ['damage', 'debuff', 'execute', 'removeBlock', 'goldOnKill', 'maxHpOnKill', 'penetrate', 'steal', 'lifeSteal', 'absorb', 'swapHpPercent', 'executeDamage', 'percentDamage', 'blockBurst'].includes(effect.type) && (!effect.target || effect.target === 'enemy' || effect.target === 'single'));
    const hasMultipleEnemies = (Array.isArray(this.enemies) ? this.enemies : []).filter(enemy => enemy && enemy.currentHp > 0).length > 1;
    const costText = card.consumeCandy ? '消耗奶糖' : `消耗 ${Math.max(0, Math.floor(Number(this.getEffectiveCardCost(card)) || 0))} 灵力`;
  ...
  resolveBattleTacticalCardPlanMeta(threatProfile = null, recommendation = null) {
    ...
    const cardMeta = hand.map((card, index) => {
      ...
      const hpGate = card.condition && card.condition.type === 'hp' ? Math.max(0, Number(card.condition.min) || 0) : 0;
      const hpAllowed = !hpGate || Number(this.player?.currentHp || 0) >= hpGate;
      const energyAllowed = card.consumeCandy ? Math.max(0, Number(this.player?.milkCandy) || 0) >= 1 : Math.max(0, Number(this.player?.currentEnergy) || 0) >= cost;
      ...
      const reason = tags.length > 0 ? tags.map(item => item.label).join(' + ') : defaultReasonMap[card.type] || '补节奏';
      return {
        index,
        card,
        playable,
    ...
    const playableCards = cardMeta.filter(item => item.playable).sort((a, b) => b.score - a.score);
  ...
  previewAdvisorCard(cardIndex) {
    ...
    const needsTarget = Array.isArray(card.effects) && card.effects.some(effect => ['damage', 'debuff', 'execute', 'removeBlock', 'goldOnKill', 'maxHpOnKill', 'penetrate', 'steal', 'lifeSteal', 'absorb', 'swapHpPercent', 'executeDamage', 'percentDamage', 'blockBurst'].includes(effect.type) && (!effect.target || effect.target === 'enemy' || effect.target === 'single'));
  ...
  getEffectiveCardCost(card) {
    ...
    if (this.activeEnvironment && typeof this.activeEnvironment.modifyCardCost === 'function') {
      try {
        cost = this.activeEnvironment.modifyCardCost({
          ...card,
          cost
        });
      } catch (e) {
  ...
  onCardHover(cardIndex) {
    ...
    const damageEffects = card.effects.filter(e => ['damage', 'penetrate', 'randomDamage', 'damageAll', 'execute', 'executeDamage'].includes(e.type));
  ...
  onCardClick(cardIndex) {
    ...
    const needsTarget = card.effects && card.effects.some(e => ['damage', 'debuff', 'execute', 'removeBlock', 'goldOnKill', 'maxHpOnKill', 'penetrate', 'steal', 'lifeSteal', 'absorb', 'swapHpPercent', 'executeDamage', 'percentDamage'].includes(e.type) && (!e.target || e.target === 'enemy' || e.target === 'single'));
  ...
  async playCardOnTarget(cardIndex, targetIndex) {
    ...
    const needsTarget = Array.isArray(card.effects) && card.effects.some(e => ['damage', 'debuff', 'execute', 'removeBlock', 'goldOnKill', 'maxHpOnKill', 'penetrate', 'steal', 'lifeSteal', 'absorb', 'swapHpPercent', 'executeDamage', 'percentDamage', 'blockBurst'].includes(e.type) && (!e.target || e.target === 'enemy' || e.target === 'single'));
    let target = null;
    if (needsTarget) {
    ...
    try {
      ...
      const results = this.player.playCard(cardIndex, target, {
        energyCostOverride: effectiveCost
      });
      if (results === false) {
        return;
      }
      ...
      const previousCardSnapshot = this.lastPlayerCardSnapshot ? JSON.parse(JSON.stringify(this.lastPlayerCardSnapshot)) : null;
    ...
    switch (result.type) {
      ...
      case 'penetrate':
        if (target) {
          ...
          const thunderSword = Array.isArray(this.player.activeResonances) ? this.player.activeResonances.find(r => r.id === 'thunderSword') : null;
          if (thunderSword) {
            // 穿透命中后附加易伤，作为“麻痹”表现。
      ...
      case 'echoLastPlayedCard':
        {
          ...
          const nestedResults = Array.isArray(result.results) ? result.results : Array.isArray(result.echoedResults) ? result.echoedResults : [];
          const echoedCardName = result.cardName || result.echoedCardName || result.sourceCardName || result.card && result.card.name || '';
          ...
          const preferredIndex = Number.isFinite(Number(result.targetIndex)) ? Number(result.targetIndex) : Number.isFinite(Number(result.echoTargetIndex)) ? Number(result.echoTargetIndex) : targetIndex;
      ...
      case 'consumeOathDebt':
        {
          ...
          const nestedResults = Array.isArray(result.results) ? result.results : Array.isArray(result.followUps) ? result.followUps : [];
          ...
          const totalDebtDamage = Number.isFinite(Number(result.damageBonus)) ? Math.floor(Number(result.damageBonus)) : Math.floor(Number(result.value) || 0);
          ...
          const preferredIndex = Number.isFinite(Number(result.targetIndex)) ? Number(result.targetIndex) : targetIndex;
          const nestedTarget = resolveEnemyTargetByIndex(preferredIndex, target);
          const nestedTargetIndex = resolveEnemyIndex(nestedTarget, targetIndex);
          if (nestedResults.length > 0) {
            for (const nestedResult of nestedResults) {
      ...
      case 'createCard':
        {
          ...
          const names = Array.isArray(result.cards) ? Array.from(new Set(result.cards.map(card => card && card.name).filter(Boolean))) : [];
      ...
      case 'debuff':
        if (target) {
          ...
          if (result.buffType === 'stun') {
            ...
            // Boss Immunity
            if (target.isBoss && Math.random() < 0.8) {
              // Boss 80% resist stun
              immune = true;
  ...
  dealDamageToEnemy(enemy, amount, sourceElement = null) {
    ...
    const doctrine = this.player && this.player.legacyRunDoctrine ? this.player.legacyRunDoctrine : null;
    const runPathMeta = this.player && typeof this.player.getRunPathMeta === 'function' ? this.player.getRunPathMeta() : null;
    const runPathEffects = this.player && typeof this.player.getRunPathEffects === 'function' ? this.player.getRunPathEffects() : null;
    ...
    const vowEffects = this.player && typeof this.player.getRunVowEffects === 'function' ? this.player.getRunVowEffects() : null;
    const vowBattleState = this.player && this.player.runVowBattleState ? this.player.runVowBattleState : null;
    const spiritMeta = this.player && typeof this.player.getSpiritCompanionMeta === 'function' ? this.player.getSpiritCompanionMeta() : null;
    const spiritPassive = this.player && typeof this.player.getSpiritCompanionEffects === 'function' ? this.player.getSpiritCompanionEffects().passive || {} : {};
    if (runPathMeta && runPathEffects && runPathBattleState && Number(runPathEffects.firstAttackBonusPerBattle) > 0 && !runPathBattleState.firstAttackBonusUsed && sourceElement !== 'plasma_proc') {
    ...
    if (destinyMeta && destinyEffects && destinyBattleState && Number(destinyEffects.firstAttackBonusPerBattle) > 0 && !destinyBattleState.firstAttackBonusUsed && sourceElement !== 'plasma_proc') {
    ...
    if (vowEffects && vowBattleState && Number(vowEffects.firstAttackBonusPerBattle) > 0 && !vowBattleState.firstAttackBonusUsed && sourceElement !== 'plasma_proc') {
    ...
    if (spiritMeta && Number(spiritPassive.debuffedTargetBonusDamage) > 0 && sourceElement !== 'spirit_active' && sourceElement !== 'spirit_passive' && enemy && enemy.buffs && Object.keys(enemy.buffs).some(key => Number(enemy.buffs[key]) > 0)) {
    ...
    if (spiritMeta && Number(spiritPassive.blockedTargetBonusDamage) > 0 && sourceElement !== 'spirit_active' && sourceElement !== 'spirit_passive' && Math.max(0, Math.floor(Number(enemy?.block) || 0)) > 0) {
    ...
    if (doctrine && doctrine.firstAttackBonusPerBattle > 0 && !this.playerFirstAttackBoostUsed && sourceElement !== 'plasma_proc') {
    ...
    if (this.player.activeResonances) {
      ...
      if (extreme && sourceElement === 'fire') {
        if ((enemy.buffs.weak || 0) > 0 || enemy.stunned) {
          // Weak as Slow proxy
    ...
    if (enemy.buffs.vulnerable && enemy.buffs.vulnerable > 0) {
      ...
      if ((hasStormcraftResonance || hasStormcraftDoctrine) && !resonanceUsed && !doctrineUsed) {
        const bonusDamage = Math.max(1, hasStormcraftResonance ? Math.floor(Number(resonance.vulnerableBonusDamage) || 0) : 0, hasStormcraftDoctrine ? Math.floor(Number(doctrine.stormcraftLegacyBonusDamage) || 0) : 0);
        amount += bonusDamage;
        ...
        const drawCount = Math.max(hasStormcraftResonance ? Math.floor(Number(resonance.firstVulnerableHitDraw) || 0) : 0, hasStormcraftDoctrine ? Math.floor(Number(doctrine.stormcraftLegacyDraw) || 0) : 0);
        if (drawCount > 0) {
          this.player.drawCards(drawCount);
          this.markUIDirty('hand', 'piles');
        }
    ...
    if (sourceElement && enemy.element) {
      ...
      if (multiplier !== 1.0) {
        ...
        } else if (multiplier < 1 && multiplier > 0.75) {
          // Same element 0.8
      ...
      if (resistMod !== 0) {
        ...
        if (resistMod > 0) Utils.showBattleLog(`敌方抗性生效！伤害减少 ${Math.floor(resistMod * 100)}%`);else Utils.showBattleLog(`敌方弱点！伤害增加 ${Math.floor(Math.abs(resistMod) * 100)}%`);
      }
    }
    ...
    if (destinyMeta && destinyEffects && Number(destinyEffects.lowHpDamageBonusPct) > 0 && Number(destinyEffects.lowHpThreshold) > 0 && this.player && Math.max(1, Number(this.player.maxHp) || 1) > 0 && (Number(this.player.currentHp) || 0) / Math.max(1, Number(this.player.maxHp) || 1) <= Number(destinyEffects.lowHpThreshold)) {
      amount = Math.floor(amount * (1 + Math.max(0, Number(destinyEffects.lowHpDamageBonusPct) || 0)));
    }
    if (vowEffects && Number(vowEffects.lowHpDamageBonusPct) > 0 && Number(vowEffects.lowHpThreshold) > 0 && this.player && Math.max(1, Number(this.player.maxHp) || 1) > 0 && (Number(this.player.currentHp) || 0) / Math.max(1, Number(this.player.maxHp) || 1) <= Number(vowEffects.lowHpThreshold)) {
    ...
    for (let i = 0; i < this.enemies.length; i++) {
      ...
      try {
        ...
        if (enemy.isBoss) {
          ...
          // 30% 几率净化一个负面效果
          if (Math.random() < 0.3) {
            const debuffs = Object.keys(enemy.buffs).filter(k => ['poison', 'burn', 'weak', 'vulnerable', 'stun', 'freeze'].includes(k) && enemy.buffs[k] > 0);
            if (debuffs.length > 0) {
        ...
        if (enemy.stunned) {
          ...
          if (this.player.realm >= 16) {
            let resistChance = 0;
            if (this.player.realm === 16) resistChance = 0.3;else if (this.player.realm === 17) resistChance = 0.4;else if (this.player.realm >= 18) resistChance = 0.5;
  ...
  applyGuardBreakPressure(enemy, amount) {
    ...
    const shattered = Math.min(currentBlock, Math.max(3, Math.min(shatterCap, Math.floor(currentBlock * shatterRate))));
  ...
  async executeEnemyAction(enemy, index) {
    ...
    const pattern = enemy.patterns[safeIndex] || {
      type: 'attack',
      value: 1,
      intent: '⚔️'
    };
      ...
      case 'innerDemon':
        {
          ...
          if (demonCardDef) {
            for (let c = 0; c < count; c++) {
              const demonCard = {
                ...demonCardDef,
                instanceId: this.player.generateCardId()
              };
              // Random insert
  ...
  resolveBattleTacticalAdvisorSnapshot(state = null, threatProfile = null) {
    ...
    const cardPlanHint = cardPlanMeta && typeof cardPlanMeta.text === 'string' ? cardPlanMeta.text : '';
    const cardPlanSteps = cardPlanMeta && Array.isArray(cardPlanMeta.steps) ? cardPlanMeta.steps.filter(item => item && Number.isFinite(Number(item.index))).slice(0, 2).map(item => ({
    ...
    telemetry.suggestedStepKeys = cardPlanSteps.map(item => this.getAdvisorCardKey(Array.isArray(this.player?.hand) ? this.player.hand[item.index] : null)).filter(Boolean);
    let lastModeLabel = '';
    let pendingModeLabel = '';
    const matrixControls = [];
    const squad = this.activeSquadEcology && typeof this.activeSquadEcology === 'object' ? this.activeSquadEcology : null;
    if (squad && squad.id) {
    ...
    if (endlessActive && this.game && typeof this.game.getEndlessCycleThemeProfile === 'function') {
      ...
      if (theme && typeof theme === 'object') {
        ...
        const themeHint = directiveHintMap[directive] || '该轮段节奏较均衡，可按当前构筑正常展开。';
    ...
    if (chapterBattlefield) {
      ...
      if (chapterSegments.length > 0) {
        ...
        formationHint = formationHint ? `${formationHint} ${chapterHint}` : chapterHint;
      }
    }
    if (matrixMeta) {
      ...
      pendingModeLabel = pendingModeId === 'auto' ? '自动判断' : `已预设 ${pendingMode.label}`;
      const modeChoices = [{
        id: 'auto',
        label: '自适应'
      }, {
  ...
  resolveBattleSpiritCompanionDisplay() {
    ...
    if (!meta) return null;
    const state = typeof this.player.ensureSpiritCompanionBattleState === 'function' ? this.player.ensureSpiritCompanionBattleState() : this.player.spiritCompanionBattleState || {};
  ...
  applySpiritCompanionBattleStart() {
    ...
    const effects = typeof this.player.getSpiritCompanionEffects === 'function' ? this.player.getSpiritCompanionEffects() : {
      passive: {}
    };
  ...
  handleSpiritCompanionCardPlayed(card = null) {
    ...
    const state = typeof this.player.ensureSpiritCompanionBattleState === 'function' ? this.player.ensureSpiritCompanionBattleState() : null;
    let triggered = false;
    const nthEnergy = passive.everyNthCardEnergy && typeof passive.everyNthCardEnergy === 'object' ? passive.everyNthCardEnergy : null;
    if (nthEnergy && Math.max(0, Math.floor(Number(nthEnergy.count) || 0)) > 0 && this.cardsPlayedThisTurn > 0 && this.cardsPlayedThisTurn % Math.max(1, Math.floor(Number(nthEnergy.count) || 1)) === 0) {
  ...
  handleSpiritCompanionPlayerDamaged(hpDamage = 0) {
    ...
    const enemies = Array.isArray(this.enemies) ? this.enemies.filter(enemy => enemy && enemy.currentHp > 0) : [];
  ...
  activateSpiritCompanion() {
    if (!this.player || this.currentTurn !== 'player' || this.battleEnded || this.isProcessingCard || this.isTurnTransitioning || typeof this.player.getSpiritCompanionMeta !== 'function') {
      return false;
    }
    const meta = this.player.getSpiritCompanionMeta();
    if (!meta) return false;
    ...
    const livingEnemies = Array.isArray(this.enemies) ? this.enemies.filter(enemy => enemy && enemy.currentHp > 0) : [];
    ...
    switch (String(active.type || '')) {
      ...
      case 'star_sift':
        {
          ...
          const reducible = Array.isArray(this.player.hand) ? this.player.hand.filter(card => card && Number(card.cost) > 0).slice(0, Math.max(0, Math.floor(Number(active.reduceHandCost) || 0))) : [];
      ...
      case 'sunder_strike':
        {
          const target = livingEnemies.slice().sort((a, b) => {
      ...
      case 'relic_overclock':
        ...
        if (this.commandState && typeof this.commandState === 'object') {
          ...
          if (gain > 0) {
            this.commandState.points = Math.min(Math.max(0, Math.floor(Number(this.commandState.maxPoints) || 12)), Math.max(0, Math.floor(Number(this.commandState.points) || 0)) + gain);
          }
        }
        Utils.showBattleLog(`灵契【${meta.name}】发动【${meta.activeLabel}】`);
        break;
  ...
  syncTacticalAdvisorPresentation() {
    ...
    const advisor = panel && typeof panel.querySelector === 'function' ? panel.querySelector('#battle-tactical-advisor') : null;
  ...
  clampBattleCommandPanelPosition(left, top, width, height) {
    if (typeof DefierBattleHud !== 'undefined' && DefierBattleHud && typeof DefierBattleHud.clampFloatingPanelPosition === 'function') {
      return DefierBattleHud.clampFloatingPanelPosition({
        left,
        top,
        width,
        height,
  ...
  beginBattleCommandPanelDrag(event) {
    ...
    const onMove = moveEvent => {
      ...
      const next = this.clampBattleCommandPanelPosition(this.battleCommandDragState.left + dx, this.battleCommandDragState.top + dy, rect.width || panel.offsetWidth, rect.height || panel.offsetHeight);
      this.battleCommandPanelPosition = next;
      panel.style.left = `${next.left}px`;
      panel.style.top = `${next.top}px`;
      panel.style.transform = 'none';
  ...
  bindBattleIntentTooltipDelegation(root) {
    ...
    root.addEventListener('mouseover', forward);
    root.addEventListener('mousemove', forward);
    root.addEventListener('mouseout', forward);
    root.dataset.intentTooltipBound = '1';
  }
  handleBattleIntentTooltipEvent(event) {
    const target = event.target instanceof Element ? event.target.closest('.enemy-intent[data-tooltip-safe]') : null;
    if (!target) {
      if (event.type === 'mouseout') Utils.hideTooltip();
      return;
    }
    if (event.type === 'mouseout') {
      const related = event.relatedTarget instanceof Element ? event.relatedTarget.closest('.enemy-intent[data-tooltip-safe]') : null;
      if (related === target) return;
      Utils.hideTooltip();
      return;
    }
    Utils.showTooltip(target.dataset.tooltipSafe || '', event.clientX, event.clientY);
  }
  bindBattleCommandPanelDelegation(panel) {
    if (!panel || panel.dataset.commandDelegated === '1') return;
    panel.addEventListener('click', event => {
      const trigger = event.target instanceof Element ? event.target.closest('[data-action]') : null;
      if (!trigger) return;
      switch (trigger.dataset.action) {
        case 'activate-battle-command':
          this.activateBattleCommand(trigger.dataset.commandId || '');
          break;
        case 'set-resonance-matrix-mode':
          this.setResonanceMatrixSignalMode(trigger.dataset.mode || 'auto');
          break;
        case 'preview-advisor-card':
          this.previewAdvisorCard(trigger.dataset.cardIndex || 0);
          break;
        case 'activate-spirit-companion':
          this.activateSpiritCompanion();
          break;
        case 'toggle-tactical-advisor':
          this.toggleTacticalAdvisor();
          break;
        default:
          break;
    ...
    this.bindBattleIntentTooltipDelegation(panel);
    panel.dataset.commandDelegated = '1';
  }
  ...
  updateBattleCommandUI() {
    ...
    const commandButtonData = state.commands.map(command => {
      const cost = this.resolveBattleCommandEffectiveCost(command);
      const cooldownRemaining = Math.max(0, Math.floor(Number(command.cooldownRemaining) || 0));
      const disabled = this.currentTurn !== 'player' || this.battleEnded || this.isProcessingCard || this.isTurnTransitioning || cooldownRemaining > 0 || points < cost;
      const statusText = cooldownRemaining > 0 ? `冷却 ${cooldownRemaining}` : points >= cost ? '可发动' : '槽能不足';
      const classes = ['battle-command-btn', cooldownRemaining > 0 ? 'cooldown' : '', points >= cost && cooldownRemaining === 0 ? 'ready' : '', disabled ? 'disabled' : ''].filter(Boolean).join(' ');
      return {
        id: command.id,
        icon: command.icon,
        name: command.name,
    ...
    panel.innerHTML = DefierBattleHud.buildBattleCommandPanelMarkup({
      points,
      maxPoints,
      progress,
  ...
  updateEnvironmentUI() {
    ...
    if (this.activeEnvironment || encounter || squad || chapter) {
      ...
      envEl.innerHTML = `
                ...
                ${chapter?.skyOmen ? `<span class="chapter-omen-chip" title="${escapeHtml(chapterField?.omen?.phaseDesc || chapter.skyOmen.desc || '')}">
                        <span class="chapter-rule-icon">☁️</span>
                        <span class="chapter-rule-name">天象·${escapeHtml(chapter.skyOmen.name || '未定')}</span>
                        ${chapterField?.omen?.phaseLabel ? `<span class="chapter-rule-stage">${escapeHtml(chapterField.omen.phaseLabel)}</span>` : ''}
                    </span>` : ''}
                ${chapter?.leyline ? `<span class="chapter-leyline-chip" title="${escapeHtml(chapterField?.leyline?.activeDesc || chapter.leyline.desc || '')}">
                        <span class="chapter-rule-icon">⛰️</span>
                        <span class="chapter-rule-name">地脉·${escapeHtml(chapter.leyline.name || '未定')}</span>
                        ${chapterField?.leyline?.activeLabel ? `<span class="chapter-rule-stage">${escapeHtml(chapterField.leyline.activeLabel)}</span>` : ''}
                    </span>` : ''}
                ${chapterField?.formation ? `<span class="chapter-formation-chip" title="${escapeHtml(chapterField.formation.hint || chapterField.formation.desc || '')}">
                ...
                ${this.activeEnvironment ? `<span class="env-main"><span class="env-icon">${envIcon}</span><span class="env-name">${escapeHtml(envName)}</span></span>` : ''}
                ${encounter ? `<span class="encounter-theme-chip" title="${escapeHtml(encounter.description)}">
                ...
                ${squad ? `<span class="squad-formation-chip" title="${escapeHtml(squad.desc || '敌方编队协同中')}">
