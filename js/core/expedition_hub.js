...
const expeditionHubMethods = Object.create(null);
  ...
  let originalStartRealm = null;
  let originalAdvanceToNextRealm = null;
  ...
  let originalGetBuildSnapshotData = null;
  let originalGetSanctumOverviewData = null;
  let originalMapRender = typeof GameMap !== 'undefined' && GameMap?.prototype ? GameMap.prototype.render : null;
  let originalMapUpdateState = typeof GameMap !== 'undefined' && GameMap?.prototype ? GameMap.prototype.updateMapState : null;
  let originalMapOnNodeClick = typeof GameMap !== 'undefined' && GameMap?.prototype ? GameMap.prototype.onNodeClick : null;
  let originalMapCompleteNode = typeof GameMap !== 'undefined' && GameMap?.prototype ? GameMap.prototype.completeNode : null;
  ...
  const intersectNodeTypes = (...groups) => {
    const normalized = groups.map(group => readArray(group).map(value => String(value || '')).filter(Boolean)).filter(group => group.length > 0);
  ...
  const pickUnique = (list, count, seed = 0) => {
    ...
    const pool = source.map((item, index) => ({
      item,
      weight: hashString(`${seed}:${item.id || item.name || index}`),
      index
    })).sort((a, b) => a.weight - b.weight || a.index - b.index).map(entry => entry.item);
  ...
  const EXPEDITION_OBSERVATORY_THEME_KEY_BY_LABEL = Object.entries(EXPEDITION_OBSERVATORY_THEME_LABELS).reduce((result, [key, label]) => {
  ...
  const normalizeExpeditionObservatoryResonance = (resonance = null) => {
    ...
    return {
      ...
      rewardLine,
      completed,
      statusLine: completed ? `已兑现 ${label}${rewardLine ? `（${rewardLine}）` : ''}` : `命盘共鸣 ${progress}/${target}`,
  ...
  const normalizeExpeditionObservatoryRoutePact = (routePact = null) => {
    ...
    const progress = Math.min(clampInt(source.progress, 0, 99), target);
    const completed = !!source.completed;
    const summary = String(source.summary || `已把「${bountyName}」并入本章答卷，沿 ${formatExpeditionNodeLabels(focusNodeTypes, '关键线路')} 线命中 ${target} 次，可额外兑现 ${label}${rewardLine ? `：${rewardLine}` : ''}。`);
    return {
      label,
      ...
      completed,
      summary,
      statusLine: completed ? `已合卷 ${bountyName}${rewardLine ? `（${rewardLine}）` : ''}` : `${label} ${progress}/${target} · ${bountyName}`,
  ...
  const retargetExpeditionObservatoryResonanceForBonus = (resonance = null, bonus = null) => {
    ...
    const resolvedFocus = focusNodeTypes.length > 0 ? focusNodeTypes : selectedBonus.nodeTypes.slice(0, 4);
  ...
  const normalizeExpeditionObservatoryLink = (link = null) => {
    ...
    const resonance = normalizeExpeditionObservatoryResonance(source.resonance) || (selectedBonus ? retargetExpeditionObservatoryResonanceForBonus(fallbackResonance, selectedBonus) : fallbackResonance);
  ...
  const buildExpeditionPracticeTopic = (link = null, branchOptions = [], chapterIndex = 1) => {
    ...
    const fallbackBranches = readArray(branchOptions).map(entry => ({
      id: String(entry?.id || ''),
      name: String(entry?.name || ''),
      matchCount: intersectNodeTypes(entry?.nodeBias, focusNodeTypes).length
    })).filter(entry => entry.id && entry.matchCount > 0).sort((a, b) => b.matchCount - a.matchCount || a.name.localeCompare(b.name, 'zh-Hans-CN')).slice(0, 3);
    const recommendedBranches = readArray(source.recommendedBranches).length > 0 ? readArray(source.recommendedBranches).map(entry => ({
      id: String(entry.id || ''),
      name: String(entry.name || ''),
      matchCount: clampInt(entry.matchCount, 0, 9)
    })).filter(entry => entry.id) : fallbackBranches;
  ...
  const normalizeExpeditionPracticeTopic = (topic = null, link = null, branchOptions = [], chapterIndex = 1) => {
    ...
    return {
      ...
      themeKey: resolveExpeditionObservatoryThemeKey(source?.themeKey || fallback?.themeKey, source?.themeLabel || fallback?.themeLabel),
  ...
  const normalizeExpeditionAnswerGoal = (goal = null, index = 0) => {
    ...
    return {
      ...
      stateTone: ['completed', 'selected', 'suggested', 'idle'].includes(String(source.stateTone || '')) ? String(source.stateTone) : 'idle',
  ...
  const normalizeExpeditionAnswerReview = (review = null) => {
    ...
    return {
      ...
      ratingTone: ['completed', 'selected', 'suggested', 'idle'].includes(String(source.ratingTone || '')) ? String(source.ratingTone) : 'idle',
  ...
  const buildExpeditionAnswerSheet = (state = null, practiceTopic = null) => {
    ...
    const executionFocusNodeTypes = state.observatoryLink?.selectedBonusId && resonance?.focusNodeTypes?.length ? readArray(resonance.focusNodeTypes) : readArray(practiceTopic.focusNodeTypes);
    const executionFocusLine = formatExpeditionNodeLabels(executionFocusNodeTypes, '关键线路', 4);
    const routeCompleted = !!selectedBranch && (recommendedBranchIds.length > 0 ? recommendedBranchIds.includes(selectedBranch.id) : intersectNodeTypes(selectedBranch.nodeBias, practiceTopic.focusNodeTypes).length > 0);
    ...
    const executionProgress = Math.min(executionTarget, executionFocusNodeTypes.reduce((sum, nodeType) => sum + clampInt(state.stats?.nodeVisits?.[nodeType], 0, 999), 0));
    ...
    if (!selectedBonus) {
      ...
      nextSuggestion = routePact ? `再沿 ${formatExpeditionNodeLabels(routePact.focusNodeTypes, '关键线路', 4)} 推进 ${Math.max(0, routePact.target - routePact.progress)} 次，把「${routePact.bountyName}」真正并卷。` : `承接一条与 ${executionFocusLine} 同线的章节悬赏，把研判真正写进答卷。`;
    } else {
    ...
    return {
      ...
      clueStatusLine: selectedBonus ? `已锁定线索「${selectedBonus.label}」` : '待从精选命盘里锁 1 条线索',
      ratingLabel,
      ratingTone,
      completedGoals,
  ...
  const serializeSeasonBoardSnapshot = (board = null) => {
    ...
    const laneRewards = readArray(source.laneRewards).map(reward => serializeLaneReward(reward)).filter(Boolean);
    const canonicalLaneRewards = laneRewards.length > 0 ? laneRewards : lanes.map(lane => lane.reward).filter(Boolean);
  ...
  const isPressureBounty = (bounty = null) => {
    const condition = bounty?.condition || {};
    return ['battleWins', 'eliteWins', 'noRestBossWin'].includes(condition.type) || String(bounty?.id || '') === 'altar_oath' || String(bounty?.id || '') === 'trial_verdict';
  };
  const isStabilityBounty = (bounty = null) => String(bounty?.condition?.type || '') === 'hpAboveOnBossWin';
  ...
  const normalizeNemesisHistoryEntry = (entry = null, index = 0) => {
    ...
    const severity = ['low', 'medium', 'high'].includes(String(source.severity || '')) ? String(source.severity) : ['guarding', 'evolved', 'escaped'].includes(status) ? 'high' : ['allied', 'recurring', 'hunting'].includes(status) ? 'medium' : 'low';
  ...
  const normalizeConflictWarning = (entry = null, index = 0) => {
    const source = entry && typeof entry === 'object' ? entry : {};
    const severity = ['low', 'medium', 'high'].includes(String(source.severity || '')) ? String(source.severity) : 'low';
  ...
  const getFactionStatusMeta = (stance = 0) => {
    ...
    if (stance > 0) return {
      tone: 'warm',
      label: '偏友',
      nextHint: `再 +${Math.max(1, 2 - stance)} 可结盟`
    };
    if (stance < 0) return {
      tone: 'cold',
      label: '偏冷',
      nextHint: `再 ${Math.max(1, Math.abs(-2 - stance))} 会敌对`
    };
    return {
      tone: 'neutral',
      label: '中立',
  ...
  const normalizeNemesisStatus = (value = 'hunting') => ALL_NEMESIS_STATUSES.includes(String(value || '').trim()) ? String(value || '').trim() : 'hunting';
  ...
  const getNemesisStatusMeta = (status = 'hunting') => NEMESIS_STATUS_META[normalizeNemesisStatus(status)] || NEMESIS_STATUS_META.hunting;
  const getNemesisPressureMeta = (score = 0) => {
    const safeScore = clampInt(score, 0, 100);
    if (safeScore >= 78) return {
      tierId: 'extreme',
      label: '极高压'
    };
    if (safeScore >= 58) return {
      tierId: 'high',
      label: '高压'
    };
    if (safeScore >= 34) return {
      tierId: 'medium',
      label: '拉扯'
    };
    return {
  ...
  expeditionHubMethods.loadActiveExpeditionState = function () {
    try {
  ...
  expeditionHubMethods.persistActiveExpeditionState = function () {
    try {
  ...
  expeditionHubMethods.loadRunSlateArchive = function () {
    try {
      if (typeof localStorage === 'undefined') return [];
  ...
  expeditionHubMethods.persistRunSlateArchive = function () {
    try {
      if (typeof localStorage === 'undefined') return;
  ...
  expeditionHubMethods.normalizeRunSlateArchive = function (rawArchive = null) {
    ...
    return source.filter(entry => entry && typeof entry === 'object').map(entry => ({
      ...
      practiceTopic: normalizeExpeditionPracticeTopic(entry.practiceTopic, entry.observatoryLink, [], clampInt(entry.chapterIndex, 1, 6) || 1),
    ...
    })).sort((a, b) => clampInt(b.timestamp, 0) - clampInt(a.timestamp, 0)).slice(0, 16);
  };
  expeditionHubMethods.normalizeExpeditionState = function (rawState = null) {
    ...
    normalizedState.practiceTopic = normalizeExpeditionPracticeTopic(source.practiceTopic, normalizedState.observatoryLink, normalizedState.branchOptions, normalizedState.chapterIndex);
    normalizedState.answerSheet = buildExpeditionAnswerSheet(normalizedState, normalizedState.practiceTopic);
    return normalizedState;
  };
  expeditionHubMethods.ensureExpeditionBootState = function () {
    if (!Array.isArray(this.runSlateArchive)) {
      this.runSlateArchive = expeditionHubMethods.loadRunSlateArchive.call(this);
    }
    if (!this.expeditionState || typeof this.expeditionState !== 'object') {
      this.expeditionState = expeditionHubMethods.loadActiveExpeditionState.call(this);
    } else if (!Array.isArray(this.expeditionState.branchOptions) || !Array.isArray(this.expeditionState.bountyDraft) || !this.expeditionState.practiceTopic || !this.expeditionState.answerSheet) {
      this.expeditionState = expeditionHubMethods.normalizeExpeditionState.call(this, this.expeditionState);
    }
  };
  expeditionHubMethods.getExpeditionState = function () {
    try {
      expeditionHubMethods.ensureExpeditionBootState.call(this);
      return this.expeditionState || null;
    } catch (error) {
      console.warn('Expedition state bootstrap failed:', error);
  ...
  expeditionHubMethods.syncExpeditionPracticeState = function (state = null) {
    const source = state && typeof state === 'object' ? state : this.getExpeditionState();
    if (!source) return null;
    source.practiceTopic = normalizeExpeditionPracticeTopic(source.practiceTopic, source.observatoryLink, source.branchOptions, source.chapterIndex);
    source.answerSheet = buildExpeditionAnswerSheet(source, source.practiceTopic);
    return source;
  };
  expeditionHubMethods.getExpeditionPracticeTopic = function (state = null) {
    const source = state || this.getExpeditionState();
    return source?.practiceTopic || null;
  };
  expeditionHubMethods.getExpeditionAnswerSheet = function (state = null) {
    const source = state || this.getExpeditionState();
    return source?.answerSheet || null;
  };
  expeditionHubMethods.getExpeditionBranchPool = function (chapterIndex = 1) {
    const pool = typeof EXPEDITION_BRANCH_REGIONS !== 'undefined' && EXPEDITION_BRANCH_REGIONS ? EXPEDITION_BRANCH_REGIONS[getChapterKey(chapterIndex)] : [];
    return readArray(pool).map(entry => clone(entry));
  };
  expeditionHubMethods.getExpeditionFactionPool = function () {
    const source = typeof EXPEDITION_FACTION_PROFILES !== 'undefined' && EXPEDITION_FACTION_PROFILES ? EXPEDITION_FACTION_PROFILES : {};
    return Object.values(source || {}).filter(Boolean).map(entry => clone(entry));
  };
  expeditionHubMethods.getExpeditionNemesisPool = function (chapterIndex = 1) {
    const source = typeof EXPEDITION_NEMESIS_PROFILES !== 'undefined' && EXPEDITION_NEMESIS_PROFILES ? EXPEDITION_NEMESIS_PROFILES[getChapterKey(chapterIndex)] : [];
    return readArray(source).map(entry => clone(entry));
  };
  expeditionHubMethods.getExpeditionBountyPool = function (chapterIndex = 1) {
    const source = typeof EXPEDITION_BOUNTY_TEMPLATES !== 'undefined' && Array.isArray(EXPEDITION_BOUNTY_TEMPLATES) ? EXPEDITION_BOUNTY_TEMPLATES : [];
    return source.filter(entry => !Array.isArray(entry?.chapters) || entry.chapters.includes(chapterIndex)).map(entry => clone(entry));
  };
  expeditionHubMethods.buildExpeditionObservatoryBonusOptions = function (guide = null) {
  ...
  expeditionHubMethods.getFallbackObservatoryExpeditionGuide = function (options = {}) {
    const realm = clampInt(options.realm, 1, 99) || clampInt(this.player?.realm, 1, 99) || 1;
    const sourceBranches = Array.isArray(options.branchOptions) && options.branchOptions.length > 0
      ? options.branchOptions
      : this.getExpeditionBranchPool(getChapterIndexForRealm(realm));
    const nodeCounts = new Map();
    sourceBranches.forEach(entry => {
      readArray(entry?.nodeBias).forEach(nodeType => {
        const safeNodeType = String(nodeType || '').trim();
        if (!safeNodeType) return;
        nodeCounts.set(safeNodeType, (nodeCounts.get(safeNodeType) || 0) + 1);
      });
    });
    const preferredNodes = Array.from(nodeCounts.entries())
      .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0], 'zh-Hans-CN'))
      .map(entry => entry[0])
      .filter(Boolean)
      .slice(0, 4);
    const resolvedPreferredNodes = preferredNodes.length > 0 ? preferredNodes : ['observatory', 'elite', 'trial'];
    const focusLine = formatExpeditionNodeLabels(resolvedPreferredNodes, '关键线路', 4);
    const routeFocusLine = `优先节点：${focusLine}`;
    const trainingTags = getExpeditionNodeLabels(resolvedPreferredNodes, 3);
    const chapterIndex = getChapterIndexForRealm(realm);
    return {
      id: `fallback_expedition_guide_${chapterIndex}`,
      title: `观星精选·第${chapterIndex}章临摹`,
      score: 0,
      note: '',
      icon: '🔭',
      type: 'challenge',
      ...
      themeLabel: '前压爆发',
      featuredTier: '章节临摹',
      featuredTags: trainingTags,
      metricLine: '',
      preferredNodes: resolvedPreferredNodes,
      routeFocusLine,
      insight: null,
      trainingTags,
  ...
  expeditionHubMethods.buildExpeditionObservatoryLink = function (realm = 1, branchOptions = []) {
    const sourceBranches = Array.isArray(branchOptions) && branchOptions.length > 0 ? branchOptions : this.getExpeditionBranchPool(getChapterIndexForRealm(realm));
    const selectedGuide = typeof this.getSelectedObservatoryExpeditionGuide === 'function'
      ? this.getSelectedObservatoryExpeditionGuide({
          silentSync: true
        })
      : null;
    const guide = selectedGuide || (typeof this.getFallbackObservatoryExpeditionGuide === 'function'
      ? this.getFallbackObservatoryExpeditionGuide({
          realm,
          branchOptions: sourceBranches
        })
      : null);
    if (!guide) return null;
    const preferredNodes = readArray(guide.preferredNodes);
    const recommendedBranches = sourceBranches.map(entry => ({
      id: String(entry.id || ''),
      name: String(entry.name || ''),
      matchCount: readArray(entry.nodeBias).filter(nodeType => preferredNodes.includes(nodeType)).length
    })).filter(entry => entry.id && entry.matchCount > 0).sort((a, b) => b.matchCount - a.matchCount || a.name.localeCompare(b.name, 'zh-Hans-CN')).slice(0, 3);
    const fallbackRecommendedBranches = recommendedBranches.length > 0
      ? recommendedBranches
      : sourceBranches.map(entry => ({
          id: String(entry.id || ''),
          name: String(entry.name || ''),
          matchCount: readArray(entry.nodeBias).length
        })).filter(entry => entry.id && entry.matchCount > 0).sort((a, b) => b.matchCount - a.matchCount || a.name.localeCompare(b.name, 'zh-Hans-CN')).slice(0, 3);
    return normalizeExpeditionObservatoryLink({
      ...
      recommendedBranches: fallbackRecommendedBranches,
      bonusOptions: this.buildExpeditionObservatoryBonusOptions(guide),
      selectedBonusId: '',
      resonance: buildExpeditionObservatoryResonance(guide)
    });
  };
  expeditionHubMethods.getSelectedExpeditionObservatoryBonus = function (state = null) {
    const source = state || this.getExpeditionState();
  ...
  expeditionHubMethods.getExpeditionObservatoryResonance = function (state = null) {
    const source = state || this.getExpeditionState();
    return normalizeExpeditionObservatoryResonance(source?.observatoryLink?.resonance);
  };
  expeditionHubMethods.buildExpeditionObservatoryRoutePact = function (state = null, bounty = null) {
    const source = state || this.getExpeditionState();
    const rawBounty = bounty && typeof bounty === 'object' ? bounty : source?.bountyDraft?.find(entry => entry.id === String(bounty || '')) || null;
    const link = source?.observatoryLink;
    ...
    const focusNodeTypes = branchAligned ? branchFocus : engineeringAligned ? engineeringFocus : overlapFocus;
    const target = branchAligned || engineeringAligned ? 2 : 3;
    const reward = buildObservatoryRoutePactReward(focusNodeTypes, {
      branchAligned,
      engineeringAligned
    });
    const rewardLine = formatObservatoryRewardLine(reward);
    const leadParts = [`已把「${targetBounty.name}」并入本章答卷`, selectedBranch?.name ? `支线锚点为「${selectedBranch.name}」` : '', engineeringInfluence?.engineeringTrackName ? `工程主轴为 ${engineeringInfluence.engineeringTrackName}` : ''].filter(Boolean);
  ...
  expeditionHubMethods.getExpeditionObservatoryRoutePact = function (state = null) {
    const source = state || this.getExpeditionState();
    return normalizeExpeditionObservatoryRoutePact(source?.observatoryLink?.routePact);
  };
  expeditionHubMethods.tryArmExpeditionObservatoryRoutePact = function (state = null, preferredBountyId = '') {
    const source = state || this.getExpeditionState();
    const link = source?.observatoryLink;
    if (!source || !link?.selectedBonusId || !source.selectedBranchId || link.routePact) return source;
    const candidateIds = [preferredBountyId, ...readArray(source.activeBountyIds)].map(value => String(value || '')).filter(Boolean);
    const targetBounty = candidateIds.map(id => source.bountyDraft.find(entry => entry.id === id) || null).find(entry => entry && this.buildExpeditionObservatoryRoutePact(source, entry)) || null;
  ...
  expeditionHubMethods.isExpeditionRecommendedBranch = function (state = null, branchId = '') {
    const source = state || this.getExpeditionState();
  ...
  expeditionHubMethods.selectExpeditionRecommendedBranch = function (branchId = '') {
  ...
  expeditionHubMethods.selectExpeditionObservatoryBonus = function (bonusId = '') {
    const state = this.getExpeditionState();
  ...
  expeditionHubMethods.grantExpeditionObservatoryReward = function (reward = null, label = '') {
  ...
  expeditionHubMethods.consumeExpeditionObservatoryBonus = function (state = null, triggerType = 'node_visit', nodeType = '') {
    const source = state || this.getExpeditionState();
    const link = source?.observatoryLink;
  ...
  expeditionHubMethods.advanceExpeditionObservatoryResonance = function (state = null, nodeType = '') {
    const source = state || this.getExpeditionState();
    const link = source?.observatoryLink;
    const safeNodeType = String(nodeType || '').trim();
  ...
  expeditionHubMethods.advanceExpeditionObservatoryRoutePact = function (state = null, nodeType = '') {
    const source = state || this.getExpeditionState();
    const link = source?.observatoryLink;
    const safeNodeType = String(nodeType || '').trim();
    if (!link || !link.selectedBonusId || !safeNodeType) return source;
  ...
  expeditionHubMethods.getExpeditionFactionDisplayName = function (state = null, factionId = '') {
    const source = state || this.getExpeditionState();
    if (!source || !factionId) return '';
    return source.factions.find(entry => entry.id === factionId)?.name || '';
  };
  expeditionHubMethods.resolveExpeditionNemesisReward = function (nemesis = null, outcome = 'defeated') {
  ...
  expeditionHubMethods.grantExpeditionNemesisReward = function (reward = null, status = 'defeated', silent = false) {
  ...
  expeditionHubMethods.applyExpeditionNemesisOutcome = function (state = null, outcome = 'escaped', options = {}) {
    const source = state || this.getExpeditionState();
  ...
  expeditionHubMethods.chooseExpeditionNemesisVariant = function (state = null, nodeType = '') {
    const source = state || this.getExpeditionState();
    const nemesis = source?.activeNemesis;
    if (!nemesis) return normalizeNemesisVariant(null, 0);
    const variants = Array.isArray(nemesis.battleVariants) && nemesis.battleVariants.length > 0 ? nemesis.battleVariants : [normalizeNemesisVariant(null, 0)];
    let targetId = 'hunt';
    ...
    return variants.find(entry => entry.id === targetId) || variants[0] || normalizeNemesisVariant(null, 0);
  };
  expeditionHubMethods.advanceExpeditionNemesisState = function (state = null, node = null, options = {}) {
    const source = state || this.getExpeditionState();
    if (!source || !source.activeNemesis) return source;
    const nemesis = source.activeNemesis;
    ...
    const hostileFaction = source.factions.filter(entry => entry.stance <= -2 && entry.pressureNodeTypes.includes(nodeType)).sort((a, b) => a.stance - b.stance)[0] || null;
    ...
    const tradeEligible = caravan && nodeType && nemesis.tradeNodeTypes.includes(nodeType) && !nemesis.engaged && !nemesis.rewardGranted && (/market|dock|bazaar|vault/.test(branchId) || (source.stats.nodeVisits?.shop || 0) > 0);
    if (tradeEligible) {
    ...
    const releaseEligible = starSeers && nodeType && nemesis.releaseNodeTypes.includes(nodeType) && nemesis.clueRevealed && !nemesis.engaged && !nemesis.rewardGranted && (/spire|archive|orbit|atrium/.test(branchId) || (source.stats.nodeVisits?.observatory || 0) > 0);
    if (releaseEligible) {
    ...
    if (nodeType === 'boss' && nemesis.bossGuardEligible && ACTIVE_NEMESIS_STATUSES.includes(nemesis.status) && (nemesis.status === 'allied' || nemesis.engagedCount > 0 || nemesis.recurrenceCount > 0)) {
  ...
  expeditionHubMethods.createExpeditionStateForRealm = function (realm = 1) {
    const safeRealm = clampInt(realm, 1, 18);
    const chapterIndex = getChapterIndexForRealm(safeRealm);
    const chapter = typeof this.getChapterDisplaySnapshot === 'function' ? this.getChapterDisplaySnapshot(safeRealm) : null;
    const seed = hashString([safeRealm, this.selectedCharacterId || this.player?.characterId || '', this.selectedRunDestinyId || this.player?.runDestiny?.id || '', this.selectedSpiritCompanionId || this.player?.spiritCompanion?.id || ''].join(':'));
    ...
    if (bountyDraft.length < 3) {
      pickUnique(bountyPool.filter(item => !bountyDraft.find(picked => picked.id === item.id)), 3 - bountyDraft.length, seed + 133).forEach(item => bountyDraft.push(item));
    }
  ...
  expeditionHubMethods.initializeExpeditionForRealm = function (realm = 1, force = false) {
  ...
  expeditionHubMethods.findExpeditionFaction = function (factionId = '') {
    const state = this.getExpeditionState();
    if (!state) return null;
    return state.factions.find(entry => entry.id === factionId) || null;
  };
  expeditionHubMethods.appendExpeditionFactionHistory = function (state = null, faction = null, delta = 0, reason = '', options = {}) {
  ...
  expeditionHubMethods.applyExpeditionFactionDelta = function (state = null, factionId = '', delta = 0, reason = '', options = {}) {
  ...
  expeditionHubMethods.getRecentExpeditionFactionLogs = function (state = null, limit = 3) {
    const source = state || this.getExpeditionState();
    if (!source) return [];
    const safeLimit = clampInt(limit, 1, MAX_FACTION_HISTORY);
    const history = readArray(source.factionHistory).map((entry, index) => normalizeFactionHistoryEntry(entry, index)).slice(-safeLimit).reverse();
    if (history.length > 0) return history;
    return readArray(source.factions).filter(entry => entry.lastReason && entry.lastReason !== DEFAULT_FACTION_REASON).sort((a, b) => Math.abs(clampInt(b.stance, -3, 3)) - Math.abs(clampInt(a.stance, -3, 3))).slice(0, safeLimit).map((entry, index) => normalizeFactionHistoryEntry({
  ...
  expeditionHubMethods.appendExpeditionNemesisHistory = function (state = null, entry = null) {
    ...
    if (previous && String(previous.status || '') === normalized.status && String(previous.line || '') === normalized.line && String(previous.sourceType || '') === normalized.sourceType) {
  ...
  expeditionHubMethods.getRecentExpeditionNemesisLogs = function (state = null, limit = 3) {
    const source = state || this.getExpeditionState();
    if (!source) return [];
    const safeLimit = clampInt(limit, 1, MAX_NEMESIS_HISTORY);
    const history = readArray(source.nemesisHistory).map((entry, index) => normalizeNemesisHistoryEntry(entry, index)).slice(-safeLimit).reverse();
    if (history.length > 0) return history;
    const nemesis = source.activeNemesis;
    ...
    return fallback.slice(0, safeLimit).map((entry, index) => normalizeNemesisHistoryEntry(entry, index));
  };
  expeditionHubMethods.getStrategicEngineeringExpeditionInfluence = function (state = null) {
    const source = state || this.getExpeditionState();
    const snapshot = typeof this.getStrategicEngineeringSnapshot === 'function' ? this.getStrategicEngineeringSnapshot() : null;
    ...
    const pressureDelta = safeNumber(pressureDeltas[Math.min(tier, Math.max(0, pressureDeltas.length - 1))], 0);
    return {
  ...
  expeditionHubMethods.getExpeditionBranchEngineeringInsight = function (state = null, branch = null, influence = null) {
    ...
    const matchNodeTypes = Array.from(new Set(branchNodeBias.filter(type => activeInfluence.routeNodeTypes.includes(type)))).slice(0, 4);
    ...
    switch (activeInfluence.engineeringTrackId) {
      case 'observatory':
        engineeringNote = matchNodeTypes.length > 0 ? `${activeInfluence.engineeringTrackName}会在 ${matchLabels} 线提前暴露追猎窗口。` : `${activeInfluence.engineeringTrackName}更偏 ${targetLabels} 线，这条路线的信息收益会缩水。`;
        pressureBias = matchNodeTypes.length > 0 ? '低压锁线' : '信息偏移';
        rewardBias = matchNodeTypes.length > 0 ? '情报前置' : '预警缩水';
        break;
      case 'memory_rift':
        engineeringNote = matchNodeTypes.length > 0 ? `${activeInfluence.engineeringTrackName}会把这条路线改写成 ${matchLabels} 的高收益高波动窗口。` : `${activeInfluence.engineeringTrackName}正在把收益牵向 ${targetLabels} 线，这条路线更容易出现分岔。`;
        pressureBias = matchNodeTypes.length > 0 ? '裂隙改道' : '分岔上升';
        rewardBias = '高收益改写';
        routeDivergence = matchNodeTypes.length > 0 ? 'volatile' : 'drifting';
        break;
      case 'forbidden_altar':
        engineeringNote = matchNodeTypes.length > 0 ? `${activeInfluence.engineeringTrackName}会把这条路线抬成 ${matchLabels} 的高压追猎线。` : `${activeInfluence.engineeringTrackName}正在抬升 ${targetLabels} 线压强，这条路线会更吃资源与净化。`;
        pressureBias = '禁术压强';
        rewardBias = '献祭收割';
        routeDivergence = 'volatile';
        break;
      case 'spirit_grotto':
        engineeringNote = matchNodeTypes.length > 0 ? `${activeInfluence.engineeringTrackName}会把这条路线稳成 ${matchLabels} 的护送线。` : `${activeInfluence.engineeringTrackName}更偏 ${targetLabels} 线，这条路线暂时吃不到稳线补给。`;
  ...
  expeditionHubMethods.getExpeditionBountyEngineeringInsight = function (state = null, bounty = null, selectedBranch = null, influence = null) {
    const activeInfluence = influence || this.getStrategicEngineeringExpeditionInfluence(state);
    ...
    const focusMatch = Array.from(new Set(focusNodeTypes.filter(type => activeInfluence.routeNodeTypes.includes(type)))).slice(0, 4);
    const branchMatch = Array.from(new Set(branchNodeBias.filter(type => activeInfluence.routeNodeTypes.includes(type)))).slice(0, 4);
    ...
    switch (activeInfluence.engineeringTrackId) {
      case 'observatory':
        engineeringNote = aligned ? `${activeInfluence.engineeringTrackName}会把这条赏单锁到 ${matchLabels} 线，追猎窗口会更早显露。` : `${activeInfluence.engineeringTrackName}当前更偏 ${targetLabels} 线，这条赏单需要额外绕路才能吃满情报收益。`;
        ...
        break;
      case 'memory_rift':
        engineeringNote = aligned ? `${activeInfluence.engineeringTrackName}会把这条赏单拖进 ${matchLabels} 的改写窗口，奖励更高但路线更抖。` : `${activeInfluence.engineeringTrackName}正在把收益牵向 ${targetLabels} 线，这条赏单更容易出现分岔。`;
        summaryLine = `工程牵引：${engineeringNote}`;
        ...
        if (stabilityBounty || !aligned) {
          warning = {
            ...
            detail: stabilityBounty ? `${activeInfluence.engineeringTrackName}会把节奏推向 ${targetLabels} 线，稳线类赏单更容易在改道时掉进度。` : `${activeInfluence.engineeringTrackName}正在把路线扯向 ${targetLabels}，这条赏单很难顺带完成。`,
            suggestion: stabilityBounty ? '接单前先确认补给余量，并给改线预留一个节点。' : '若想吃满奖励，优先锁能碰到记忆裂隙 / 事件的路线。',
            engineeringTrackId: activeInfluence.engineeringTrackId,
            engineeringTrackName: activeInfluence.engineeringTrackName,
            engineeringNote,
            routeDivergence,
        ...
        break;
      case 'forbidden_altar':
        engineeringNote = aligned || pressureBounty ? `${activeInfluence.engineeringTrackName}会把这条赏单抬成 ${matchLabels} 的高压收割线，追猎与反噬都会被放大。` : `${activeInfluence.engineeringTrackName}正在抬升 ${targetLabels} 线压强，这条赏单会被迫承担更多风险。`;
        summaryLine = `工程牵引：${engineeringNote}`;
        pressureBias = '禁术压强';
        rewardBias = '献祭收割';
        routeDivergence = 'volatile';
        warning = {
          ...
          detail: stabilityBounty ? `${activeInfluence.engineeringTrackName}会持续抬高追猎压强，稳线类赏单会被迫在保血和结单之间二选一。` : `${activeInfluence.engineeringTrackName}会放大这条赏单的高压节奏，势力与仇敌都更容易同步加压。`,
          suggestion: stabilityBounty ? '若坚持稳线赏单，优先准备净化、护盾与一次补给节点。' : '承接前确认本章有足够资源顶住两轮连续硬战。',
          engineeringTrackId: activeInfluence.engineeringTrackId,
          engineeringTrackName: activeInfluence.engineeringTrackName,
          engineeringNote,
          routeDivergence,
        ...
        break;
      case 'spirit_grotto':
        engineeringNote = aligned || stabilityBounty ? `${activeInfluence.engineeringTrackName}会把这条赏单稳成 ${matchLabels} 的护送线，容错更高。` : `${activeInfluence.engineeringTrackName}更偏 ${targetLabels} 线，这条赏单暂时吃不到稳线补给。`;
        summaryLine = `工程牵引：${engineeringNote}`;
  ...
  expeditionHubMethods.getExpeditionObservatoryEngineeringIntel = function (state = null, nemesisForecast = null, bountyConflictWarnings = null, influence = null) {
    const source = state || this.getExpeditionState();
    const link = source?.observatoryLink;
    ...
    const warnings = Array.isArray(bountyConflictWarnings) ? bountyConflictWarnings : this.getExpeditionBountyConflictWarnings(source);
    ...
    return {
      ...
      engineeringTierLabel: activeInfluence.engineeringTierLabel,
      huntIntel,
      conflictPreview: leadWarning ? `冲突预告：${leadWarning.line}` : `${activeInfluence.engineeringTrackName} 暂未侦测到新的悬赏撕裂，可继续按 ${activeInfluence.routeDirective} 节奏推进。`,
      signalLine: `工程情报：${huntIntel}`
    };
  };
  expeditionHubMethods.getExpeditionNemesisForecast = function (state = null) {
    const source = state || this.getExpeditionState();
    const nemesis = source?.activeNemesis;
    ...
    const overlapNodeTypes = Array.from(new Set(triggerNodeTypes.filter(type => branchNodeBias.includes(type) || bountyFocusNodeTypes.includes(type) || hostilePressureNodeTypes.includes(type)))).slice(0, 4);
    const focusNodeTypes = overlapNodeTypes.length > 0 ? overlapNodeTypes : triggerNodeTypes.length > 0 ? triggerNodeTypes : branchNodeBias;
    const focusNodeLabels = getExpeditionNodeLabels(focusNodeTypes, 3);
    const engineeringInfluence = this.getStrategicEngineeringExpeditionInfluence(source);
    const engineeringFocusNodeTypes = engineeringInfluence ? Array.from(new Set((focusNodeTypes.length > 0 ? focusNodeTypes : triggerNodeTypes).filter(type => engineeringInfluence.routeNodeTypes.includes(type)))).slice(0, 4) : [];
    ...
    let pressureIndex = clampInt((baseScoreMap[status] ?? 42) + Math.min(18, overlapNodeTypes.length * 7) + Math.min(16, clampInt(nemesis.engagedCount, 0, 99) * 4) + Math.min(18, clampInt(nemesis.recurrenceCount, 0, 9) * 8) + (nemesis.alliedFactionName ? 8 : 0) + (nemesis.clueRevealed ? 4 : 0) + (activeBounties.length > 0 && overlapNodeTypes.some(type => bountyFocusNodeTypes.includes(type)) ? 6 : 0) - (alliedSupportNodeTypes.some(type => focusNodeTypes.includes(type)) ? 4 : 0), 0, 100);
    let nextWindowLabel = status === 'guarding' ? '终章首领窗口' : status === 'allied' ? `下个 ${formatExpeditionNodeLabels(focusNodeTypes, '合围')} 节点` : status === 'recurring' ? `下个 ${formatExpeditionNodeLabels(focusNodeTypes, '复现')} 窗口` : status === 'evolved' ? '后续章节同类线路' : ['defeated', 'released', 'traded'].includes(status) ? '本章已结算' : status === 'escaped' ? '章末外溢窗口' : `下个 ${formatExpeditionNodeLabels(focusNodeTypes, '追猎')} 窗口`;
    const drivers = [];
    ...
    if (engineeringInfluence) {
      const engineeringLabels = formatExpeditionNodeLabels(engineeringFocusNodeTypes.length > 0 ? engineeringFocusNodeTypes : engineeringInfluence.routeNodeTypes, '关键线路');
    ...
    let line = status === 'guarding' ? `${nemesis.name} 将在终章首领窗口以「${this.chooseExpeditionNemesisVariant(source, 'boss').label || '护卫终局'}」施压。` : ['defeated', 'released', 'traded'].includes(status) ? `${nemesis.name} · ${statusMeta.label} · ${nemesis.outcomeNote || '当前追猎线已阶段性结算。'}` : `${nemesis.name} · ${nextWindowLabel} · ${drivers[0] || `预计会在 ${formatExpeditionNodeLabels(focusNodeTypes, '关键线路')} 线继续施压。`}`;
    let counterplay = status === 'guarding' ? '终章前保留净化、护盾与一轮爆发，避免护卫战把答卷直接锁死。' : status === 'allied' ? `${nemesis.alliedFactionName || hostileOverlap?.name || '敌对势力'} 已参与追猎，先拆敌意路线或改走 ${formatExpeditionNodeLabels(focusNodeTypes, '低压')} 线。` : status === 'recurring' ? '它已经记住了上次暴露的缺口，下一次接战前先补好护盾与过牌。' : status === 'escaped' ? '章末外溢说明你没把追猎线收干净，后续优先选能提前锁线的事件与观星节点。' : status === 'evolved' ? '同类追猎题会在后续变得更凶，接下来要优先做能稳血和拆压制的构筑。' : ['defeated', 'released', 'traded'].includes(status) ? '当前可把资源重新转回章节主线，把追猎收益兑现成收官优势。' : alliedCover ? `${alliedCover.name} 能在这条线提供援护，先借支援稳住再找反打窗口。` : `在 ${formatExpeditionNodeLabels(focusNodeTypes, '关键线路')} 线保留 1 轮爆发或控制链，别让它先手滚雪球。`;
    if (engineeringInfluence) {
      switch (engineeringInfluence.engineeringTrackId) {
        case 'observatory':
  ...
  expeditionHubMethods.buildExpeditionConflictWarning = function (bounty = null, source = {}) {
  ...
  expeditionHubMethods.getExpeditionBountySignalModel = function (state = null, bounty = null) {
    const source = state || this.getExpeditionState();
    if (!source || !bounty) {
      return {
        focusNodeTypes: [],
    ...
    readArray(source.factions).forEach(faction => {
      ...
      const hostilePressure = focusNodeTypes.filter(type => Number(faction.stance) <= -1 && (readArray(faction.pressureNodeTypes).includes(type) || readArray(faction.likes).includes(type)));
    ...
    peerBounties.forEach(entry => {
      if (isPressureBounty(bounty) && isStabilityBounty(entry) || isStabilityBounty(bounty) && isPressureBounty(entry)) {
        warnings.push(this.buildExpeditionConflictWarning(bounty, {
          severity: 'medium',
    ...
    const deduped = warnings.reduce((list, entry) => {
      ...
      return list;
    }, []).sort((a, b) => getConflictSeverityWeight(b.severity) - getConflictSeverityWeight(a.severity) || a.label.localeCompare(b.label, 'zh-Hans-CN'));
  ...
  expeditionHubMethods.getExpeditionBountyConflictWarnings = function (state = null, signalMap = null) {
    const source = state || this.getExpeditionState();
    if (!source) return [];
    ...
    return warnings.map((entry, index) => normalizeConflictWarning(entry, index)).sort((a, b) => getConflictSeverityWeight(b.severity) - getConflictSeverityWeight(a.severity)).slice(0, 4);
  };
  expeditionHubMethods.applyExpeditionFactionShift = function (factionId = '', delta = 0, reason = '', options = {}) {
    const state = this.getExpeditionState();
    if (!state || !factionId || !delta) return null;
  ...
  expeditionHubMethods.selectExpeditionBranch = function (branchId = '') {
    const state = this.getExpeditionState();
  ...
  expeditionHubMethods.toggleExpeditionBounty = function (bountyId = '') {
    const state = this.getExpeditionState();
    if (!state) return false;
  ...
  expeditionHubMethods.getActiveExpeditionBounties = function (state = null) {
    const source = state || this.getExpeditionState();
    if (!source) return [];
    return source.bountyDraft.filter(entry => source.activeBountyIds.includes(entry.id));
  };
  expeditionHubMethods.evaluateExpeditionBounty = function (bounty, state = null) {
    const source = state || this.getExpeditionState();
    ...
    switch (condition.type) {
      ...
      case 'noRestBossWin':
        ...
        break;
      case 'hpAboveOnBossWin':
        progress = clampInt(stats.bossWins, 0, 1) > 0 && safeNumber(stats.currentHpRatio, 0) >= safeNumber(condition.threshold, 0) ? 1 : 0;
        completed = progress >= 1;
        break;
      default:
        progress = 0;
  ...
  expeditionHubMethods.grantExpeditionBountyReward = function (bounty, options = {}) {
  ...
  expeditionHubMethods.refreshExpeditionProgress = function (silent = false) {
    const state = this.getExpeditionState();
    if (!state) return null;
    const nextDraft = state.bountyDraft.map(entry => {
      ...
      if (updated.completed && !entry.rewardGranted) {
        ...
        this.grantExpeditionBountyReward(updated, {
          silent
        });
      }
      return updated;
    ...
    state.stats.currentHpRatio = this.player ? Math.max(0, Math.min(1, safeNumber(this.player.currentHp, 0) / Math.max(1, safeNumber(this.player.maxHp, 1)))) : safeNumber(state.stats.currentHpRatio, 0);
  ...
  expeditionHubMethods.recordExpeditionNodeVisit = function (node = null) {
    const state = this.getExpeditionState();
    ...
    this.advanceExpeditionNemesisState(state, {
      type
    }, {
      silent: false
    });
  ...
  expeditionHubMethods.applyExpeditionBattleModifiers = function (enemies = [], node = null) {
    const state = this.getExpeditionState();
    ...
    if (nemesis && ACTIVE_NEMESIS_STATUSES.includes(nemesis.status) && nemesis.triggerNodeTypes.includes(nodeType) && nextEnemies[0]) {
      this.advanceExpeditionNemesisState(state, {
        type: nodeType
      }, {
        silent: true
      ...
      target.patterns = Array.isArray(target.patterns) ? target.patterns.map(pattern => {
        const copied = {
          ...pattern
        };
    ...
    if (hostileFactions.length > 0) {
      ...
      nextEnemies = nextEnemies.map((enemy, index) => {
        ...
        copied.patterns = Array.isArray(copied.patterns) ? copied.patterns.map(pattern => {
          const next = {
            ...pattern
          };
  ...
  expeditionHubMethods.applyFriendlyFactionBattleSupport = function (node = null) {
    const state = this.getExpeditionState();
  ...
  expeditionHubMethods.recordExpeditionBattleVictory = function (node = null, enemies = []) {
    const state = this.getExpeditionState();
    if (!state) return null;
    const nodeType = String(node?.type || this.currentBattleNode?.type || '');
    ...
    const nemesisDefeated = state.activeNemesis && ACTIVE_NEMESIS_STATUSES.includes(state.activeNemesis.status) && Array.isArray(enemies) && enemies.some(enemy => String(enemy?.id || '').startsWith(`nemesis_${state.activeNemesis.id}`));
    if (nemesisDefeated) {
      const canRecur = Array.isArray(state.activeNemesis.recursOnVictoryNodeTypes) && state.activeNemesis.recursOnVictoryNodeTypes.includes(nodeType) && state.activeNemesis.recurrenceCount < 1 && state.activeNemesis.status !== 'guarding';
      if (canRecur) {
  ...
  expeditionHubMethods.resolvePendingExpeditionNemesisOutcome = function (state = null, reason = 'realm_clear') {
    const source = state || this.getExpeditionState();
    if (!source || !source.activeNemesis) return source;
    const nemesis = source.activeNemesis;
    if (FINAL_NEMESIS_OUTCOMES.includes(nemesis.status)) return source;
  ...
  expeditionHubMethods.getExpeditionEndingMeta = function (endingId = 'fracture') {
    const source = typeof EXPEDITION_CHAPTER_ENDINGS !== 'undefined' && EXPEDITION_CHAPTER_ENDINGS ? EXPEDITION_CHAPTER_ENDINGS[endingId] : null;
    return source || {
      id: endingId,
      icon: '🧭',
      name: '章节留痕',
  ...
  expeditionHubMethods.determineExpeditionEnding = function (state = null) {
    const source = state || this.getExpeditionState();
  ...
  expeditionHubMethods.buildRunSlateEntry = function (state = null) {
    const source = state || this.getExpeditionState();
    if (!source) return null;
    ...
    const currentVariant = Array.isArray(source.activeNemesis?.battleVariants) ? source.activeNemesis.battleVariants.find(entry => entry.id === source.activeNemesis.currentVariantId) || source.activeNemesis.battleVariants[0] : null;
    ...
    const score = clampInt(completedBounties.reduce((sum, entry) => sum + clampInt(entry.reward?.score || 0, 0, 9999), 0) + clampInt(nemesisReward.score || 0, 0, 9999) + (ending.id === 'alliance' ? 30 : 0) + (ending.id === 'assault' ? 40 : 0) + (ending.id === 'hunt' ? 55 : 0) + (ending.id === 'sealed' ? 26 : 0) + (observatoryResonance?.completed ? OBSERVATORY_RESONANCE_SCORE_BONUS : 0) + (observatoryRoutePact?.completed ? OBSERVATORY_ROUTE_PACT_SCORE_BONUS : 0), 0, 9999);
    return {
      ...
      observatoryLink: normalizeExpeditionObservatoryLink(source.observatoryLink),
      answerReview,
      tags: [branch?.name || '未锁支线', answerReview?.ratingLabel ? `答卷·${answerReview.ratingLabel}` : '', source.activeNemesis?.name ? `宿敌·${nemesisStatusMeta.label}` : '', practiceTopic?.themeLabel ? `课题·${practiceTopic.themeLabel}` : '', source.observatoryLink?.sourceThemeLabel ? `观星·${source.observatoryLink.sourceThemeLabel}` : '', source.observatoryLink?.trainingTags?.[0] ? `训练·${source.observatoryLink.trainingTags[0]}` : '', selectedObservatoryBonus?.label ? `线索·${selectedObservatoryBonus.label}` : '', observatoryResonance?.completed ? `共鸣·${observatoryResonance.label}` : '', observatoryRoutePact?.completed ? `合卷·${observatoryRoutePact.bountyName}` : '', ...completedBounties.map(entry => entry.name), ...source.factions.filter(entry => entry.stance >= 2).map(entry => `${entry.name}结盟`)].filter(Boolean).slice(0, 6),
      timestamp: Date.now()
    };
  };
  expeditionHubMethods.buildRewardExpeditionMeta = function (slate = null, options = {}) {
    ...
    const breakdown = readArray(source.scoreBreakdown).map(line => String(line || '').trim()).filter(Boolean);
    const focusLines = answerReview ? readArray(answerReview.goalHighlights).map(line => String(line || '').trim()).filter(Boolean).slice(0, 3) : [];
    ...
    const agendaFocusLine = agendaResolution?.summaryLine ? String(agendaResolution.summaryLine || '').trim() : '';
    const agendaRecoveryLine = agendaResolution?.recoveryLine ? String(agendaResolution.recoveryLine || '').trim() : '';
    const agendaContractLine = agendaResolution?.contractResolutionLine ? String(agendaResolution.contractResolutionLine || '').trim() : '';
    const agendaContractCostLine = agendaResolution?.contractSignCostLine ? `契押：${String(agendaResolution.contractSignCostLine || '').trim()}` : '';
    const agendaBreakdownLine = agendaResolution ? `洞府议程：${String(agendaResolution.outcomeLabel || '研究未成').trim()}${agendaResolution.grantedLine ? ` · ${String(agendaResolution.grantedLine || '').trim()}` : agendaResolution.reasonLine ? ` · ${String(agendaResolution.reasonLine || '').trim()}` : ''}` : '';
    const agendaContractBreakdownLine = agendaContractLine ? `锁线契约：${agendaContractLine}` : '';
    const lineage = typeof this.getFateLineageSnapshot === 'function' ? this.getFateLineageSnapshot({
      latestSlate: source
    }) : null;
    const lineageSummaryLine = lineage?.summaryLine ? `命盘谱系：${String(lineage.summaryLine || '').trim()}` : '';
    const lineageDetailLine = lineage?.detailLine ? `谱系留痕：${String(lineage.detailLine || '').trim()}` : '';
    const aftereffects = typeof this.getFateAftereffectSnapshot === 'function' ? this.getFateAftereffectSnapshot({
      latestRunId: String(source.id || ''),
      latestSlate: source,
      currentChapterIndex: clampInt(source.chapterIndex, 0, 999)
    }) : null;
    const seasonBoard = typeof this.getSeasonBoardSnapshot === 'function' ? this.getSeasonBoardSnapshot({
      latestSlate: source
    }) : null;
    const aftereffectSummaryLine = aftereffects?.summaryLine ? `界痕后效：${String(aftereffects.summaryLine || '').trim()}` : '';
    const aftereffectStatusLine = aftereffects?.currentStatusLine ? `后效状态：${String(aftereffects.currentStatusLine || '').trim()}` : '';
    const aftereffectDetailLine = aftereffects?.detailLine ? `契约后效：${String(aftereffects.detailLine || '').trim()}` : '';
    const seasonSettlementLine = seasonBoard?.settlement?.summaryLine ? `赛季押卷：${String(seasonBoard.settlement.summaryLine || '').trim()}` : '';
    const seasonDebtLine = seasonBoard?.debtPack?.summaryLine ? String(seasonBoard.debtPack.summaryLine || '').trim() : '';
    const seasonVerificationLine = seasonBoard?.verificationOrders?.[0]?.summaryLine ? `结业验证：${String(seasonBoard.verificationOrders[0].summaryLine || '').trim()}` : '';
    const nemesisParts = [source.nemesisName || '', source.nemesisStatusLabel || source.nemesisStatus || '', source.nemesisFactionName || ''].map(value => String(value || '').trim()).filter(Boolean);
    ...
    return {
      ...
      tags: [...(seasonBoard?.settlement?.outcomeLabel ? [`押卷·${String(seasonBoard.settlement.outcomeLabel || '').trim()}`] : []), ...(seasonBoard?.debtPack?.id ? ['债账·待清'] : []), ...(agendaResolution ? [`议程·${String(agendaResolution.outcomeLabel || '研究未成').trim()}`] : []), ...(agendaResolution?.selectedContractLabel ? [`契约·${agendaResolution.contractSuccess ? '兑现' : '未兑现'}`] : []), ...(agendaResolution?.recoveryEligible ? [`回收·${String(agendaResolution.recoveryTierLabel || agendaResolution.recoveryLabel || '残卷').trim()}`] : []), ...(lineage?.styleTrack?.dominantLabel ? [`谱系·${String(lineage.styleTrack.dominantLabel || '').trim()}`] : []), ...(aftereffects?.primary?.templateLabel ? [`后效·${String(aftereffects.primary.templateLabel || '').trim()}`] : []), ...readArray(source.tags).map(tag => String(tag || '').trim()).filter(Boolean)].filter(Boolean).slice(0, 6),
      branchName,
  ...
  expeditionHubMethods.finalizeExpeditionChapter = function (reason = 'realm_clear') {
    const state = this.getExpeditionState();
    if (!state) return null;
    ...
    const agendaResolution = typeof this.resolveSanctumAgenda === 'function' ? this.resolveSanctumAgenda(reason, {
      state,
      slate,
      answerSheet: typeof this.getExpeditionAnswerSheet === 'function' ? this.getExpeditionAnswerSheet(state) : null
    }) : null;
    this.runSlateArchive = this.normalizeRunSlateArchive([slate, ...(this.runSlateArchive || [])]);
    this.persistRunSlateArchive();
    this.expeditionState = null;
    this.persistActiveExpeditionState();
    this.lastExpeditionRewardMeta = reason === 'realm_clear' || agendaResolution?.recoveryEligible ? this.buildRewardExpeditionMeta(slate, {
      agendaResolution
    }) : null;
    if (reason === 'realm_clear' && typeof this.setObservatoryTrainingFocus === 'function') {
      this.setObservatoryTrainingFocus(this.buildObservatoryTrainingFocusFromSlate(slate), {
        silent: true
      });
    }
    this.renderExpeditionMapPanels();
  ...
  expeditionHubMethods.getLatestRunSlate = function () {
    this.ensureExpeditionBootState();
    return Array.isArray(this.runSlateArchive) && this.runSlateArchive.length > 0 ? this.runSlateArchive[0] : null;
  };
  expeditionHubMethods.getExpeditionPayload = function () {
    const state = this.getExpeditionState();
    ...
    const agenda = typeof this.getSanctumAgendaExpeditionSnapshot === 'function' ? this.getSanctumAgendaExpeditionSnapshot({
    ...
    const observatoryEngineering = state ? this.getExpeditionObservatoryEngineeringIntel(state, nemesisForecast, bountyConflictWarnings, engineeringInfluence) : null;
    const selectedBranchEngineering = state?.selectedBranchId ? branchEngineeringMap.get(state.selectedBranchId) || null : null;
  ...
  expeditionHubMethods.getRunSlateArchiveSummary = function () {
    this.ensureExpeditionBootState();
  ...
  expeditionHubMethods.renderExpeditionMapPanels = function () {
    ...
    const nemesisVariant = state.activeNemesis ? this.chooseExpeditionNemesisVariant(state, state.activeNemesis.lastEncounterNodeType || state.activeNemesis.triggerNodeTypes?.[0] || '') : null;
    ...
    const currentSuggestionLine = relayAdvice && answerSheet?.nextSuggestion && answerSheet.nextSuggestion !== relayAdvice ? `本章推进：${answerSheet.nextSuggestion}` : '';
    ...
    container.innerHTML = `
      ...
      return `
                        <article class="expedition-choice-card ${entry.id === state.selectedBranchId ? 'selected' : ''} ${this.isExpeditionRecommendedBranch(state, entry.id) ? 'suggested' : ''}">
                            ...
                            ${branchEngineering?.engineeringNote ? `<div class="expedition-choice-meta">
      ...
      const signal = bountySignalMap.get(entry.id) || {
        conflictWarnings: [],
        summaryLine: '暂无悬赏情报'
      };
      return `
                        <article class="expedition-choice-card ${state.activeBountyIds.includes(entry.id) ? 'selected' : ''} ${entry.completed ? 'completed' : ''}">
                            ...
                            ${signal.conflictWarnings.length > 0 ? `<div class="expedition-warning-list">
                                    ...
                                    `).join('')}
                                </div>` : `<div class="expedition-observatory-note expedition-bounty-signal">${escapeHtml(signal.summaryLine || '当前赏单与路线暂未出现明显冲突。')}</div>`}
                            ${signal.engineeringNote ? `<div class="expedition-observatory-note expedition-bounty-signal">${escapeHtml(`工程联动：${signal.engineeringNote}`)}</div>` : ''}
                            ${observatoryRoutePact?.bountyId === entry.id ? `<div class="expedition-observatory-note expedition-bounty-signal">${escapeHtml(observatoryRoutePact.completed ? `路线合卷已兑现：${observatoryRoutePact.bountyName}${observatoryRoutePact.rewardLine ? `（${observatoryRoutePact.rewardLine}）` : ''}` : `路线合卷 ${observatoryRoutePact.progress}/${observatoryRoutePact.target}：沿 ${formatExpeditionNodeLabels(observatoryRoutePact.focusNodeTypes, '关键线路')} 线推进，可额外兑现 ${observatoryRoutePact.rewardLine || '章节奖励'}`)}</div>` : ''}
                            <button type="button" class="collection-inline-btn ${state.activeBountyIds.includes(entry.id) ? 'secondary' : ''}"
                                data-expedition-action="toggle-bounty"
                                data-bounty-id="${escapeHtml(entry.id)}">${state.activeBountyIds.includes(entry.id) ? '取消承接' : '承接悬赏'}</button>
                        </article>
                    `;
    }).join('')}
                </div>
            ...
            <section class="expedition-panel-card expedition-signals-card">
                ...
                <div class="expedition-signal-grid">
                    <div class="expedition-signal-block">
                        ...
                        ${recentFactionLogs.length > 0 ? `<div class="expedition-signal-list">
                                ...
                                `).join('')}
                            </div>` : '<div class="expedition-empty-note">最近还没有新的势力波动，先锁路线或踩关键节点再观察本章关系转折。</div>'}
                    </div>
                    <div class="expedition-signal-block warning">
                        ...
                        ${bountyConflictWarnings.length > 0 ? `<div class="expedition-signal-list">
                                ...
                                `).join('')}
                            </div>` : '<div class="expedition-empty-note">当前承接的悬赏与路线暂未出现明显冲突，可以继续按既定节奏推进。</div>'}
                    </div>
                    <div class="expedition-signal-block nemesis ${escapeHtml(nemesisForecast?.pressureTier || 'low')}">
                        ...
                        ${nemesisForecast ? `<article class="expedition-signal-item nemesis ${escapeHtml(nemesisForecast.pressureTier || 'low')}">
                            ...
                            </article>` : '<div class="expedition-empty-note">当前仇敌链路尚未形成明确压制窗口，先踩事件或高压节点再观察追猎方向。</div>'}
                        ${recentNemesisLogs.length > 0 ? `<div class="expedition-signal-list">
            ...
            <section class="expedition-panel-card expedition-observatory-card">
                ...
                ${observatoryLink ? `
                    <div class="expedition-observatory-source">
                        ...
                        ${observatoryLink.sourceFeaturedTags.length > 0 ? `<div class="expedition-chip-row">
                            ${observatoryLink.sourceFeaturedTags.map(tag => `<span class="expedition-chip">${escapeHtml(tag)}</span>`).join('')}
                        </div>` : ''}
                        ${observatoryLink.trainingTags.length > 0 ? `<div class="expedition-chip-row">
                            ${observatoryLink.trainingTags.map(tag => `<span class="expedition-chip">${escapeHtml(tag)}</span>`).join('')}
                        </div>` : ''}
                        ${observatoryLink.routeFocusLine ? `<div class="expedition-observatory-note">${escapeHtml(observatoryLink.routeFocusLine)}</div>` : ''}
                        ${observatoryLink.drillObjective ? `<div class="expedition-observatory-note">${escapeHtml(`演练目标：${observatoryLink.drillObjective}`)}</div>` : observatoryLink.coachBrief ? `<div class="expedition-observatory-note">${escapeHtml(`教练提示：${observatoryLink.coachBrief}`)}</div>` : ''}
                        ${observatoryLink.compareHint ? `<div class="expedition-observatory-note">${escapeHtml(`对比抓手：${observatoryLink.compareHint}`)}</div>` : ''}
                        ${trainingFocus ? `<div class="expedition-observatory-note" data-observatory-training-focus="true">${escapeHtml(`${trainingFocus.chapterName || '上章归卷'}主练回流：${trainingFocus.trainingAdvice}`)}</div>` : ''}
                        ${practiceTopic ? `<div class="expedition-choice-list expedition-practice-topic-list">
                            <article class="expedition-choice-card suggested expedition-practice-topic-card" data-practice-topic-card="true">
                                ...
                                ${practiceTopic.trainingTags.length > 0 ? `<div class="expedition-chip-row">
                        ...
                        ${observatoryRecommendedSummary ? `<div class="expedition-observatory-note">${escapeHtml(observatoryRecommendedSummary)}</div>` : ''}
      ...
      return `
                                <div class="expedition-observatory-action ${isSelected ? 'selected' : ''}" data-observatory-recommended-card="${escapeHtml(entry.id)}"${isSelected ? ' data-selected-recommended-branch="true"' : ''}>
                                    ...
                                    <button type="button" class="collection-inline-btn ${isSelected ? 'secondary' : ''}"
                                        ...
                                        data-expedition-action="select-recommended-branch"
                                        data-recommended-branch-id="${escapeHtml(entry.id)}"
                                        ${isSelected ? 'disabled' : ''}
                        ...
                        ${answerSheet ? `<div class="expedition-choice-list expedition-answer-sheet-list">
                            ...
                            ${answerReview ? `<article class="expedition-choice-card ${escapeHtml(answerReview.ratingTone === 'completed' ? 'completed' : answerReview.ratingTone === 'selected' ? 'selected' : 'suggested')}" data-answer-sheet-summary="true">
                                <div class="expedition-choice-head">
                                ...
                                ${answerReview.tags.length > 0 ? `<div class="expedition-chip-row">
                        ...
                        ${observatoryLink.bonusOptions.map(entry => `
                            <article class="expedition-choice-card ${observatoryLink.selectedBonusId === entry.id ? 'selected' : ''} ${entry.consumed ? 'completed' : ''}">
                                ...
                                <button type="button" class="collection-inline-btn ${observatoryLink.selectedBonusId === entry.id ? 'secondary' : ''}"
                                    data-expedition-action="select-observatory-bonus"
                                    data-bonus-id="${escapeHtml(entry.id)}"
                                    ${observatoryLink.selectedBonusId ? 'disabled' : ''}
                                    >${observatoryLink.selectedBonusId === entry.id ? entry.consumed ? '已触发' : '当前线索' : '启用线索'}</button>
                            </article>
                        `).join('')}
                    </div>
      ...
      return `
        ...
        `;
    this.bindExpeditionPanelDelegates(container);
    syncChapterBrief();
  };
  expeditionHubMethods.bindExpeditionPanelDelegates = function (container = null) {
    if (typeof document === 'undefined') return;
    const root = container && typeof container.addEventListener === 'function' ? container : document.getElementById('map-expedition-panels');
    if (!root || root.__expeditionPanelDelegatesBound) return;
    root.addEventListener('click', event => {
      const target = event.target;
      if (!target || typeof target.closest !== 'function') return;
      const actionButton = target.closest('[data-expedition-action]');
      if (!actionButton || actionButton.disabled) return;
      const action = String(actionButton.dataset.expeditionAction || '');
      if (action === 'select-branch') {
        const branchId = String(actionButton.dataset.branchId || '');
        if (branchId) {
          this.selectExpeditionBranch(branchId);
        }
        return;
      }
      if (action === 'toggle-bounty') {
        const bountyId = String(actionButton.dataset.bountyId || '');
        if (bountyId) {
          this.toggleExpeditionBounty(bountyId);
        }
        return;
      }
      if (action === 'select-recommended-branch') {
        const branchId = String(actionButton.dataset.recommendedBranchId || actionButton.dataset.observatoryRecommendedBranch || '');
        if (branchId) {
          this.selectExpeditionRecommendedBranch(branchId);
        }
        return;
      }
      if (action === 'select-observatory-bonus') {
        const bonusId = String(actionButton.dataset.bonusId || '');
        if (bonusId) {
          this.selectExpeditionObservatoryBonus(bonusId);
        }
      }
    });
    root.__expeditionPanelDelegatesBound = true;
  };
  expeditionHubMethods.startRealm = function (realmLevel, isReplay = false) {
    const result = typeof originalStartRealm === 'function' ? originalStartRealm.call(this, realmLevel, isReplay) : undefined;
    if (this.expeditionHub) {
      registerExpeditionHubMapHooks(this.expeditionHub, this);
    }
  ...
  expeditionHubMethods.advanceToNextRealm = function (clearEssence = 0) {
    const result = typeof originalAdvanceToNextRealm === 'function' ? originalAdvanceToNextRealm.call(this, clearEssence) : undefined;
    if (this.expeditionHub) {
      registerExpeditionHubMapHooks(this.expeditionHub, this);
    }
    if (!this.isEndlessActive()) {
  ...
  expeditionHubMethods.onBattleWon = async function (enemies) {
  ...
  expeditionHubMethods.onBattleLost = async function () {
    ...
    if (this.mode !== 'pvp' && this.getExpeditionState()) {
      ...
      const agendaResolution = typeof this.getSanctumAgendaExpeditionSnapshot === 'function' ? this.getSanctumAgendaExpeditionSnapshot({
        latestRunId: String(slate?.id || '')
      })?.lastResolved || null : null;
      preservedAgendaState = typeof this.getSanctumAgendaSaveState === 'function' ? this.getSanctumAgendaSaveState() : null;
      failureRecoveryNotice = typeof this.buildSanctumAgendaFailureRecoveryNotice === 'function' ? this.buildSanctumAgendaFailureRecoveryNotice(agendaResolution) : null;
    }
    const result = typeof originalOnBattleLost === 'function' ? originalOnBattleLost.call(this) : undefined;
    ...
    if (failureRecoveryNotice) {
      const gameOverText = typeof document !== 'undefined' ? document.getElementById('game-over-text') : null;
      if (gameOverText && failureRecoveryNotice.summaryLine && !String(gameOverText.textContent || '').includes(failureRecoveryNotice.summaryLine)) {
        ...
        gameOverText.textContent = baseText ? `${baseText}｜${failureRecoveryNotice.summaryLine}` : failureRecoveryNotice.summaryLine;
      }
      if (typeof this.showRewardModal === 'function') {
        this.showRewardModal(failureRecoveryNotice.title, failureRecoveryNotice.message || failureRecoveryNotice.summaryLine, failureRecoveryNotice.icon || '📘');
      }
  ...
  expeditionHubMethods.startBattle = function (enemies, node = null) {
    ...
    const prepared = !isPvpBattle ? this.applyExpeditionBattleModifiers(enemies, battleNode) : enemies;
    if (!isPvpBattle) {
      const state = this.getExpeditionState();
      if (state) {
    ...
    const result = typeof originalStartBattle === 'function' ? originalStartBattle.call(this, prepared, battleNode) : undefined;
    if (!isPvpBattle) {
  ...
  expeditionHubMethods.showScreen = function (screenId) {
  ...
  expeditionHubMethods.renderGameToText = function () {
    ...
    try {
      ...
      const rewardSeasonBoard = payload?.reward?.expedition?.seasonBoard && typeof payload.reward.expedition.seasonBoard === 'object' ? JSON.parse(JSON.stringify(payload.reward.expedition.seasonBoard)) : null;
      if (rewardSeasonBoard && payload.expedition) {
        payload.expedition.seasonBoard = rewardSeasonBoard;
      }
      if (payload?.map?.chapter && payload.expedition) {
        ...
        payload.map.chapter.latestAnswerReview = payload.expedition.latestSlate?.answerReview ? normalizeExpeditionAnswerReview(payload.expedition.latestSlate.answerReview) : null;
        ...
        const cloneSeasonBoardFrontier = (frontier = null) => {
          ...
          return {
            ...
            resolution,
            chronicleArchive,
            items: readArray(frontier.items).map((item, index) => ({
              id: item?.id || '',
              laneId: item?.laneId || '',
              label: item?.label || '',
        ...
        payload.map.chapter.seasonBoard = payload.expedition.seasonBoard ? {
          ...
          laneRewards: readArray(payload.expedition.seasonBoard.laneRewards).map(reward => cloneSeasonBoardLaneReward(reward)).filter(Boolean),
          laneRewardSummary: {
  ...
  expeditionHubMethods.loadGame = function () {
    ...
    if (!this.isEndlessActive() && this.player?.realm) {
      ...
      this.expeditionState = active && clampInt(active.realm, 1, 18) === clampInt(this.player.realm, 1, 18) ? this.normalizeExpeditionState(active) : this.createExpeditionStateForRealm(this.player.realm);
      this.persistActiveExpeditionState();
    }
    return result;
  };
  expeditionHubMethods.clearSave = function (options = {}) {
  ...
  expeditionHubMethods.getBuildSnapshotData = function () {
    const snapshot = typeof originalGetBuildSnapshotData === 'function' ? originalGetBuildSnapshotData.call(this) : {};
    ...
    const trainingFocus = typeof this.getObservatoryTrainingFocus === 'function' ? this.getObservatoryTrainingFocus() : null;
    const agendaSnapshot = typeof this.getSanctumAgendaExpeditionSnapshot === 'function' ? this.getSanctumAgendaExpeditionSnapshot({
      latestRunId: String(this.getLatestRunSlate?.()?.id || '')
    }) : null;
    const activeAgenda = agendaSnapshot?.active || null;
    const nextMandateTask = mandateSnapshot && Array.isArray(mandateSnapshot.lanes) ? mandateSnapshot.lanes.flatMap(lane => Array.isArray(lane.tasks) ? lane.tasks : []).find(task => !task.completed) || null : null;
  ...
  expeditionHubMethods.getSanctumOverviewData = function () {
    const data = typeof originalGetSanctumOverviewData === 'function' ? originalGetSanctumOverviewData.call(this) : {
      progress: {},
      rooms: [],
      researches: [],
      goals: [],
    ...
    data.progress = data.progress && typeof data.progress === 'object' ? {
      ...data.progress
    } : {};
    data.progress.runSlateArchives = archiveSummary.count;
    data.progress.topRunSlateScore = archiveSummary.topScore;
    const mandate = typeof this.getHeavenlyMandateExpeditionSnapshot === 'function' ? this.getHeavenlyMandateExpeditionSnapshot() : null;
    const seasonBoard = typeof this.getSeasonBoardSnapshot === 'function' ? this.getSeasonBoardSnapshot() : null;
    const aftereffects = typeof this.getFateAftereffectSnapshot === 'function' ? this.getFateAftereffectSnapshot() : null;
    ...
    data.rooms.push({
      ...
      note: latestSlate ? `最新命盘：${latestSlate.chapterName} · ${latestSlate.endingName} · 评分 ${latestSlate.score}` : '当前还没有远征命盘归档，打通一章后会自动留下第一张答卷。',
      actionLabel: '查看归卷书架',
    ...
    if (!data.goals.find(goal => goal.id === 'run_slate_archive_goal')) {
      data.goals.push({
        ...
        note: latestSlate ? `最新评分 ${latestSlate.score}，可继续通过更高悬赏完成度与仇敌追猎抬高命盘质量。` : '推进任意主线章节，洞府会自动记录章节结局与势力走向。',
        action: 'collection',
        value: 'slates',
...
const rebindExpeditionHubOriginals = game => {
  if (game && typeof game.startRealm === 'function') originalStartRealm = game.startRealm;
  if (game && typeof game.advanceToNextRealm === 'function') originalAdvanceToNextRealm = game.advanceToNextRealm;
  ...
  if (game && typeof game.getBuildSnapshotData === 'function') originalGetBuildSnapshotData = game.getBuildSnapshotData;
  if (game && typeof game.getSanctumOverviewData === 'function') originalGetSanctumOverviewData = game.getSanctumOverviewData;
};
const bindExpeditionHubFacade = (controller, game) => {
  Object.entries(expeditionHubMethods).forEach(([name, fn]) => {
...
const wrapExpeditionHubNoHookMapInstance = mapInstance => {
  if (!mapInstance || typeof mapInstance.completeNode !== 'function' || mapInstance.__expeditionNoHookCompleteWrapped) return;
  const originalCompleteNode = mapInstance.completeNode.bind(mapInstance);
  mapInstance.completeNode = node => {
    const wasCompleted = isMapNodeMarkedCompleted(mapInstance, node);
    const result = originalCompleteNode(node);
    const completedAfterCall = isMapNodeMarkedCompleted(mapInstance, node);
    if (!wasCompleted && result !== false && completedAfterCall && mapInstance.game && typeof mapInstance.game.recordExpeditionNodeVisit === 'function') {
      mapInstance.game.recordExpeditionNodeVisit(node);
    }
    return result;
  };
  mapInstance.__expeditionNoHookCompleteWrapped = true;
};
const installExpeditionHubNoHookMapBridge = controller => {
  if (controller._expeditionNoHookMapBridgeInstalled) return;
  const runtimeGameMap = expeditionHubGlobal?.GameMap || (typeof GameMap === 'function' ? GameMap : null);
  if (typeof runtimeGameMap !== 'function' || typeof runtimeGameMap.prototype?.registerHook === 'function') return;
  if (runtimeGameMap.__expeditionNoHookFactoryWrapped) {
    controller._expeditionNoHookMapBridgeInstalled = true;
    return;
  }
  const WrappedGameMap = class ExpeditionHubWrappedGameMap extends runtimeGameMap {
    constructor(...args) {
      super(...args);
      wrapExpeditionHubNoHookMapInstance(this);
    }
  };
  Object.defineProperty(WrappedGameMap, '__expeditionNoHookFactoryWrapped', {
    configurable: true,
    value: true
  });
  if (expeditionHubGlobal) {
    expeditionHubGlobal.GameMap = WrappedGameMap;
  }
  try {
    (0, eval)('GameMap = globalThis.GameMap;');
  } catch (error) {
    // Ignore when the runtime does not expose a writable script-global GameMap binding.
  }
  try {
    const syncRuntimeGameMap = Function('wrapped', 'GameMap = wrapped; return GameMap;');
    syncRuntimeGameMap(WrappedGameMap);
  } catch (error) {
    // Ignore when running in ESM environments where imported bindings are immutable.
  }
  controller._expeditionNoHookMapBridgeInstalled = true;
};
const registerExpeditionHubMapHooks = (controller, game) => {
  if (!game) return;
  if (!game.map || typeof game.map.registerHook !== 'function') {
    if (game.map) {
      wrapExpeditionHubNoHookMapInstance(game.map);
      controller._expeditionMapHooksRegistered = true;
    } else {
      installExpeditionHubNoHookMapBridge(controller);
    }
    return;
  }
  if (controller._expeditionMapHooksRegistered) return;
  controller._expeditionAfterRenderHook = () => {
    if (game && typeof game.renderExpeditionMapPanels === 'function') {
      game.renderExpeditionMapPanels();
    }
  };
  controller._expeditionAfterUpdateStateHook = () => {
    if (game && typeof game.renderExpeditionMapPanels === 'function') {
      game.renderExpeditionMapPanels();
    }
  };
  controller._expeditionAfterCompleteNodeHook = ({ node, nodeCompletedProcessing }) => {
    if (nodeCompletedProcessing && node && game && typeof game.recordExpeditionNodeVisit === 'function') {
      game.recordExpeditionNodeVisit(node);
    }
  };
  game.map.registerHook('afterRender', controller._expeditionAfterRenderHook);
  game.map.registerHook('afterUpdateState', controller._expeditionAfterUpdateStateHook);
  game.map.registerHook('afterCompleteNode', controller._expeditionAfterCompleteNodeHook);
  controller._expeditionMapHooksRegistered = true;
};
class ExpeditionHubController {
  constructor(game) {
    this.game = game;
    rebindExpeditionHubOriginals(game);
    bindExpeditionHubFacade(this, game);
    registerExpeditionHubMapHooks(this, game);
  }
}
function attachExpeditionHubController(game) {
  if (game.expeditionHub instanceof ExpeditionHubController) {
    registerExpeditionHubMapHooks(game.expeditionHub, game);
    return game.expeditionHub;
  }
  game.expeditionHub = new ExpeditionHubController(game);
  if (typeof game.getSelectedObservatoryExpeditionGuide !== 'function' && typeof game.getFallbackObservatoryExpeditionGuide === 'function') {
...
if (expeditionHubGlobal) {
  expeditionHubGlobal.__attachExpeditionHubController = attachExpeditionHubController;
}
return {
  ExpeditionHubController,
  attachExpeditionHubController
};
})();
export const ExpeditionHubController = ExpeditionHubModule.ExpeditionHubController;
export const attachExpeditionHubController = ExpeditionHubModule.attachExpeditionHubController;
