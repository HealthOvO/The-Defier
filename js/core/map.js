...
export class GameMap {
  constructor(game) {
    ...
    this.hooks = {
      afterRender: new Set(),
      afterUpdateState: new Set(),
      afterCompleteNode: new Set()
    };
  }

  registerHook(name, callback) {
    if (!name || typeof callback !== 'function') return callback;
    if (!this.hooks) {
      this.hooks = {
        afterRender: new Set(),
        afterUpdateState: new Set(),
  ...
  unregisterHook(name, callback) {
    if (!name || !callback || !this.hooks?.[name]) return;
    this.hooks[name].delete(callback);
  }

  runHooks(name, payload = {}) {
    if (!name || !this.hooks?.[name]) return;
    this.hooks[name].forEach(callback => {
      try {
        ...
        console.warn(`Map hook failed: ${name}`, error);
      }
    });
  }
  ...
  // 生成地图
  generate(realm) {
    const cacheKey = this.game && typeof this.game.getMapCacheKey === 'function' ? this.game.getMapCacheKey(realm) : `realm:${realm}`;
    ...
    const cachedMap = cachePool ? cachePool[cacheKey] || (!endlessActive ? cachePool[realm] : null) : null;
    ...
    // 获取层配置
    let config = LEVEL_CONFIG ? LEVEL_CONFIG.getRealmConfig(realm) : {
      rows: 8,
      nodesSequence: []
    };
  ...
  saveStateToCache(cacheKey) {
    ...
    const key = cacheKey || (this.game && typeof this.game.getMapCacheKey === 'function' ? this.game.getMapCacheKey(this.game.player.realm) : this.game.player.realm);
  ...
  getDynamicNodeWeights(row, totalRows, realm, context = null) {
    ...
    const upgradableCount = typeof canUpgradeCard === 'function' ? deck.filter(card => canUpgradeCard(card)).length : 0;
    ...
    const pathDoctrineProfile = player && typeof player.getPathDoctrineProfile === 'function' ? player.getPathDoctrineProfile() : null;
    this.applyFatePathNodeBias(weights, fateRingPath, progress, pathDoctrineProfile);
    if (this.game && typeof this.game.isEndlessActive === 'function' && this.game.isEndlessActive()) {
      const modifiers = typeof this.game.getEndlessModifiers === 'function' ? this.game.getEndlessModifiers() : null;
      const shift = modifiers && modifiers.mapWeightShift && typeof modifiers.mapWeightShift === 'object' ? modifiers.mapWeightShift : {};
    ...
    const rumorProfile = this.game && typeof this.game.getPendingRouteRumorProfile === 'function' ? this.game.getPendingRouteRumorProfile(realm) : null;
    const rumorShift = rumorProfile && rumorProfile.shift && typeof rumorProfile.shift === 'object' ? rumorProfile.shift : null;
    if (rumorShift) {
    ...
    const vowEffects = player && typeof player.getRunVowEffects === 'function' ? player.getRunVowEffects() : null;
    const vowShift = vowEffects && vowEffects.mapWeightShift && typeof vowEffects.mapWeightShift === 'object' ? vowEffects.mapWeightShift : null;
    if (vowShift) {
    ...
    const runPathEffects = player && typeof player.getRunPathEffects === 'function' ? player.getRunPathEffects() : null;
    const runPathShift = runPathEffects && runPathEffects.mapWeightShift && typeof runPathEffects.mapWeightShift === 'object' ? runPathEffects.mapWeightShift : null;
    if (runPathShift) {
    ...
    const engineeringShift = this.game && typeof this.game.getStrategicEngineeringWeightShift === 'function' ? this.game.getStrategicEngineeringWeightShift() : null;
    ...
    const agendaShift = this.game && typeof this.game.getSanctumAgendaWeightShift === 'function' ? this.game.getSanctumAgendaWeightShift() : null;
    ...
    const aftereffectShift = this.game && typeof this.game.getFateAftereffectWeightShift === 'function' ? this.game.getFateAftereffectWeightShift() : null;
    ...
    const seasonBoardShift = this.game && typeof this.game.getSeasonBoardWeightShift === 'function' ? this.game.getSeasonBoardWeightShift() : null;
  ...
  applyFatePathNodeBias(weights, path, progress = 0, doctrineProfile = null) {
    ...
    const doctrineTier = doctrineProfile && doctrineProfile.path === path && Number.isFinite(Number(doctrineProfile.tier)) ? Math.max(0, Math.floor(Number(doctrineProfile.tier) || 0)) : 0;
  ...
  applyStrategicNodeBias(weights, row, totalRows, realm, context = null) {
    ...
    const pendingRumor = this.game && typeof this.game.getPendingRouteRumorProfile === 'function' ? this.game.getPendingRouteRumorProfile(realm + 1) : null;
    const destiny = player && typeof player.getRunDestinyMeta === 'function' ? player.getRunDestinyMeta() : null;
    const destinyBase = destiny && typeof RUN_DESTINIES !== 'undefined' ? RUN_DESTINIES[destiny.id] : null;
    const destinyMaxTier = destinyBase && Array.isArray(destinyBase.tiers) ? Math.max(1, destinyBase.tiers.length) : 1;
    const vows = player && typeof player.getRunVowMetas === 'function' ? player.getRunVowMetas() : [];
    ...
    const spirit = player && typeof player.getSpiritCompanionMeta === 'function' ? player.getSpiritCompanionMeta() : null;
    ...
    const hasStrategicNode = recentRows.some(rowNodes => this.rowContainsType(rowNodes, 'observatory') || this.rowContainsType(rowNodes, 'spirit_grotto') || this.rowContainsType(rowNodes, 'forbidden_altar') || this.rowContainsType(rowNodes, 'memory_rift'));
  ...
  applyWeightPenaltyAndRedistribute(weights, targetType, penalty, boostPlan = []) {
    ...
    const validBoosts = Array.isArray(boostPlan) ? boostPlan.filter(([type, factor]) => type !== targetType && Object.prototype.hasOwnProperty.call(weights, type) && Number.isFinite(Number(factor)) && Number(factor) > 0) : [];
  ...
  applyRouteDiversityPressure(weights, row, totalRows, context = null) {
    ...
    const dominantToBoostPlan = type => {
      if (isCombat(type)) {
        return [['event', 0.22], ['trial', 0.18], ['shop', 0.14], ['forge', 0.12], ['observatory', 0.12], ['spirit_grotto', 0.1], ['memory_rift', 0.12], ['forbidden_altar', 0.06], ['rest', 0.04]];
      }
      return [['enemy', 0.24], ['elite', 0.14], ['event', 0.12], ['trial', 0.12], ['shop', 0.12], ['observatory', 0.1], ['spirit_grotto', 0.1], ['memory_rift', 0.1], ['forbidden_altar', 0.06]];
    };
    ...
    Object.keys(currentCounts).forEach(type => {
      ...
      const inRowBoostPlan = isCombat(type) ? [['event', 0.22], ['trial', 0.18], ['shop', 0.14], ['forge', 0.1], ['observatory', 0.14], ['spirit_grotto', 0.12], ['memory_rift', 0.14], ['forbidden_altar', 0.06], ['rest', 0.02]] : [['enemy', 0.24], ['elite', 0.16], ['trial', 0.12], ['event', 0.12], ['shop', 0.1], ['observatory', 0.1], ['spirit_grotto', 0.1], ['memory_rift', 0.1], ['forbidden_altar', 0.06]];
  ...
  applyLongTermDiversityPressure(weights, row, totalRows, context = null) {
    ...
    const {
      counts,
      total
    } = this.collectRecentTypeCounts(row, 4, context);
    if (total < 6) return;
    ...
    const planByType = type => {
      if (isCombat(type)) {
        return [['event', 0.2], ['trial', 0.18], ['forge', 0.12], ['shop', 0.12], ['observatory', 0.14], ['spirit_grotto', 0.1], ['memory_rift', 0.14], ['forbidden_altar', 0.06], ['rest', 0.04]];
      }
      return [['enemy', 0.24], ['elite', 0.16], ['trial', 0.12], ['event', 0.1], ['shop', 0.12], ['observatory', 0.1], ['spirit_grotto', 0.1], ['memory_rift', 0.1], ['forbidden_altar', 0.06]];
    };
  ...
  applyNodePityPressure(weights, row, totalRows, context = null) {
    ...
    const containsStrategic = recentRows.some(rowNodes => this.rowContainsType(rowNodes, 'observatory') || this.rowContainsType(rowNodes, 'forbidden_altar') || this.rowContainsType(rowNodes, 'memory_rift'));
    const progress = row / Math.max(1, totalRows - 1);
    if (!containsEvent) {
      weights.event += 0.028;
      weights.enemy -= 0.012;
  ...
  applyPathSynergyComboBonus(player, path, node, qualifiedHit) {
    ...
    const healPlayer = (amount, logText = '') => {
      ...
      if (heal <= 0) return;
      player.currentHp = Math.min(Math.max(1, Math.floor(Number(player.maxHp) || 1)), Math.max(0, Math.floor(Number(player.currentHp) || 0)) + heal);
      if (logText && typeof Utils !== 'undefined' && typeof Utils.showBattleLog === 'function') {
        Utils.showBattleLog(logText);
      }
    };
  ...
  getNodeLayoutSignature() {
    ...
    return this.nodes.map((row, rowIndex) => {
  ...
  updateMapState() {
    ...
    this.runHooks('afterUpdateState');
  }
  renderV3Nodes() {
    if (!this.mapView) {
      this.mapView = new MapView(this.game, this);
  ...
  getRealmEnvironment(realm) {
    const envs = {
      ...
      8: {
        name: '天道压制',
        desc: '所有卡牌效果降低 20%',
        effect: 'heaven_suppress'
      },
      9: {
        name: '生死轮回',
        desc: '受到致死伤害时有 50% 几率复活并回满血（限一次）',
        effect: 'rebirth'
      },
      10: {
        name: '大地束缚',
        desc: '灵力上限-1，且闪避率降低20%',
        effect: 'earth_bind'
      },
      11: {
        name: '天人五衰',
      ...
      14: {
        name: '混元无极',
        desc: '敌人对所有伤害拥有20%抗性，且有50%几率免疫眩晕',
        effect: 'chaos_immune'
      },
      15: {
        name: '大道独行',
        desc: '最大生命值降低 30%，但造成的伤害提升50%',
        effect: 'final_trial'
      },
      16: {
        name: '太乙神雷',
        desc: '敌人攻击自带20%吸血，且每回合获得攻击力+1',
        effect: 'vampire_scaling'
      },
      17: {
        name: '大罗法身',
        desc: '敌人免疫控制效果，且每回合回复 20% 最大生命',
        effect: 'immunity_regen'
      },
      18: {
        name: '混沌终焉',
        desc: '玩家所有属性减半，敌人全属性翻倍',
        effect: 'chaos_end'
      }
    };
    return envs[realm] || {
  ...
  updateStatusBar() {
    ...
    const displayRealmName = this.game && typeof this.game.getDisplayRealmName === 'function' ? this.game.getDisplayRealmName(player.realm) : this.getRealmName(player.realm);
  ...
  getDangerTierMetaByIndex(index = 0) {
    ...
    if (safeIndex >= 76) return {
      id: 'extreme',
      label: '极高'
    };
    if (safeIndex >= 61) return {
      id: 'high',
      label: '高压'
    };
    if (safeIndex >= 46) return {
      id: 'medium',
      label: '中压'
    };
    return {
  ...
  resolveNodeRiskDominantModifier(nodeType = '', dominantRisk = '') {
    ...
    return Math.max(-8, Math.min(12, Math.floor(Number(dominantModifiers[dominantRisk]?.[nodeType]) || 0)));
  }
  resolveNodeRiskProfile(node, chapter = null) {
    ...
    const expeditionPayload = this.game && typeof this.game.getExpeditionPayload === 'function' ? this.game.getExpeditionPayload() : null;
    ...
    const hostileCount = Array.isArray(expeditionPayload?.factions) ? expeditionPayload.factions.filter(entry => Number(entry?.stance) <= -2).length : 0;
    const alliedCount = Array.isArray(expeditionPayload?.factions) ? expeditionPayload.factions.filter(entry => Number(entry?.stance) >= 2).length : 0;
    ...
    if (nemesis && ['hunting', 'recurring', 'guarding', 'allied'].includes(String(nemesis.status || '')) && Array.isArray(nemesis.triggerNodeTypes) && nemesis.triggerNodeTypes.includes(node.type)) {
      index += 12;
      modifiers.push(`${nemesis.name} 可能在此现身`);
    }
    const activeBounties = Array.isArray(expeditionPayload?.activeBounties) ? expeditionPayload.activeBounties : [];
    ...
    const reserveParts = [this.getChapterRiskResourceGuidance(dangerProfile, nemesis).replace(/[。！!]+$/g, ''), blueprint.reserveHint];
  ...
  getAccessibleNodeRiskForecast(chapter = null) {
    const accessibleNodes = typeof this.getAccessibleNodes === 'function' ? this.getAccessibleNodes() : [];
    const nodeRisks = accessibleNodes.map(node => this.resolveNodeRiskProfile(node, chapter)).filter(Boolean).sort((a, b) => {
    ...
    return {
      topRisk,
      nodeRisks,
      summary: topRisk ? `${topRisk.label} · DRI ${topRisk.index} · ${topRisk.summary}` : '当前暂无可选节点'
    };
  }
  buildNodeTooltipHtml(node, chapter = null) {
    ...
    const pollutionLine = node && node.polluted ? '<div class="node-tooltip-risk danger">煞气激荡：此处灵脉受损，不可恢复生命，且能量消耗增加。</div>' : '';
    if (!risk) {
      return `
  ...
  collectSpecialWarnings(limit = 3) {
    ...
    return Array.from(new Set(accessibleNodes.map(node => specialLabelMap[node?.type] || null).filter(Boolean))).slice(0, Math.max(1, Math.floor(Number(limit) || 1)));
  }
  resolveFactionTendency(expeditionPayload = null) {
    const factions = Array.isArray(expeditionPayload?.factions) ? expeditionPayload.factions : [];
    ...
    const dominant = [...factions].sort((a, b) => Math.abs(Number(b?.stance) || 0) - Math.abs(Number(a?.stance) || 0))[0];
    const dominantLabel = dominant && dominant.name ? `${dominant.name}${Number(dominant.stance) > 0 ? '↑' : Number(dominant.stance) < 0 ? '↓' : '·'}` : '';
    return `盟友 ${allied} · 对立 ${hostile}${dominantLabel ? ` · 主轴 ${dominantLabel}` : ''}`;
  }
  resolveRecentFactionSignal(expeditionPayload = null) {
    const logs = Array.isArray(expeditionPayload?.recentFactionLogs) ? expeditionPayload.recentFactionLogs : [];
    if (logs.length === 0) {
    ...
    const direction = Number(lead.delta) > 0 ? `↑${Math.abs(Number(lead.delta) || 0)}` : Number(lead.delta) < 0 ? `↓${Math.abs(Number(lead.delta) || 0)}` : String(lead.stanceLabel || '变动');
  ...
  resolveBountyProgress(expeditionPayload = null) {
    ...
    if (activeBounties.length > 0) {
      ...
      const progressSuffix = focusBounty && focusBounty.progressText ? ` · ${focusBounty.name} ${focusBounty.progressText}` : '';
      return `${completedCount}/${activeBounties.length} 已完成${progressSuffix}`;
    }
    const bountyDraft = Array.isArray(expeditionPayload?.bountyDraft) ? expeditionPayload.bountyDraft : [];
  ...
  resolveBountyConflictSignal(expeditionPayload = null) {
    const warnings = Array.isArray(expeditionPayload?.bountyConflictWarnings) ? expeditionPayload.bountyConflictWarnings : [];
  ...
  resolveNemesisForecastSignal(expeditionPayload = null) {
    const forecast = expeditionPayload?.nemesisForecast && typeof expeditionPayload.nemesisForecast === 'object' ? expeditionPayload.nemesisForecast : null;
    if (!forecast) {
    ...
    const parts = [forecast.pressureLabel || '', forecast.windowLabel || ''].filter(Boolean);
    return parts.join(' · ') || '当前仇敌追猎线尚未形成明确压制窗口。';
  }
  normalizeMapCopy(text = '') {
    return String(text || '').replace(/\s+/g, ' ').replace(/。{2,}/g, '。').replace(/；{2,}/g, '；').replace(/。；/g, '；').replace(/；。/g, '；').trim();
  }
  ...
  getStrategicEngineeringSnapshot() {
    return this.game && typeof this.game.getStrategicEngineeringSnapshot === 'function' ? this.game.getStrategicEngineeringSnapshot() : null;
  }
  ...
  resolveStrategicEngineeringRiskSignal() {
    ...
    const sideTrack = Array.isArray(snapshot.activeTracks) ? snapshot.activeTracks.find(entry => entry && entry.trackId !== focus.trackId) : null;
  ...
  getMapSituationOverviewModel(chapter = null) {
    ...
    const tags = [];
    (Array.isArray(chapter?.focusTags) ? chapter.focusTags : []).slice(0, 2).forEach(tag => {
    ...
    return {
      ...
      coreTags,
      riskLevel: dangerProfile ? `DRI ${dangerProfile.index} / 100 · ${dangerProfile.tierLabel || '未定'}` : 'DRI 待推演 · 风险结构未定',
      riskTier: dangerProfile?.tierId || 'none',
      frontierRisk: frontierRisk ? `${frontierRisk.label} · DRI ${frontierRisk.index} · ${frontierRisk.tierLabel}${frontierRisk.modifierLines.length > 0 ? ` · ${frontierRisk.modifierLines[0]}` : ''}` : '暂无前路情报',
  ...
  getChapterRiskResourceGuidance(dangerProfile = null, nemesis = null) {
    ...
    const tierId = dangerProfile && reserveProfile[dangerProfile.tierId] ? dangerProfile.tierId : 'none';
    ...
    const notes = [`生命建议 ≥ ${hpTarget}`, `灵石预留 ≥ ${profile.gold}`];
    if (nemesis && ['hunting', 'recurring', 'guarding', 'allied'].includes(nemesis.status)) {
      notes.push('保留 1 轮爆发或控制链');
    }
    const expeditionPayload = this.game && typeof this.game.getExpeditionPayload === 'function' ? this.game.getExpeditionPayload() : null;
    const activeBountyCount = Array.isArray(expeditionPayload?.activeBounties) ? expeditionPayload.activeBounties.length : 0;
  ...
  updateChapterRiskCardPanel(chapter = null, specialWarnings = []) {
    ...
    const nemesis = chapter && chapter.nemesis ? chapter.nemesis : null;
    const frontierRisk = this.getAccessibleNodeRiskForecast(chapter).topRisk;
    const warningLine = Array.isArray(specialWarnings) && specialWarnings.length > 0 ? `前路异象：${specialWarnings.join(' / ')}` : '前路异象：当前以常规战斗节点为主';
    const highRiskMechanic = dangerProfile ? this.normalizeMapCopy(`${dangerProfile.tierLabel || '风险未定'} · ${dangerProfile.summary || '高压机制正在形成'}`) : '风险结构待推演，先用稳态路线收集战场情报。';
    const frontierLine = frontierRisk ? this.normalizeMapCopy(`${frontierRisk.label} · DRI ${frontierRisk.index} · ${frontierRisk.summary}`) : '当前暂无可选节点，等待地图推进后再评估前线风险。';
    const expeditionPayload = this.game && typeof this.game.getExpeditionPayload === 'function' ? this.game.getExpeditionPayload() : null;
    ...
    const defenseStrategy = this.joinDistinctMapLines([dangerProfile?.counterplay || '', frontierRisk?.counterplay || '', nemesis?.counterplay || '', expeditionPayload?.nemesisForecast?.counterplay || ''], '。') || '优先保留防御链与过牌，先稳住前两轮，再决定爆发窗口。';
  ...
  updateChapterBriefPanel() {
    ...
    const frontierRisk = this.getAccessibleNodeRiskForecast(chapter).topRisk;
    const expeditionPayload = this.game && typeof this.game.getExpeditionPayload === 'function' ? this.game.getExpeditionPayload() : null;
    const seasonBoardFrontier = expeditionPayload?.seasonBoard?.frontier && typeof expeditionPayload.seasonBoard.frontier === 'object' ? expeditionPayload.seasonBoard.frontier : null;
    ...
    const seasonBoardChapterArc = expeditionPayload?.seasonBoard?.chapterArc && typeof expeditionPayload.seasonBoard.chapterArc === 'object' ? expeditionPayload.seasonBoard.chapterArc : null;
    ...
    const seasonBoardFrontierLine = seasonBoardFrontier ? [seasonBoardFrontier.summaryLine || `诸界战线：${seasonBoardFrontier.primaryFrontLabel || seasonBoardFrontier.primaryFrontShortLabel || '主战线'} · ${seasonBoardFrontier.pressureLabel || seasonBoardFrontier.statusLabel || '稳态'}`, seasonBoardFrontier.actionTargetLabel ? `下一跳 ${seasonBoardFrontier.actionTargetLabel}` : ''].filter(Boolean).join(' · ') : '';
    const seasonBoardFrontierDecreeLine = seasonBoardFrontierDecree ? [seasonBoardFrontierDecree.summaryLine || seasonBoardFrontierDecree.title || '本周法旨', seasonBoardFrontierDecree.constraintLine || '', seasonBoardFrontierDecree.actionTargetLabel ? `下一跳 ${seasonBoardFrontierDecree.actionTargetLabel}` : ''].filter(Boolean).join(' · ') : '';
    const seasonBoardFrontierChronicleLine = seasonBoardFrontierChronicle ? [seasonBoardFrontierChronicle.summaryLine || seasonBoardFrontierChronicle.title || '战役史卷', seasonBoardFrontierChronicle.progressLine || '', seasonBoardFrontierChronicle.actionTargetLabel ? `下一跳 ${seasonBoardFrontierChronicle.actionTargetLabel}` : ''].filter(Boolean).join(' · ') : '';
    const seasonBoardFrontierCouncilLine = seasonBoardFrontierCouncil ? [seasonBoardFrontierCouncil.summaryLine || seasonBoardFrontierCouncil.title || '诸界会审', seasonBoardFrontierCouncil.verdictLine || '', seasonBoardFrontierCouncil.supportLine || ''].filter(Boolean).join(' · ') : '';
    const seasonBoardChapterArcLine = seasonBoardChapterArc ? [seasonBoardChapterArc.summaryLine ? `${seasonBoardChapterArc.arcLabel || seasonBoardChapterArc.chapterLabel || '当前章程'} · ${seasonBoardChapterArc.summaryLine}` : `三周一章：${seasonBoardChapterArc.arcLabel || seasonBoardChapterArc.chapterLabel || '当前章程'} · 第 ${seasonBoardChapterArc.weekSlot || 1}/${seasonBoardChapterArc.targetWeeks || 3} 周`, seasonBoardChapterArcPressureWindow?.shortLine || seasonBoardChapterArcPressureWindow?.reasonLine || '', seasonBoardChapterArcObjective?.summaryLine || '', seasonBoardChapterArc.feedbackLine || (seasonBoardChapterArcRescue?.open ? seasonBoardChapterArcRescue.guideLine || seasonBoardChapterArcRescue.reasonLine || seasonBoardChapterArcRescue.statusLabel || '' : seasonBoardChapterArcReview?.endingPreviewLine || seasonBoardChapterArc.statusLine || seasonBoardChapterArc.goalLine || ''), seasonBoardChapterArc.progressText ? `归卷 ${seasonBoardChapterArc.progressText}` : ''].filter(Boolean).join(' · ') : '';
    const seasonBoardChapterArcChipLine = seasonBoardChapterArc ? `三周一章 · 第 ${seasonBoardChapterArc.weekSlot || 1}/${seasonBoardChapterArc.targetWeeks || 3} 周 · ${seasonBoardChapterArcRescue?.statusLabel || seasonBoardChapterArcReview?.statusLabel || seasonBoardChapterArc.windowLabel || '章程'}` : '';
    const seasonBoardChapterArcPressureChipLine = seasonBoardChapterArcPressureWindow?.available ? `章势 · ${seasonBoardChapterArcPressureWindow.statusLabel || '章势压强'} · ${seasonBoardChapterArcPressureWindow.shortLine || '待同步'}` : '';
    const seasonBoardChapterArcObjectiveChipLine = seasonBoardChapterArcObjective?.available ? `章目标 · ${seasonBoardChapterArcObjective.statusLabel || seasonBoardChapterArcObjective.label || '经营目标'} · ${seasonBoardChapterArcObjective.focusLaneLabel || '本周主线'}` : '';
    const bossInfo = this.game && typeof this.game.getRealmBossInfo === 'function' ? this.game.getRealmBossInfo(this.game.player.realm) : null;
    const bossLine = bossInfo && bossInfo.bossName ? `${bossInfo.bossName}${chapter.bossPrompt ? ` · ${chapter.bossPrompt}` : ''}` : chapter.bossPrompt || '本章主宰尚未显形。';
    const dangerProfile = chapter && chapter.dangerProfile ? chapter.dangerProfile : null;
    const dangerLine = dangerProfile ? `DRI ${dangerProfile.index} · ${dangerProfile.tierLabel} · ${dangerProfile.summary || '风险结构已锁定'}` : 'DRI 待推演 · 风险结构尚未生成';
    const dangerCounterplay = dangerProfile && dangerProfile.counterplay ? dangerProfile.counterplay : '先以稳态路线探路，再根据战场反馈调整资源分配。';
    const nemesis = chapter && chapter.nemesis ? chapter.nemesis : null;
    const nemesisLine = nemesis ? `${nemesis.icon || '🎯'} ${nemesis.name} · ${nemesis.statusLabel || '追猎中'}${nemesis.currentVariantLabel ? ` · ${nemesis.currentVariantLabel}` : ''}${nemesis.triggerNodeLabel ? ` · 出没 ${nemesis.triggerNodeLabel}` : ''}${nemesis.alliedFactionName ? ` · 投靠 ${nemesis.alliedFactionName}` : ''}` : '暂无锁定宿敌，当前以章节规则压力为主。';
    const nemesisCounterplay = nemesis && nemesis.counterplay ? nemesis.counterplay : '优先确认高压节点的资源底线，避免被突发追击直接打乱节奏。';
    const nemesisClue = nemesis && nemesis.clueRevealed && nemesis.clueLine ? `线索：${nemesis.clueLine}` : '线索：尚未显露，优先在事件/观星/记忆裂隙里找追猎痕迹。';
    const nemesisForecast = expeditionPayload?.nemesisForecast && typeof expeditionPayload.nemesisForecast === 'object' ? expeditionPayload.nemesisForecast : null;
    const nemesisForecastLine = nemesisForecast ? `${nemesisForecast.pressureLabel || '追猎预判'} · ${nemesisForecast.line || nemesisForecast.windowLabel || '继续观察仇敌链路。'}` : '预判：当前还没有足够线索锁定下一次追猎窗口。';
    ...
    if (panel) {
      ...
      panel.innerHTML = `
            ...
            ${seasonBoardChapterArc ? `<div class="chapter-brief-line compact"
                ...
                data-season-board-chapter-arc-pressure-status="${this.escapeMapText(seasonBoardChapterArcPressureWindow?.statusId || '')}"
            ...
            <div class="chapter-brief-chip-row">
                ...
                ${seasonBoardChapterArcPressureChipLine ? `<span class="chapter-brief-chip warning" data-map-season-board-chip="chapter-arc-pressure">${this.escapeMapText(seasonBoardChapterArcPressureChipLine)}</span>` : ''}
                ...
                ${(Array.isArray(chapter.focusTags) ? chapter.focusTags : []).slice(0, 3).map(tag => `<span class="chapter-brief-chip">${tag}</span>`).join('')}
                <span class="chapter-brief-chip warning">前路异象：${specialWarnings.length > 0 ? specialWarnings.join(' / ') : '常规战斗为主'}</span>
            </div>
        `;
      panel.title = [chapter.stageDesc || '', chapter.routePrompt || '', chapter.bossPrompt || '', nemesis && nemesis.intro ? `${nemesis.name}：${nemesis.intro}` : ''].filter(Boolean).join(' ｜ ');
    }
  }
  updateAdventureBuffPanel() {
    const container = document.getElementById('map-adventure-buffs');
    if (!container) return;
    const buffs = this.game.player && this.game.player.adventureBuffs && typeof this.game.player.adventureBuffs === 'object' ? this.game.player.adventureBuffs : {};
    const buffDefs = [{
      ...
      id: 'victoryHealBoostBattles',
      icon: '🩹',
      name: '战后医护'
    }];
    const active = buffDefs.map(def => {
  ...
  getRouteHintProfile() {
    ...
    const chips = [];
    const pendingRumor = this.game && typeof this.game.getPendingRouteRumorProfile === 'function' ? this.game.getPendingRouteRumorProfile() : null;
    ...
    const activeVows = this.game && this.game.player && typeof this.game.player.getRunVowMetas === 'function' ? this.game.player.getRunVowMetas() : [];
    ...
    const hasStrategicInRows = recentRows.some(rowNodes => this.rowContainsType(rowNodes, 'observatory') || this.rowContainsType(rowNodes, 'spirit_grotto') || this.rowContainsType(rowNodes, 'forbidden_altar') || this.rowContainsType(rowNodes, 'memory_rift'));
    ...
    const counts = this.collectRecentTypeCounts(frontierRow, 4, {
  ...
  updateEndlessPanel() {
    ...
    const state = typeof this.game.ensureEndlessState === 'function' ? this.game.ensureEndlessState() : null;
    const mods = typeof this.game.getEndlessModifiers === 'function' ? this.game.getEndlessModifiers() : {
      enemyHpMul: 1,
      enemyAtkMul: 1,
      rewardGoldMul: 1,
      rewardExpMul: 1
    };
    const pressureProfile = typeof this.game.getEndlessPressureBehaviorProfile === 'function' ? this.game.getEndlessPressureBehaviorProfile() : null;
    ...
    const cycleTheme = typeof this.game.getEndlessCycleThemeProfile === 'function' ? this.game.getEndlessCycleThemeProfile() : null;
    ...
    const mutators = state && Array.isArray(state.activeMutators) && typeof this.game.getEndlessMutatorPool === 'function' ? state.activeMutators.map(id => this.game.getEndlessMutatorPool().find(item => item.id === id)).filter(Boolean) : [];
    panel.style.display = 'block';
    ...
    const behaviorHint = pressureProfile && pressureProfile.summary ? pressureProfile.summary : '敌方行动维持常态';
    const themeChipClass = cycleTheme ? `segment-${Math.max(1, Math.min(5, Math.floor(Number(cycleTheme.segmentIndex) || 1)))}` : 'segment-0';
    const themeChipText = cycleTheme ? `轮段 ${cycleTheme.segmentIndex} · ${cycleTheme.shortName || cycleTheme.name || '稳衡'}` : '轮段 · 稳衡';
    const themeDesc = cycleTheme && cycleTheme.desc ? cycleTheme.desc : '轮段稳定，敌方与收益维持均衡节奏。';
    const seasonChipText = seasonProfile ? `${seasonProfile.icon || '🜁'} ${seasonProfile.name} · ${seasonProfile.weekTag}` : '🜁 赛季待命';
    const directiveChipText = seasonProfile && seasonProfile.directiveName ? seasonProfile.directiveName : '稳态季签';
    const seasonDesc = seasonProfile ? `${seasonProfile.desc || ''}｜季签：${seasonProfile.directiveName}（${seasonProfile.directiveRiskLabel || '平衡'} / ${seasonProfile.selectionModeLabel || '轮转推荐'}）· ${seasonProfile.directiveDesc || '保持稳态推进。'}` : '赛季尚未激活，等待进入无尽轮回后自动加载。';
    const dangerLine = dangerProfile && dangerProfile.line ? dangerProfile.line : '轮回压强 DRI 待推演 · 进入轮回后自动生成主轴';
    const dangerSummary = dangerProfile && dangerProfile.summary ? dangerProfile.summary : '等待压力、轮段与赛季数据收束后生成危险画像。';
    const dangerCounterplay = dangerProfile && dangerProfile.counterplay ? dangerProfile.counterplay : '先稳住资源底线，再根据本轮主轴调整路线。';
    const dangerReserve = dangerProfile && dangerProfile.reserveGuidance ? dangerProfile.reserveGuidance : '保留一条减伤、补件或净化线，避免深轮直接断档。';
    const dangerAxes = dangerProfile && Array.isArray(dangerProfile.axes) ? dangerProfile.axes : [];
    ...
    const seasonGoals = seasonProfile && Array.isArray(seasonProfile.goals) ? seasonProfile.goals : [];
    const collapseSummary = seasonProfile && Array.isArray(seasonProfile.collapseSummary) ? seasonProfile.collapseSummary : [];
    const lastCollapse = seasonProfile && seasonProfile.lastCollapse ? seasonProfile.lastCollapse : null;
    const paranoia = typeof this.game.getEndlessParanoiaEffects === 'function' ? this.game.getEndlessParanoiaEffects() : null;
    ...
    const paranoiaSummary = latestBurden && latestBoon ? `偏执 ${paranoiaLevel} 层：${latestBurden.shortLabel || latestBurden.name} / ${latestBoon.shortLabel || latestBoon.name}` : '尚未触发轮回偏执';
    ...
    panel.innerHTML = `
            ...
            <div class="endless-danger-band ${dangerProfile ? `tier-${dangerProfile.tierId || 'controlled'}` : 'tier-none'}">
                ...
                <div class="endless-danger-grid">
                    ${dangerAxes.length > 0 ? dangerAxes.map(axis => `
                            <div class="endless-danger-chip">
                                <strong>${axis.label}</strong>
                                <span>${Math.max(0, Math.floor(Number(axis.value) || 0))}</span>
                            </div>
                        `).join('') : '<div class="endless-danger-chip idle"><strong>风险维度</strong><span>待推演</span></div>'}
                </div>
            ...
            <div class="endless-season-goals">
                ...
                <div class="endless-season-goal-grid">
                    ${seasonGoals.length > 0 ? seasonGoals.map(goal => `
                        ...
                        `).join('') : '<div class="endless-season-goal pending"><div class="goal-title">赛季目标待生成</div><div class="goal-desc">进入本周无尽后会自动生成目标链。</div></div>'}
                </div>
            </div>
            <div class="endless-collapse-ledger">
                <div class="endless-section-title">崩盘账本</div>
                <div class="endless-collapse-list">
                    ${collapseSummary.length > 0 ? collapseSummary.map(item => `<span class="endless-collapse-chip">${item.label} ${item.count} 次</span>`).join('') : '<span class="endless-collapse-chip idle">本周暂无崩盘记录</span>'}
                </div>
                <div class="endless-collapse-note">
                    ${lastCollapse && lastCollapse.label ? `最近一次：${lastCollapse.label}${lastCollapse.desc ? `｜${lastCollapse.desc}` : ''}` : '账本会在无尽失败后记录主因，方便复盘赛季失分点。'}
                </div>
            </div>
            <div class="endless-paranoia-summary">${paranoiaSummary}</div>
            <div class="endless-paranoia-effects">
                ${paranoiaImpact.length > 0 ? paranoiaImpact.map(item => `<span class="endless-paranoia-effect">${item}</span>`).join('') : '<span class="endless-paranoia-effect idle">等待下一次大轮回抉择</span>'}
            </div>
            <div class="endless-mutators">
                ${mutators.length > 0 ? mutators.map(item => `<span class="endless-mutator-chip" title="${item.desc}">${item.name}</span>`).join('') : '<span class="endless-mutator-chip">当前无词缀</span>'}
            </div>
        `;
  ...
  updateLegacyMissionTracker() {
    const updatePanel = (panelId, payload = null, titlePrefix = '') => {
      ...
      if (progressEl) {
        progressEl.textContent = payload.completed ? `已达成 ${target}/${target}` : `${progress}/${target}`;
      }
    };
    ...
    const runPathTracker = this.game && typeof this.game.getRunPathTrackerState === 'function' ? this.game.getRunPathTrackerState() : null;
    ...
    const nextText = flash.completed ? '命途主线已圆满，后续地图将按这条完成路线继续收束。' : flash.nextPhaseTitle ? `下一阶段：${[flash.nextPhaseLabel, flash.nextPhaseTitle].filter(Boolean).join(' · ')}` : '下一阶段已就绪。';
  ...
  startTrialNode(node, trialConfig = null) {
    ...
    const armedTrial = trialConfig && this.game && typeof this.game.armTrialChallenge === 'function' ? this.game.armTrialChallenge(trialConfig) : trialConfig;
    ...
    const enemyDebuff = armedTrial && armedTrial.enemyDebuff && typeof armedTrial.enemyDebuff === 'object' ? armedTrial.enemyDebuff : null;
    ...
    trialEnemy.patterns = (trialEnemy.patterns || []).map(pattern => {
      if (pattern.type === 'attack' || pattern.type === 'multiAttack') {
        return {
          ...pattern,
          value: Math.floor((pattern.value || 0) * 1.2 * atkMul)
        };
      }
      return {
        ...pattern
  ...
  triggerEvent(node) {
    ...
    if (event) {
      ...
      if (this.game.showRewardModal) {
        this.game.showRewardModal('神秘迷雾', '迷雾散去，你在地上发现了一些东西...\n获得 30 灵石\n获得 15 命环经验', '🌫️', () => {
          this.completeNode(node);
        });
      } else {
        this.completeNode(node);
  ...
  openForgeNode(node) {
    ...
    if (this.game && typeof this.game.handleRunPathProgress === 'function') {
      this.game.handleRunPathProgress('strategicNodeVisit', 1, {
        nodeType: 'forge'
      });
    }
    if (this.game && typeof this.game.showForgeChoiceModal === 'function') {
      this.game.showForgeChoiceModal(node, {
        forgeCost,
        premiumCost,
        temperCost
      });
    ...
    this.applyForgeChoice(node, 'steady', {
      forgeCost,
      premiumCost,
      temperCost
    });
  ...
  pickAndUpgradeCards(count = 1) {
    ...
    const shuffled = typeof Utils !== 'undefined' && Utils.shuffle ? Utils.shuffle(indexed.slice()) : indexed.slice().sort(() => Math.random() - 0.5);
  ...
  applyForgeChoice(node, choice, costs = {}) {
    ...
    const hasFirstForgeBoost = !!(doctrine && doctrine.firstForgeExtraUpgradeOnce > 0 && !doctrine.firstForgeBoostUsed);
  ...
  completeNode(node) {
    ...
    const agendaProgress = this.game && typeof this.game.recordSanctumAgendaNodeProgress === 'function' ? this.game.recordSanctumAgendaNodeProgress(node?.type, {
      realm: this.game?.player?.realm || 0,
    ...
    const cacheKey = this.game && typeof this.game.getMapCacheKey === 'function' ? this.game.getMapCacheKey(this.game.player.realm) : this.game.player.realm;
    this.saveStateToCache(cacheKey);
    if (agendaProgress && this.game && typeof this.game.autoSave === 'function') {
      this.game.autoSave();
    }
    this.runHooks('afterCompleteNode', {
      node,
      nodeCompletedProcessing
    });
    this.render();
  }
