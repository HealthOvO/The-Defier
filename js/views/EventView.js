export class EventView {
    ...
    getEventNarrativePresentation(event, node = null) {
        const currentChapter = this.game.player && typeof this.game.getChapterDisplaySnapshot === 'function'
    ...
    showEventModal(event, node) {
        ...
        const chapterSnapshot = (typeof this.game.getChapterDisplaySnapshot === 'function')
            ? this.game.getChapterDisplaySnapshot(this.game.player?.realm || 1)
            : null;
    ...
    showTrialChallengeSelection(node) {
        ...
        if (!modal || !titleEl || !iconEl || !descEl || !choicesEl || challenges.length === 0) {
            const fallback = challenges[0] || this.game.armTrialChallenge({ id: 'speedKill', name: '逐光试斩', conditions: { maxTurns: 4 }, rewardMultiplier: 1.4, reward: 'law' });
    ...
    showForgeChoiceModal(node, costs = {}) {
        ...
        const upgradableCount = Array.isArray(this.game.player.deck)
            ? this.game.player.deck.filter(c => typeof canUpgradeCard === 'function' && canUpgradeCard(c)).length
            : 0;
        const equippedTreasures = Array.isArray(this.game.player?.equippedTreasures)
            ? this.game.player.equippedTreasures
            : [];
        ...
        const infusionEligibleCount = this.game.player && typeof this.game.player.isTreasureSpiritInfusionEligible === 'function'
            ? equippedTreasures.filter((treasure) => this.game.player.isTreasureSpiritInfusionEligible(treasure)).length
            : 0;
        const spiritMeta = this.game.player && typeof this.game.player.getSpiritCompanionMeta === 'function'
            ...
            : null;
        const workshopTags = this.game.player && typeof this.game.player.getTreasureWorkshopSnapshot === 'function'
            ? this.game.player.getTreasureWorkshopSnapshot('equipped')
                .flatMap((entry) => {
                    const labels = [];
            ...
            if (option.canChoose) {
                btn.onclick = () => {
                    ...
                    if (this.game.map && typeof this.game.map.applyForgeChoice === 'function') {
                        this.game.map.applyForgeChoice(node, option.id, { forgeCost, premiumCost, temperCost });
                    }
                    this.game.autoSave();
                };
            } else {
    ...
    showForgeTreasureDraft(node, mode = 'reforge', costs = {}) {
        ...
        const spiritMeta = this.game.player && typeof this.game.player.getSpiritCompanionMeta === 'function'
            ...
            : null;
        const equippedTreasures = Array.isArray(this.game.player?.equippedTreasures)
            ? this.game.player.equippedTreasures.slice()
            : [];
        ...
        if (mode === 'reforge') {
            ...
            candidates = equippedTreasures.filter((treasure) => (
                this.game.player
                && typeof this.game.player.isTreasureSpiritInfusionEligible === 'function'
                && this.game.player.isTreasureSpiritInfusionEligible(treasure)
            ));
        ...
        candidates.forEach((treasure) => {
            const setLabel = this.game.player && typeof this.game.player.getTreasureSetLabel === 'function'
                ? this.game.player.getTreasureSetLabel(treasure.setTag || '')
                : (treasure.setTag || '散修');
            const statusText = this.describeTreasureWorkshopStatus(treasure);
            const research = this.game.player && typeof this.game.player.getTreasureResearchEntry === 'function'
                ? this.game.player.getTreasureResearchEntry(treasure)
                : null;
            ...
            btn.onclick = () => {
                ...
                if (mode === 'reforge' && typeof this.game.player?.applyTreasureReforge === 'function') {
                    ...
                    gainedInsight = this.game.grantStrategicCurrencies({ insight: 1 }, '器灵灌注').insight || 0;
                    ...
                    gainedInsight = this.game.grantStrategicCurrencies({ insight: 1 }, '套装修正').insight || 0;
