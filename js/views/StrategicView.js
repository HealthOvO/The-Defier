export class StrategicView {
    ...
    showStrategicCardDraftModal(config = {}) {
        const { modal, titleEl, iconEl, descEl, choicesEl } = this.game.getEventModalRefs();
    ...
    showObservatoryNode(node) {
        ...
        if (!modal || !titleEl || !iconEl || !descEl || !choicesEl) {
            ...
            const gained = this.game.grantStrategicCurrencies({ insight: 1 }, '观星推演');
            this.game.finishStrategicNode(
                node,
                '星轨已锁定',
                `第 ${this.game.player.realm + 1} 重将偏向 ${forecast?.label || '机缘补给线'}。\n天机 +${gained.insight || 0}。`,
                forecast?.icon || '🔭'
            );
            return;
        }
        ...
        const nextRealmName = typeof this.game.getDisplayRealmName === 'function'
            ? this.game.getDisplayRealmName(nextRealm)
            : (this.game.map && typeof this.game.map.getRealmName === 'function' ? this.game.map.getRealmName(nextRealm) : `第 ${nextRealm} 重`);
        const env = this.game.map && typeof this.game.map.getRealmEnvironment === 'function'
            ? this.game.map.getRealmEnvironment(nextRealm)
            : { name: '未知天象', desc: '天机仍被迷雾遮蔽。' };
        const bossInfo = typeof this.game.getRealmBossInfo === 'function'
            ? this.game.getRealmBossInfo(nextRealm)
            : null;
        ...
        appendChoice('🗺️', '锁定福缘星轨', '偏向商路、观星、营地与平稳事件。', () => {
            ...
            this.game.finishStrategicNode(
                node,
                '福缘星轨已定',
                `第 ${this.game.player.realm + 1} 重路线趋向：${forecast.label}。\n${forecast.desc}\n天机 +${gained.insight || 0}。`,
                forecast.icon || '🗺️'
            );
        });
        ...
        appendChoice('✨', '校准星图战利', '锁定 1 次高稀有奖励，并获取 1 点天机。', () => {
            ...
            const gained = this.game.grantStrategicCurrencies({ insight: 1 }, '星图校准');
            this.game.closeModal();
            this.game.finishStrategicNode(
                node,
                '星图校准完成',
    ...
    showForbiddenAltarNode(node) {
        const { modal, titleEl, iconEl, descEl, choicesEl } = this.game.getEventModalRefs();
        if (!modal || !titleEl || !iconEl || !descEl || !choicesEl) {
            const gained = this.game.grantStrategicCurrencies({ karma: 1 }, '禁坛残响');
        ...
        const activeVows = this.game.player && typeof this.game.player.getRunVowMetas === 'function'
            ...
            () => {
                ...
                this.showStrategicCardDraftModal({
                    ...
                    onSelect: (card) => {
                        ...
                        const gained = this.game.grantStrategicCurrencies({ karma: 1 }, '禁术血契');
                        this.game.finishStrategicNode(
                            node,
                            '禁术血契完成',
        ...
        appendChoice('🗿', '灾像供契', '向牌组加入【心魔·疑心】，换取一件法宝与 1 点业果。', () => {
            ...
            const treasure = typeof this.game.getWeightedRandomTreasure === 'function'
                ? this.game.getWeightedRandomTreasure()
                : null;
            if (treasure) {
                player.addTreasure(treasure.id);
            }
            const gained = this.game.grantStrategicCurrencies({ karma: 1 }, '灾像供契');
            this.game.closeModal();
            this.game.finishStrategicNode(
                node,
                '灾像供契完成',
    ...
    showForbiddenAltarVowDraft(node, hpCost, draftIds = null) {
        ...
        const picks = Array.isArray(draftIds) && draftIds.length > 0
            ? draftIds.slice(0, 3)
            : this.game.draftRunVowChoices(this.game.player.realm);
        if (!modal || !titleEl || !iconEl || !descEl || !choicesEl || picks.length === 0) {
            this.showForbiddenAltarNode(node);
            return;
        }

        const activeMetas = this.game.player && typeof this.game.player.getRunVowMetas === 'function'
            ? this.game.player.getRunVowMetas()
            : [];
        ...
        picks.forEach((vowId) => {
            ...
            btn.onclick = () => {
                ...
                const gained = this.game.grantStrategicCurrencies({ karma: 1 }, '血祭立誓');
    ...
    showMemoryRiftNode(node) {
        const { modal, titleEl, iconEl, descEl, choicesEl } = this.game.getEventModalRefs();
        if (!modal || !titleEl || !iconEl || !descEl || !choicesEl) {
            const gained = this.game.grantStrategicCurrencies({ insight: 1 }, '裂隙回响');
            this.game.finishStrategicNode(node, '裂隙回响', `天机 +${gained.insight || 0}。`, '🪞');
            return;
        }

        const destinyMeta = this.game.player && typeof this.game.player.getRunDestinyMeta === 'function'
            ? this.game.player.getRunDestinyMeta()
            : null;
        const destinyText = destinyMeta
            ? `${destinyMeta.icon || '✦'} ${destinyMeta.name} ${destinyMeta.tierLabel}`
            : '暂无命格响应';
        const realmText = this.game.map && typeof this.game.map.getRealmName === 'function'
            ? this.game.map.getRealmName(this.game.player.realm)
            : `第 ${this.game.player.realm} 重`;
        ...
        appendChoice('✦', '追忆命格', '优先提升当前命格阶位；若已满阶，则转化为天机与感悟。', () => {
            ...
            const gained = this.game.grantStrategicCurrencies({ insight: 1 }, '裂隙残响');
        ...
        appendChoice('📚', '撕取残章', '从 3 张构筑相关卡中选择 1 张，并获得 1 点天机。', () => {
            ...
            this.showStrategicCardDraftModal({
                ...
                onSelect: (card) => {
                    ...
                    const gained = this.game.grantStrategicCurrencies({ insight: 1 }, '残章采撷');
        ...
        appendChoice('🧭', '逆写路标', '锁定“裂隙回响线”，并让下一场战斗的命环收益更高。', () => {
            ...
            const gained = this.game.grantStrategicCurrencies({ insight: 1 }, '裂隙定标');
            this.game.closeModal();
            this.game.finishStrategicNode(
                node,
                '裂隙路标已改写',
    ...
    showSpiritGrottoDraft(node, draftIds = null) {
        ...
        const picks = Array.isArray(draftIds) && draftIds.length > 0
            ? draftIds.slice(0, 3)
            : this.game.draftSpiritCompanionsForCharacter(this.game.player?.characterId || 'linFeng');
        if (!modal || !titleEl || !iconEl || !descEl || !choicesEl || picks.length === 0) {
            this.showSpiritGrottoNode(node);
            return;
        }

        const currentMeta = this.game.player && typeof this.game.player.getSpiritCompanionMeta === 'function'
            ? this.game.player.getSpiritCompanionMeta()
            : null;
        ...
        picks.forEach((spiritId) => {
            ...
            const modeLabel = canUpgradeCurrent
                ? `维持契约 · 升至 ${this.game.getSpiritCompanionMetaById(meta.id, Math.min(meta.maxTier, 2))?.tierLabel || '下一阶'}`
            ...
            `;
            btn.onclick = () => {
                const gained = this.game.grantStrategicCurrencies({ insight: 1 }, '灵契换契');
                let resultMeta = null;
    ...
    showSpiritGrottoNode(node) {
        const { modal, titleEl, iconEl, descEl, choicesEl } = this.game.getEventModalRefs();
        if (!modal || !titleEl || !iconEl || !descEl || !choicesEl) {
            const gained = this.game.grantStrategicCurrencies({ insight: 1 }, '灵契残响');
            this.game.finishStrategicNode(node, '灵契残响', `天机 +${gained.insight || 0}。`, '🪷');
            return;
        }

        const spiritMeta = this.game.player && typeof this.game.player.getSpiritCompanionMeta === 'function'
            ? this.game.player.getSpiritCompanionMeta()
            : null;
        ...
        appendChoice(
            ...
            () => {
                ...
                const gained = this.game.grantStrategicCurrencies({ insight: 1 }, '灵契共鸣');
        ...
        appendChoice('📖', '追索灵痕', '获得 1 点天机，并让接下来 1 场战斗的命环经验额外提升。', () => {
            ...
            const gained = this.game.grantStrategicCurrencies({ insight: 1 }, '灵痕追索');
...
if (typeof window !== 'undefined') {
    window.StrategicView = StrategicView;
}
