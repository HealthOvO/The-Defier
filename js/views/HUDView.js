...
export class HUDView {
  ...
  showCombo() {
    ...
    if (display && countEl && bonusEl) {
      ...
      if (this.game.comboCount >= 4) display.classList.add('level-4');else if (this.game.comboCount >= 3) display.classList.add('level-3');else display.classList.add('level-2');
    }
  }
  renderTreasures() {
    if (!this.game.player || !this.game.player.treasures) return;
    const containers = [document.getElementById('map-treasures'), document.getElementById('battle-treasures'), document.getElementById('treasures-container') // 顶部栏 (如有)
    ];
