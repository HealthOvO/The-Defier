...
`;
    ...
    const updatesContent = `
            <div class="intro-section">
                ...
                <p class="intro-text">
                    V11 命途长卷 · 三证归卷当前的 v2 新周卷改为 3 章各 3 条誓约组成服务器权威 run 主线；每章保留原稳健/进取誓约，并新增一条在中段二选一出现的处置誓约。失败不扣次数，也不会回退已完成章节。
            ...
            <div class="intro-section">
                ...
                <ul class="intro-list">
                    ...
                    <li><strong>命途长卷</strong>：当前 v2 新周卷为每周 3 章各 3 条誓约；每章原稳健/进取之外，再加入一条处置誓约，由服务器按选定路线创建绑定的权威 run；标准牌组、生命、敌人与周种子不读取本地主线永久战力。</li>
                    <li><strong>三证归卷</strong>：命途长卷、众生试炼、世界裂隙、实时论道与同道远征均可铸证；2/5 发基础荣誉，3/5 及以上只提升档案展示。</li>
                    <li><strong>处置誓约</strong>：定稿誓、审镜誓、封卷誓分别在对应章节中段出现二选一，只改变后续敌人池、路线合同和构筑奖励偏向，不提供永久战力。</li>
                    <li><strong>权威试炼</strong>：赛季司提供 PVE、挑战、远征三种服务器裁定路线；浏览器只提交命令，断线后恢复服务器卷面，完整重放通过后才写入权威赛季进度。旧玩法继续保留离线兼容，不会被误记为权威成绩。</li>
                    <li><strong>战后构筑</strong>：每战奖励提供两张互不重复的新牌和一个服务器指定的精修目标；第二战后可裁去基础牌，牌组不会低于 8 张，并会保留必要的伤害牌与格挡牌。</li>
                    <li><strong>路线合同</strong>：权威战局提供稳进、争衡、险锋三档路线合同，选择前展示敌方压力、构筑回报与路线分；风险越高，敌方生命与意图压力越高，服务器同时锁定对应回报和终局留痕。</li>
                    <li><strong>裂隙战役指令</strong>：每周提供个人、小队与全服三条共同题面；进度只认服务器重放后的正式结算，可选参与，奖励只用于外观与档案。</li>
            ...
            <div class="intro-section">
                <h3>🧭 推荐体验路线</h3>
                <ul class="intro-list">
                    <li>先进入命途长卷选定一条誓约，用固定权威命盘熟悉三章推进、章节中段的处置誓约二选一，以及完整重放。</li>
