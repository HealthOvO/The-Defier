...
import { LAWS, LAW_RESONANCES, getLawPassiveDescription } from "../data/laws.js";
import { TREASURE_CONFIG, TREASURES } from "../data/treasures.js";
/**
 * InventoryView
...
export class InventoryView {
  ...
  showDeckModal(type) {
    ...
    if (!header || !contentContainer) {
      ...
      const oldCloseBtnHtml = closeBtn ? closeBtn.outerHTML : '<button class="modal-close" type="button" data-inventory-action="close-deck-modal">×</button>';
    ...
    switch (type) {
      case 'deck':
        cards = this.game.player.deck;
        deckName = '当前牌组';
        break;
      case 'draw':
        cards = this.game.player.drawPile;
        deckName = '识海';
        break;
      case 'discard':
        cards = this.game.player.discardPile;
        deckName = '轮回';
        break;
    }
    ...
    if (!modal.__inventoryDeckDelegatesBound) {
      modal.addEventListener('click', event => {
        const target = event.target;
        if (!target || typeof target.closest !== 'function') return;
        const closeBtn = target.closest('[data-inventory-action="close-deck-modal"]');
        if (closeBtn && modal.contains(closeBtn)) {
          this.game.closeModal();
        }
      });
      modal.__inventoryDeckDelegatesBound = true;
    }
  ...
  showTreasureBag() {
    ...
    if (!modal) {
      ...
      modal.innerHTML = `
            <div class="modal-content large-modal">
                    <button type="button" class="close-btn" data-inventory-action="close-treasure-bag" aria-label="关闭法宝囊">&times;</button>
                    <h2>🎒 法宝囊</h2>
    ...
    if (!modal.__treasureBagDelegatesBound) {
      modal.addEventListener('click', e => {
        const target = e.target;
        if (!target || typeof target.closest !== 'function') return;
        if (target === modal) {
          this.closeTreasureBagModal(modal);
          return;
        }
        const closeBtn = target.closest('[data-inventory-action="close-treasure-bag"]');
        if (closeBtn && modal.contains(closeBtn)) {
          this.closeTreasureBagModal(modal);
          return;
        }
        const unequipBtn = target.closest('[data-inventory-action="unequip-treasure"]');
        if (unequipBtn && modal.contains(unequipBtn)) {
          const treasureId = String(unequipBtn.dataset.treasureId || '');
          if (treasureId) {
            this.game.player.unequipTreasure(treasureId);
            if (typeof audioManager !== 'undefined') audioManager.playSFX('click');
            this.updateTreasureBagUI();
          }
          return;
        }
        const treasureCard = target.closest('[data-inventory-treasure-id]');
        if (!treasureCard || !modal.contains(treasureCard)) return;
        const treasureId = String(treasureCard.dataset.inventoryTreasureId || '');
        const treasureName = String(treasureCard.dataset.inventoryTreasureName || treasureId || '法宝');
        const treasureDesc = String(treasureCard.dataset.inventoryTreasureDesc || '');
        const source = String(treasureCard.dataset.inventoryTreasureSource || '');
        if (source === 'inventory') {
          if (this.game.player.equipTreasure(treasureId)) {
            if (typeof audioManager !== 'undefined') audioManager.playSFX('equip');
            this.updateTreasureBagUI();
          } else if (this.game.player.equippedTreasures.length >= this.game.player.getMaxTreasureSlots()) {
            this.game.showAlertModal(`⚠️ 法宝槽位已满！请先卸下其他法宝。`, '无法装备');
          }
          return;
        }
        if (source === 'equipped') {
          this.game.showAlertModal(treasureDesc, treasureName);
        }
      });
      modal.addEventListener('contextmenu', e => {
        ...
        const treasureCard = target.closest('[data-inventory-treasure-id]');
        if (!treasureCard || !modal.contains(treasureCard)) return;
        const treasureName = String(treasureCard.dataset.inventoryTreasureName || '法宝');
        const treasureDesc = String(treasureCard.dataset.inventoryTreasureDesc || '');
        e.preventDefault();
        e.stopPropagation();
        this.game.showAlertModal(treasureDesc, treasureName);
    ...
    for (let i = 0; i < maxSlots; i++) {
      ...
      if (treasure) {
        ...
        slot.innerHTML = `
                    ...
                    <button class="unequip-btn" type="button" data-inventory-action="unequip-treasure" data-treasure-id="${treasure.id}">卸下</button>
                `;
        slot.dataset.inventoryTreasureId = treasure.id;
        slot.dataset.inventoryTreasureName = name;
        slot.dataset.inventoryTreasureDesc = desc;
        slot.dataset.inventoryTreasureSource = 'equipped';
      } else {
    ...
    if (inventory.length === 0) {
      ...
      inventory.forEach(t => {
        ...
        el.innerHTML = `
                ...
                `;
        el.title = `${name}: ${desc}`;
        el.dataset.inventoryTreasureId = t.id;
        el.dataset.inventoryTreasureName = name;
        el.dataset.inventoryTreasureDesc = desc;
        el.dataset.inventoryTreasureSource = 'inventory';
  ...
  normalizeLawCodexFilterState(rawState = null) {
    ...
    const allowedElements = ['all', 'thunder', 'fire', 'ice', 'sword', 'void', 'chaos', 'blood', 'life', 'earth', 'wind', 'time', 'space', 'karma', 'reversal', 'metal', 'wood'];
  ...
  initCollection() {
    ...
    const totalResonanceCount = typeof LAW_RESONANCES !== 'undefined' && LAW_RESONANCES ? Object.keys(LAW_RESONANCES).length : 0;
    ...
    const resonanceEntries = typeof LAW_RESONANCES !== 'undefined' && LAW_RESONANCES ? Object.values(LAW_RESONANCES).map(resonance => ({
      resonance,
    ...
    if (summaryEl) {
      summaryEl.innerHTML = ['<span class="codex-side-kicker">收集总览</span>', '<h3>法则收藏进度</h3>', `<div class="codex-summary-metric"><strong>${collectedLawCount}</strong><span>/ ${totalLawCount} 已收录</span></div>`, `<div class="codex-progress-track"><div class="codex-progress-fill" style="width:${lawProgress}%"></div></div>`, '<ul class="codex-side-list compact">', `<li>完成度 ${lawProgress}% · 越接近满库，共鸣路线越完整。</li>`, `<li>当前检索结果 ${filteredLawEntries.length} 条${lawFilterLabels.length > 0 ? ` · 条件 ${lawFilterLabels.join(' / ')}` : ' · 条件 全部法则'}。</li>`, '<li>未收录法则会保留在主区，便于直观看到缺口。</li>', '</ul>'].join('');
    }
    if (resonanceSummaryEl) {
      const visibleActiveResonances = filteredResonanceEntries.filter(entry => entry.state === 'active').length;
      resonanceSummaryEl.innerHTML = ['<span class="codex-side-kicker">当前共鸣</span>', '<h3>羁绊装配</h3>', '<div class="codex-summary-grid two-cols">', `<div class="codex-summary-chip"><strong>${visibleActiveResonances}</strong><span>激活中</span></div>`, `<div class="codex-summary-chip"><strong>${filteredResonanceEntries.length}</strong><span>当前结果 / ${totalResonanceCount}</span></div>`, '</div>', `<p class="codex-side-note">${lawFilterLabels.length > 0 ? `当前已按 ${lawFilterLabels.join(' / ')} 检索。` : '优先补齐同元素法则，可更快点亮主力共鸣链。'}</p>`].join('');
    }
    ...
    for (const entry of filteredLawEntries) {
      ...
      let daoType = '小道';
      if (law.rarity === 'legendary') daoType = '无上大道';else if (law.rarity === 'epic') daoType = '三千大道';else daoType = '旁门小道';
  ...
  resolveLawCodexResonanceRecordState(resonance) {
    ...
    const collectedIds = new Set(Array.isArray(this.game.player?.collectedLaws) ? this.game.player.collectedLaws.map(entry => entry?.id).filter(Boolean) : []);
    const socketedIds = new Set(this.game.player?.fateRing && typeof this.game.player.fateRing.getSocketedLaws === 'function' ? this.game.player.fateRing.getSocketedLaws().filter(Boolean) : []);
  ...
  passesLawCodexLawFilter(entry) {
    ...
    const haystack = [entry.law.id, entry.law.name, entry.law.description, this.game.getLawElementLabel(entry.law.element), this.game.getLawRarityText(entry.law.rarity), ...relatedResonanceNames, ...(Array.isArray(entry.law.unlockCards) ? entry.law.unlockCards : [])].join(' ').toLowerCase();
  ...
  passesLawCodexResonanceFilter(entry) {
    ...
    const haystack = [entry.resonance.id, entry.resonance.name, entry.resonance.description, ...lawNames].join(' ').toLowerCase();
    return haystack.includes(filterState.query.toLowerCase());
  }
  ...
  getTreasureCompendiumPresets() {
    ...
    try {
      ...
      this.treasureCompendiumPresetCache = Array.isArray(parsed) ? parsed.slice(0, 3).map(entry => entry && typeof entry === 'object' ? {
  ...
  getTreasureResearchData(treasure) {
    ...
    const playerResearch = this.game.player && typeof this.game.player.getTreasureResearchEntry === 'function' ? this.game.player.getTreasureResearchEntry(safeTreasure.id) : null;
    ...
    const favoredSets = Array.isArray(runPathMeta?.treasureSynergy?.favoredSets) ? runPathMeta.treasureSynergy.favoredSets : [];
    ...
    const runPathSynergyText = runPathMeta?.treasureSynergy ? runPathSynergyActive ? `命途协同：当前【${runPathMeta.name}】推荐 ${setMeta?.icon || '✦'}${setMeta?.label || '该套装'}，${runPathMeta.treasureSynergy.bonusDesc || runPathMeta.treasureSynergy.summary || '能进一步放大当前构筑。'}` : `当前命途推荐：${runPathMeta.name} 更偏向 ${favoredLabels.join(' / ')}，若要走长线可优先补这些套装。` : '';
    const setText = setMeta ? [`${setMeta.icon || '✦'} ${setMeta.label} · ${setMeta.theme}`, `2件：${setMeta.twoPiece}`, `3件：${setMeta.threePiece}`, playerResearch?.setPieces > 0 ? `当前已装备视作 ${playerResearch.setPieces} 件。` : '当前尚未把该套装正式带入战斗。'].join('<br>') : '暂无套装归属，更多承担单卡对策或前期过渡补强。';
    const buildFitText = [focusTags.length > 0 ? `适配方向：${focusTags.join(' / ')}` : '适配方向：偏泛用补件', role.summary, runPathSynergyText].join('<br>');
    const forgeLines = Array.isArray(playerResearch?.workshopLines) && playerResearch.workshopLines.length > 0 ? playerResearch.workshopLines.slice() : ['尚未进行炼器改造，可先在炼器坊确认是走重铸、器灵还是套装修正。'];
  ...
  getTreasureResearchOverviewData() {
    ...
    const overview = this.game.player && typeof this.game.player.getTreasureWorkshopResearchOverview === 'function' ? this.game.player.getTreasureWorkshopResearchOverview() : fallbackOverview;
    const runPathMeta = this.game.player && typeof this.game.player.getRunPathMeta === 'function' ? this.game.player.getRunPathMeta() : null;
    const favoredSets = Array.isArray(runPathMeta?.treasureSynergy?.favoredSets) ? runPathMeta.treasureSynergy.favoredSets : [];
    const favoredLabels = favoredSets.map(setId => this.game.player?.getTreasureSetLabel?.(setId) || setId);
    const favoredProgress = favoredSets.map(setId => overview.setProgress?.find(entry => entry && entry.id === setId)).filter(Boolean).map(entry => `${entry.icon || '✦'} ${entry.label} ${entry.owned}/${entry.total}（视作 ${entry.pieces} 件）`);
    return {
      ...overview,
      spotlight: [...(runPathMeta?.treasureSynergy ? [`当前命途推荐：${runPathMeta.icon || '🧭'} ${runPathMeta.name} · ${favoredLabels.join(' / ')} · ${runPathMeta.treasureSynergy.summary || '优先围绕推荐套装补件。'}`, favoredProgress.length > 0 ? `当前命途协同进度：${favoredProgress.join(' / ')}` : `当前命途协同仍未成型，可优先补 ${favoredLabels.join(' / ')}。`] : []), `核心件 ${overview.coreOwned || 0}/${overview.coreTotal || 0} · 形态件 ${overview.formOwned || 0}/${overview.formTotal || 0}`, `已激活铭刻 ${overview.activeWorkshops || 0} 条：重铸 ${overview.activeReforges || 0} / 器灵 ${overview.activeInfusions || 0} / 套装修正 ${overview.activeSetEchoes || 0}`, overview.readyInfusions && overview.readyInfusions.length > 0 ? `当前可灌注：${overview.readyInfusions.join('、')}` : '当前没有已装备核心件待灌注，可继续先收集或装备核心法宝。']
    };
  }
  ...
  passesTreasureCompendiumFilter(item) {
    ...
    const research = typeof this.getTreasureResearchData === 'function' ? this.getTreasureResearchData(item?.data || {}) : {
      role: {
        label: ''
      },
      setLabel: '',
    ...
    if (query) {
      const haystack = [item?.id, item?.data?.name, item?.data?.description, item?.data?.lore, this.game.getTreasureSource(item?.data || {}), ...sourceTags, research?.role?.label || '', research?.setLabel || '', research?.setMeta?.theme || '', ...(research?.focusTags || []), this.game.getRarityLabel(rarity).replace(/<[^>]+>/g, '')].join(' ').toLowerCase();
  ...
  showTreasureCompendium() {
    if (this.game && typeof this.game.showScreen === 'function') {
      ...
      this.game.currentScreen = 'treasure-compendium';
    }
    ...
    document.querySelectorAll('#treasure-compendium [data-filter-chip-group]').forEach(chip => {
      ...
      const active = group === 'status' ? filterState.status === value : group === 'rarity' ? filterState.rarities.includes(value) : filterState.sources.includes(value);
    ...
    if (summaryEl) {
      summaryEl.innerHTML = ['<span class="codex-side-kicker">藏品总览</span>', '<h3>法宝收藏进度</h3>', `<div class="codex-summary-metric"><strong>${ownedCount}</strong><span>/ ${allTreasures.length} 已收录</span></div>`, `<div class="codex-progress-track"><div class="codex-progress-fill" style="width:${progress}%"></div></div>`, '<ul class="codex-side-list compact">', `<li>当前筛选结果 ${filteredTreasures.length} 件 · 条件 ${activeFilterLabels.length > 0 ? activeFilterLabels.join(' / ') : '全部法宝'} / 排序 ${sortLabelMap[this.treasureCompendiumSort] || this.treasureCompendiumSort}。</li>`, '<li>点击主区任意法宝即可查看来源、套装关系、适配流派与当前铭刻状态。</li>', '</ul>'].join('');
    }
    if (rarityEl) {
      rarityEl.innerHTML = ['<span class="codex-side-kicker">稀有度分布</span>', '<h3>稀有度概览</h3>', '<div class="codex-summary-grid">', ...rarityCounts.map(entry => `<div class="codex-summary-chip rarity-${entry.rarity}"><strong>${entry.owned}/${entry.total}</strong><span>${rarityNameMap[entry.rarity]}</span></div>`), '</div>', '<p class="codex-side-note">顶部 quick filter 可快速切换，下面多选 chip 可叠加来源与稀有度条件。</p>'].join('');
    }
    if (researchEl) {
      researchEl.innerHTML = ['<span class="codex-side-kicker">炼器研究</span>', '<h3>套装 / 核心件</h3>', '<div class="treasure-research-grid">', ...researchOverview.setProgress.map(entry => `
                ...
                `), '</div>', '<ul class="codex-side-list compact">', ...researchOverview.spotlight.map(line => `<li>${line}</li>`), '</ul>'].join('');
    }
  }
  showTreasureDetail(treasure, isUnlocked) {
    ...
    if (elInfusionState) {
      ...
      if (research?.infusionEligible) elInfusionState.classList.add('eligible');else elInfusionState.classList.add('muted');
    }
    if (!isUnlocked) {
