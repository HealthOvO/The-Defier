...
export const SKILLS = {
  ...
  lifeBloom: {
    ...
    getDescription: level => {
      let heal = 10;
      if (level >= 4) heal = 30;else if (level === 3) heal = 25;else if (level === 2) heal = 15;
  ...
  vajraIndestructible: {
    ...
    getDescription: level => {
      let block = 10;
      if (level >= 4) block = 30;else if (level === 3) block = 25;else if (level === 2) block = 15;
  ...
  starOath: {
    ...
    effect: (player, battle) => {
      ...
      const lawCount = typeof player?.fateRing?.getSocketedLaws === 'function' ? player.fateRing.getSocketedLaws().length : 0;
