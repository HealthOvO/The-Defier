 ...
 */
export function cloneNarrativeTemplate(value) {
...
export function createChapterNarrativeTemplate(config = {}) {
  const chapterIndex = Math.max(1, Math.floor(Number(config.chapterIndex) || 1));
  const beats = Array.isArray(config.beats) ? config.beats.map((beat, index) => ({
...
export function createSpiritStoryTemplate(config = {}) {
  return {
...
export function createCharacterIdentityTemplate(config = {}) {
  return {
...
export function createWorldviewRecallTemplate(config = {}) {
  return {
...
export const V6_CHARACTER_IDENTITY_TEMPLATES = {
...
export const V6_CHAPTER_NARRATIVE_ARCS = {
...
export const V6_SPIRIT_STORY_TEMPLATES = {
...
export const V6_WORLDVIEW_RECALL = [createWorldviewRecallTemplate({
  id: 'recall_fate',
  label: '命格',
...
export const V6_EVENT_PRESENTATION_TEMPLATES = {
  generic: {
    tone: 'chapter',
