export const EXPEDITION_BRANCH_REGIONS = {
  chapter1: [{
    id: 'ember_mines',
    chapterIndex: 1,
    icon: '⛏️',
...
export const EXPEDITION_BOUNTY_TEMPLATES = [{
  id: 'battle_chain',
  name: '破阵快押',
  icon: '⚔️',
  type: 'battle',
...
export const buildNemesisVariants = (profile = {}) => [{
  id: 'hunt',
  label: '追猎压制',
...
export const createNemesisProfile = (profile = {}) => {
  const triggerNodeTypes = Array.isArray(profile.triggerNodeTypes) ? profile.triggerNodeTypes.map(value => String(value || '')).filter(Boolean).slice(0, 4) : ['elite'];
  return {
    ...profile,
    clueLine: String(profile.clueLine || '命盘碎屑里留下了一句不完整的追猎暗号。'),
    clueNodeTypes: Array.isArray(profile.clueNodeTypes) && profile.clueNodeTypes.length > 0 ? profile.clueNodeTypes.map(value => String(value || '')).filter(Boolean).slice(0, 4) : ['event', 'observatory', 'memory_rift'],
    releaseNodeTypes: Array.isArray(profile.releaseNodeTypes) && profile.releaseNodeTypes.length > 0 ? profile.releaseNodeTypes.map(value => String(value || '')).filter(Boolean).slice(0, 3) : ['event', 'observatory'],
    tradeNodeTypes: Array.isArray(profile.tradeNodeTypes) && profile.tradeNodeTypes.length > 0 ? profile.tradeNodeTypes.map(value => String(value || '')).filter(Boolean).slice(0, 3) : ['shop'],
    alliedFactionHints: Array.isArray(profile.alliedFactionHints) && profile.alliedFactionHints.length > 0 ? profile.alliedFactionHints.map(value => String(value || '')).filter(Boolean).slice(0, 3) : ['wild_hunt', 'ash_covenant'],
    recursOnVictoryNodeTypes: Array.isArray(profile.recursOnVictoryNodeTypes) ? profile.recursOnVictoryNodeTypes.map(value => String(value || '')).filter(Boolean).slice(0, 3) : triggerNodeTypes.includes('enemy') ? ['enemy'] : [],
    bossGuardEligible: profile.bossGuardEligible !== false,
    battleVariants: Array.isArray(profile.battleVariants) && profile.battleVariants.length >= 2 ? profile.battleVariants : buildNemesisVariants(profile)
  };
};
...
export const EXPEDITION_NEMESIS_PROFILES = Object.fromEntries(Object.entries(NEMESIS_PROFILES).map(([chapterKey, profiles]) => [chapterKey, Array.isArray(profiles) ? profiles.map(profile => createNemesisProfile(profile)) : []]));
export const EXPEDITION_CHAPTER_ENDINGS = {
  assault: {
    id: 'assault',
    icon: '⚔️',
    name: '强袭过章',
    desc: '高压推进、主动抢节奏，把本章压成一段持续进攻。'
  },
  alliance: {
    id: 'alliance',
    icon: '🤝',
    name: '结盟过章',
    desc: '通过势力支持和路线经营，把压力提前转化为稳定收益。'
  },
  hunt: {
    id: 'hunt',
    icon: '🎯',
    name: '追猎过章',
    desc: '把章节中的仇敌与高压节点一起清算，换来更高命盘评价。'
  },
  sealed: {
    id: 'sealed',
    icon: '🔒',
    name: '封印过章',
    desc: '更强调读局与资源控制，以较少失误拿到完整答卷。'
  },
  scarred: {
    id: 'scarred',
    icon: '🩹',
    name: '残胜过章',
    desc: '勉强冲线但仍保住主线进度，命盘会记下这次危险的答案。'
  },
  fracture: {
    id: 'fracture',
    icon: '🪓',
    name: '裂局过章',
    desc: '虽然勉强前进，但本章的目标没有被完整兑现。'
  }
};
