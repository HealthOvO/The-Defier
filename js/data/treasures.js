...
const getBattleEnemies = player => {
  const battle = player?.game?.battle || null;
  return Array.isArray(battle?.enemies) ? battle.enemies : Array.isArray(player?.game?.currentEnemies) ? player.game.currentEnemies : [];
};
/**
 * The Defier - 法宝数据
  ...
  'phoenix_feather': {
    ...
    callbacks: {
      onBattleStart: player => {
        ...
        const enemies = getBattleEnemies(player);
        if (enemies.length > 0) {
          enemies.forEach(e => {
...
export function canAddTreasure(playerTreasures, newTreasure) {
  ...
  const sameRarityCount = playerTreasures.filter(t => TREASURES[t]?.rarity === rarity).length;
