...
export const ENEMIES = {
  ...
  mirrorDemon: {
    ...
    stealChance: 0.6,
    stealLaw: 'chaosLaw',
    // Changed from chaosControl to match existing law types or chaosLaw if generalized
    gold: {
      min: 500,
      max: 650
    },
  ...
  voidDevourer: {
    ...
    // Devour: Exiles top card of deck
    {
      type: 'heal',
      value: 50,
      intent: '💚'
...
export const BOSS_PHASE_BLUEPRINTS = {
  swordElder: {
    actTwo: {
      threshold: 0.72,
      name: '剑阵封域',
...
export function enrichEnemyMetadata() {
  const createScaledPhasePatterns = (basePatterns = [], config = {}) => {
    ...
    (config.appendPatterns || []).forEach(pattern => {
      if (pattern && typeof pattern === 'object') patterns.push({
        ...pattern
      });
    });
    return patterns;
  ...
  Object.values(ENEMIES).forEach(enemy => {
    if (!enemy.aiProfile) {
      if (enemy.isBoss) enemy.aiProfile = 'boss_adaptive';else if ((enemy.patterns || []).some(p => p.type === 'debuff')) enemy.aiProfile = 'control';else if ((enemy.patterns || []).some(p => p.type === 'defend' || p.type === 'heal')) enemy.aiProfile = 'sustain';else enemy.aiProfile = 'aggressive';
    }
    if (!enemy.resistTags) {
      enemy.resistTags = enemy.resistances ? Object.entries(enemy.resistances).filter(([, v]) => v > 0).map(([k]) => `resist_${k}`) : [];
    }
  ...
  const phaseBossIds = ['swordElder', 'danZun', 'heavenlyDao', 'karmaArbiter', 'ancientSpirit', 'swordSaint', 'tribulationCloud10', 'tribulationCloud15'];
    ...
    boss.phaseConfig = [{
      ...
      patterns: basePatterns.map(p => {
        ...
        if (p.type === 'defend') {
          return {
            ...p,
            value: Math.floor((p.value || 0) * 0.8)
          };
        }
        return {
          ...p
...
export const ELITE_MODIFIERS = [{
  ...
  hpMultiplier: 1.2
} // Changed to dodge chance
]; // 根据天域获取敌人
export function getEnemiesForRealm(realm) {
  return Object.values(ENEMIES).filter(e => e.realm === realm && !e.isBoss && !e.isMinion).map(e => JSON.parse(JSON.stringify(e)));
} // 根据天域获取BOSS
export function getBossForRealm(realm) {
  const boss = Object.values(ENEMIES).find(e => e.realm === realm && e.isBoss);
  return boss ? JSON.parse(JSON.stringify(boss)) : null;
} // 获取随机敌人
export function getRandomEnemy(realm) {
...
export function createEliteEnemy(realm) {
