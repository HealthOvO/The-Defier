...
- 2026-07-15: V11-S114 权威战局 · 敌策读构与战后校正 V1
  - 本轮完成
    - 权威内容目录提升为不可变 `authoritative-trials-v9`，协议、schema 与既有命令面保持不变；敌策 V1 为代表敌人配置三套服务端私有决策策略，前两个玩家回合固定沿用基础敌意，第三个玩家回合是全场唯一读构窗口，之后回到基础循环。
    - 敌策只读取本局冻结牌组、当前血线、上一回合公开反制结果和已选路线合同，并以确定性优先级选择变招；限时题面仅允许显式限时分支，稳进路线不会被隐藏增压。玩家回合开始即冻结当前敌意，出牌过程不能中途改写目标或提示。
    - 公开投影只包含版本、提示编号、标题、说明与已冻结敌意；策略编号、分支编号、阈值、偏好牌型、优先级和意图来源继续留在 canonical 私有状态，不进入公开投影、回执、终局或浏览器文本。
    - 非终局战后根据本战公开表现生成攻式、守式或节奏校正，但只会在原本已经洗出的卡牌报价中标记至多一张匹配候选；不会替换报价、增加候选数量、追加货币或提供永久战力。战斗提示、校正理由、奖励标记和终局敌策摘要已接入既有权威战局与命途长卷界面。
    - v1-v8 继续按 run 创建时冻结的目录、动作语义和哈希链恢复；S114 规则只由 v9 快照中的 `enemyDecision.version=1` 启用，不从当前目录向历史 run 回填。桌面和 390px 手机继续复用现有战斗、奖励与结算区域，不新增页面壳或独立系统。
  - 已验证
    - 最终完整 Node 门禁 `PVP_LIVE_WS_FANOUT_MESSAGE_TIMEOUT_MS=60000 npm run test:node` 通过并输出 `All node checks passed.`；敌策专项覆盖唯一第三回合窗口、冻结意图、限时/稳进边界、私有字段隔离、确定性重放、旧目录兼容和校正报价不替换不增数。
    - 24 个固定种子乘 3 档合同乘 3 类代表场景的 216 局战术平衡门禁通过，共覆盖 3972 次反制机会；64 种子路线门禁与 6 分支乘 24 种子的命途门禁同步通过。挑战策略的稳进、争衡、险锋通关率分别为 100%、100%、79.69%，中位分 764、795、852.5，中位受伤 8、8、14，中位回合 12、13、15，风险与回报仍保持可辨识梯度。
    - `npm run build:pages` 与 `npm run test:frontend:budget` 通过；变基合入当前 `main` 前端优化后，入口 JS gzip 667.1 KB、eager JS gzip 776.0 KB、eager CSS gzip 100.7 KB，均在现有预算内。当前 v9 内容哈希锁定为 `7f2d9e95543a616228436d6c6fe79a7bed79a7452060ad955063e2dd942fec9b`。
    - 最终 fresh 浏览器门禁 `output/s114-enemy-decision-final9` 汇总权威战局与命途长卷 98/98 findings、40 张截图、0 failed、0 console error，expected/report 均为 2，missing / duplicate / unknown module 均为空；真实后端验证冻结敌意、公开提示、终局摘要、校正报价不增数，以及桌面和 390px 战斗/奖励布局。
    - 首轮 `gpt-5.5 + xhigh` 挑战者未发现实现侧 P1；其指出 S114 封板记录缺失和命途 finding 仍写 v8 两项 P2 后，已补齐本条记录、同步为 v9，并在变基到最新 `main` 后重跑完整 Node、生产构建、预算与 fresh final9。第二轮同规格挑战者复核前端 UX、PVP deferred reveal 与 S114 组合差异，确认无剩余 actionable P1/P2。
  - 当前结论
    - S114 已完成设计、开发、历史兼容、平衡回归、完整 Node、生产构建、真实浏览器验收与挑战者终审；本轮仅快进合并并推送 `main`，不再次部署。`https://080305.xyz/` 继续运行已验证的 S113 `authoritative-trials-v8`。

- 2026-07-15: V11-S113 权威战局 · 双线反制押注 V2
  - 本轮完成
    - 权威内容目录提升为不可变 `authoritative-trials-v8`，协议、schema 与既有五类命令保持不变；每个敌意题面同时公开更稳的基础解和收益更高的逆解，服务器在结束回合时自动选择已完成的最高档路线。
    - 攻击逆解要求攻式后接守式且最多 3 张牌，结印逆解要求恰用 2 张牌完成更高伤害，攻守一体逆解要求守式后接攻式且最多 3 张牌；纯攻击/纯格挡角色由服务端卡面效果推导，混合与功能牌不伪造顺序进度。
    - 逆解只提高本次公开减伤或破势，不增加失败惩罚；未达成时仍按原始敌意结算，不弃牌、不加伤、不施加隐藏状态。终局在既有机会数、成功数和反制率外增加逆解成功数。
    - v7 继续作为 Combat Tactics V1 冻结快照；v1-v7 历史 run 按创建时内容恢复，投影、回执、重放和哈希不由 v8 回填。
    - 权威战局与命途长卷沿用现有战斗区，不增加命令、页面、货币或永久成长；桌面并排展示两条公开路线，390px 手机改为单列，并在手牌、动作回执、路线留痕、奖励回看和终局摘要持续显示卡牌角色与服务器最终选线。
  - 已验证
    - 完整 Node 门禁 `PVP_LIVE_WS_FANOUT_MESSAGE_TIMEOUT_MS=60000 npm run test:node` 通过并输出 `All node checks passed.`；覆盖引擎、历史迁移、客户端、UI、路线、命途、挑战、裂隙、接力、实时 PVP、存档与真实后端 E2E。
    - 24 个固定种子乘 3 档合同乘 3 类代表场景的 216 局战术平衡门禁通过；三类敌意均同时存在基础解和逆解成功样本，首张牌不会直接完成任一逆解。64 种子路线门禁与命途分支门禁同步通过，风险收益和分支代价仍可辨识。
    - 历史兼容门禁不再从当前 v8 近似反推 v5/v6：现分别锁定真实发布目录哈希 `b1787bc0...72b280` 与 `25e7e2f3...1c9abc`，恢复当时的卡牌、章节场景和分支字段形状；v7 继续锁定 `a89a387d...a7a062`。
    - `npm run build:pages` 与 `npm run test:frontend:budget` 通过；入口 JS gzip 662.6 KB、eager JS gzip 771.3 KB、eager CSS gzip 94.7 KB，均在现有预算内。
    - 最终 fresh 浏览器门禁 `output/s113-dual-counterplay-final2` 汇总权威战局与命途长卷 94/94 findings、37 张截图、0 failed、0 console error，missing / duplicate / unknown module 均为空；真实后端强制完成一次双线均失败且 0 额外惩罚的回执，并完成守式后接攻式的 `turnabout` 逆解，核对伤害与格挡各削减 3 点。桌面、390px 和两张定向回执截图已人工复核无重叠、黑屏或横向溢出。
    - 首轮挑战者发现 v5/v6 冻结夹具与真实浏览器失败/混合逆解证据不足后均已补强；最终 `gpt-5.5 + xhigh` 挑战者复核完整 diff 与 fresh 产物，确认两项问题关闭且无剩余 actionable P1/P2。
  - 当前结论
    - S113 已完成设计、开发、兼容回放、平衡回归和真实浏览器验收，并于 2026-07-15 部署到 `https://080305.xyz/`。生产侧 `authoritative-trials-v8` 哈希为 `e41817fa...3db`，注册、登录、存档、长期进度、残影与 PVP smoke、数据库/schema、systemd 和 Nginx 均通过；S114 开发不会自动触发第二次部署。

- 2026-07-15: V11-S112 权威战局 · 敌意题面与守势反制 V1
  - 本轮完成
    - 权威内容目录新增不可变 `authoritative-trials-v7`，继续沿用 `authoritative-run-v2 / schemaVersion 2` 与既有命令面；攻击、结印、攻守一体三类敌意分别公开精确反制目标、当前进度与成功收益，失败只按原始公开意图结算，不追加隐藏惩罚。
    - 修复敌方格挡生命周期：结印或攻守一体产生的格挡会保留到下一玩家回合、先吸收卡牌伤害，并在下一次敌方意图结算前清除；旧格挡不会提前消失，也不会跨无关回合叠加。
    - 新增承势步与破印诀两张条件反制牌，分别在攻击意图和敌方已有格挡时获得额外收益；两牌只进入 V7 既有构筑奖励池，不新增大厅、货币、永久战力或客户端自报结果。
    - canonical state、动作回执、公开投影、路线/奖励回看与终局摘要已记录敌意题面机会、成功次数和反制率；玩家界面只展示精确目标与收益，不展示阈值系数等私有目录配置，390px 手机保持单列可读且无横向溢出。
    - 规则完全由 run 创建时冻结的内容快照启用；v1-v6 缺少 `combatTactics` 时继续保留原 RNG、投影、敌方格挡清理、动作哈希与重放语义。迁移门禁锁定独立 v5/v6 历史行，并验证并发启动只插入一条 v7。
    - 战术加入后重新校准命途分支：审镜兑付切换为虚空司书，抢卷分支补足评分倍率；六条分支仍保留可见的回合、受伤和余血代价，没有无代价支配。
  - 已验证
    - 完整 Node 门禁 `PVP_LIVE_WS_FANOUT_MESSAGE_TIMEOUT_MS=60000 npm run test:node` 通过并输出 `All node checks passed.`；覆盖引擎、迁移、客户端、UI、命途、挑战、裂隙、接力、实时 PVP、存档与真实后端 E2E。
    - 新增 24 个固定种子乘 3 档合同乘 3 类代表场景的 216 局战术平衡门禁，共结算 3985 次敌意题面；攻击、结印与攻守一体均同时出现成功和失败，承势步与破印诀均覆盖触发与未触发路径。既有 64 种子路线门禁和 144 局命途分支门禁同步通过。
    - `npm run build:pages` 与 `npm run test:frontend:budget` 通过；入口 JS gzip 662.6 KB、eager JS gzip 771.3 KB、eager CSS gzip 94.7 KB，均在现有预算内。
    - 最终 fresh 浏览器门禁 `output/s112-authoritative-counterplay-final2` 汇总权威战局与命途长卷 93/93 findings、34 张截图、0 failed、0 console error，missing / duplicate / unknown module 均为空；真实页面验证敌方格挡跨回合保留、按原始 10 点伤害吸收 5 点后只损失 5 HP、桌面与 390px 战术面板、回执和终局统计。
    - `gpt-5.4` 挑战者未发现实现侧 P1/P2；其发现真实浏览器格挡吸收断言未独立核对原始伤害的 P2 后，门禁已补为同时校验原始伤害、吸收量、剩余格挡与 HP 实伤，挑战者复核确认关闭。
  - 当前结论
    - S112 已完成设计、开发、兼容回放、平衡回归、真实浏览器验收与挑战者终审；本轮只合并推送，不部署生产，`https://080305.xyz/` 继续运行已验证的 S110。
...
- 2026-07-14: V11-S109 权威路线风险收益与难度契约 V1
  - 本轮完成
    ...
    - 路线合同在选择前展示敌方压力、构筑回报与路线分：稳进使用基础敌方强度；争衡使用 112.5% 生命并给攻击/格挡意图各加 1；险锋使用 125% 生命并给两类意图各加 2，同时提供额外卡牌候选和更高路线分。风险越高，敌方生命与意图压力越高，但回报不会通过永久战力或客户端自报结果发放。
    - 服务端把完整敌方与奖励系数保存在 canonical state，公开投影只给浏览器可读标签、难度、烈度、摘要和分数；`enemyAdjustments / rewardAdjustments` 不会出现在路线、战斗、奖励、回放或终局公开 JSON。状态与内容目录版本不一致时，动作直接 fail closed。
    - 选定合同会贯穿战斗敌意、战后奖励、路线留痕和终局结算；`scoreBreakdown / routeResolution` 以新增字段给出基础分、路线分、场景倍率和逐站选择，同时保留众生试炼、世界裂隙、同道远征与命途长卷依赖的原顶层结算字段。
    - 赛季司与命途长卷 UI 已在择路、战斗、奖励和终局展示合同；旧 v4 投影继续走原界面。390px 小屏会把合同字段收为单列，玩家界面不展示 `contractId`、倍率字段名、内容 hash 或私有系数。
    - 路线选择进入战斗、奖励或下一站时，新阶段会成为可聚焦 region 并滚到当前容器顶部，避免点击底部路线后停在已移除按钮留下的空白区域；键盘焦点不会丢回页面根节点。
    - 完整 Node 回归额外暴露并修复同道远征抢棒竞态：第二个领取者看到其他成员的 `reserved` 棒次时不再代替对方发车并返回成功，只有同一成员和完全相同的请求哈希可以恢复该预留，其他领取者统一得到 `relay_leg_claim_raced`。
  - 已验证
    - 新增 64 个共享固定种子、5 类代表场景、3 种路线策略的确定性平衡门禁，共 960 条完整 run；PVE、挑战、远征、接力先锋与五战长卷均保持风险分数单调、回合成本单调和受伤代价，所有样本在 256 动作内终止。
    - 挑战模式的低压策略为 64/64 通关，高压策略为 60/64 通关；高压策略的中位分更高，但中位受伤、回合数和剩余生命更差，证明路线不是只换皮的分数按钮，也没有在当前自动策略下全面支配稳进路线。
    - 完整 `npm run test:node` 通过，覆盖引擎 golden/replay、v1-v4 迁移、客户端默认 v5、公开投影隔离、UI legacy fallback/转义安全、release marker、接力跨进程竞态和既有 PVP/存档/E2E 回归。
    - 真实后端浏览器中，权威试炼 56/56 findings、命途长卷 27/27 findings，均为 0 failed、0 console error；两条链都先把容器强制置为 `scrollTop=0`，再确认新 `battle` region 位于可视顶部且成为 `document.activeElement`。PVE 与长卷还真实选择险锋路线并验证高压战斗、额外奖励候选和终局路线分。
    - 最终生产产物发布门禁 `output/s109-full-release-final3` 汇总 34/34 模块、1428 findings、714 张截图，0 failed、0 console error，missing / duplicate / unexpected module 均为空；其中前端布局覆盖 424/424 个桌面、矮屏、手机与平板场景。
    - 两轮 `gpt-5.4` 挑战者发现并推动补齐险锋真实浏览器路径、过时长卷目录说明、接力抢棒直接分支测试和阶段焦点连续性；修复后回看未发现剩余 P1/P2。
  - 当前结论
    - S109 已完成设计、开发、平衡模拟、完整回归、真实浏览器验收和挑战者终审，并由本轮 fast-forward 合并 `main` 后推送；本轮不把 S109 再次部署到生产，`https://080305.xyz/` 继续运行已验证的 S108。

- 2026-07-14: V11-S108 权威构筑深化 V1
  - 本轮完成
    - 权威内容目录升级为不可变 `authoritative-trials-v4`，协议和状态结构继续保持 `authoritative-run-v2 / schemaVersion 2`；旧 v1-v3 目录、卡牌实例、动作事件和哈希链按原始形状恢复，不会被补写 `upgraded=false` 或新字段。
    - 每战奖励提供两张互不重复的新牌、一个服务器指定的精修目标，以及按生命和牌组状态出现的调息、固本或裁牌选择。奖励 ID 绑定确切卡牌实例，客户端只提交 `rewardId`，不能自行指定精修或裁牌目标。
    - 第二战后可裁去基础牌，牌组不会低于 8 张；服务端还会重新校验至少保留 2 张直接伤害牌和 2 张格挡牌、整局最多裁牌 2 次。观微、裂隙、流炁等循环或倍率核心不提供直接精修，所有卡牌继续保持至少 1 点能量成本。
    - 12 套基础、接力与长卷场景拥有各自奖励池、精修优先级、裁牌优先级与整备阈值；战间固定回复同步收紧，构筑选择不再只是三种模式共用同一套奖励模板。
    - 玩家界面新增精修牌标识、精确目标说明、精修/裁牌领取回执、局中牌组状态和终局构筑摘要；旧目录缺少 `cardId` 的历史新牌回执仍显示“新卡牌”，不会退化成“未知牌”。
  - 已验证
    - 引擎黄金回放、性质模拟、客户端、UI、迁移、平台与 release marker 定向测试均通过；性质模拟覆盖基础 5 模式、3 套接力谱和 6 条长卷誓约，验证终态、动作预算、精修/裁牌约束与逐动作哈希复演；另以真实持久化 journal 闭环验证 v1-v3 旧 run 的 `/replay` 与 `/settle` 不受 v4 字段影响。
    - 额外对 12 套独立场景各运行 60 个确定性种子，共 720 局全部终止于动作预算内，且所有通关局都保留合法精修、裁牌、伤害牌与格挡牌下限；该数据用于稳定性验证，不作为最终难度标尺。
    - 真实后端浏览器审计：权威试炼 48/48 findings、命途长卷 20/20 findings，均为 0 failed、0 console error；真实页面完成精修与裁牌，结算完整复演通过，并生成本轮 fresh 精修/裁牌与终局构筑截图；移动端三证归卷读取失败态只保留一条告警。
    - 完整本地 Node 门禁：`PVP_LIVE_WS_FANOUT_MESSAGE_TIMEOUT_MS=60000 npm run test:node`，覆盖现有后端平台、跨进程同步、PVP、核心战斗与 E2E 回归。
    - 2026-07-14 正式部署前已备份 `/www/backup/the-defier/wwwroot_20260714_160517.tar.gz` 与 `/www/backup/the-defier/backend_20260714_160517.tar.gz`；静态站和后端同步后，`the-defier-backend` 为 active、`nginx -t` 通过，公网首页与 `/api/health` 均为 200，线上 schema 为 V11 `0011_authoritative_fate_chronicle`。
    - 生产只读、环境和写入 smoke 全部通过，覆盖注册、登录、存档、全局进度、残影上传/拉取与现有真人 PVP 链；生产浏览器最终补跑前端布局 424/424，0 failed、0 console error。
  - 当前结论
    - S108 已完成设计、开发、全量回归、合并推送、生产备份部署和线上 smoke；正式地址为 `https://080305.xyz/`。后续难度与路线风险收益使用 S109 的独立基线，不把 S108 的自动策略稳定性误当成最终平衡目标。

- 2026-07-13: V11 玩家版本说明同步（命途长卷与三证归卷）
  - 本轮完成
    - `game-intro.html` 的“当前版本重点 / 当前迭代重点”已切到 `V11 命途长卷 · 三证归卷`：当前玩家版本口径更新为“V11 命途长卷 · 三证归卷”，并明确命途长卷是 3 章各 2 条誓约、按周目录发车的服务器权威 run 主线。
    - 同步写明失败不扣次数、放弃不扣次数、过期不扣次数，且不会回退已完成章节；同章双解只记档案展示与纯外观里程碑，不把奖励变成战力来源。
    - 同步写明三证归卷只认 5 类权威活动铸证：命途长卷、众生试炼、世界裂隙、实时论道、同道远征；其中 2/5 领取 120 荣誉，3/5 只提升档案等级，4/5 与 5/5 继续只展示荣耀。
    - 当前说明额外强调非强制 PVP：实时论道是可选证源，正式完赛即可计证，胜负都算；玩家不用依赖真人排位也能完成本周主要经济收益。
  - 已验证
    - `node tests/sanity_intro_progress_sync_checks.cjs`
  - 当前结论
    - 历史版本记录中的 `V10 同道远征 · 权威接力闭环` 已保留；当前对外说明与同步测试已切换到 V11 命途长卷与三证归卷。
...
## 2026-07-10 全游戏版本化云状态与恢复平台 V2

- 设计 / 实现
  - 四个存档槽与账号全局数据统一进入服务端 revision/CAS 模型；每次有效写入生成不可变 revision，并记录完整业务字段签名、内容哈希、mutationId、来源 revision 与操作类型。
  - 新客户端持久化并提交 base revision；并发写入不再依赖时间戳静默覆盖，冲突会返回当前 head 并进入已有的本地 / 云端显式选择流程。mutationId 重试保持幂等，旧 API 与旧客户端继续可读写。
  - 云端历史入口接入真实存档位 UI；查看历史不会改变当前 slot，恢复会创建新的 head revision，原 revision 保持不变。弹窗补齐桌面与移动端布局、加载 / 空态 / 错误态、当前版本标记和二次确认。
  - revision 缓存按账号隔离，所有异步读写绑定发起账号；退出登录或切换账号后，旧请求不能更新缓存、驱动恢复或覆盖本地存档。
  - SQLite migration `0004_cloud_state_v2` 支持旧数据并发回填与超大历史数据兼容；每个 scope 保留最近 20 个版本，并额外保留仍被窗口内恢复记录引用的来源版本，总上限 40。mutation 与原始运维事件保留 30 天，累计计数独立保存。
  - 运维概览补齐 revision、冲突、幂等重放、恢复、清理与迁移状态的脱敏计数；完整协议、冲突语义、迁移、保留策略和恢复流程记录于 `docs/backend_cloud_state_v2.md`。

- 验证 / 反向审查
  - 后端 API、签名、CAS 并发、幂等、历史恢复、旧客户端兼容、迁移并发、保留上限、账号隔离与运维统计测试通过；前端 revision 缓存、保存队列、冲突选择和 UI 静态检查通过。
  - `output/cloud-state-v2-auth-ui-v10-mobile/report.json`：3/3 真实浏览器流程通过、0 console error，覆盖真实注册 / 登录 / 云保存、CAS 竞争、历史恢复、登录失效、恢复中切换账号和移动端无横向溢出。
  - 最终 release run `release-1783696222-7497` 的 `output/release-browser-audits-local/report.json`：29/29 模块、1010 条 finding、0 失败、0 console error、412 张截图，`missingModules=[]`、`duplicateModules=[]`、`unknownModules=[]`。
  - 后端与客户端两条挑战者审查均完成；发现的超大旧数据迁移、保留边界、并发回填计数、历史 slot 副作用、跨账号异步响应与双弹窗等问题均已修复，最终无剩余 actionable finding。
