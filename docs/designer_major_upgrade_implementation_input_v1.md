...
S8T 补充后，前端 live WS 异常关闭会快速重连：`PvpLiveSession` 在非手动 `onClose` 且仍有 pending `join_match` 时进入 `reconnecting`，短延迟后创建新连接并重放 `join_match + lastSeenRevision`；`disconnectRealtime()` / `reset()` 会清理 timer 和 pending join，不把主动离开误当作弱网重连。S8U 已补 UI 传输状态可见化；该结论只覆盖 session 层传输恢复，不代表离页保活、intent in-flight 防双击、跨进程终局 fanout、生产 smoke 或线上部署完成。

S8U 补充后，本地实时传输状态已可见：`PVPScene.getLiveSnapshot()` 和 `render_game_to_text()` 会透出 `realtimeStatus`、`lastRealtimeSyncAt`、`realtimeReport`；live 面板新增 `data-live-realtime-status`，把本机 WS/实时通道 connected / reconnecting / error 与服务端双方 heartbeat / grace 的 `data-live-connection-status` 分开显示。该结论只覆盖传输状态可见化，不代表离页保活、intent in-flight 防双击、跨进程终局 fanout、生产 smoke 或线上部署完成。
