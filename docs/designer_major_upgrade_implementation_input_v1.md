...
S8H stale save 权威回读补充：`recordHeartbeat()` 和 `submitIntent()` 现在会识别 `stale_state_version` 写输结果，绕过本地 `matches` cache 直接用 `loadMatchForUser(userId, matchId)` 回读 authoritative persisted match。heartbeat 分支返回权威 `stateView`；intent 分支返回 `sync_required / stale_state_version / events=[] / authoritative stateView`，避免把未持久化的本地 accepted 结果和旧事件流发给玩家。该切片只覆盖 heartbeat 与 `submitIntent()` 的 active 非终局保存写输回读，不代表 route 级统一 rehydrate、终局 settlement 写输收口、同版本双写级 CAS、DB 原子撮合 / 并发 claim 防重、跨进程 WS fanout、生产 smoke 或线上部署已经完成。
