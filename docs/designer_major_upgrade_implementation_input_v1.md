...
S8I terminal accepted intent stale-save settlement guard 补充：`completeFinishedMatch()` 现在会读取 finished state 首次 `saveMatch()` 的 `saveResult`；如果 `saveMatch returned stale_state_version`，必须立刻停止本地 dirty finished state 的正式 settlement / settlement report 保存 / release。`submitIntent()` 的 terminal accepted intent 分支会回读 authoritative persisted match，并返回 `sync_required / stale_state_version / events=[] / authoritative stateView`，避免 surrender 等终局动作在旧进程里写输后还污染正式胜负。该切片不包含 route-level unified rehydrate、settlement report 二次保存写输补偿、same-version CAS、Redis / 多实例强一致队列、跨进程 WS fanout、生产 smoke 或线上部署。

S8J timeout / read-path terminal stale rehydrate 补充：`sweepMatchTimeout()` 现在会把 timeout、connection-timeout 和 invalidated finishers 的 completion `saveResult` 冒给 `getMatchForUser()` / `getActiveMatchForUser()`；`releaseIfTerminal()` 也会返回 terminal completion。GET read path 遇到 `stale_state_version` 时会回读 authoritative persisted match，并返回 authoritative `stateView`，防止旧进程在自动 timeout 或读取已终局脏缓存时把本地 dirty finished / invalidated 视图发给双方。当前行为证据覆盖 `getMatchForUser()` 的 timeout finished 写输回源，以及 `getActiveMatchForUser()` 的 invalidated dirty cache 读时释放写输回源。该切片不包含 route-level unified rehydrate、settlement report 二次保存写输补偿、same-version CAS、Redis / 多实例强一致队列、跨进程 WS fanout、生产 smoke 或线上部署。

S8K active match same-version conflict guard 补充：`saveMatch()` 现在会读取 persisted `state_json` / `state_version` 快照，active 状态下若 incoming `stateVersion` 等于权威版本但 `state_json` 内容不同，则跳过保存并返回 `conflicting_state_version`，防止同一 base revision 产生的同版本旧进程写入静默覆盖权威 active match。SQL upsert 也会拒绝 active 同版本不同内容的后到写入，并在 no-op 后二次读取权威版本；若权威版本已更高，继续返回 `stale_state_version`。store 会把这两类结果纳入 authoritative rehydrate / `sync_required` 路径，避免玩家继续看到本地 dirty `stateView`。同版本同内容保存仍可更新 `connection_json`。该切片不包含 settlement report 二次保存写输补偿、route-level unified rehydrate、Redis / 多实例强一致队列、跨进程 WS fanout、生产 smoke 或线上部署。
...
- S8K 已补 active match same-version content conflict guard，但这仍不是完整 active match revision/CAS；settlement report 二次保存写输补偿、Redis / 多实例强一致队列、跨进程 WS fanout、生产 smoke 和线上部署仍未封板。
