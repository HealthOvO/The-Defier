...
S8V 补充后，live PVP 前端已具备 intent in-flight 防双击：`PVPScene.submitLiveIntent()` 在 WS 快路径成功发送后会按 action / social 两类记录 pending，action 类覆盖 `play_card`、`mulligan`、`ready`、`end_turn`、`surrender`，social 类覆盖 `emote`；`PvpLiveSession` 会把 `intent_result` 投影成 `lastRealtimeIntentResult`，scene 层用同一 `intentId` 的 ack 或 action 权威 `stateVersion` 推进释放锁。该结论只覆盖客户端重复提交体验保护，不代表服务端完整 active revision/CAS、离页保活、跨进程终局 fanout、生产 smoke 或线上部署完成。
