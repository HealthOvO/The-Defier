...
截至 2026-06-20，live PVP 已继续补到准备调息、先后手语义预算、开局护体、后手开局公开护盾、护体后反打缓冲、每回合抽牌、连接宽限、赛后复盘、练习承接、长等待练习承接、真实后端长等待 smoke、active match 低版本晚写保护、stale save 写输合同、heartbeat / 非终局 intent 写输后权威回读、terminal accepted intent 写输结算保护、settlement report 二次保存写输补偿、好友约战、Bo3 友谊再战、定向约战收件箱和公平保护可见化。`openingSafeguardReport` 已把首动伤害预算、后手 3 点公开护盾、开局护体保底血量、护体后首回合护盾缓冲、当前行动席位预算和公开状态投影给双方 UI；`firstMatchGuide` 已从数据层推进到 DOM 层完整展示 exceptionBranches / reviewActions；`practice-live` 在长等待后会先退出排队，确认成功才生成 no-score / replay-only / practice-only 的 drillScenario；若退出排队时票据已被真人撮合消费，则优先同步权威战局，不强开练习；真实后端 smoke 已用独立账号证明 `waitingReport.longWait=true`、`practice-live` 可点击并打开 no-score drill；SQLite 持久化已按 `state_version` 拒绝低版本 active match 晚写，并对老行 `state_json.stateVersion` 做迁移兜底；`saveMatch()` 已用 `saved/skipped/reason` 向 store 暴露写输结果，store 在 state 保存被跳过时不再追加旧事件表；heartbeat / 非终局 accepted intent 遇到 `stale_state_version` 后会回读 authoritative persisted match，避免继续返回旧进程 dirty `stateView`；terminal accepted intent 写输后会跳过本地 dirty finished settlement，并返回 `sync_required / stale_state_version` 的权威视图；terminal settlement report 第二次保存写输时，会回读 authoritative finished match 并把缺失的权威结算报告补写回去。`server/pvp-live/content/pvp-live-v1-content.js` 已落 8 套 V10-S2 基准谱 / bot policy 单点事实源，`tests/sanity_pvp_live_balance_simulation_checks.cjs` 已接入 10,048 样本 quick gate，并实际执行 10,000 个 opening pressure probe。当前 S2-C 证据显示：quick gate 先手胜率 50.14%，8 类 opening 脚本各 1250 条，后手首行动前死亡、无行动线、不可读爆发和非游戏局为 0。
