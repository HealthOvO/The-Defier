...
- 当前 live PVP 已覆盖 HTTP 撮合、入队评分锁定和近分优先候选选择、长等待后双方显式同意的 `200-399` 宽分差成局、queue status 跨进程 active-match handoff、SQLite queue ticket atomic claim / duplicate-match prevention、长等待 no-score 练习承接、真实后端 long-wait smoke、active match 低版本晚写保护、active match 同版本内容冲突保护、same-version connection timeline 单调合并、stale save 写输合同、heartbeat / 非终局 intent 写输后权威回读、terminal accepted intent 写输结算保护、settlement report 二次保存写输补偿、route-level 读路径权威回源、WS `sync_required` 双方即时同步、WS heartbeat missed-events 补发、WS heartbeat 有界跨进程状态追赶证据、SQLite-backed 跨进程 WS `state_sync` fanout、客户端 WS 快速重连、本地实时传输状态可见化、live intent in-flight 防双击、权威 reducer、seat-scoped StateView、intent 幂等、S2-C full gate、S2-B artifact foundation、S2-D reducer-backed golden replay、S2-E store-backed 重连 / timeout / invalidated golden replay、S2-F runtime round14 结算、S4A active 回合计时独立锚点、S4B 前端权威心跳调度、S5A 最小赛后 replay API、S5B append-only `pvp_live_match_events` 回放真源、S6A 单进程 WebSocket 权威同步与公开事件补发、S7B live ranked 权威结算回执可见化、S7C live ranked 非强度赛季荣誉进度、S7D 真实后端 invalidated / replay 浏览器负向证据、S7F 非强度荣誉收藏持久化、S8A live PVP 移动端真实后端主链路 smoke、S8D 首局引导 DOM 完整展示、S8E 真实后端长等待练习证据、S8F 迁移安全的 lower-version stale save guard、S8G saved/skipped contract、S8H heartbeat / non-terminal submitIntent stale-save rehydrate、S8I terminal accepted intent stale-save settlement guard、S8K same-version active content conflict guard、S8N read-path authoritative rehydrate、S8O heartbeat events replay catch-up、S8P heartbeat state catch-up、S8Q connection timeline monotonic merge、S8S SQLite signal fanout、S8T client reconnect、S8U realtime status visibility、S8V intent in-flight lock、持久化邀请 / 再战、重连宽限和关键 UI 审计，但不包含完整正式赛季奖励系统、Redis / 多实例强一致队列、完整 active match revision/CAS、跨进程终局结算 fanout smoke、生产 API smoke 或线上部署。
- S8J 已补 timeout / connection-timeout / invalidated / GET read path 的 terminal stale rehydrate，S8M 已补 settlement report 二次保存写输补偿，S8N 已补读路径权威回源和 WS sync_required fanout，S8S 已补 SQLite signal state_sync fanout，但这仍不等于完整 same-version active match revision/CAS、Redis / 多实例强一致或跨进程终局结算 fanout 已完成。
- S8K 已补 active match same-version content conflict guard，S8Q 已补同版本同内容 connection timeline 单调合并，但这仍不是完整 active match revision/CAS；Redis / 多实例强一致队列、跨进程终局结算 fanout smoke、生产 smoke 和线上部署仍未封板。
- S8L 已补共享 SQLite 下的 queue ticket atomic claim / duplicate-match prevention，S8S 已补 SQLite-backed WS state_sync fanout，但 Redis / 多实例强一致队列、跨进程终局结算 fanout smoke、生产 smoke 和线上部署仍未封板。
