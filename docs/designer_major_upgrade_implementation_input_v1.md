...
截至 2026-06-20，live PVP 已继续补到准备调息、先后手语义预算、开局护体、后手开局公开护盾、护体后反打缓冲、每回合抽牌、连接宽限、赛后复盘、练习承接、长等待练习承接、真实后端长等待 smoke、active match 低版本晚写保护、stale save 写输合同、heartbeat / 非终局 intent 写输后权威回读、terminal accepted intent 写输结算保护、settlement report 二次保存写输补偿、重复终局 release 不重入 settlement provider、stale duplicate intent 权威幂等回放、首次超时软托管写输回源、route-level 读路径权威回源、WS `sync_required` / `duplicate_action` 双方即时同步、WS heartbeat missed-events 补发、WS heartbeat 跨进程权威状态追赶证据、SQLite-backed 跨进程 WS `state_sync` fanout、跨进程终局 WS fanout smoke、跨进程 `duplicate_action` WS fanout smoke、客户端 WS 快速重连、前后台恢复即时权威追帧、本地实时传输状态可见化、live intent in-flight 防双击、same-version connection timeline 写入时单调合并、好友约战、Bo3 友谊再战、定向约战收件箱、公平保护可见化、公开局势 / 反打窗口持续 HUD 和赛后公平回执。`openingSafeguardReport` 已把首动伤害预算、后手 3 点公开护盾、开局护体保底血量、护体后首回合护盾缓冲、当前行动席位预算和公开状态投影给双方 UI；`duelMomentumReport` 已把公开血线百分比、当前行动席位、开局护体窗口、后手护盾、反打缓冲状态和行动窗口解释持续投给双方 HUD，并固定 `sourceVisibility: public_state`、`usesHiddenInformation: false`、`rankedImpact: none`，不读取隐藏手牌、牌库、完整斗法谱、原始事件 payload、rating / ELO 或奖励信息；`fairnessReceipt` 已把赛后公开体验检查归纳为 `pvp-live-fairness-receipt-v1`，解释开战、首动预算、护体 / 反打窗口、公开行动窗口和终局边界，并固定 `sourceVisibility: public_events`、`usesHiddenInformation: false`、`rankedImpact: none`，不读取隐藏手牌、牌库、原始事件明细、rating / ELO 或奖励信息；`firstMatchGuide` 已从数据层推进到 DOM 层完整展示 exceptionBranches / reviewActions；`practice-live` 在长等待后会先退出排队，确认成功才生成 no-score / replay-only / practice-only 的 drillScenario；若退出排队时票据已被真人撮合消费，则优先同步权威战局，不强开练习；真实后端 smoke 已用独立账号证明 `waitingReport.longWait=true`、`practice-live` 可点击并打开 no-score drill；SQLite 持久化已按 `state_version` 拒绝低版本 active match 晚写，并对老行 `state_json.stateVersion` 做迁移兜底；`saveMatch()` 已用 `saved/skipped/reason` 向 store 暴露写输结果，store 在 state 保存被跳过时不再追加旧事件表；heartbeat / 非终局 accepted intent 遇到 `stale_state_version` 后会回读 authoritative persisted match，避免继续返回旧进程 dirty `stateView`；terminal accepted intent 写输后会跳过本地 dirty finished settlement，并返回 `sync_required / stale_state_version` 的权威视图；若 stale save 后权威 state 已经记录同一 intent，store 会复用 reducer 的 `duplicate` / `duplicate_action_conflict` 幂等语义并返回 authoritative `stateView`，前端 WS 收到 `duplicate / duplicate_action` 时也按幂等成功回执清理错误提示；首次超时软托管的 active 保存写输后也会回读权威状态，不再把本地自动行动视图发给玩家；terminal settlement report 第二次保存写输时，会回读 authoritative finished match 并把缺失的权威结算报告补写回去；finished state 已带权威 settlementReport 时，重复终局 release 会保存并释放状态，但不再二次调用 settlement provider；active / direct read path 在本地 cache 命中时也会先回源 authoritative persisted match，WS intent 返回 `sync_required` 或 `duplicate_action` 幂等重放时会把权威 `state_sync` 推给发起方和对手；WS heartbeat 明确携带 `lastSeenRevision` 时会从事件真源补发 missed public events；两个独立 WS server 的测试已证明对手连在另一进程时，可在下一次 heartbeat 内从共享权威状态拿到新版 `state_sync`；两个真实 `server/app.js` 进程共用临时 SQLite 的测试已证明跨进程 WS signal 可主动 fanout `state_sync`；跨进程终局 smoke 已证明 surrender 终局可在不依赖对手 heartbeat 的情况下向另一进程推送 finished state、赢家复盘和正式结算投影，终局公开事件时间线可由 heartbeat events_replay 补齐；跨进程 duplicate smoke 已证明 `duplicate_action` 幂等重放可向另一进程对手推送权威 `state_sync`，且同一 match / signal_type / stateVersion / reason 的 `sync_required` 与 `duplicate_action` durable signal 会通过 SQLite `INSERT ... WHERE NOT EXISTS` 共享去重；前端 WS 异常 close 后会进入 `reconnecting` 并在短延迟重连后重放 pending `join_match`；页面从 hidden / pageshow / focus 恢复到前台时，scene 会在短窗口去重后立即恢复实时通道并补一次权威 heartbeat，`PvpLiveSession.resumeRealtime()` 会用最新公开事件高水位重放 pending `join_match` 并在重连打开后补发 realtime heartbeat；live 面板和 `render_game_to_text()` 会把本地 `realtimeStatus` / `lastRealtimeSyncAt` / `realtimeReport` 与服务端 `connectionReport` 分开展示；前端会把 WS `intent_result` 投影成 `lastRealtimeIntentResult`，并用 action / social 两类 in-flight 锁防止同一权威版本重复战斗动作或表情连发，同时不让 pending 表情阻塞出牌 / 结束回合；active match 同版本同内容的 heartbeat / connection 写入会在 SQLite upsert 时按当前行与 incoming 做 per-seat 时间戳 `MAX()` 合并，不能把另一个进程已推进的在线 / 重连时间线写回旧值。`server/pvp-live/content/pvp-live-v1-content.js` 已落 8 套 V10-S2 基准谱 / bot policy 单点事实源，`tests/sanity_pvp_live_balance_simulation_checks.cjs` 已接入 10,048 样本 quick gate，并实际执行 10,000 个 opening pressure probe。当前 S2-C 证据显示：quick gate 先手胜率 50.14%，8 类 opening 脚本各 1250 条，后手首行动前死亡、无行动线、不可读爆发和非游戏局为 0。
...
S9E 补充后，live PVP 的社交舒适度从“一局内可静音”推进到“本机偏好可持久”：玩家静音对手预设表情后，会写入 `pvp-live-social-preferences-v1` 本地偏好，重新进入 live 面板仍保持静音；`PVPScene.getLiveSnapshot().social` 明确输出 `preferenceScope: local_only`、`sourceVisibility: local_preference`、`rankedImpact: none` 和 `persistence: local_storage`。服务端预设表情仍只允许白名单和冷却，但 accepted emote 会推进公开 `stateVersion`，保证 active 对局中的社交事件能被持久化和跨端同步，不再因为同版本 active state 内容冲突返回 `sync_required / conflicting_state_version`；前端 action in-flight 锁也同步收紧为同 intent ack 或“版本推进 + pending 建立后的匹配 action 公开事件证据”才释放，避免 `emote_sent` 社交事件叠加旧 action 事件残留后误解锁出牌 / 结束回合 pending。该偏好只改变本机事件流展示，不屏蔽服务端权威事件、不影响对手发送预设表情、不写正式积分、奖励、匹配、赛季验证或结算，也不引入自由文本聊天、好友黑名单、举报后台或账号级封禁。

S9F 补充后，live PVP 的公平保护从“规则已生效 / 事件可回看”推进到“局中持续可读”：`pvp-live-duel-momentum-v1` 会把公开血线百分比、当前行动席位、开局护体窗口、后手护盾、反打缓冲状态和下一行动窗口解释投给 `data-live-duel-momentum`，并同步进入 `PVPScene.getLiveSnapshot()` 与 `render_game_to_text()`。该报告只读公开 seat 状态和既有 `openingSafeguardReport`，固定 `public_state / false / none`，不读取隐藏手牌、牌库、完整斗法谱、原始事件 payload、rating / ELO 或奖励；它只提升双方对“为什么先手不能秒杀、为什么护体后还有反打窗口”的理解，不改变伤害、生命、抽牌、灵力、起手、匹配、正式积分、奖励、赛季验证或结算。

S9G 补充后，live PVP 的赛后复盘从“关键回合 + 细项体验检查”推进到“先给双方一个可读公平回执”：`pvp-live-fairness-receipt-v1` 会把公开体验检查归纳成开战回执、反先手秒杀结论、首动预算说明、护体 / 反打窗口说明、公开行动窗口、终局边界和下一步建议，并投给 `data-live-fairness-receipt`、`PVPScene.getLiveSnapshot()` 与 `render_game_to_text()`。该回执只读公开复盘证据和公开事件序号，固定 `public_events / false / none`，不读取隐藏手牌、牌库、原始事件明细、rating / ELO 或奖励；它只提升终局后的理解、信任和再来一局动机，不改变伤害、生命、抽牌、灵力、起手、匹配、正式积分、奖励、赛季验证或结算。

S9H 补充后，低压力再战等待从“发起后只能等对手接受”推进到完整 pending 生命周期：服务端提供 pending status、发起方 cancel、TTL expiry 和重启后 pending 读取；pending TTL 锚定本次等待创建时间，不继承整条 Bo3 系列的旧 `createdAt`，因此长时间 Bo3 后新发起的决胜局等待不会秒过期。前端 `waiting_rematch` 会展示 `data-live-action="cancel-rematch"`，刷新后可从 terminal 复盘恢复到等待态，取消或过期后回到 finished 复盘并保留 `friendlySeries.status=cancelled/expired` 的可读边界。取消只允许 pending 发起方执行，原对手和旁观者不能清空等待；取消后可重新发起并正常让对手接受。该切片只提升连续对局的掌控感和失败路径确定性，不改变 Bo3 计分、战斗数值、正式积分、奖励、赛季验证或结算。
...
S8O 补充后，WS heartbeat 已支持 missed-events catch-up：前端 `heartbeatRealtime()` 会按 `lastEvents` 与 `stateView.recentEvents` 的最大公开事件序号发送 `lastSeenRevision`；后端 heartbeat 在返回 `presence` 后，只有收到该字段时才从 `pvp_live_match_events` / state events 补发白名单 `events_replay`。空 `events_replay` 不会清掉前端已见公开事件高水位，重新 join / reconnect 也会把 replay 事件高水位纳入 `lastSeenRevision`。这让弱网、短断线或跨进程读路径回源后的玩家不用等重新 join 才看到漏掉的公开行动事件。该结论只覆盖 heartbeat 驱动的公开事件补漏，不代表 Redis / 多实例 WS fanout、完整 active match revision/CAS、生产 smoke 或线上部署完成。

S8P 补充后，跨进程 WS 已有 heartbeat bounded state catch-up 协议证据：测试用两个独立 `attachLivePvpWebSocket()` server 模拟两个进程，并用共享 fake authority 表达权威 `stateVersion`；A 在进程 1 推进版本后，已 join 但连在进程 2 的 B 不依赖跨进程主动广播，而是在下一次 heartbeat 经 `recordHeartbeat()` / `getMatchForUser()` 投影权威状态，并收到 seat-scoped `state_sync`。该结论只覆盖 WS 编排层的 heartbeat interval 内有界状态追赶，不代表 Redis / 多实例主动 fanout、真实多实例 persistence smoke、完整 active match revision/CAS、生产 smoke 或线上部署完成。

S8Q 补充后，同版本同内容的 active heartbeat / connection 保存不再整段覆盖 `connection_json`：SQLite upsert 会在写入时基于当前权威行和 incoming JSON 对双方 `connectedAt`、`lastHeartbeatAt`、`reconnectedAt` 做 `MAX()` 合并。普通旧连接快照和读后写前已有并发心跳推进的窗口都不能把在线 / 重连时间线回退。该结论只覆盖 active connection timeline 单调性，不代表完整 active match revision/CAS、Redis / 多实例强一致队列、跨进程 WS 主动 fanout、生产 smoke 或线上部署完成。

S8R 补充后，首次回合超时软托管也纳入 stale-save 回源：`executeFirstTimeoutAutomation()` 会把 active 自动行动保存结果冒泡给 `sweepMatchTimeout()`，若保存写输则由读路径回读 authoritative persisted match。这样旧进程不能把未持久化的本地自动防御 / 自动结束回合视图继续发给双方。该结论只覆盖 first-timeout active automation 的写输回源，不代表完整 active match revision/CAS、Redis / 多实例强一致队列、跨进程 WS 主动 fanout、生产 smoke 或线上部署完成。

S8S 补充后，跨进程 WS 已有 SQLite-backed 主动 `state_sync` fanout：状态推进会写入 `pvp_live_state_signals`，不同 `server/app.js` 进程上的 WS client 通过 per-client signal cursor 读取共享 SQLite 信号并跳过本进程自发信号；join race baseline cursor 和 `sync_required` 节流用于避免启动旧信号或重复 fanout。两个真实 Node 进程共用临时 SQLite 的 smoke 已证明，A/B 连到不同进程时也能收到对方推进后的 `state_sync`。该结论只覆盖 SQLite 单库 signal fanout，不代表 Redis / 多实例强一致、跨进程终局结算专门 smoke、生产 API smoke 或线上部署完成。

S8T 补充后，前端 live WS 异常关闭会快速重连：`PvpLiveSession` 在非手动 `onClose` 且仍有 pending `join_match` 时进入 `reconnecting`，短延迟后创建新连接并重放 `join_match + lastSeenRevision`；`disconnectRealtime()` / `reset()` 会清理 timer 和 pending join，不把主动离开误当作弱网重连。S8U 已补 UI 传输状态可见化，S8V 已补 live intent in-flight 防双击；该结论只覆盖 session 层传输恢复，不代表离页保活、跨进程终局 fanout、生产 smoke 或线上部署完成。

S8U 补充后，本地实时传输状态已可见：`PVPScene.getLiveSnapshot()` 和 `render_game_to_text()` 会透出 `realtimeStatus`、`lastRealtimeSyncAt`、`realtimeReport`；live 面板新增 `data-live-realtime-status`，把本机 WS/实时通道 connected / reconnecting / error 与服务端双方 heartbeat / grace 的 `data-live-connection-status` 分开显示。该结论只覆盖传输状态可见化，不代表离页保活、跨进程终局 fanout、生产 smoke 或线上部署完成。

S8V 补充后，live PVP 前端已具备 intent in-flight 防双击：`PVPScene.submitLiveIntent()` 在 WS 快路径成功发送后会按 action / social 两类记录 pending，action 类覆盖 `play_card`、`mulligan`、`ready`、`end_turn`、`surrender`，social 类覆盖 `emote`；`PvpLiveSession` 会把 `intent_result` 投影成 `lastRealtimeIntentResult`，scene 层用同一 `intentId` 的 ack 释放锁。S9E 已进一步把 action 的权威版本兜底释放收紧为“`stateVersion` 推进 + pending 建立后的对应 action 公开事件证据”，避免社交事件推进公开版本并叠加历史 action 事件残留后误释放 action pending。该结论只覆盖客户端重复提交体验保护，不代表服务端完整 active revision/CAS、离页保活、跨进程终局 fanout、生产 smoke 或线上部署完成。

S8W 补充后，SQLite-backed 跨进程 WS fanout 已有 surrender 终局专门 smoke：两个真实 `server/app.js` 进程共用临时 SQLite 时，A/B 分别连到不同进程，A 投降后 B 不发送 heartbeat 也会通过 `state_sync` 收到 `status=finished`、赢家 post-match review、`finishReason=surrender`、正式结算投影和 `match_finished` 公开事件；随后 B 带终局前 `lastSeenRevision` heartbeat 会收到 `player_surrendered` / `match_finished` 的 `events_replay`。该结论只覆盖 SQLite 单库 signal fanout 的终局证据，不代表 Redis / 多实例强一致、生产 API smoke 或线上部署完成。

S8X 补充后，已有权威 settlement report 的 terminal completion 不再重入正式结算 provider：`completeFinishedMatch()` 在 finished state 已带 `pvp-live-settlement-report-v1` 时，只执行保存 / release 并直接返回。重复投降、重连重放或读路径再次触发 terminal release 时，store 层会先截断重复结算入口；底层 `pvp_live_match_settlements` 幂等仍保留为 DB 级兜底。该结论只覆盖已有报告的终局重入保护，不代表完整 active match revision/CAS、Redis / 多实例强一致、生产 API smoke 或线上部署完成。

S8Y 补充后，stale duplicate intent 已有 authoritative replay 语义：`submitIntent()` 在 accepted intent 保存写输后回读 authoritative match，若权威 `processedIntents` 已记录同一 `seatId:intentId`，则在权威 state 上复用 reducer 返回 `duplicate / duplicate_action` 或 `rejected / duplicate_action_conflict`，并投影 authoritative `stateView`；只有权威未记录该 intent 时才继续泛化为 `sync_required`。终局分支只在 terminal state 初始保存写输且本地尚未附 settlement report 时走该 replay，避免把 settlement report 二次保存失败或 outcome mismatch 误判为 duplicate；前端 WS 收到 `duplicate / duplicate_action` 时会与 HTTP duplicate 一样清掉实时错误提示，不把幂等重放显示成“行动需要处理”。该结论只覆盖 stale duplicate/conflict intent 的发起方权威回执和 WS 发起方错误提示收口，不代表完整 active match revision/CAS、WS duplicate peer fanout、Redis / 多实例强一致、生产 API smoke 或线上部署完成。

S8Z 补充后，WS `duplicate / duplicate_action` 幂等重放也会触发双方权威 `state_sync`：`live-ws` 在 result 带 authoritative `stateView` 时会把 duplicate_action 纳入 `shouldFanoutState`，同一 WS server 上的发起方与对手都会收到 seat-scoped 权威视图；`appendLiveWsSignal()` 对 `duplicate_action` 使用与 `sync_required` 相同的 match / stateVersion / reason 节流，避免弱网重复点击写出重复 signal。`duplicate_action_conflict` 仍保持 rejected，不广播对手。该结论只覆盖 duplicate_action 幂等成功重放的 WS 双方同步和 signal 节流合同，不代表完整 active match revision/CAS、专门跨进程 duplicate smoke、Redis / 多实例强一致、生产 API smoke 或线上部署完成。

S9A 补充后，跨进程 `duplicate_action` 已有真实双 `server/app.js` / 共享 SQLite smoke：B 在一个后端进程 ready accepted 后，原样重放同一 intent 会收到 `duplicate / duplicate_action`，A 连在另一个后端进程且不发送 heartbeat 也会收到同版本 authoritative `state_sync`。同一测试还用 B 的第二条 WS 连接落到另一个后端进程再次重放 duplicate intent，证明 `appendLiveWsSignalRow()` 对 `sync_required` 和 `duplicate_action` 的 durable signal 使用 `INSERT ... WHERE NOT EXISTS` 按 match / signal_type / stateVersion / reason 共享去重，不再只依赖单进程内存节流。该结论只覆盖 SQLite 单库跨进程 duplicate fanout 与 signal 去重，不代表 Redis / 多实例强一致、生产 API smoke 或线上部署完成。

S9B 补充后，前后台恢复已有即时权威追帧合同：`PvpLiveSession.resumeRealtime()` 会保留 pending `join_match` 的最高 `lastSeenRevision`，在页面恢复时立刻重放 join 并补 realtime heartbeat；若 WS 正处于重连等待，恢复会清理延迟 timer、立即建新连接，连接打开后再补 heartbeat。`PVPScene` 会一次性监听 `visibilitychange`、`pageshow` 和 `focus`，忽略 hidden 状态，并对同一次前台恢复短窗口去重后调用 `sendLiveHeartbeat({ resumeRealtime: true })`；reconnecting 状态下仍保留 HTTP heartbeat fallback，避免玩家回到页面后还要等下一次 interval 或手点刷新。浏览器审计已覆盖真实页面不点击 refresh 的恢复路径：UI 先停在对手 `grace`，再通过前台恢复事件重连 WS、replay `join_match`、发送 realtime heartbeat 和一次 HTTP heartbeat fallback，最终 realtime 与 UI 连接状态同步回双方 online。该结论只覆盖前端 hidden-tab / focus 恢复追帧，不代表后台常驻保活、sendBeacon、生产 smoke 或线上部署完成。

S2-B artifact foundation 已把 fixture 路径从“未冻结示例”推进到可提交、可读取的冻结合同：`server/pvp-live/fixtures/baseline_loadouts_v1.json`、`baseline_bot_policies_v1.json`、`opening_scripts_v1.jsonl`、`golden_replays_v1.jsonl` 已生成并被 `tests/sanity_pvp_live_balance_artifact_checks.cjs` 校验；`server/pvp-live/balance-artifacts.js` 负责构建 / 写入 artifact bundle，`runBalanceSimulationFullGate()` 固定 8x8x500 = 32,000 局 full 样本。S2-C full gate 已通过：32,000 局先手胜率 49.98%，8 套构筑胜率落在 49.73%-50.32%，pair first-seat 全部在 45%-55%，`archetypeSpread` 无 dominant / false archetype 风险，后手首行动前死亡、无行动线、不可读爆发、非游戏局均为 0。S2-D 已完成第一批 reducer-backed golden replay：9 条公开回放样本直接跑 reducer，校验 event sequence、稳定 replay/final hash、`replay_public` / `audit_safe` 可见性分支、字段路径隐藏扫描、双方 post-match review 不泄露、公开复盘可推导和重复 intent 幂等。S2-E 已继续把重连、首次 timeout 托管、严重 timeout 判负、setup ready timeout invalidated 推进为 store-backed golden replay。S2-F 已把第 14 整轮 runtime 结算接入 reducer：`round14_score` 按公开长局分差判胜负，`round14_draw` 不强造 loser、生成 draw review，并把 `golden-draw-round14-001` 从 simulation-only 升级为 reducer-backed golden replay；settlement 层已把 `round14_draw` 固定为 no-ranked-impact，不写正式积分、奖励、settlement gate 或历史，`round14_score` 已锁定正式 settlement / history / rating 写账门禁。S3A 已把 live ranked 公共队列从 FIFO / `mvp_open_pool` 展示占位推进到入队时评分快照：`pvp_live_queue_tickets` 保存 `rating_score / rating_bucket / rating_season_id / rating_provisional`，`joinQueue()` 会恢复完整候选池、用 `pvp_ranks` 默认 provider 读取分数、优先选择分差最近候选，双方均未定级时才保持 open pool；自动匹配只允许 `near_0_99` 和长等待后的 `fair_100_199`，`200+` 分差当前不自动匹配且不泄露精确 rating；饱和队列回归已覆盖第 34 条近分候选不能因重启恢复上限被漏掉。S4A 已把 active 回合倒计时从 `match.updatedAt` 中拆出为独立 `turnTiming`：合法出牌仍可更新 match 最近变更时间，但不会延长当前行动窗口；只有开战、换手和 timeout 托管换手会刷新下一行动席位的 deadline。S4B 已把前端 HTTP heartbeat 调度从硬编码 `5000ms` 改成消费 `stateView.connectionReport.heartbeatIntervalMs`，interval 变化时重建 timer，保证客户端节奏跟服务端连接 SLA 对齐。S5A 已补上最小赛后 replay API：参与者可在终局后拉取 `replay_self`、`replay_public`、`audit_safe`，active 对局返回 `replay_not_ready`，非参与者 404，公开层只暴露 `matchRef` / `replayHash` / 脱敏 public event timeline / `hiddenScan`，不返回 raw `matchId`、手牌、牌库、实例 id、原始 payload 或 RNG。S5B 已把 replay 真源从单一 `state_json.events` 推进到独立 append-only `pvp_live_match_events`：`saveMatch()` 统一幂等补写事件，回放 API 优先读取事件表并在旧局缺表记录时回退 `state.events`；持久化测试已覆盖清空 `state_json.events` 后仍能从事件表恢复 `battle_started` 和 `match_finished` 公开时间线。S6A 已接入单进程 WebSocket 权威同步最小闭环：`/api/pvp/live/ws` 使用 session token 鉴权，连接后返回 `connected` 与权威 heartbeat interval，`join_match` 返回 seat-scoped `state_sync` 并按 `lastSeenRevision` 补发公开 `events_replay`，`heartbeat` 返回 `presence` 且在携带 `lastSeenRevision` 时补发 missed public events，`intent` 返回 `intent_result` 并广播双方最新 StateView；前端 session / scene 已在 heartbeat 生命周期内启动 WS 并保留 HTTP fallback。S7A 已先收正式赛季验证写回权限：`game.js` 只有显式 `formalSeasonVerification=true` 且来源为 live ranked 的服务端权威结果才允许写 `seasonVerificationState`，旧镜像演武、本地回执、降级回执、Bmob 旧在线和旧 `server_authoritative` 均不得污染正式赛季验证；实时论道首屏和 `game-intro.html` 已同步“正式真人入口 / 练习隔离 / 好友约战 / 镜像演武不是真人排位”的玩家可见口径。S7D 已补齐真实后端浏览器负向证据：`replay_public` / `audit_safe` 不暴露 requester seat、post-match review、正式结算回执或赛季荣誉，`ready_timeout` invalidated 在 DOM、live snapshot、text snapshot 和固定 match 读取中都不显示正式结算或赛季荣誉。S7F 已把 S7E 的非强度外观目标推进到 `pvp_economy.economy_data.seasonHonorCollection` 持久化：只保存 `cosmetic_only / powerImpact none` 的荣誉收藏状态，不写 `ownedItems`、不进入旧商店强度项、不影响卡牌、属性、资源、起手、匹配或战斗效果；该状态只在 seat-scoped ranked settlement / `replay_self` 可见。S8A 已补 live PVP 移动端真实后端 smoke：同一真实后端双账号链路在 390x844 touch 视口下跑通入队、匹配、ready、终局、正式结算、赛季荣誉收藏、公开 replay / audit_safe 和 invalidated 负向检查，并用几何 / top-hit 断言防止终局复盘裁切、横向溢出和复盘按钮不可点。S8D 已补首局引导和长等待练习承接：首局卡片在 DOM 展示异常分支和复盘动作；长等待练习会取消排队并打开 no-score challenge drill，移动端首局卡片不再硬裁切。S8E 已补真实后端长等待 smoke：真实 `server/app.js`、真实浏览器账号和临时 SQLite 会验证 long-wait waiting report、动态阈值文案、`practice-live` 启用和 no-score drill handoff。S8F 已补 active match 低版本晚写持久化保护：`pvp_live_matches.state_version` 会随 match 保存，低于现有权威版本的晚到 `saveMatch()` 不再覆盖 `state_json`，迁移旧行会先从 `state_json.stateVersion` 推导现有版本。S8G 已补 stale save 写输合同：`saveMatch()` 明确返回 saved/skipped/reason，store 会透传 skipped 并停止旧事件追加。S8H 已补 heartbeat / non-terminal submitIntent stale-save authoritative rehydrate：lower-version 写输后不再把本地 dirty `stateView` 回给玩家，而是回读权威 match；非终局 intent 分支返回 `sync_required / stale_state_version`。S8I 已补 terminal accepted intent stale-save settlement guard：surrender 等终局 accepted intent 保存写输后不会继续本地 dirty settlement / release，而是回读权威 match 并返回 `sync_required / stale_state_version`。下一步仍需进入更广义正式赛季入口、多实例共享队列、跨进程 WS/队列共享、生产 smoke 和线上部署，不能把本地仿真、golden replay、评分快照、计时锚点、前端心跳调度、最小 replay API、事件表真源、单进程 WS、WS heartbeat missed-events 补发、S7A 写回权限收口、S7D 浏览器负向证据、S7F 荣誉收藏、S8A 移动端真实后端主链路、S8D 首局 / 长等待体验收口、S8E 真实后端 long-wait smoke、S8F 低版本晚写保护、S8G 写输合同、S8H heartbeat / 非终局 intent 回读或 S8I terminal settlement guard 误写成线上封板、route 级统一 rehydrate 或完整同版本并发 CAS。
...
S8K active match same-version conflict guard 补充：`saveMatch()` 现在会读取 persisted `state_json` / `state_version` 快照，active 状态下若 incoming `stateVersion` 等于权威版本但 `state_json` 内容不同，则跳过保存并返回 `conflicting_state_version`，防止同一 base revision 产生的同版本旧进程写入静默覆盖权威 active match。SQL upsert 也会拒绝 active 同版本不同内容的后到写入，并在 no-op 后二次读取权威版本；若权威版本已更高，继续返回 `stale_state_version`。store 会把这两类结果纳入 authoritative rehydrate / `sync_required` 路径，避免玩家继续看到本地 dirty `stateView`。同版本同内容保存仍可更新 `connection_json`，但 S8Q 已把该更新收紧为写入时连接时间线单调合并。该切片不包含 route-level unified rehydrate、Redis / 多实例强一致队列、跨进程 WS fanout、生产 smoke 或线上部署。

S8Q same-version connection timeline monotonic merge 补充：同 `stateVersion` / 同 `state_json` 的 active heartbeat 或 connection 保存仍可成功，但不能把旧 `connection_json` 整段覆盖权威行。`ON CONFLICT DO UPDATE` 会在写入时用当前 DB 行和 incoming JSON 合并双方 `connectedAt`、`lastHeartbeatAt`、`reconnectedAt`，确保普通旧快照和读后写前并发心跳都只会让时间线前进。该切片只覆盖连接时间线单调性，不包含完整 active revision/CAS、Redis / 多实例强一致、跨进程 WS fanout、生产 smoke 或线上部署。
...
- 当前 live PVP 已覆盖 HTTP 撮合、入队评分锁定和近分优先候选选择、长等待后双方显式同意的 `200-399` 宽分差成局、queue status 跨进程 active-match handoff、SQLite queue ticket atomic claim / duplicate-match prevention、长等待 no-score 练习承接、真实后端 long-wait smoke、active match 低版本晚写保护、active match 同版本内容冲突保护、same-version connection timeline 单调合并、stale save 写输合同、heartbeat / 非终局 intent 写输后权威回读、terminal accepted intent 写输结算保护、settlement report 二次保存写输补偿、route-level 读路径权威回源、WS `sync_required` / `duplicate_action` 双方即时同步、WS heartbeat missed-events 补发、WS heartbeat 有界跨进程状态追赶证据、SQLite-backed 跨进程 WS `state_sync` fanout、跨进程终局 WS fanout smoke、客户端 WS 快速重连、前后台恢复即时权威追帧、本地实时传输状态可见化、live intent in-flight 防双击、权威 reducer、seat-scoped StateView、intent 幂等、S2-C full gate、S2-B artifact foundation、S2-D reducer-backed golden replay、S2-E store-backed 重连 / timeout / invalidated golden replay、S2-F runtime round14 结算、S4A active 回合计时独立锚点、S4B 前端权威心跳调度、S5A 最小赛后 replay API、S5B append-only `pvp_live_match_events` 回放真源、S6A 单进程 WebSocket 权威同步与公开事件补发、S7B live ranked 权威结算回执可见化、S7C live ranked 非强度赛季荣誉进度、S7D 真实后端 invalidated / replay 浏览器负向证据、S7F 非强度荣誉收藏持久化、S8A live PVP 移动端真实后端主链路 smoke、S8D 首局引导 DOM 完整展示、S8E 真实后端长等待练习证据、S8F 迁移安全的 lower-version stale save guard、S8G saved/skipped contract、S8H heartbeat / non-terminal submitIntent stale-save rehydrate、S8I terminal accepted intent stale-save settlement guard、S8K same-version active content conflict guard、S8N read-path authoritative rehydrate、S8O heartbeat events replay catch-up、S8P heartbeat state catch-up、S8Q connection timeline monotonic merge、S8S SQLite signal fanout、S8W terminal WS fanout smoke、S8X terminal settlement re-entry guard、S8Y stale duplicate intent authoritative replay、S8Z duplicate_action WS peer fanout、S8T client reconnect、S8U realtime status visibility、S8V intent in-flight lock、S9B foreground resume catch-up、S9G 赛后公平回执、持久化邀请 / 再战、重连宽限和关键 UI 审计，但不包含完整正式赛季奖励系统、Redis / 多实例强一致队列、完整 active match revision/CAS、生产 API smoke 或线上部署。
- S8J 已补 timeout / connection-timeout / invalidated / GET read path 的 terminal stale rehydrate，S8M 已补 settlement report 二次保存写输补偿，S8N 已补读路径权威回源和 WS sync_required fanout，S8S 已补 SQLite signal state_sync fanout，S8W 已补 surrender 终局跨进程 state_sync / review / settlement fanout smoke，S8X 已补已有 settlementReport 的终局重入不再调用 settlement provider，S8Y 已补 stale duplicate/conflict intent 的发起方权威回执和 WS 发起方错误提示收口，S8Z 已补 duplicate_action 幂等重放的双方 state_sync fanout 与同版本 signal 节流，但这仍不等于完整 same-version active match revision/CAS、专门跨进程 duplicate smoke、Redis / 多实例强一致或生产封板已完成。
- S8K 已补 active match same-version content conflict guard，S8Q 已补同版本同内容 connection timeline 单调合并，但这仍不是完整 active match revision/CAS；Redis / 多实例强一致队列、生产 smoke 和线上部署仍未封板。
- S8L 已补共享 SQLite 下的 queue ticket atomic claim / duplicate-match prevention，S8S 已补 SQLite-backed WS state_sync fanout，S8W 已补跨进程终局 fanout smoke，但 Redis / 多实例强一致队列、生产 smoke 和线上部署仍未封板。
