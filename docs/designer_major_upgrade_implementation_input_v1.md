...
截至 2026-06-20，live PVP 已继续补到准备调息、先后手语义预算、开局护体、后手开局公开护盾、护体后反打缓冲、每回合抽牌、连接宽限、赛后复盘、练习承接、长等待练习承接、真实后端长等待 smoke、active match 低版本晚写保护、stale save 写输合同、heartbeat / 非终局 intent 写输后权威回读、terminal accepted intent 写输结算保护、好友约战、Bo3 友谊再战、定向约战收件箱和公平保护可见化。`openingSafeguardReport` 已把首动伤害预算、后手 3 点公开护盾、开局护体保底血量、护体后首回合护盾缓冲、当前行动席位预算和公开状态投影给双方 UI；`firstMatchGuide` 已从数据层推进到 DOM 层完整展示 exceptionBranches / reviewActions；`practice-live` 在长等待后会先退出排队，确认成功才生成 no-score / replay-only / practice-only 的 drillScenario；若退出排队时票据已被真人撮合消费，则优先同步权威战局，不强开练习；真实后端 smoke 已用独立账号证明 `waitingReport.longWait=true`、`practice-live` 可点击并打开 no-score drill；SQLite 持久化已按 `state_version` 拒绝低版本 active match 晚写，并对老行 `state_json.stateVersion` 做迁移兜底；`saveMatch()` 已用 `saved/skipped/reason` 向 store 暴露写输结果，store 在 state 保存被跳过时不再追加旧事件表；heartbeat / 非终局 accepted intent 遇到 `stale_state_version` 后会回读 authoritative persisted match，避免继续返回旧进程 dirty `stateView`；terminal accepted intent 写输后会跳过本地 dirty finished settlement，并返回 `sync_required / stale_state_version` 的权威视图。`server/pvp-live/content/pvp-live-v1-content.js` 已落 8 套 V10-S2 基准谱 / bot policy 单点事实源，`tests/sanity_pvp_live_balance_simulation_checks.cjs` 已接入 10,048 样本 quick gate，并实际执行 10,000 个 opening pressure probe。当前 S2-C 证据显示：quick gate 先手胜率 50.14%，8 类 opening 脚本各 1250 条，后手首行动前死亡、无行动线、不可读爆发和非游戏局为 0。
...
S8C 跨进程 queue status 成局回收补充：`getQueueStatus()` 现在不再只依赖撮合进程内的 `pendingQueueResults`。撮合创建 match 后、删除 queue row 前会写入 `pvp_live_queue_handoffs(queueTicket -> userId -> matchId)`；当等待方 queue row 已被另一进程消费或删除滞后时，原等待进程或无状态第三进程可以通过本地旧票据或持久化 handoff 恢复自己的 active match，并拿到 `matched / matchId / seatId / stateView`。同一进程的旧 ticket 首次返回后仍会按既有合同消费，重复轮询返回空；伪造 ticket 和无 handoff 的 stale row 不会借 active match 返回 matched。该切片只解决 HTTP queue status 的 active-match handoff，不代表完整多实例强一致队列已经完成；DB 原子撮合 / 并发 claim 防重、active match revision/CAS、跨进程 WS fanout、生产 smoke 和线上部署仍需后续封板。
...
S8F active match 低版本晚写保护补充：SQLite `pvp_live_matches` 新增 `state_version` 持久化列，`saveMatch()` 会先读取当前列版本和 `state_json.stateVersion`，低于现有权威版本的旧进程晚写会被跳过，避免把已推进的 active 对局回退到旧 HP / 旧 stateVersion；旧库迁移后即使 `state_version=0`，也会从 `state_json` 推导现有版本。该切片只解决 lower-version stale save，不代表两个进程基于同一 base revision 同时生成同一新 `stateVersion` 的完整 CAS、DB 原子撮合 / 并发 claim 防重、跨进程 WS fanout、生产 smoke 或线上部署已经完成。

S8G stale save 写输合同补充：`saveMatch()` 现在把保存结果显式返回为 `saved/skipped/reason`，低版本晚写会返回 `saved=false / skipped=true / stale_state_version`，正常保存返回 `saved=true / skipped=false / saved`。`LivePvpStore.saveMatch()` 会透传该结果，并在 state 保存被 skipped 时停止 `saveMatchEvents()`，避免旧状态写输后仍追加旧事件流。该切片只提供 store 可感知写输的稳定接口，不代表完整同版本并发 CAS、DB 原子撮合 / 并发 claim 防重、跨进程 WS fanout、生产 smoke 或线上部署已经完成。

S8H stale save 权威回读补充：`recordHeartbeat()` 和 active 非终局 `submitIntent()` 现在会识别 `stale_state_version` 写输结果，绕过本地 `matches` cache 直接用 `loadMatchForUser(userId, matchId)` 回读 authoritative persisted match。heartbeat 分支返回权威 `stateView`；非终局 intent 分支返回 `sync_required / stale_state_version / events=[] / authoritative stateView`，避免把未持久化的本地 accepted 结果和旧事件流发给玩家。该切片只覆盖 heartbeat 与 `submitIntent()` 的 active 非终局保存写输回读，不代表 route 级统一 rehydrate、完整 settlement 写输补偿、同版本双写级 CAS、DB 原子撮合 / 并发 claim 防重、跨进程 WS fanout、生产 smoke 或线上部署已经完成。

S8I terminal accepted intent stale-save settlement guard 补充：`completeFinishedMatch()` 现在会读取 finished state 首次 `saveMatch()` 的 `saveResult`；如果 `saveMatch returned stale_state_version`，必须立刻停止本地 dirty finished state 的正式 settlement / settlement report 保存 / release。`submitIntent()` 的 terminal accepted intent 分支会回读 authoritative persisted match，并返回 `sync_required / stale_state_version / events=[] / authoritative stateView`，避免 surrender 等终局动作在旧进程里写输后还污染正式胜负。该切片不包含 route-level unified rehydrate、settlement report 二次保存写输补偿、same-version CAS、DB atomic matchmaking、跨进程 WS fanout、生产 smoke 或线上部署。

S8J timeout / read-path terminal stale rehydrate 补充：`sweepMatchTimeout()` 现在会把 timeout、connection-timeout 和 invalidated finishers 的 completion `saveResult` 冒给 `getMatchForUser()` / `getActiveMatchForUser()`；`releaseIfTerminal()` 也会返回 terminal completion。GET read path 遇到 `stale_state_version` 时会回读 authoritative persisted match，并返回 authoritative `stateView`，防止旧进程在自动 timeout 或读取已终局脏缓存时把本地 dirty finished / invalidated 视图发给双方。当前行为证据覆盖 `getMatchForUser()` 的 timeout finished 写输回源，以及 `getActiveMatchForUser()` 的 invalidated dirty cache 读时释放写输回源。该切片不包含 route-level unified rehydrate、settlement report 二次保存写输补偿、same-version CAS、DB atomic matchmaking、跨进程 WS fanout、生产 smoke 或线上部署。

S8K active match same-version conflict guard 补充：`saveMatch()` 现在会读取 persisted `state_json` / `state_version` 快照，active 状态下若 incoming `stateVersion` 等于权威版本但 `state_json` 内容不同，则跳过保存并返回 `conflicting_state_version`，防止同一 base revision 产生的同版本旧进程写入静默覆盖权威 active match。SQL upsert 也会拒绝 active 同版本不同内容的后到写入，并在 no-op 后二次读取权威版本；若权威版本已更高，继续返回 `stale_state_version`。store 会把这两类结果纳入 authoritative rehydrate / `sync_required` 路径，避免玩家继续看到本地 dirty `stateView`。同版本同内容保存仍可更新 `connection_json`。该切片不包含 DB atomic matchmaking、settlement report 二次保存写输补偿、route-level unified rehydrate、跨进程 WS fanout、生产 smoke 或线上部署。
...
- 当前 live PVP 已覆盖 HTTP 撮合、入队评分锁定和近分优先候选选择、长等待后双方显式同意的 `200-399` 宽分差成局、queue status 跨进程 active-match handoff、长等待 no-score 练习承接、真实后端 long-wait smoke、active match 低版本晚写保护、active match 同版本内容冲突保护、stale save 写输合同、heartbeat / 非终局 intent 写输后权威回读、terminal accepted intent 写输结算保护、权威 reducer、seat-scoped StateView、intent 幂等、S2-C full gate、S2-B artifact foundation、S2-D reducer-backed golden replay、S2-E store-backed 重连 / timeout / invalidated golden replay、S2-F runtime round14 结算、S4A active 回合计时独立锚点、S4B 前端权威心跳调度、S5A 最小赛后 replay API、S5B append-only `pvp_live_match_events` 回放真源、S6A 单进程 WebSocket 权威同步与公开事件补发、S7B live ranked 权威结算回执可见化、S7C live ranked 非强度赛季荣誉进度、S7D 真实后端 invalidated / replay 浏览器负向证据、S7F 非强度荣誉收藏持久化、S8A live PVP 移动端真实后端主链路 smoke、S8D 首局引导 DOM 完整展示、S8E 真实后端长等待练习证据、S8F 迁移安全的 lower-version stale save guard、S8G saved/skipped contract、S8H heartbeat / non-terminal submitIntent stale-save rehydrate、S8I terminal accepted intent stale-save settlement guard、S8K same-version active content conflict guard、持久化邀请 / 再战、重连宽限和关键 UI 审计，但不包含完整正式赛季奖励系统、DB 原子撮合 / 并发 claim 防重、route 级统一 rehydrate、settlement report 二次保存写输补偿、完整 active match revision/CAS、跨进程 WS fanout、生产 smoke 或线上部署。
- S8J 已补 timeout / connection-timeout / invalidated / GET read path 的 terminal stale rehydrate，但这仍只是 lower-version stale save 的体验与一致性收口，不等于 route-level unified rehydrate、settlement report 二次保存写输补偿或 same-version active match revision/CAS 已完成。
- S8K 已补 active match same-version content conflict guard，但这仍不是完整 active match revision/CAS；DB 原子撮合 / 并发 claim 防重、settlement report 二次保存写输补偿、跨进程 WS fanout、生产 smoke 和线上部署仍未封板。
