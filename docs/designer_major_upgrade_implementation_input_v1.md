...
| `game.js` 赛季验证写回 | 只要 PVP 结果不是 rejected，仍可能继续记录 season verification | 练习 / 降级 / 旧回执不得污染赛季验证 |
| `game-intro.html` 和历史 `progress.md` 文案 | 仍残留“镜像演武兜底”等旧表述 | 玩家可见文案必须改成练习 / 历史说明，不得暗示残影是真人 |
