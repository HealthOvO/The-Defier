...
} = require('../weekly-archive/service');
    ...
    if (!integrity.ok || integrity.skipped || integrity.mode !== 'session-v2') {
        res.status(integrity.ok ? 400 : integrity.status).json({
            ...
            message: integrity.ok
                ? (integrity.skipped ? '缺少完整性签名' : '三证归卷领奖需要绑定请求路径的会话签名')
                : integrity.message
        });
        return false;
    }
...
router.get('/current', authenticate, asyncHandler(async (req, res) => {
    res.json(await getCurrentWeeklyArchive(req.user.id));
}));

router.post('/rewards/foundation/claim', authenticate, asyncHandler(async (req, res) => {
    const payload = getSignedBusinessPayload(req.body);
    if (!requireSignedPayload(req, res, payload)) return;
    res.json(await claimWeeklyArchiveFoundation(req.user.id, payload));
}));

router.get('/ops/overview', authenticate, requireOpsToken, asyncHandler(async (req, res) => {
    res.json(await getWeeklyArchiveOpsOverview());
}));

router.use((error, req, res, next) => {
    if (res.headersSent) return next(error);
    const status = Number(error && error.statusCode) || 500;
    if (status >= 500) console.error('[WeeklyArchive] Route failed:', error);
    res.status(status).json({
        success: false,
        reason: error && error.reason || 'weekly_archive_error',
        message: status >= 500 ? '三证归卷服务暂时不可用' : error.message,
